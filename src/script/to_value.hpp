...
template <typename T>
void mark_dependency_member(const T &value, const String &name,
...
template <typename T>
inline String type_name(const T &) {
    return _TYPE_("object");
}
template <typename T>
inline String type_name(const intrusive_ptr<T> &p) {
    return type_name(*p.get());
}
template <typename K, typename V>
inline String type_name(const std::pair<K, V> &p) {
    return type_name(p.second); // for maps
}

/// Script code for an object, or anything else we can show
template <typename T>
inline String to_code(const T &v) {
    return format_string((L"<%s>"), type_name(v));
}
template <typename T>
inline String to_code(const intrusive_ptr<T> &p) {
    return type_name(*p.get());
}
...
class ScriptDefault : public ScriptValue {
  public:
    inline ScriptDefault(ScriptValueP const &un_default)
    ...
    virtual String toFriendlyString() const {
        return un_default->toFriendlyString();
    }
    virtual String toCode() const {
        return (L"mark_default(") + un_default->toCode() + (L")");
    }
    ...
    virtual CompareWhat compareAs(String &compare_str,
                                  void const *&compare_ptr) const {
        return un_default->compareAs(compare_str, compare_ptr);
    }
...
inline ScriptValueP with_defaultness_of(ScriptValueP const &of,
                                        ScriptValueP const &x) {
...
class ScriptLocalFileName : public ScriptValue {
  public:
    ...
    inline ScriptLocalFileName(Packaged* package, String const& fn, Type
file_type = IMAGE) : package(package), fn(fn), file_type(file_type) {}
...
struct ScriptIterator : public ScriptValue {
    ...
    virtual CompareWhat
    compareAs(String &, void const *&) const; // { return COMPARE_NO; }

    /// Return the next item for this iterator, or ScriptValueP() if there is no
    /// such item
...
class ScriptCollection : public ScriptCollectionBase {
  ...
  private:
    /// Store a pointer to a collection, collections are only ever used for
    /// structures owned outside the script
...
class ScriptCustomCollection : public ScriptCollectionBase {
  public:
    ...
    /// The collection as a list (contains only the values that don't have a
    /// key)
...
class ScriptConcatCollection : public ScriptCollectionBase {
  public:
    inline ScriptConcatCollection(ScriptValueP a, ScriptValueP b)
        : a(a), b(b) {}
    virtual ScriptValueP getMember(const String &name) const;
    virtual ScriptValueP getIndex(int index) const;
    virtual ScriptValueP makeIterator() const;
...
class ScriptObject : public ScriptValue {
  public:
    ...
    virtual ScriptType type() const {
        ...
        return d ? d->type() : SCRIPT_OBJECT;
    }
    virtual String typeName() const { return type_name(*value); }
    virtual String toString() const {
        ScriptValueP d = getDefault();
        return d ? d->toString() : ScriptValue::toString();
    }
    virtual int toInt() const {
        ScriptValueP d = getDefault();
        return d ? d->toInt() : ScriptValue::toInt();
    }
    virtual double toDouble() const {
        ScriptValueP d = getDefault();
        return d ? d->toDouble() : ScriptValue::toDouble();
    }
    virtual bool toBool() const {
        ScriptValueP d = getDefault();
        return d ? d->toBool() : ScriptValue::toBool();
    }
    virtual AColor toColor() const {
        ScriptValueP d = getDefault();
        return d ? d->toColor() : ScriptValue::toColor();
    }
    virtual String toCode() const {
        ScriptValueP d = getDefault();
        return d ? d->toCode() : to_code(*value);
    }
    virtual GeneratedImageP toImage() const {
        ScriptValueP d = getDefault();
        return d ? d->toImage() : ScriptValue::toImage();
    }
    virtual ScriptValueP getMember(const String &name) const {
#if USE_SCRIPT_PROFILING
        Timer t;
        Profiler prof(t, (void *)mangled_name(typeid(T)),
                      (L"get member of ") + type_name(*value));
#endif
        GetMember gm(name);
        gm.handle(*value);
        if (gm.result())
    ...
    virtual ScriptValueP getIndex(int index) const {
        ScriptValueP d = getDefault();
        return d ? d->getIndex(index) : ScriptValue::getIndex(index);
    }
    virtual ScriptValueP dependencyMember(const String &name,
                                          const Dependency &dep) const {
        mark_dependency_member(*value, name, dep);
...
inline ScriptValueP to_script(const vector<T> *v) {
    return intrusive(new ScriptCollection<vector<T>>(v));
}
template <typename K, typename V>
inline ScriptValueP to_script(const map<K, V> *v) {
    return intrusive(new ScriptMap<map<K, V>>(v));
}
template <typename K, typename V>
inline ScriptValueP to_script(const IndexMap<K, V> *v) {
    return intrusive(new ScriptMap<IndexMap<K, V>>(v));
}
template <typename T>
inline ScriptValueP to_script(const intrusive_ptr<T> &v) {
    return intrusive(new ScriptObject<intrusive_ptr<T>>(v));
}
template <typename T>
...
template <typename T>
inline T from_script(const ScriptValueP &value) {
...
template <>
inline ScriptValueP from_script<ScriptValueP>(const ScriptValueP &value) {
    return value;
}
template <>
inline String from_script<String>(const ScriptValueP &value) {
    return value->toString();
}
template <>
inline int from_script<int>(const ScriptValueP &value) {
    return value->toInt();
}
template <>
inline double from_script<double>(const ScriptValueP &value) {
    return value->toDouble();
}
template <>
inline bool from_script<bool>(const ScriptValueP &value) {
    return value->toBool();
}
template <>
inline Color from_script<Color>(const ScriptValueP &value) {
    return value->toColor();
}
template <>
inline AColor from_script<AColor>(const ScriptValueP &value) {
    return value->toColor();
}
template <>
inline wxDateTime from_script<wxDateTime>(const ScriptValueP &value) {
    return value->toDateTime();
}
template <>
inline GeneratedImageP from_script<GeneratedImageP>(const ScriptValueP &value) {
