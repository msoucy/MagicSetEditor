 ...
 *
 *  The context contains a normal pointer to the set, not a
 * shared/intrusive_ptr, because the set
...
class SetScriptManager : public SetScriptContext, public ActionListener {
  ...
  private:
    ...
    void alsoUpdate(std::deque<ToUpdate> &to_update,
                    const vector<Dependency> &deps, const CardP &card);

    /// Delayed update for (bitmask)...
    enum Delay { DELAY_KEYWORDS = 0x01, DELAY_CARDS = 0x02 };
    int delay;
