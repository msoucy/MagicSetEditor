...
enum CompareWhat { COMPARE_NO, COMPARE_AS_STRING, COMPARE_AS_POINTER };
...
class ScriptValue : public IntrusivePtrBaseWithDelete {
  public:
	...
	/// A string that can be shown to a user (i.e. not a template developer),
	/// should not contains tags
	...
	/// Signal that a script depends on a member of container, with the name of
	/// this
	...
	/** Should return a simplification of the closure or null to keep the
	 * closure.
	 *  Alternatively, the closure may be modified in place.
	 */
	virtual ScriptValueP simplifyClosure(ScriptClosure &) const;

	/// Return an iterator for the current collection, an iterator is a value
	/// that has next()
...
extern ScriptValueP
	dependency_dummy; ///< Dummy value used during dependency analysis
