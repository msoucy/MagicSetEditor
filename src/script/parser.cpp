...
void TokenIterator::readToken() {
    ...
    if (c == (L'\n')) {
        ...
        input = read_utf8_line(*is, true, true);
               ...
               (isDigit(c) && !buffer.empty() && buffer.back() == (L"."))) {
        ...
        if (c == (L'"') && !open_braces.empty() &&
            ...
            if (c == (L'n'))
                ...
                add_error(String::Format(
                    (L"Invalid string escape sequence: \"\\%c\""), c));
                str += (L'\\') + c; // ignore
            }
        } else if (c == (L'{')) {
            // smart string
...
ExprType parseExpr(TokenIterator &input, Script &script, Precedence minPrec) {
    ...
    if (token == (L"(")) {
        ...
        if (t == EXPR_STATEMENT) {
            input.add_error(
                (L"Warning: last statement of a function should be an "
                 L"expression, i.e. it should return a result in all cases."));
        }
        expectToken(input, (L"}"), &token);
    ...
    while (true) {
        ...
        if (minPrec <= PREC_SEQ && token == (L";")) {
            ...
            if (type == EXPR_STATEMENT) {
                input.add_error((L"Warning: the right hand side of an "
                                 L"assignment should always yield a value."));
            }
        } else if (minPrec <= PREC_AND && token == (L"orelse"))
        ...
        } else if (minPrec <= PREC_AND && token == (L"xor"))
            parseOper(input, script, PREC_CMP, I_BINARY, I_XOR);
        else if (minPrec <= PREC_CMP && token == (L"=")) {
            if (minPrec <= PREC_SET) {
                input.add_error((L"Use of '=', did you mean ':=' or '=='? I "
                                 L"will assume '=='"));
            }
