...
void store(const ScriptValueP &val, Defaultable<String> &var) {
	var.assign(val->toString());
}
void store(const ScriptValueP &val, Defaultable<Color> &var) {
	var.assign(val->toColor());
}
void store(const ScriptValueP &val, Defaultable<AColor> &var) {
	var.assign(val->toColor());
}
void store(const ScriptValueP &val, Alignment &var) {
	var = from_string(val->toString());
}
void store(const ScriptValueP &val, Direction &var) {
	parse_enum(val->toString(), var);
}
	...
	for (size_t i = 0; i < errors.size(); ++i) {
		...
		if (!e.filename.empty()) {
			...
			include_warnings +=
				...
				e.ParseError::what();
			if (i + 1 >= errors.size() ||
				errors[i + 1].filename != e.filename) {
				reader.warning(_("In include file '") + e.filename + _("'") +
							   include_warnings);
				include_warnings.clear();
			}
		} else {
			// use ParseError::what because we don't want e.start in the error
			// message
...
void OptionalScript::initDependencies(Context &ctx,
...
template <>
void Reader::handle(OptionalScript &os) {
	handle(os.unparsed);
	os.parse(*this);
}

template <>
void Writer::handle(const OptionalScript &os) {
	if (os.script) {
		handle(os.unparsed);
	}
}
...
template <>
void GetDefaultMember::handle(const OptionalScript &os) {
...
const String &StringScript::get() const { return unparsed; }
...
template <>
void Reader::handle(StringScript &os) {
	handle(os.unparsed);
	os.parse(*this, true);
}
...
template <>
void Writer::handle(const StringScript &os) {
	handle(os.unparsed);
}

template <>
void GetDefaultMember::handle(const StringScript &os) {
	// reflect as the script itself
	if (os.script) {
		handle(os.script);
	} else {
