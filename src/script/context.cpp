...
void instrBinary(BinaryInstructionType i, ScriptValueP &a,
				 const ScriptValueP &b);

// Perform a ternary simple instruction, store the result in a (not in *a)
void instrTernary(TernaryInstructionType i, ScriptValueP &a,
				  const ScriptValueP &b, const ScriptValueP &c);

// Perform a quaternary simple instruction, store the result in a (not in *a)
void instrQuaternary(QuaternaryInstructionType i, ScriptValueP &a,
					 const ScriptValueP &b, const ScriptValueP &c,
					 const ScriptValueP &d);
	...
	try {
		...
		while (instr < end) {
			...
			case I_GET_VAR: {
				...
				if (!value)
					throw ScriptErrorNoVariable(
						variable_to_string((Variable)i.data));
			...
			case I_MEMBER_C: {
				stack.back() = stack.back()->getMember(
					script.constants[i.data]->toString());
				break;
			}
			...
			case I_TAILCALL: {
				...
				try {
					...
					Timer timer;
					const Instruction *instr_bt =
						script.backtraceSkip(instr - i.data - 2, i.data);
					...
					// there is no need to open a new scope for this function,
					// since we already did so for the arguments
...
//%timer.exclude_time();
					...
					if (instr_bt) {
						throw ScriptError(
							_ERROR_2_("in function", e.what(),
									  script.instructionName(instr_bt)));
					} else {
						throw e; // rethrow
					}
				}
...
ScriptValueP Context::getVariable(const String &name) {
	...
	if (!value)
		throw ScriptErrorNoVariable(name);
	return value;
}
...
int Context::getVariableScope(Variable var) {
	if (variables[var].value)
		return level - variables[var].level;
	else
		return -1;
}
...
ScriptValueP Context::makeClosure(const ScriptValueP &fun) {
	...
	// we can find out which variables are in the last level by looking at
	// shadowed
...
void instrBinary(BinaryInstructionType i, ScriptValueP &a,
	...
	default:
			...
			if (bt == SCRIPT_INT) {
				...
				if (at == SCRIPT_DOUBLE) {
					...
					if (bi == 0)
						...
						a = to_script(aa * aa * aa);
					else
						a = to_script(pow(aa, bi));
				} else {
					int aa = a->toInt();
					if (bi == 0)
						a = to_script(1);
		...
		case I_MEMBER:
			throw InternalError(
				_("ITERATOR_R/MEMBER instruction fell through!"));
		}
	}
}
...
void Context::makeClosure(size_t n, const Instruction *&instr) {
	intrusive_ptr<ScriptClosure> closure(
		new ScriptClosure(stack[stack.size() - n - 1]));
