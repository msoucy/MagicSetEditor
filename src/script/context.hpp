...
class Context {
  public:
	 ...
	 *  @param openScope if false, variables set in this eval call will leak
	 * out.
	 */
	...
	/// Get the value of a variable, returns ScriptValue() if it is not set
	inline ScriptValueP getVariableOpt(Variable var) {
		return variables[var].value;
	}
	/// Get the value of a variable only if it was set in the current scope,
	/// returns ScriptValue() if it is not set
	...
	/// Make a closure of the function with the direct parameters of the current
	/// call
  ...
  public: // public for FOR_EACH
	...
	/// Record of a variable binding that is being shadowed (overwritten) by
	/// another binding
	struct Binding {
  ...
  private:
	...
	/// Make an object with n elements, popping 2n values from the stack, and
	/// push it onto the stack
	void makeObject(size_t n);
	/// Make a closure with n arguments
	void makeClosure(size_t n, const Instruction *&instr);

	/// Get a variable name givin its value, returns (Variable)-1 if not found
	/// (slow!)
