...
SetScriptContext::SetScriptContext(Set &set) : set(set) {}
...
Context &SetScriptContext::getContext(const StyleSheetP &stylesheet) {
	...
	if (it != contexts.end()) {
		...
		// variables
		//  NOTE: do not use a smart pointer for the pointer to the set, because
		//  the set owns this
		//        which would lead to a reference cycle.
		init_script_functions(*ctx);
		ctx->setVariable(SCRIPT_VAR_set,
						 intrusive(new ScriptObject<Set *>(&set)));
		ctx->setVariable(SCRIPT_VAR_game, to_script(set.game));
		ctx->setVariable(SCRIPT_VAR_stylesheet, to_script(stylesheet));
		ctx->setVariable(SCRIPT_VAR_card_style,
						 to_script(&stylesheet->card_style));
		ctx->setVariable(SCRIPT_VAR_card,
						 set.cards.empty()
							 ? script_nil
							 : to_script(set.cards.front())); // dummy value
		ctx->setVariable(SCRIPT_VAR_styling,
						 to_script(&set.stylingDataFor(*stylesheet)));
		try {
			// perform init scripts, don't use a scope, variables stay bound in
			// the context
...
Context &SetScriptContext::getContext(const CardP &card) {
	...
	if (card) {
		...
		ctx.setVariable(SCRIPT_VAR_styling,
						to_script(&set.stylingDataFor(card)));
	} else {
...
SetScriptManager::SetScriptManager(Set &set) : SetScriptContext(set), delay(0) {
	// add as an action listener for the set, so we receive actions
	set.actions.addListener(this);
}

SetScriptManager::~SetScriptManager() { set.actions.removeListener(this); }
...
void SetScriptManager::initDependencies(Context &ctx, Game &game) {
	if (game.dependencies_initialized)
		return;
...
void SetScriptManager::initDependencies(Context &ctx, StyleSheet &stylesheet) {
	if (stylesheet.dependencies_initialized)
		return;
...
void SetScriptManager::onAction(const Action &action, bool undone) {
	TYPE_CASE(action, ValueAction) {
		if (action.card) {
			...
			updateValue(*action.valueP, intrusive_from_existing(
											const_cast<Card *>(action.card)),
						&action);
			return;
		} else {
			...
			if (value) {
				if (value->underlying == &value->keyword.match) {
					...
					value->update(ctx);
					// changed the 'match' string of a keyword, rebuild database
					// and regex so matching is correct
					value->keyword.prepare(set.game->keyword_parameter_types,
										   true);
	...
	for (auto const s : styles) {
		...
		try {
			if (int change = s->update(ctx)) {
				...
				s->tellListeners(
					change |
					(only_content_dependent ? CHANGE_ALREADY_PREPARED : 0));
			}
		} catch (const ScriptError &e) {
			// NOTE: don't handle errors now, we are likely in an onPaint
			// handler
			handle_error(ScriptError(e.what() +
	...
	for (auto card : set.cards) {
		...
		for (auto v : card->data) {
			try {
				...
				Timer t;
				Profiler prof(t, v->fieldP.get(),
							  _("update card.") + v->fieldP->name);
#endif
				v->update(ctx);
			} catch (const ScriptError &e) {
				handle_error(ScriptError(e.what() +
...
void SetScriptManager::updateToUpdate(const ToUpdate &u,
	...
	if (starting_age <= age)
		return;			// this value was already updated
	...
	for (auto const d : deps) {
		...
		case DEP_CARD_FIELD: {
			if (card) {
				...
				// There is no card, so the update should affect all cards (fall
				// through).
			}
		}
		...
		case DEP_EXTRA_CARD_FIELD: {
			/*	// Not needed, extra card fields are handled in updateStyles()
			   ...
			   reinterpret_cast<StyleSheet*>(d.data);
