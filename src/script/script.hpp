 ...
 *  Some stupid compilers use signed integers for bitfields, so values should be
 * < 32
 */
enum InstructionType
// Basic
{ I_NOP = 0 ///< arg = *          : no operation, used as placeholder for extra
            ///data values
  ,
  I_PUSH_CONST = 1 ///< arg = const val  : push a constant onto the stack
  ,
  I_JUMP = 2 ///< arg = address    : move the instruction pointer to the given
             ///position
  ,
  I_JUMP_IF_NOT = 3 ///< arg = address    : move the instruction pointer if the
                    ///top of the stack is false
  ,
  I_JUMP_SC_AND = 19 ///< arg = address    : (short-circuiting and) jump and
                     ///don't pop if the top of the stack is false
  ,
  I_JUMP_SC_OR = 20 ///< arg = address    : (short-circuiting or)  jump and
                    ///don't pop if the top of the stack is true
                    // Variables
  ,
  I_GET_VAR = 4 ///< arg = var        : find a variable, push its value onto the
                ///stack, it is an error if the variable is not found
  ,
  I_SET_VAR = 5 ///< arg = var        : assign the top value from the stack to a
                ///variable (doesn't pop)
                // Objects
  ,
  I_MEMBER_C = 6 ///< arg = const name : finds a member of the top of the stack
                 ///replaces the top of the stack with the member
  ,
  I_LOOP = 7 ///< arg = address    : loop over the elements of an iterator,
             ///which is the *second* element of the stack (this allows for
             ///combing the results of multiple iterations)
  ///<                    at the end performs a jump and pops the iterator.
  ///note: The second element of the stack must be an iterator!
  ,
  I_LOOP_WITH_KEY = 8 ///< arg = address    : loop, but also pushing the key
  ,
  I_MAKE_OBJECT = 9 ///< arg = int        : make a list/map with n elements,
                    ///pops 2n values of the stack, n key/value pairs
                    // Functions
  ,
  I_CALL = 10 ///< arg = int, n*var : call the top item of the stack, with the
              ///given number of arguments (set with SET_VAR, but in the
              ///activation record of the call)
  ,
  I_CLOSURE = 11 ///< arg = int, n*var : construct a call closure object with
                 ///the given arguments
  ,
  I_TAILCALL = 12 ///< arg = int, n*var : perform a tail call - like I_CALL,
                  ///except faster
                  // Simple instructions
  ,
  I_UNARY =
      13 ///< arg = 1ary instr : pop 1 value,  apply a function, push the result
  ,
  I_BINARY =
      14 ///< arg = 2ary instr : pop 2 values, apply a function, push the result
  ,
  I_TERNARY =
      15 ///< arg = 3ary instr : pop 3 values, apply a function, push the result
  ,
  I_QUATERNARY =
      16 ///< arg = 4ary instr : pop 4 values, apply a function, push the result
  ,
  I_DUP =
      17 ///< arg = int        : duplicate the k-from-top element of the stack
  ,
...
enum Variable {
    ...
    ,
    SCRIPT_VAR_CUSTOM_LOTS =
        0xFFFFFF // ensure that sizeof(Variable) is large enough
};
...
class Script : public ScriptValue {
  public:
    ...
    /// Add a jump instruction, later comeFrom should be called on the returned
    /// value
    ...
    /** The instruction at pos must be a jumping instruction, it is changed so
  ...
  private:
     ...
     */
    const Instruction *backtraceSkip(const Instruction *instr,
                                     int to_skip) const;
