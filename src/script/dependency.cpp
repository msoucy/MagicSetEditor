...
// NOTE: dependency.cpp has nothing to do with dependency.hpp, the latter
// defines the dependency
// type, which is used here as an abstract type. The header for this source file
// is context.hpp
...
class DependencyDummy : public ScriptIterator {
  public:
	...
	virtual ScriptValueP dependencyName(const ScriptValue &,
										const Dependency &) const {
...
class DependencyUnion : public ScriptValue {
  public:
	...
	virtual String typeName() const {
		return _("union of ") + a->typeName() + _(" and ") + b->typeName();
	}

	virtual ScriptValueP dependencies(Context &ctx,
	...
	virtual ScriptValueP dependencyMember(const String &name,
										  const Dependency &dep) const {
		return unified(a->dependencyMember(name, dep),
					   b->dependencyMember(name, dep));
	}
	virtual ScriptValueP dependencyName(const ScriptValue &container,
										const Dependency &dep) const {
		return unified(a->dependencyName(container, dep),
					   b->dependencyName(container, dep));
	}
...
void unify(ScriptValueP &a, const ScriptValueP &b) {
	assert(a && b);
	if (a != b)
		a = intrusive(new DependencyUnion(a, b));
}
// Unify two values from different execution paths
ScriptValueP unified(const ScriptValueP &a, const ScriptValueP &b) {
	assert(a && b);
	if (a == b)
		...
		return intrusive(new DependencyUnion(a, b));
}
...
class ScriptMissingVariable : public ScriptValue {
  public:
	...
	virtual String typeName() const {
		return _("missing variable '") + name + _("'");
	}
...
struct Context::Jump {
	...
	vector<ScriptValueP> stack_top; ///< The top part of the stack, everything
	/// local to the current call
	vector<Binding>
		bindings; ///< The bindings made up to this point in the current scope
};
// an ordering on jumps by their target, lowest target = highest priority
struct Context::JumpOrder {
	inline bool operator()(Jump *a, Jump *b) { return a->target > b->target; }
};

// -----------------------------------------------------------------------------
// : Dependency analysis

ScriptValueP Context::dependencies(const Dependency &dep,
								   const Script &script) {
	// Dependency analysis proceeds in the same way as normal evaluation.
	// Operator calls will be replaced by "push dummy", we don't care about
	// values.
	...
	// Jumps are tricky:
	//   - I_LOOP:        We want to prevent infinite loops, the solution is
	//   that after the first
	//                    iteration we set the looper to a dummy value, so the
	//                    loop is only executed once.
	//                    TODO: This could result in false negatives when
	//                    iterating over things like fields.
	//                          We ignore this, because loops are usually only
	//                          used for exporting, where dependency
	//                          analysis is not used anyway.
	//   - I_JUMP_IF_NOT: We don't know the value of the condition, so we must
	//   evaluate both branches.
	//                    The simple solution would be to use recursion to fork
	//                    off one of the cases.
	//                    This could result in an exponential increase in
	//                    execution time,
	//                    because the analysis after an if statement is
	//                    duplicated.
	//                    A better solution is to evalutate branches 'in
	//                    parallel'.
	//                    We create a jump record for taking the branch, and
	//                    evaluate the fall through case.
	//                    When later a jump record points to the current
	//                    instruction the stack and variables of that
	//                    record are unify with the current execution path.
	//   - I_JUMP:        We must can not follow all jumps, because they may
	//   lead to a point beyond a jump record,
	//                    we can then no longer hope to unify with that jump
	//                    record.
	//                    Instead we create a new jump record, and follow the
	//                    jump record with the lowest target address.
	//                    This story doesn't hold for backwards jumps, we can
	//                    safely follow those (see I_LOOP above)
	...
	try {
		...
		while (true) {
				...
				for (auto &v : j->bindings) {
					...
					if (old_value) {
						setVariable(v.variable,
									unified(old_value, v.value.value));
					}
				}
				delete j;
			}

			if (instr >= &script.instructions[0] + script.instructions.size())
				break; // end of script
			...
			case I_JUMP: {
				if (&script.instructions[0] + i.data >= instr) {
					...
					jump->stack_top.assign(stack.begin() + stack_size,
										   stack.end());
					...
					resetBindings(scope);
					// we don't follow this jump just yet, there may be jumps
					// that point to earlier positions
					...
					// backward jump: just follow it, someone else (I_LOOP) will
					// make sure
			...
			case I_JUMP_SC_OR: {
				...
				// create jump record
				Jump *jump = new Jump;
				jump->target = &script.instructions[0] +
							   i.data; // note: operator[] triggers assertion
									   // (in msvc>=9) failure if
									   // i.data==instructions.size()
				...
				if (i.instr != I_JUMP_IF_NOT) {
					stack.pop_back(); // pop condition afterwards, so it is not
									  // poped when jump is taken
				}
				break;
			}
			...
			case I_MEMBER_C: {
				...
				stack.back() = stack.back()->dependencyMember(
					name, dep); // dependency on member
				break;
			}
			// Loop over a container, push next value or jump (almost as normal)
			case I_LOOP: {
				ScriptValueP &it =
					stack[stack.size() - 2]; // second element of stack
				ScriptValueP val = it->next();
				if (val) {
					// we have not been through the body
					it = dependency_dummy; // invalidate iterator, so we loop
										   // only once
					stack.push_back(val);
				} else {
			...
			case I_TAILCALL: {
				...
				for (unsigned int j = 0; j < i.data; ++j) {
					setVariable((Variable)instr[i.data - j - 1].data,
								stack.back());
					stack.pop_back();
				}
				instr += i.data; // skip arguments, there had better not be any
								 // jumps into the argument list
			...
			case I_GET_VAR: {
				...
				if (!value) {
					value =
						intrusive(new ScriptMissingVariable(variable_to_string(
							(Variable)i.data))); // no errors here
				}
		...
		// the only place where exceptions should be possible is in
		// someValue->getMember
		if (scope)
			closeScope(scope);	// restore scope
