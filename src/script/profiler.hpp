...
inline const char *mangled_name(const type_info &t) { return t.raw_name(); }
#else
// clock() has nanosecond resolution on Linux
// on any other platform, stil the best way.
typedef clock_t ProfileTime;

inline ProfileTime timer_now() { return clock(); }
inline ProfileTime timer_resolution() { return CLOCKS_PER_SEC; }

inline const char *mangled_name(const std::type_info &t) { return t.name(); }
#endif
...
class FunctionProfile : public IntrusivePtrBase<FunctionProfile> {
  public:
    ...
    /** we (ab)use the fact that all pointers are even to store both pointers
     * and ids */
    ...
    inline double total_time() const {
        return time_ticks / (double)timer_resolution();
    }
    inline double avg_time() const { return total_time() / calls; }
    inline double max_time() const {
        return time_ticks_max / (double)timer_resolution();
    }
};
...
/// Return a simplified profile, where all things beyond a cerrain level are
/// agragated
...
class Profiler {
  public:
    /// Log the fact that the function  function_name  is entered, ends when
    /// profiler goes out of scope.
    /** Time between the construction of Timer and the construction of Profiler
     * is excluded from ALL profiles.
     */
...
// Profile the current function (all following code in the current block) under
// the given name
