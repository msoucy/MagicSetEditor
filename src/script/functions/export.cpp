...
void guard_export_info(const String &fun, bool need_template = false) {
	if (!export_info() && (!need_template || export_info()->export_template)) {
		throw ScriptError(_("Can only use ") + fun +
						  _(" from export templates"));
	} else if (export_info()->directory_relative.empty()) {
		throw ScriptError(_("Can only use ") + fun +
						  _(" when 'create directory' is set to true"));
	}
}
...
String get_export_full_path(String &rel_name) {
	...
	if (!ei.allow_writes_outside) {
		...
		if (p2.empty() || p1.size() < p2.size() ||
			p1.substr(0, p2.size() - 1) != p2.substr(0, p2.size() - 1)) {
			throw ScriptError(_("Not a relative filename: ") + rel_name);
		}
	}
	rel_name = fn.GetFullName(); // TODO: does this work correctly with
								 // subdirectories in target dir?
...
class TagStack {
  public:
	void open(String &ret, Tag &tag) { add(ret, NegTag(&tag, false)); }
	void close(String &ret, Tag &tag) { add(ret, NegTag(&tag, true)); }
  ...
  private:
	...
	vector<NegTag> pending_tags; ///< Tags opened in the tagged string, but not
								 ///(yet) in the output
				...
				if (tags[i] == tag.tag) {
					...
					for (int j = (int)tags.size() - 1; j > i; --j) {
						pending_tags.push_back(
							NegTag(tags[j], true)); // close tag, top down
						tags.pop_back();
					}
					pending_tags.push_back(tag); // now close tag i
					for (int j = i + 1; j < (int)tags.size(); ++j) {
						pending_tags.push_back(
							NegTag(tags[j], false)); // reopen later, bottom up
						tags.pop_back();
					}
					tags.resize(i);
					return;
...
String symbols_to_html(const String &str, SymbolFont &symbol_font,
					   double size) {
	...
	for (auto &sym : symbols) {
		String filename =
			symbol_font.name() + _("-") + clean_filename(sym.text) + _(".png");
		...
		if (it == ei.exported_images.end()) {
			...
			it = ei.exported_images.insert(make_pair(filename,
													 wxSize(img.GetWidth(),
															img.GetHeight())))
					 .first;
		}
		html += _("<img src='") + filename + _("' alt='") +
				html_escape(sym.text) + _("' width='") +
				(String() << it->second.x) + _("' height='") +
				(String() << it->second.y) + _("'>");
	}
	return html;
}
...
String to_html(const String &str_in, const SymbolFontP &symbol_font,
	...
	String ret;
	Tag bold(_("<b>"), _("</b>")), italic(_("<i>"), _("</i>")),
...
SCRIPT_FUNCTION(to_html) {
	...
	if (symbol_font_size <= 0)
		symbol_font_size = 12; // a default
...
// copy from source package -> destination directory, return new filename
// (relative)
SCRIPT_FUNCTION(copy_file) {
	...
	if (!out.Ok())
		throw Error(_("Unable to open file '") + out_path + _("' for output"));
	out.Write(*in);
...
SCRIPT_FUNCTION(write_image_file) {
	...
	ScriptObject<CardP> *card =
		dynamic_cast<ScriptObject<CardP> *>(input.get()); // is it a card?
	Image image;
	GeneratedImage::Options options(width, height, ei.export_template.get(),
									ei.set.get());
	if (card) {
		image = conform_image(
			export_bitmap(ei.set, card->getValue()).ConvertToImage(), options);
	} else {
		image = input->toImage()->generateConform(options);
	}
	if (!image.Ok())
		throw Error(_("Unable to generate image for file ") + file);
	// write
	image.SaveFile(out_path);
	ei.exported_images.insert(
		make_pair(file, wxSize(image.GetWidth(), image.GetHeight())));
	SCRIPT_RETURN(file);
}
