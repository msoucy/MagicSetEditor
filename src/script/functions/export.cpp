...
void guard_export_info(const String &fun, bool need_template = false) {
    if (!export_info() && (!need_template || export_info()->export_template)) {
        throw ScriptError((L"Can only use ") + fun +
                          (L" from export templates"));
    } else if (export_info()->directory_relative.empty()) {
        throw ScriptError((L"Can only use ") + fun +
                          (L" when 'create directory' is set to true"));
    }
}
...
String symbols_to_html(const String &str, SymbolFont &symbol_font,
    ...
    for (auto &sym : symbols) {
        String filename =
            symbol_font.name() + (L"-") + clean_filename(sym.text) + (L".png");
        ...
        html += (L"<img src='") + filename + (L"' alt='") +
                html_escape(sym.text) + (L"' width='") +
                (String() << it->second.x) + (L"' height='") +
                (String() << it->second.y) + (L"'>");
    }
    return html;
}
...
String to_html(const String &str_in, const SymbolFontP &symbol_font,
    ...
    String ret;
    Tag bold((L"<b>"), (L"</b>")), italic((L"<i>"), (L"</i>")),
...
SCRIPT_FUNCTION(copy_file) {
    ...
    if (!out.Ok())
        throw Error((L"Unable to open file '") + out_path + (L"' for output"));
    out.Write(*in);
...
SCRIPT_FUNCTION(write_image_file) {
    ...
    if (!image.Ok())
        throw Error((L"Unable to generate image for file ") + file);
    // write
