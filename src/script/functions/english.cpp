...
bool is_vowel(Char c) {
	return c == _('a') || c == _('e') || c == _('i') || c == _('o') ||
		   c == _('u') || c == _('A') || c == _('E') || c == _('I') ||
		   c == _('O') || c == _('U');
}
inline bool is_constant(Char c) { return !is_vowel(c); }
...
String english_number(int i) {
	...
	default: {
		if (i < 0 || i >= 100) {
			...
			// <a>ty-<b>
			return english_number(i / 10 * 10) + _("-") +
				   english_number(i % 10);
		}
	}
	}
		...
		if (start != String::npos) {
			...
			if (end != String::npos) {
				...
				if (is.ToLong(&i)) {
					if (i == 1) {
						return _("<hint-1>") +
							   substr_replace(input, start, end, fun(i));
					} else {
						return _("<hint-2>") +
							   substr_replace(input, start, end, fun(i));
					}
...
String english_singular(const String &str) {
	if (str.size() > 3 && is_substr(str, str.size() - 3, _("ies"))) {
		...
		return str.substr(0, str.size() - 2);
	} else if (str.size() > 5 && is_substr(str, str.size() - 3, _("ves")) &&
			   (is_substr(str, str.size() - 5, _("el")) ||
				is_substr(str, str.size() - 5, _("ar")))) {
 ...
 *   -  "[^., ]a <param-..>[aeiou]" ---> "\1 an \2"   (articla 'an', case
 *insensitive)
 *   -  "<hint-?>"                  ---> ""           (remove <hint>s
 *afterwards)
 *
		...
		if (is_substr(str, i, _("<hint-"))) {
			...
			if (after != String::npos) {
				...
				if (singplur == 1 && is_substr(str, after, _("a</param-"))) {
					...
					if (after_end != String::npos &&
						after_end + 1 < str.size() &&
						isSpace(str.GetChar(after_end)) &&
						is_vowel(str.GetChar(after_end + 1))) {
					...
					if ((ret.size() == 2 ||
						 !isAlpha(ret.GetChar(ret.size() - 3))) &&
						(last == _("a ") || last == _("A "))) {
						ret.insert(ret.size() - 1, _('n'));
					}
				} else if (is_substr(str, after, _("</param-")) &&
						   ret.size() >= 1 &&
...
void init_script_english_functions(Context &ctx) {
	...
	ctx.setVariable(_("english_number_multiple"),
					script_english_number_multiple);
