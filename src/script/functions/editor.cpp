...
SCRIPT_FUNCTION_WITH_DEP(combined_editor) {
	...
	for (int i = 0 ; ; ++i) {
		String name = (L"field"); if (i > 0) name = name << i;
		SCRIPT_OPTIONAL_PARAM_N(ValueP, name, value) {
			TextValue* text_value = dynamic_cast<TextValue*>(value.get());
			if (!text_value) throw ScriptError((L"Argument '")+name+(L"' should be a text field"));
	...
	for (int i = 0 ; ; ++i) {
		String name = (L"separator"); if (i > 0) name = name << i;
		SCRIPT_OPTIONAL_PARAM_N(String, name, separator) {
			separators.push_back(separator);
		} else if (i > 0) break;
	}
	if (separators.size() < values.size() - 1) {
		throw ScriptError(String::Format((L"Not enough separators for combine_editor, expected %d"), values.size()-1));
	}
	// the value
	...
	if (!new_value_empty || !hide_when_empty) {
		if (!suffix.empty()) {
			if (is_substr(new_value, size_before_last, (L"<sep-soft>")) && value_parts.size() >= 2) {
...
SCRIPT_FUNCTION_DEPENDENCIES(combined_editor) {
	...
	else                                 throw InternalError((L"Finding dependencies of combined error for non card/set field"));
...
SCRIPT_FUNCTION(primary_choice) {
	...
	if (!value) {
		throw ScriptError((L"Argument to 'primary_choice' should be a choice value"));
	}
...
bool next_choice(size_t& start, size_t& end, const String& input) {
	...
	// ingore whitespace
	while (start < input.size() && input.GetChar(start) == (L' ')) ++start;
	// find end
...
void init_script_editor_functions(Context& ctx) {
	ctx.setVariable((L"forward_editor"),           script_combined_editor); // compatability
	...
	ctx.setVariable((L"require_exclusive_choice"), script_require_exclusive_choice);
