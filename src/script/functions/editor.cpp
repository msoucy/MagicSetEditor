...
SCRIPT_FUNCTION_WITH_DEP(combined_editor) {
	...
	for (int i = 0;; ++i) {
		...
		SCRIPT_OPTIONAL_PARAM_N(ValueP, name, value) {
			...
			if (!text_value)
				throw ScriptError(_("Argument '") + name +
								  _("' should be a text field"));
	...
	if (separators.size() < values.size() - 1) {
		throw ScriptError(String::Format(
			_("Not enough separators for combine_editor, expected %d"),
			values.size() - 1));
	}
	// the value
	...
	if (!new_value_empty || !hide_when_empty) {
		if (!suffix.empty()) {
			if (is_substr(new_value, size_before_last, _("<sep-soft>")) &&
				value_parts.size() >= 2) {
				...
				new_value = new_value.substr(0, size_before_last) + _("<sep>") +
							suffix + _("<soft>") +
							separators[value_parts.size() - 2] +
							_("</soft></sep>") + new_value.substr(after);
			} else {
...
SCRIPT_FUNCTION_DEPENDENCIES(combined_editor) {
	...
	if (dep.type == DEP_CARD_FIELD)
		...
		throw InternalError(
			_("Finding dependencies of combined error for non card/set field"));
...
SCRIPT_FUNCTION(primary_choice) {
	...
	if (!value) {
		throw ScriptError(
			_("Argument to 'primary_choice' should be a choice value"));
	}
...
bool next_choice(size_t &start, size_t &end, const String &input) {
	if (start >= input.size())
		return false;
	// ingore whitespace
	while (start < input.size() && input.GetChar(start) == _(' '))
		++start;
	// find end
	...
	if (count > max) {
		for (size_t i = choices.size() - 1; i >= 0; --i) {
			if (count <= max)
				break;
			if (seen[i]) {
				if (max > 0 && choices[i] == prefered)
					continue; // we would rather not remove prefered choice
				seen[i] = false;
				--count;
			}
		}
...
void init_script_editor_functions(Context &ctx) {
	ctx.setVariable(_("forward_editor"),
	...
	ctx.setVariable(_("require_exclusive_choice"),
