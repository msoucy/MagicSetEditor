...
// The combined value is  value1 <sep>something</sep> value2
// <sep>something</sep> value3
//
...
SCRIPT_FUNCTION_WITH_DEP(combined_editor) {
    ...
    for (int i = 0;; ++i) {
        ...
        SCRIPT_OPTIONAL_PARAM_N(ValueP, name, value) {
            ...
            if (!text_value)
                throw ScriptError((L"Argument '") + name +
                                  (L"' should be a text field"));
            values.push_back(text_value);
        }
        else if (i > 0) break;
    }
    if (values.empty()) {
        throw ScriptError((L"No fields specified for combined_editor"));
    }
    // read 'separator#' arguments
    vector<String> separators;
    for (int i = 0;; ++i) {
        String name = (L"separator");
        if (i > 0)
            name = name << i;
    ...
    if (separators.size() < values.size() - 1) {
        throw ScriptError(String::Format(
            (L"Not enough separators for combine_editor, expected %d"),
            values.size() - 1));
    }
    // the value
    ...
    for (size_t i = 1; i < value_parts.size(); ++i) {
        ...
        if (value_parts[i].second && new_value_empty && hide_when_empty) {
            ...
            // soft separator
            new_value += (L"<sep-soft>") + separators[i - 1] + (L"</sep-soft>");
            new_value_empty = false;
        } else {
            // normal separator
            new_value += (L"<sep>") + separators[i - 1] + (L"</sep>");
            new_value_empty = false;
        }
    ...
    if (!new_value_empty || !hide_when_empty) {
        if (!suffix.empty()) {
            if (is_substr(new_value, size_before_last, (L"<sep-soft>")) &&
                value_parts.size() >= 2) {
                // If the value ends in a soft separator, we have this
                // situation:
                ...
                // Which might be slightly incorrect, but soft text doesn't
                // matter anyway.
                size_t after =
                    min(new_value.size(),
                        match_close_tag_end(new_value, size_before_last));
                new_value = new_value.substr(0, size_before_last) + (L"<sep>") +
                            suffix + (L"<soft>") +
                            separators[value_parts.size() - 2] +
                            (L"</soft></sep>") + new_value.substr(after);
            } else {
                new_value += (L"<suffix>") + suffix + (L"</suffix>");
            }
        }
        if (!prefix.empty()) {
            new_value = (L"<prefix>") + prefix + (L"</prefix>") + new_value;
        }
    }
...
SCRIPT_FUNCTION_DEPENDENCIES(combined_editor) {
    ...
    GameP game = set->game;
    FieldP target_field;
    if (dep.type == DEP_CARD_FIELD)
        target_field = game->card_fields[dep.index];
    else if (dep.type == DEP_SET_FIELD)
        target_field = game->set_fields[dep.index];
        ...
        throw InternalError(
            (L"Finding dependencies of combined error for non card/set field"));
...
SCRIPT_FUNCTION(primary_choice) {
    ...
    if (!value) {
        throw ScriptError(
            (L"Argument to 'primary_choice' should be a choice value"));
    }
...
bool next_choice(size_t &start, size_t &end, const String &input) {
    if (start >= input.size())
        return false;
    // ingore whitespace
    while (start < input.size() && input.GetChar(start) == (L' '))
        ++start;
    // find end
    end = input.find_first_of((L','), start);
    if (end == String::npos)
        end = input.size();
    return true;
}
...
bool chosen(const String &choice, const String &input) {
    for (size_t start = 0, end = 0; next_choice(start, end, input);
         start = end + 1) {
 ...
 */
String filter_choices(const String &input, const vector<String> &choices,
                      int min, int max, String prefered) {
    if (choices.empty())
        return input; // do nothing, shouldn't happen, but better make sure
    String ret;
    int count = 0;
...
void read_choices_param(Context &ctx, vector<String> &choices_out) {
    SCRIPT_OPTIONAL_PARAM_C(String, choices) {
        for (size_t start = 0, end = 0; next_choice(start, end, choices);
             start = end + 1) {
...
SCRIPT_FUNCTION(require_choice) {
    ...
    SCRIPT_RETURN(
        filter_choices(input, choices, 1, (int)choices.size(), last_change));
}
...
SCRIPT_FUNCTION(remove_choice) {
    ...
    vector<String> choices;
    read_choices_param(ctx, choices);
    SCRIPT_RETURN(filter_choices(input, choices, 0, 0, (L"")));
}
...
SCRIPT_FUNCTION(count_chosen) {
    ...
    SCRIPT_OPTIONAL_PARAM_C(String, choices) {
        ...
        for (size_t start = 0, end = 0; next_choice(start, end, choices);
             start = end + 1) {
            if (chosen(choices.substr(start, end - start), input))
                ++count;
        }
        SCRIPT_RETURN(count);
    }
...
void init_script_editor_functions(Context &ctx) {
    ctx.setVariable((L"forward_editor"),
                    script_combined_editor); // compatability
    ctx.setVariable((L"combined_editor"), script_combined_editor);
    ctx.setVariable((L"primary_choice"), script_primary_choice);
    ctx.setVariable((L"chosen"), script_chosen);
    ctx.setVariable((L"count_chosen"), script_count_chosen);
    ctx.setVariable((L"require_choice"), script_require_choice);
    ctx.setVariable((L"exclusive_choice"), script_exclusive_choice);
    ctx.setVariable((L"require_exclusive_choice"),
                    script_require_exclusive_choice);
    ctx.setVariable((L"remove_choice"), script_remove_choice);
    ctx.setVariable((L"is_default"), script_is_default);
    ctx.setVariable((L"mark_default"), script_mark_default);
}
