 ...
 *   }
 *  @endcode
 *  This declares a value 'script_my_function' which can be added as a variable
 * to a context
 *  using:
 *  @code
...
#define SCRIPT_FUNCTION_WITH_DEP(name)                                         \
    SCRIPT_FUNCTION_AUX(name, virtual ScriptValueP dependencies(               \
                                  Context &, const Dependency &) const;)

#define SCRIPT_FUNCTION_DEPENDENCIES(name)                                     \
    ScriptValueP ScriptBuiltIn_##name::dependencies(                           \
        Context &ctx, const Dependency &dep) const

/// Macro to declare a new script function with custom closure simplification
#define SCRIPT_FUNCTION_WITH_SIMPLIFY(name)                                    \
    SCRIPT_FUNCTION_AUX(                                                       \
        name, virtual ScriptValueP simplifyClosure(ScriptClosure &) const;)

#define SCRIPT_FUNCTION_SIMPLIFY_CLOSURE(name)                                 \
    ScriptValueP ScriptBuiltIn_##name::simplifyClosure(ScriptClosure &closure) \
        const
...
inline Type from_script(const ScriptValueP &v, Variable var) {
    try {
        ...
        throw ScriptError(
            _ERROR_2_("in parameter", variable_to_string(var), e.what()));
    }
}
 ...
 */
#define SCRIPT_PARAM(Type, name) SCRIPT_PARAM_N(Type, _(#name), name)
...
#define SCRIPT_PARAM_C(Type, name) SCRIPT_PARAM_N(Type, SCRIPT_VAR_##name, name)
...
#define SCRIPT_RULE_2_C(funname, type1, name1, type2, name2)                   \
    SCRIPT_RULE_2_N(funname, type1, SCRIPT_VAR_##name1, name1, type2,          \
                    SCRIPT_VAR_##name2, name2)
...
/// Utility for defining a script rule with two named parameters, with
/// dependencies
#define SCRIPT_RULE_2_N_DEP(funname, type1, str1, name1, type2, str2, name2)   \
    SCRIPT_RULE_2_N_AUX(                                                       \
        funname, type1, str1, name1, type2, str2, name2,                       \
        virtual ScriptValueP dependencies(Context &, const Dependency &)       \
            const;                                                             \
...
#define SCRIPT_RULE_2_N_AUX(funname, type1, str1, name1, type2, str2, name2,   \
                            dep, more)                                         \
    class ScriptRule_##funname : public ScriptValue {                          \
      public:                                                                  \
        ...
        virtual ScriptType type() const { return SCRIPT_FUNCTION; }            \
        virtual String typeName() const { return _(#funname)(L"_rule"); }      \
        dep protected : virtual ScriptValueP do_eval(Context &ctx,             \
                                                     bool) const;              \
                                                                               \
      private:                                                                 \
        type1 name1;                                                           \
    ...
    more ScriptValueP ScriptRule_##funname::do_eval(Context &ctx, bool) const

#define SCRIPT_RULE_2_DEPENDENCIES(name)                                       \
    ScriptValueP ScriptRule_##name::dependencies(Context &ctx,                 \
