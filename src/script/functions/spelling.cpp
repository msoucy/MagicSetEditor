...
inline size_t spelled_correctly(const String &input, size_t start, size_t end,
								SpellChecker **checkers,
								const ScriptValueP &extra_test, Context &ctx) {
	// untag
	...
	if (extra_test) {
		...
		// try on tagged
		ctx.setVariable(SCRIPT_VAR_input,
						to_script(input.substr(start, end - start)));
		if (extra_test->eval(ctx)->toBool()) {
			return true;
		}
	}
...
void check_word(const String &tag, const String &input, String &out,
				size_t start, size_t end, SpellChecker **checkers,
				const ScriptValueP &extra_test, Context &ctx) {
	if (start >= end)
		return;
...
void check_word(const String &tag, const String &input, String &out, Char sep,
				size_t prev, size_t start, size_t end, size_t after,
				SpellChecker **checkers, const ScriptValueP &extra_test,
				Context &ctx) {
	if (start == end) {
		// word consisting of whitespace/punctuation only
		if (untag(input.substr(prev, after - prev)).empty()) {
			if (isSpace(sep) &&
				(after == input.size() || isSpace(input.GetChar(after)))) {
				// double space
...
SCRIPT_FUNCTION(check_spelling) {
	...
	while (pos < input.size()) {
		...
		if (c == _('<')) {
			if (word_start == pos) {
				// prefer to place word start inside tags, i.e. as late as
				// possible
			...
			check_word(tag, input, result, sep, prev_end, word_start, word_end,
					   pos, checkers, extra_match, ctx);
			// next
			sep = c;
	...
	// last word
	check_word(tag, input, result, sep, prev_end, word_start, word_end, pos,
			   checkers, extra_match, ctx);
	// done
