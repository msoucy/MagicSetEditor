...
SCRIPT_FUNCTION(new_card) {
    ...
    while (ScriptValueP v = it->next(&key)) {
        ...
        IndexMap<FieldP, ValueP>::const_iterator value_it =
            new_card->data.find(name);
        if (value_it == new_card->data.end()) {
            throw ScriptError(
                format_string((L"Card doesn't have a field named '%s'"), name));
        }
        // set it
...
void init_script_construction_functions(Context &ctx) {
    ctx.setVariable((L"new_card"), script_new_card);
}
