...
enum DependencyType {
	...
	,
	DEP_CARD_STYLE ///< dependency of a script in a "style" property, data gives
				   ///the stylesheet
	,
	DEP_EXTRA_CARD_FIELD ///< dependency of a script in an extra stylesheet
						 ///specific card field
	,
...
class Dependency {
  public:
	...
	inline Dependency makeCardIndependend() const {
		return Dependency(type == DEP_CARD_FIELD ? DEP_CARDS_FIELD : type,
						  index, data);
	}
