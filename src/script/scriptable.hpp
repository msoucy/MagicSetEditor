...
template <typename T>
inline void change(T &v, const T &new_v) {
	v = new_v;
}
template <typename T>
inline void change(Defaultable<T> &v, const Defaultable<T> &new_v) {
	v.assignDontChangeDefault(new_v());
}
...
class OptionalScript {
  public:
	...
	/// Invoke the script, return the result, or script_nil if there is no
	/// script
	...
	/// Initialize things this script depends on by adding dep to their list of
	/// dependent scripts
...
class Scriptable {
  public:
	...
	inline void operator=(const T &value) { this->value = value; }
	...
	/// Updates the value by executing the script, returns true if the value has
	/// changed
	inline bool update(Context &ctx) { return script.invokeOn(ctx, value); }
