        ...
        for (auto &error : errors) {
            if (multiline) {
                cli.show_message(MESSAGE_ERROR,
                                 String::Format(L"On line %d:\t", error.line) +
                                     error.what());
            } else {
    ...
    try {
        if (command.empty()) {
            ...
            if (before == L":q" || before == L":quit") {
                ...
                running = false;
            } else if (before == L":?" || before == L":h" ||
                       before == L":help") {
                showUsage();
            } else if (before == L":l" || before == L":load") {
                if (arg.empty()) {
                    cli.show_message(MESSAGE_ERROR,
                                     L"Give a filename to open.");
                } else {
                ...
                if (set) {
                    ...
                    cli << String::Format(L"#cards:   %d", set->cards.size())
                        << ENDL;
                } else {
                ...
                if (arg.empty()) {
                    cli.show_message(MESSAGE_ERROR,
                                     L"Give a new working directory.");
                } else {
                    if (!wxSetWorkingDirectory(arg)) {
                        cli.show_message(MESSAGE_ERROR,
                                         L"Can't change working directory to " +
                                             arg);
                    } else {
                        setExportInfoCwd();
                    }
                ...
                if (arg.empty()) {
                    cli.show_message(MESSAGE_ERROR,
                                     L"Give a shell command to execute.");
                } else {
#ifdef __WXMSW__
                ...
                cli.show_message(MESSAGE_ERROR,
                                 L"Unknown command, type :help for help.");
            }
...
void CLISetInterface::showProfilingStats(const FunctionProfile &item,
    ...
    if (level == 0) {
        ...
        for (int i = 1; i < level; ++i)
            cli << L"  ";
        cli << String::Format(L"%8.5f  %8.5f  %6d  %s", item.total_time(),
