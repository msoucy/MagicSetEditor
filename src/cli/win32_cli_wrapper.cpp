 ...
 *    - hence, there is NO way to printf from a gui program to an existing
 *console
 *
 *  This wrapper hacks around that by opening pipes for std{in,our,err}.
 *  The gui program (MSE) can then use stdout as usual, while this program
 *passes
 *  the text to the real stdout.
 *
 *  In addition, this wrapping allows us to translate a (subset of) ANSI escape
 *codes
...
const char *redirect_flags[] = {
	"-?",	"/?", "--help",   "-v",			  "--version",
	"--cli", "-c", "--export", "--export-images", "--create-installer"};

int main(int argc, char **argv) {
	// determine whether we need to wrap console i/o
	need_redirection = false;
	for (int i = 1; i < argc; ++i) {
		for (int j = 0; j < sizeof(redirect_flags) / sizeof(redirect_flags[0]);
			 ++j) {
...
break_2:
	...
	// application name
	TCHAR app_path[2048];
	GetModuleFileName(NULL /*current process*/, app_path,
					  sizeof(app_path) / sizeof(TCHAR));
	size_t app_path_length = _tcsclen(app_path);
	if (app_path_length > 4 &&
		_tcsicmp(app_path + app_path_length - 4, _T(".com")) == 0) {
	...
	if (need_redirection) {
		...
		CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)TransferThread,
					 &tranfer_in, 0, NULL);
		CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)TransferThread,
					 &tranfer_out, 0, NULL);
		CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)TransferThread,
		...
		DuplicateHandle(me, in_theirs, me, &child_startup_info.hStdInput, 0,
						TRUE, DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS);
		DuplicateHandle(me, out_theirs, me, &child_startup_info.hStdOutput, 0,
						TRUE, DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS);
		DuplicateHandle(me, err_theirs, me, &child_startup_info.hStdError, 0,
						TRUE, DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS);
		child_startup_info.dwFlags = STARTF_USESTDHANDLES;
	}

	// start the child program
	if (!CreateProcess(app_path, command_line, NULL, NULL, TRUE, 0, NULL, NULL,
					   &child_startup_info, &child_process_info)) {
...
BOOL WINAPI HandleCtrlEvent(DWORD type) {
	...
	if (wait &&
		WaitForSingleObject(child_process_info.hProcess, 100) ==
			WAIT_OBJECT_0) {
		GetExitCodeProcess(child_process_info.hProcess, &exit_code);
	} else {
...
void CopyFileBufferWithEscape(HANDLE output, char *buffer, DWORD size,
							  bool handle_escapes) {
	if (!handle_escapes) {
	...
	while (pos < size) {
		...
		while (next_pos < size &&
			   (buffer[next_pos] != '\x1B' ||
				(next_pos + 1 >= size || buffer[next_pos + 1] != '[')))
			++next_pos;
...
DWORD WINAPI TransferThread(Transfer *transfer) {
	while (true) {
		...
		// write
		CopyFileBufferWithEscape(transfer->to, buffer, bytes_read,
								 transfer->escapes);
	}
	return 0;
}
...
#endif
				attributes =
					(attributes & (FOREGROUND_COLOR | FOREGROUND_INTENSITY))
						<< 4 |
					(attributes & (BACKGROUND_COLOR | BACKGROUND_INTENSITY)) >>
						4;
