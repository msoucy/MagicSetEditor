...
const Char *BRIGHT = (L"\x1B[1m");
const Char *NORMAL = (L"\x1B[0m");
const Char *PARAM = (L"\x1B[36m");
const Char *FILE_EXT = (L"\x1B[0;1m");
const Char *GRAY = (L"\x1B[1;30m");
const Char *RED = (L"\x1B[1;31m");
const Char *YELLOW = (L"\x1B[1;33m");
const Char *ENDL = (L"\n");
...
bool StdHandleOk(DWORD std_handle) {
    // GetStdHandle sometimes returns an invalid handle instead of
...
void TextIOHandler::init() {
    ...
    if (have_console) {
        for (int i = 1; i < wxTheApp->argc; ++i) {
            if (String(wxTheApp->argv[i]) == (L"--color")) {
                escapes = true;
                break;
            }
        }
    ...
    static const Char *redirect_flags[] = {
        (L"-?"),        (L"--help"),          (L"-v"),
        (L"--version"), (L"--cli"),           (L"-c"),
        (L"--export"),  (L"--export-images"), (L"--create-installer")};
    for (int i = 1; i < wxTheApp->argc; ++i) {
        for (size_t j = 0;
             j < sizeof(redirect_flags) / sizeof(redirect_flags[0]); ++j) {
...
bool TextIOHandler::haveConsole() const { return have_console; }
...
void TextIOHandler::flush() {
    ...
    if (have_console) {
        ...
        // Show message box
        wxMessageBox(buffer, (L"Magic Set Editor"), wxOK | wxICON_INFORMATION);
        buffer.clear();
    }
}
...
String TextIOHandler::getLine() {
    ...
    while (!feof(stdin)) {
        ...
        result += buffer;
        if (result.GetChar(result.size() - 1) == (L'\n')) {
            // drop newline, done
...
bool TextIOHandler::canGetLine() { return !feof(stdin); }
...
void TextIOHandler::flushRaw() {
    ...
    if (!buffer.empty() && buffer.GetChar(buffer.size() - 1) != (L'\n')) {
        buffer += (L'\n');
    }
    // count newlines
    int newline_count = 0;
...
void TextIOHandler::show_message(MessageType type, String const &message) {
    ...
    if (type == MESSAGE_WARNING) {
        *this << YELLOW << (L"WARNING: ") << NORMAL
              << replace_all(message, (L"\n"), (L"\n         ")) << ENDL;
    } else {
        *this << RED << (L"ERROR: ") << NORMAL
              << replace_all(message, (L"\n"), (L"\n       ")) << ENDL;
    }
    flush();
    stream = stdout;
...
void TextIOHandler::print_pending_errors() {
    ...
    while (get_queued_message(type, msg)) {
        if (haveConsole()) {
            ...
            wxMessageBox(msg, wxMessageBoxCaptionStr,
                         type == MESSAGE_INFO
                             ? wxICON_INFORMATION
                             : type == MESSAGE_WARNING ? wxICON_WARNING
                                                       : wxICON_ERROR);
        }
    }
}
