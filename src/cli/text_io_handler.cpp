...
void TextIOHandler::init() {
	bool have_stderr;
		...
		static const Char* redirect_flags[] = {(L"-?"),(L"--help"),(L"-v"),(L"--version"),(L"--cli"),(L"-c"),(L"--export"),(L"--export-images"),(L"--create-installer")};
		for (int i = 1 ; i < wxTheApp->argc ; ++i) {
...
void TextIOHandler::flushRaw() {
	...
	// count newlines
	int newline_count = 0;
	for(const auto& c :buffer) if (c==(L'\n')) newline_count++;
	// write record
...
void TextIOHandler::show_message(MessageType type, String const& message) {
	...
	if (type == MESSAGE_WARNING) {
		*this << YELLOW << (L"WARNING: ") << NORMAL << replace_all(message,(L"\n"),(L"\n         ")) << ENDL;
	} else {
		*this << RED << (L"ERROR: ") << NORMAL << replace_all(message,(L"\n"),(L"\n       ")) << ENDL;
	}
	flush();
	stream = stdout;
	if (raw_mode) {
