...
class FileFormat : public IntrusivePtrVirtualBase {
  public:
    ...
    virtual String matches() { return (L"*.") + extension(); }
    /// Name of the filter
...
/** Formated as (L"All supported (type1,...)|type1,...|name|type|...|All
 * files(*.*)|*.*").
 *  For use in file selection dialogs.
 */
String import_formats();

// List of supported export formats that a set in a specific game can be
// exported.
 ...
 *  changing the recent set list could change the filename while we are opening
 * it
 *  (which would be bad)
 */
...
/** format_index specifies what format to use for saving, used as index in the
