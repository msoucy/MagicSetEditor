...
void export_mws(Window *parent, const SetP &set) {
    ...
    // Select filename
    String name =
        wxFileSelector((L"Export to file"), settings.default_export_dir, (L""),
                       (L""), (L"Text files (*.txt)|*.txt|All Files|*"),
    ...
    file.WriteString((L"Set exported using Magic Set Editor 2, version ") +
                     app_version.toString() + (L"\n\n"));
    wxDateTime now = wxDateTime::Now();
    file.WriteString((L"Spoiler List created on ") + now.FormatISODate() +
                     (L" ") + now.FormatISOTime());
    file.WriteString((L"\n\n"));
    ...
    for (auto &card : set->cards) {
        ...
        file.WriteString((L"\nCard Color:\t"));
        file.WriteString(
            card_color_mws(card->value((L"card color"))->toString()));
        file.WriteString((L"\nMana Cost:\t"));
        ...
        file.WriteString((L"\nRarity:\t\t"));
        file.WriteString(
            card_rarity_code(card->value((L"rarity"))->toString()));
        file.WriteString((L"\nCard #:\t\t"));
