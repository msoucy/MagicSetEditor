...
/// Serialize an object to a string, clipboard_package will be set to the given
/// package.
template <typename T>
...
template <typename T>
void deserialize_from_clipboard(T &object, Package &package,
								const String &data) {
...
CardsDataObject::CardsDataObject(const SetP &set, const vector<CardP> &cards) {
	// set the stylesheet, so when deserializing we know whos style options we
	// are reading
...
CardsDataObject::CardsDataObject() { SetFormat(format); }
...
KeywordDataObject::KeywordDataObject() { SetFormat(format); }
...
KeywordP KeywordDataObject::getKeyword(const SetP &set) {
	...
	deserialize_from_clipboard(data, *set, GetText());
	if (data.game_name != set->game->name())
		return KeywordP(); // Keyword is from a different game
	else
		return keyword;
}
...
CardsOnClipboard::CardsOnClipboard(const SetP &set,
