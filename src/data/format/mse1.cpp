...
class MSE1FileFormat : public FileFormat {
  public:
	...
	virtual String name() {
		return _("Magic Set Editor version 1 files (*.mse)");
	}
...
SetP MSE1FileFormat::importSet(const String &filename) {
	...
	if (format.substr(0, 8) != _("MTG Set8")) {
		throw ParseError(_(
			"Expected MSE format version 8\nTo convert files made with older "
			"versions of Magic Set Editor:\n  1. Download the latest version 1 "
			"from http:://magicsetedtitor.sourceforge.net\n  2. Open the set, "
	...
	while (!f.Eof()) {
		...
		case 'D': { // image filename
			LocalFileName image_file = set.newFileName(
				_("image"), _("")); // a new unique name in the package
		...
		default: { throw ParseError(_("Not a valid MSE1 file")); }
		}
	}
}
