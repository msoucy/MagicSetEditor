...
void ApprDistro::writeD(wxTextOutputStream &tout, const String &name, int c,
						int u, int r) {
	tout.WriteString(_("    ") + name + _("=R") << r << _(",U") << u << _(",C")
												<< c << _("\n"));
}
...
ApprCardRecord::ApprCardRecord(const Card &card, const String &sets_) {
	...
	sets =
		sets_ + _("-") + card_rarity_code(card.value(_("rarity"))->toString());
...
void ApprCardRecord::writeHead(wxDataOutputStream &strm) {
	...
	color = 0;
	int cnt = 0;
	if (cc.find_first_of(_('W')) != String::npos) {
		cnt += 1;
		color |= white;
	}
	if (cc.find_first_of(_('U')) != String::npos) {
		cnt += 1;
		color |= blue;
	}
	if (cc.find_first_of(_('B')) != String::npos) {
		cnt += 1;
		color |= black;
	}
	if (cc.find_first_of(_('R')) != String::npos) {
		cnt += 1;
		color |= red;
	}
	if (cc.find_first_of(_('G')) != String::npos) {
		cnt += 1;
		color |= green;
	}
	if (cnt > 1)
		color |= gold;
	if (cnt == 0) {
		if (type.find(_("Land")) != String::npos)
			color = land;
		else
			color = arti;
	}
...
ApprenticeExportWindow::ApprenticeExportWindow(Window *parent, const SetP &set)
	...
	if (!set->game->isMagic())
		throw Error(_("Can only export Magic sets to Apprentice"));
...
void ApprenticeExportWindow::onOk(wxCommandEvent &ev) {
	...
	progress_target =
		new ExportProgressDialog(this, _("Export to Apprentice"),
								 _("Exporting to Apprentice, please wait"));
...
bool ApprenticeExportWindow::exportSet() {
	...
	if (expan_it != expan.expansions.end()) {
		int res = wxMessageBox(
			_("There is already a set with the code '") + expan_it->first +
				_("' in the Apprentice database.\n") +
