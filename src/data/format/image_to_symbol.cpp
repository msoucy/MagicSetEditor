...
// This has to be done before includes, because the warning is reported in
// standard headers!
...
enum ImageMarker {
	...
	,
	MARKED = 2 // This cell is full, but it has been used as a starting point
			   // for finding symbols
};
...
bool is_mse1_symbol(const Image &img) {
	...
	if (img.GetWidth() != 60 || img.GetHeight() != 80)
		return false;
	...
	for (int x = 0; x < data.width; ++x) {
		for (int y = 0; y < data.height; ++y) {
			if (data(x, y) == FULL && data(x, y - 1) == EMPTY) {
				// the point above must be clear, we don't want to start in the
				// 'ground'
...
SymbolShapeP read_symbol_shape(const ImageData &data) {
	// find start point
	int xs, ys;
	if (!find_symbol_shape_start(data, xs, ys))
		return SymbolShapeP();
	data(xs, ys) |= MARKED;
	...
	xs += 1; // start right of the found point, otherwise last_move might think
			 // we came from above
	int x = xs, y = ys;
	int old_x = x, old_y = y;
	...
	do {
		...
		case 0x0110:
			y -= last_move;
			break; // diagonal, we can come here from two sides, from left and
				   // right
		case 0x0111:
			y -= 1;
			break; // last_move indicates which of {b,c} we are 'attached' to
		case 0x1000:
			x -= 1;
			break;
		case 0x1001:
		...
		shape->points.push_back(intrusive(
			new ControlPoint(double(x) / data.width, double(y) / data.height)));
		if (x > old_x)
			data(old_x, y) |= MARKED; // mark when moving right -> only mark the
									  // top of the shape
...
/** A corner is a point that has an angle between tangent greater then a
 * treshold
 */
void mark_corners(SymbolShape &shape) {
	for (int i = 0; (size_t)i < shape.points.size(); ++i) {
		...
		Vector2D before =
			.6 * shape.getPoint(i - 1)->pos + .2 * shape.getPoint(i - 2)->pos +
			.1 * shape.getPoint(i - 3)->pos + .1 * shape.getPoint(i - 4)->pos;
		Vector2D after =
			.6 * shape.getPoint(i + 1)->pos + .2 * shape.getPoint(i + 2)->pos +
			.1 * shape.getPoint(i + 3)->pos + .1 * shape.getPoint(i + 4)->pos;
...
void merge_corners(SymbolShape &shape) {
	for (int i = 0; (size_t)i < shape.points.size(); ++i) {
		...
		if (prev.lock != LOCK_FREE || cur.lock != LOCK_FREE)
			continue;
		// step 1. find tangent lines: try tangent lines to the first point, the
		// second, etc.
		// and take the one that has the largest angle with ab, i.e. the
		// smallest dot,
		...
		for (int j = 0; j < 4; ++j) {
			Vector2D a_ =
				(shape.getPoint(i - j - 1)->pos - prev.pos).normalized();
...
void merge_lines(SymbolShape &shape) {
	for (int i = 0; (size_t)i < shape.points.size(); ++i) {
		ControlPoint &cur = *shape.getPoint(i);
		if (cur.segment_before != cur.segment_after)
			continue;
		Vector2D a = shape.getPoint(i - 1)->pos, b = cur.pos,
				 c = shape.getPoint(i + 1)->pos;
		Vector2D ab = (a - b).normalized();
		Vector2D bc = (b - c).normalized();
		double angle_len =
			abs(atan2(ab.x, ab.y) - atan2(bc.x, bc.y)) * (a - c).lengthSqr();
...
double cost_of_point_removal(SymbolShape &shape, int i) {
	...
	ControlPoint &prev = *shape.getPoint(i - 1);
	ControlPoint &next = *shape.getPoint(i + 1);
	if (cur.lock != LOCK_DIR)
		return 1e100; // don't remove corners
	...
	// cost is distance to new point * length of line ~= area added/removed from
	// shape
