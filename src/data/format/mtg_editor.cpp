	...
	while (!f.Eof()) {
		...
		if        (line == (L"#SET###########")) {										// set.title
			...
			set_date = file.ReadLine();
		           ...
		           line == (L"#FONTS#########") || line == (L"#COUNT#########")) {		// ignore
			target = nullptr;
			file.ReadLine();
		} else if (line == (L"#LAYOUT########")) {										// layout type
			target = nullptr;
			layout = file.ReadLine();
		} else if (line == (L"#CARD##########") || line == (L"#EOF###########")) {	// begin/end of card
			if (!first) {
		...
		} else if (line == (L"#RARITY########") || line == (L"#FREQUENCY#####")) {	// rarity
			target = 0;
			line = file.ReadLine();
			if      (line == (L"0")) current_card->value((L"rarity")) = to_script((L"common"));
			else if (line == (L"1")) current_card->value((L"rarity")) = to_script((L"uncommon"));
			else                     current_card->value((L"rarity")) = to_script((L"rare"));
		...
		} else if (line == (L"#ILLUSTRATION##") || line == (L"#ILLUSTRATION8#")) {		// image
			target = 0;
			line = file.ReadLine();
			if (!wxFileExists(line)) {
				// based on card name and date
				line = filter1(filename) + set_date + (L"/") +
				       filter2(current_card->value((L"name"))->toString()) + card_date + (L".jpg");
			}
			// copy image into set
			if (wxFileExists(line)) {
				FileName image_file = set->newFileName((L"image"),(L""));
				if (wxCopyFile(line, set->nameOut(image_file), true)) {
					current_card->value((L"image")) = script_local_image_file(image_file);
				}
			}
		} else if (line == (L"#TOMBSTONE#####")) {										// tombstone
			target = 0;
			line = file.ReadLine();
			current_card->value((L"card symbol")) = to_script(
				line==(L"TRUE") ? (L"tombstone") : (L"none")
			);
		} else {
			// normal text
			if (target != 0) {															// value of a text field
				...
				throw ParseError((L"Error in Mtg Editor file, unexpected text:\n") + line);
			}
		}
	}
...
void MtgEditorFileFormat::translateTags(String& value) {
	...
	while (pos < value.size()) {
		...
		if (c == (L'<')) {
			...
			if (tag==(L"W") || tag==(L"U") || tag==(L"B") || tag==(L"R") || tag==(L"G") || tag==(L"X") || tag==(L"Y") || tag==(L"Z")) {
				ret += (L"<sym>") + tag + (L"</sym>");
