...
SetP MtgEditorFileFormat::importSet(const String &filename) {
	...
	while (!f.Eof()) {
		...
		if (line == _("#SET###########")) { // set.title
		...
		} else if (line == _("#RARITY########") ||
				   line == _("#FREQUENCY#####")) { // rarity
			target = 0;
			line = file.ReadLine();
			if (line == _("0"))
				current_card->value(_("rarity")) = to_script(_("common"));
			else if (line == _("1"))
				current_card->value(_("rarity")) = to_script(_("uncommon"));
			else
				current_card->value(_("rarity")) = to_script(_("rare"));
		...
		} else if (line == _("#ILLUSTRATION##") ||
				   line == _("#ILLUSTRATION8#")) { // image
			target = 0;
			line = file.ReadLine();
			if (!wxFileExists(line)) {
				// based on card name and date
				line = filter1(filename) + set_date + _("/") +
					   filter2(current_card->value(_("name"))->toString()) +
					   card_date + _(".jpg");
			}
			// copy image into set
			if (wxFileExists(line)) {
			...
			target = 0;
			line = file.ReadLine();
			current_card->value(_("card symbol")) =
				to_script(line == _("TRUE") ? _("tombstone") : _("none"));
		} else {
			// normal text
			if (target != 0) { // value of a text field
				...
				throw ParseError(
					_("Error in Mtg Editor file, unexpected text:\n") + line);
			}
		}
	}
...
void MtgEditorFileFormat::translateTags(String &value) {
	...
	while (pos < value.size()) {
		...
		if (c == _('<')) {
			...
			if (tag == _("W") || tag == _("U") || tag == _("B") ||
				tag == _("R") || tag == _("G") || tag == _("X") ||
				tag == _("Y") || tag == _("Z")) {
				ret += _("<sym>") + tag + _("</sym>");
