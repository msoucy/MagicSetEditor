...
StatsDimension::StatsDimension()
	: automatic(false), position_hint(0), numeric(false), bin_size(0),
	  show_empty(false), split_list(false) {}

StatsDimension::StatsDimension(const Field &field)
	: automatic(true), name(field.name), description(field.description),
	  position_hint(field.position_hint), icon_filename(field.icon_filename),
	  numeric(false), show_empty(false), split_list(false) {
	// choice field?
	...
	if (choice_field) {
		...
		for (auto const &g : choice_field->choices->choices) {
...
StatsCategory::StatsCategory()
	: automatic(false), position_hint(0), type(GRAPH_TYPE_BAR) {}

StatsCategory::StatsCategory(const StatsDimensionP &dim)
	: automatic(true), name(dim->name), description(dim->description),
	  position_hint(dim->position_hint), icon_filename(dim->icon_filename),
	  dimensions(1, dim), type(GRAPH_TYPE_BAR) {}
...
void StatsCategory::find_dimensions(const vector<StatsDimensionP> &available) {
	if (!dimensions.empty())
		return;
	for (auto const &n : dimension_names) {
		StatsDimensionP dim;
		for (auto const &d : available) {
			if (d->name == n) {
				dim = d;
				break;
			}
