...
SymbolFontP SymbolFont::byName(const String &name) {
    return package_manager.open<SymbolFont>(
        name.size() > 16 &&
                is_substr(name, name.size() - 16, (L".mse-symbol-font"))
            ? name
            : name + (L".mse-symbol-font"));
}
...
Image SymbolInFont::getImage(Package &pkg, double size) {
    // generate new image
    if (!image.isReady()) {
        throw Error((L"No image specified for symbol with code '") + code +
                    (L"' in symbol font."));
    }
...
void SymbolFont::split(const String &text, SplitSymbols &out) const {
    ...
    for (size_t pos = 0; pos < text.size();) {
                ...
                if (sym->regex) {
                    ...
                    if (sym->code_regex.matches(results, text.begin() + pos,
                        ...
                        if (sym->draw_text >= 0 &&
                            ...
                            out.push_back(
                                DrawableSymbol(results.str(), (L""), *sym));
                        }
                        pos += results.length();
                        goto next_symbol;
                    }
                    ...
                    if (is_substr(text, pos, sym->code)) {
                        out.push_back(DrawableSymbol(
                            sym->code, sym->draw_text >= 0 ? sym->code : (L""),
                            *sym));
                        pos += sym->code.size();
        ...
        // out.push_back(DrawableSymbol(text.substr(pos, 1), (L""),
        // defaultSymbol()));
        pos += 1;
    next_symbol:;
    }
...
SymbolInFont *SymbolFont::defaultSymbol() const {
    for (const auto &sym : symbols) {
        if (sym->enabled && sym->regex && sym->code_regex.matches((L"0")))
            return sym.get();
    }
    return nullptr;
}
    ...
    if (name().empty()) {
        ...
        if (starts_with(name(), (L"/:NO-WARN-DEP:"))) {
            ...
            StyleSheetP stylesheet =
                from_script<StyleSheetP>(ctx.getVariable((L"stylesheet")));
