...
// SymbolFont that is used for SymbolInFonts constructed with the default
// constructor
...
SymbolFont::SymbolFont()
	: img_size(12), spacing(1, 1), scale_text(false),
	  processed_insert_symbol_menu(nullptr) {}

SymbolFont::~SymbolFont() { delete processed_insert_symbol_menu; }
...
SymbolFontP SymbolFont::byName(const String &name) {
	return package_manager.open<SymbolFont>(
		name.size() > 16 &&
				is_substr(name, name.size() - 16, _(".mse-symbol-font"))
			? name
			: name + _(".mse-symbol-font"));
}
...
class SymbolInFont : public IntrusivePtrBase<SymbolInFont> {
  public:
	...
	/** This is the size of the resulting image, it does NOT convert back to
	...
	int draw_text; ///< The index of the captured regex expression to draw, or
				   ///-1 to not draw text
  ...
  private:
	...
	wxSize
		actual_size; ///< Actual image size, only known after loading the image
...
SymbolInFont::SymbolInFont()
	: enabled(true), regex(false), draw_text(-1),
	  text_alignment(ALIGN_MIDDLE_CENTER), text_margin_left(0),
	  text_margin_right(0), text_margin_top(0), text_margin_bottom(0),
	  actual_size(0, 0) {
...
Image SymbolInFont::getImage(Package &pkg, double size) {
	// generate new image
	if (!image.isReady()) {
		throw Error(_("No image specified for symbol with code '") + code +
					_("' in symbol font."));
	}
	...
	Image resampled_image((int)(actual_size.GetWidth() * size / img_size),
						  (int)(actual_size.GetHeight() * size / img_size),
						  false);
...
Bitmap SymbolInFont::getBitmap(Package &pkg, wxSize size) {
	...
	return Bitmap(image.generate(
		GeneratedImage::Options(size.x, size.y, &pkg, nullptr, ASPECT_BORDER)));
}
...
void SymbolFont::split(const String &text, SplitSymbols &out) const {
	...
	for (size_t pos = 0; pos < text.size();) {
		...
		for (auto const &sym : symbols) {
			if (!sym->code.empty() && sym->enabled) {
				if (sym->regex) {
					...
					if (sym->code_regex.matches(results, text.begin() + pos,
												text.end()) &&
						results.position() == 0 &&
						results.length() > 0) { // Matches the regex
						if (sym->draw_text >= 0 &&
							sym->draw_text < (int)results.size()) {
							out.push_back(DrawableSymbol(
								results.str(), results.str(sym->draw_text),
								*sym));
						} else {
							out.push_back(
								DrawableSymbol(results.str(), _(""), *sym));
						}
						pos += results.length();
						goto next_symbol;
					}
					...
					if (is_substr(text, pos, sym->code)) {
						out.push_back(DrawableSymbol(
							sym->code, sym->draw_text >= 0 ? sym->code : _(""),
							*sym));
						pos += sym->code.size();
		...
		// out.push_back(DrawableSymbol(text.substr(pos, 1), _(""),
		// defaultSymbol()));
		pos += 1;
	next_symbol:
		;
		...
		for (auto const &sym : symbols) {
			if (!sym->code.empty() && sym->enabled) {
				if (sym->regex) {
					Regex::Results results;
					if (!sym->code_regex.empty() &&
						sym->code_regex.matches(results, text.begin() + pos,
												text.end()) &&
						results.position() == 0 &&
						results.length() > 0) { // Matches the regex
						pos += results.length();
...
SymbolInFont *SymbolFont::defaultSymbol() const {
	for (auto const &sym : symbols) {
		if (sym->enabled && sym->regex && sym->code_regex.matches(_("0")))
			return sym.get();
	}
	return nullptr;
}
...
void SymbolFont::draw(RotatedDC &dc, Context &ctx, const RealRect &rect,
					  double font_size, const Alignment &align,
					  const String &text) {
...
void SymbolFont::draw(RotatedDC &dc, RealRect rect, double font_size,
					  const Alignment &align, const SplitSymbols &text) {
	for (auto const &sym : text) {
...
void SymbolFont::drawSymbol(RotatedDC &dc, RealRect sym_rect, double font_size,
							const Alignment &align, SymbolInFont &sym,
							const String &text) {
	// 1. draw symbol
	// find bitmap
	...
	sym_rect.width -=
		font_size * (sym.text_margin_left + sym.text_margin_right);
	sym_rect.height -=
		font_size * (sym.text_margin_top + sym.text_margin_bottom);
	...
	while (true) {
		...
		if (ts.height <= sym_rect.height) {
			if (ts.width <= sym_rect.width) {
				break; // text fits
			} else if (ts.width * sym.text_font->max_stretch <=
					   sym_rect.width) {
...
Image SymbolFont::getImage(double font_size, const DrawableSymbol &sym) {
	if (!sym.symbol)
		return Image(1, 1);
	if (sym.draw_text.empty() || !sym.symbol->text_font)
	...
	sym_rect.width -= font_size * (sym.symbol->text_margin_left +
								   sym.symbol->text_margin_right);
	sym_rect.height -= font_size * (sym.symbol->text_margin_top +
	...
	while (true) {
		if (size <= 0)
			return sym.symbol->getImage(*this, font_size); // text too small
		rdc.SetFont(*sym.symbol->text_font, size / sym.symbol->text_font->size);
		ts = rdc.GetTextExtent(sym.draw_text);
		if (ts.height <= sym_rect.height) {
			if (ts.width <= sym_rect.width) {
				break; // text fits
			} else if (ts.width * sym.symbol->text_font->max_stretch <=
					   sym_rect.width) {
				stretch = sym_rect.width / ts.width;
				ts.width = sym_rect.width; // for alignment
				break;
	...
	// align text
	RealPoint text_pos =
		align_in_rect(sym.symbol->text_alignment, ts, sym_rect);
	// draw text
	rdc.DrawTextWithShadow(sym.draw_text, *sym.symbol->text_font, text_pos,
						   font_size, stretch);
	// done
...
void SymbolFont::getCharInfo(RotatedDC &dc, Context &ctx, double font_size,
							 const String &text, vector<CharInfo> &out) {
	SplitSymbols symbols;
	update(ctx);
	split(text, symbols);
...
void SymbolFont::getCharInfo(RotatedDC &dc, double font_size,
							 const SplitSymbols &text, vector<CharInfo> &out) {
	for (auto const &sym : text) {
		...
		for (size_t i = 0; i < count; ++i) {
			out.push_back(
				CharInfo(size, i == count - 1 ? BREAK_MAYBE : BREAK_NO));
		}
	}
}
...
wxMenu *SymbolFont::insertSymbolMenu(Context &ctx) {
	if (!processed_insert_symbol_menu && insert_symbol_menu) {
		...
		processed_insert_symbol_menu =
			insert_symbol_menu->makeMenu(ID_INSERT_SYMBOL_MENU_MIN, *this);
	}
...
String SymbolFont::insertSymbolCode(int menu_id) const {
	...
	if (insert_symbol_menu) {
		return insert_symbol_menu->getCode(menu_id - ID_INSERT_SYMBOL_MENU_MIN,
										   *this);
	} else {
...
InsertSymbolMenu::InsertSymbolMenu() : type(ITEM_CODE) {}
...
wxMenuItem *InsertSymbolMenu::makeMenuItem(wxMenu *parent, int first_id,
										   SymbolFont &font) const {
	...
	if (type == ITEM_SUBMENU) {
		wxMenuItem *item =
			new wxMenuItem(parent, wxID_ANY, menu_name, wxEmptyString,
						   wxITEM_NORMAL, makeMenu(first_id, font));
...
template <>
void GetDefaultMember::handle(const InsertSymbolMenu &m) {
	handle(m.name);
}
template <>
void GetMember::handle(const InsertSymbolMenu &m) {
...
SymbolFontRef::SymbolFontRef()
	: size(12), scale_down_to(1), alignment(ALIGN_MIDDLE_CENTER) {}

bool SymbolFontRef::valid() const { return !!font; }
...
void SymbolFontRef::initDependencies(Context &ctx,
...
void SymbolFontRef::loadFont(Context &ctx) {
	if (name().empty()) {
		...
		if (starts_with(name(), _("/:NO-WARN-DEP:"))) {
			// ensure the dependency on the font is present in the stylesheet
			// this ref is in
			...
			StyleSheetP stylesheet =
				from_script<StyleSheetP>(ctx.getVariable(_("stylesheet")));
