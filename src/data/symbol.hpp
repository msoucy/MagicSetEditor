...
enum SegmentMode { SEGMENT_LINE, SEGMENT_CURVE };
...
class ControlPoint : public IntrusivePtrBase<ControlPoint> {
  public:
    ...
    Vector2D
        delta_before; ///< delta to bezier control point, for curve before point
    Vector2D
        delta_after; ///< delta to bezier control point, for curve after point
    ...
    /// Constructor for curves lines, takes postions, delta_before, delta_after
    /// and lock mode
    ControlPoint(double x, double y, double xb, double yb, double xa, double ya,
                 LockMode lock = LOCK_FREE);

    /// Must be called after delta_before/delta_after has changed, enforces lock
    /// constraints
...
class SelectedHandle {
  public:
    ...
    SelectedHandle(const WhichHandle &handle) : handle(handle) {
        assert(handle == HANDLE_NONE);
    }
    SelectedHandle(const ControlPointP &point, const WhichHandle &handle)
    ...
    bool SelectedHandle::operator == (const ControlPointP pnt) const { return
    point == pnt; }
    bool operator == (const WhichHandle& wh) const;
    bool SelectedHandle::operator == (const WhichHandle& wh) const          {
    return  handle == wh; }
    bool operator ! () const;
    bool SelectedHandle::operator ! () const                         { return
    handle == handleNone; }
    */
};
...
class Bounds {
  public:
    ...
    inline Bounds(const Vector2D &min, const Vector2D &max)
        : min(min), max(max) {}
    ...
    inline operator RealRect() const {
        return RealRect(min, RealSize(max - min));
    }
};
...
class SymbolPart : public IntrusivePtrVirtualBase {
  public:
    ...
    /// Calculate the position and size of the part using the given rotation
    /// matrix
    virtual Bounds calculateBounds(const Vector2D &origin, const Matrix2D &m,
                                   bool is_identity) = 0;
...
class SymbolShape : public SymbolPart {
  public:
    ...
    // Center of rotation, relative to the part, when the part is scaled to
    // [0..1]
...
class SymbolGroup : public SymbolPart {
  public:
    vector<SymbolPartP>
        parts; ///< The parts in this group, first item is on top
...
enum SymbolSymmetryType { SYMMETRY_ROTATION, SYMMETRY_REFLECTION };
...
class SymbolSymmetry : public SymbolGroup {
  public:
    ...
    int copies;      ///< How many times is the orignal reflected (including the
                     ///original itself)
    bool clip;       ///< Clip the orignal so it doesn't intersect the mirror(s)
    Vector2D center; ///< Center point of the mirror
    Vector2D handle; ///< A handle pointing in the direction of the original,
...
class SymbolView : public ActionListener {
  ...
  protected:
    /// The symbol that is currently being viewed, should not be modified
    /// directly!
    SymbolP symbol;
