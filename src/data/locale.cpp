...
void ignore_add_pragma(String &str) {
	if (starts_with(str, _("#_ADD ")))
		str = str.substr(6);
	else if (starts_with(str, _("#_ADD")))
		str = str.substr(5);
	else if (starts_with(str, _("#_DEL")))
		str.clear();
}
...
bool match_wildcard(const String &wildcard, const String &name) {
	return Regex(replace_all(replace_all(wildcard, _("."), _("\\.")), _("*"),
							 _(".*"))).matches(name);
}
...
String SubLocale::tr(const String &subcat, const String &key,
	...
	map<String, String>::const_iterator it =
		translations.find(subcat + _("_") + canonical_name_form(key));
	if (it == translations.end()) {
		return def(key);
	} else {
		return it->second;
...
InputStreamP load_resource_text(const String &name) {
	...
	if (hData == 0)
		throw InternalError(String::Format(_("Resource not text: %s"), name));
...
void Locale::validate(Version ver) {
	...
	// validate
	String errors;
	errors += translations[LOCALE_CAT_MENU].validate(_("menu"),
													 v.sublocales[_("menu")]);
	errors += translations[LOCALE_CAT_HELP].validate(_("help"),
													 v.sublocales[_("help")]);
	errors += translations[LOCALE_CAT_TOOL].validate(_("tool"),
													 v.sublocales[_("tool")]);
	errors += translations[LOCALE_CAT_TOOLTIP].validate(
		_("tooltip"), v.sublocales[_("tooltip")]);
	errors += translations[LOCALE_CAT_LABEL].validate(_("label"),
													  v.sublocales[_("label")]);
	errors += translations[LOCALE_CAT_BUTTON].validate(
		_("button"), v.sublocales[_("button")]);
	errors += translations[LOCALE_CAT_TITLE].validate(_("title"),
													  v.sublocales[_("title")]);
	errors += translations[LOCALE_CAT_ACTION].validate(
		_("action"), v.sublocales[_("action")]);
	errors += translations[LOCALE_CAT_ERROR].validate(_("error"),
													  v.sublocales[_("error")]);
	errors += translations[LOCALE_CAT_TYPE].validate(_("type"),
													 v.sublocales[_("type")]);
	// errors?
	if (!errors.empty()) {
	...
	for (auto const &kc : v->keys) {
		...
		if (it == translations.end()) {
			...
			errors +=
				...
				_(": ") + kc.first +
				String::Format(_("\t  expected: %d, found %d"), kc.second.args,
							   string_format_args(it->second));
		}
	}
	// 2. keys in this but not in v
	for (auto const &kv : translations) {
