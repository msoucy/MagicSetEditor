...
class ChoiceField : public Field {
  public:
	...
	map<String, Color>
		choice_colors; ///< Colors for the various choices (when color_cardlist)
	map<String, Color> choice_colors_cardlist; ///< Colors for the various
											   ///choices, for in the card list

	virtual void after_reading(Version ver);
};

enum ChoiceChoiceType { CHOICE_TYPE_CHECK, CHOICE_TYPE_RADIO };
...
class ChoiceField::Choice : public IntrusivePtrBase<ChoiceField::Choice> {
  public:
	...
	String default_name;	  ///< A default item, if this is a group and
							  ///default_name.empty() there is no default
	...
	ChoiceChoiceType type; ///< How should this item be shown, only for multiple
						   ///choice fields
	/// First item-id in this group (can be the default item)
	/** Item-ids are consecutive integers, a group uses all ids
	 * [first_id..lastId()).
	...
	/** For a single choice this is always true, for a group only if it has a
	 * default choice */
	...
	/** The internal name is formed by concatenating the names of all parents,
	 * separated by spaces.
	 *  Returns "" if id is not in this group
	 */
	String choiceName(int id) const;
	/// Formated name of a choice.
	/** Intended for use in menu structures, so it doesn't include the group
	 * name for children.
	 *  Returns "" if id is not in this group.
	 */
...
enum ChoicePopupStyle { POPUP_MENU, POPUP_DROPDOWN, POPUP_DROPDOWN_IN_PLACE };
...
enum ChoiceRenderStyle {
	...
	,
	RENDER_CHECKLIST =
		0x100 // render as a checklist, intended for multiple choice
	,
	RENDER_LIST =
		0x200 // render as a list of images/text, intended for multiple choice
	,
...
class ChoiceStyle : public Style {
  public:
	...
	map<String, ScriptableImage>
		choice_images; ///< Images for the various choices (when RENDER_IMAGE)
	...
	vector<ThumbnailStatus>
		thumbnails_status; ///< Which thumbnails are up to date?
