...
ChoiceField::ChoiceField() : choices((Choice *)new Choice) {}
...
void ChoiceField::after_reading(Version ver) {
	...
	if (initial == script_default_nil &&
		!dynamic_cast<MultipleChoiceField *>(this)) {
...
ChoiceField::Choice::Choice(const String &name, const String &caption)
	: name(name), caption(caption), line_below(false), enabled(true),
	  type(CHOICE_TYPE_CHECK), first_id(0) {}

bool ChoiceField::Choice::isGroup() const { return !choices.empty(); }
...
int ChoiceField::Choice::choiceCount() const { return lastId() - first_id; }
...
int ChoiceField::Choice::choiceId(const String &search_name) const {
	if (hasDefault() && search_name == name) {
		return first_id;
	} else if (name.empty()) { // no name for this group, forward to all
							   // children
...
IMPLEMENT_REFLECTION(ChoiceField::Choice) {
	REFLECT_IF_READING_COMPOUND_OR(isGroup() || line_below ||
								   enabled.isScripted()) {
...
ChoiceStyle::ChoiceStyle(const ChoiceFieldP &field)
	: Style(field), popup_style(POPUP_DROPDOWN), render_style(RENDER_TEXT),
	  choice_images_initialized(false), combine(COMBINE_NORMAL),
	  alignment(ALIGN_STRETCH), thumbnails(nullptr), content_width(0.0),
	  content_height(0.0) {}

ChoiceStyle::~ChoiceStyle() { delete thumbnails; }

void ChoiceStyle::initImage() {
	if (image.isSet() || choice_images.empty())
		return;
	// for, for example:
	//     choice images:
	//          a: {uvw}
...
int ChoiceStyle::update(Context &ctx) {
	...
	int change = Style::update(ctx) | font.update(ctx) * CHANGE_OTHER;
	if (!choice_images_initialized) {
		// we only want to do this once because it is rather slow, other updates
		// are handled by dependencies
...
void ChoiceStyle::initDependencies(Context &ctx, const Dependency &dep) const {
	...
	for (auto const &ci : choice_images) {
		ci.second.initDependencies(ctx, dep);
	}
}
void ChoiceStyle::checkContentDependencies(Context &ctx,
...
void ChoiceStyle::invalidate() {
	...
	for (int i = 0; i < end; ++i) {
		if (thumbnails_status[i] == THUMB_OK)
