...
BooleanStyle::BooleanStyle(const ChoiceFieldP &field) : ChoiceStyle(field) {
	...
	choice_images[_("true")] =
		ScriptableImage(intrusive(new BuiltInImage(_("bool_yes"))));
	choice_images[_("false")] =
		ScriptableImage(intrusive(new BuiltInImage(_("bool_no"))));
}
...
void BooleanStyle::after_reading(Version ver) {
	...
	if (!choice_images[_("true")].isScripted() &&
		choice_images.find(_("yes")) != choice_images.end()) {
		choice_images[_("true")] = choice_images[_("yes")];
	}
	if (!choice_images[_("false")].isScripted() &&
		choice_images.find(_("no")) != choice_images.end()) {
