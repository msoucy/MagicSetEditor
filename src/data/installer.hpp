...
//  - Installer              = an actual package available in memory, containing
//  packages to be installed
//  - DownloadableInstaller  = an installar (possibly) not yet available, i.e.
//  just its URL
//  - PackageDescription     = description of a package version
//  - InstallablePackage     = the complete status of a package, both local and
//  remote
...
/** Installers will be sent around the internet, etc. so they are fairly
 * selfcontained.
 */
...
class PackageDescription : public IntrusivePtrBase<PackageDescription> {
  public:
    ...
    /** Usually one of the descriptions will refer to the locally installed one,
     * the other to the new one */
...
class DownloadableInstaller : public IntrusivePtrBase<DownloadableInstaller> {
  public:
    ...
    String installer_file; ///< The temp file where the installer can be found
                           ///(after downloading)
    bool downloadable; ///< Is the installer downloadable (in)directly from that
                       ///url?
    vector<PackageDescriptionP>
        packages; ///< Packages provided by this installer
...
enum PackageStatus {
    ...
    PACKAGE_INSTALLER =
        0x0010 ///< Package can be installed (there is an installer)
    ,
    PACKAGE_INSTALLABLE =
        0x0110 ///< Package can be installed (and it makes sense to do so)
    ,
    PACKAGE_NOT_UP_TO_DATE =
        0x0200 ///< The local installed version (if any) is not up to date
    ,
...
// bit twidling
inline PackageAction operator|(PackageAction a, PackageAction b) {
    return (PackageAction)((int)a | (int)b);
}
...
class InstallablePackage : public IntrusivePtrVirtualBase {
  public:
    /// A new package
    InstallablePackage(const PackageDescriptionP &,
                       const DownloadableInstallerP &);
    /// An installed package
    InstallablePackage(const PackageDescriptionP &, const PackageVersionP &);

    PackageDescriptionP description; ///< The details of the package. Either
                                     ///from the installed package or from an
                                     ///installer
    PackageVersionP
        installed; ///< The information of the installed package (if installed)
    DownloadableInstallerP
        installer; ///< The installer to install from (if updates are available)
    PackageStatus status; ///< Status of installation
    PackageAction action; ///< What to do with this package?

    int automatic; ///< Install/upgrade/remove automaticly to satisfy this many
                   ///packages
...
/** The first list contains installed packages, the second list contains
 * packages in an installer
 *  @pre both lists are sorted by package name
 *  @post the output will be sorted
 */
void merge(InstallablePackages &installed,
           const InstallablePackages &from_installer);

/// Merge the packages from a DownloadableInstaller into a list of
/// InstallablePackages.
void merge(InstallablePackages &installed,
           const DownloadableInstallerP &installer);

/// Set the action to perform on a given package, makes sure the dependencies
