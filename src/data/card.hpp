...
class Card : public IntrusivePtrVirtualBase {
  public:
     ...
     *  If stylesheet is set then contains data for the this->stylesheet,
     * otherwise for set->stylesheet
     */
    ...
    vector<std::pair<Value *, const Keyword *>> keyword_usage;

    /// Get the identification of this card, an identification is something like
    /// a name, title, etc.
    /** May return "" */
...
inline String type_name(const Card &) { return _TYPE_("card"); }
inline String type_name(const vector<CardP> &) {
    return _TYPE_("cards"); // not actually used, only for locale.pl script
}

void mark_dependency_member(const Card &value, const String &name,
