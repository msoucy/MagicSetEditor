...
class ExportTemplate : public Packaged {
  public:
    ...
    bool create_directory; ///< The export creates a directory for additional
                           ///data files
    vector<FieldP> option_fields;          ///< Options for exporting
    IndexMap<FieldP, StyleP> option_style; ///< Style of the options
    OptionalScript
        script; ///< Export script, for multi file templates and initialization
...
struct ExportInfo {
    ...
    ///  When using the CLI, this can be a fake package to allow reading from
    ///  the cwd
    String directory_relative; ///< The directory for storing extra files (or ""
                               ///if !export->create_directory)
                               ///  This is just the directory name
    String directory_absolute; ///< The absolute path of the directory
    map<String, wxSize> exported_images; ///< Images (from symbol font) already
                                         ///exported, and their size
    bool allow_writes_outside; ///< Can files outside the directory be written
                               ///to?
};
