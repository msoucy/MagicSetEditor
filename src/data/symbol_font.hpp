...
class SymbolFont : public Packaged {
  public:
    ...
    class DrawableSymbol {
      public:
        inline DrawableSymbol(const String &text, const String &draw_text,
                              SymbolInFont &symbol)
    ...
    /// Split a string into separate symbols for drawing and for determining
    /// their size
    void split(const String &text, SplitSymbols &out) const;

    /// How many consecutive characters of the text, starting at start can be
    /// rendered with this symbol font?
    size_t recognizePrefix(const String &text, size_t start) const;

    /// Draw a piece of text
    void draw(RotatedDC &dc, Context &ctx, const RealRect &rect,
              double font_size, const Alignment &align, const String &text);
    /// Get information on characters in a string
    void getCharInfo(RotatedDC &dc, Context &ctx, double font_size,
                     const String &text, vector<CharInfo> &out);

    /// Draw a piece of text prepared using split
    void draw(RotatedDC &dc, RealRect rect, double font_size,
              const Alignment &align, const SplitSymbols &text);
    /// Get information on characters in a string
    void getCharInfo(RotatedDC &dc, double font_size, const SplitSymbols &text,
                     vector<CharInfo> &out);
     ...
     *  All ids used will be in the range
     * ID_INSERT_SYMBOL_MENU_MIN...ID_INSERT_SYMBOL_MENU_MAX.
  ...
  private:
    ...
    void drawSymbol(RotatedDC &dc, RealRect sym_rect, double font_size,
...
class InsertSymbolMenu : public IntrusivePtrBase<InsertSymbolMenu> {
  public:
    ...
    wxMenuItem *makeMenuItem(wxMenu *parent, int first_id,
