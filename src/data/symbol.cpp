...
ControlPoint::ControlPoint(double x, double y)
	: pos(x, y), segment_before(SEGMENT_LINE), segment_after(SEGMENT_LINE),
	  lock(LOCK_FREE) {}
ControlPoint::ControlPoint(double x, double y, double xb, double yb, double xa,
						   double ya, LockMode lock)
	: pos(x, y), delta_before(xb, yb), delta_after(xa, ya),
	  segment_before(SEGMENT_CURVE), segment_after(SEGMENT_CURVE), lock(lock) {}

void ControlPoint::onUpdateHandle(WhichHandle wh) {
	// One handle has changed, update only the other one
	if (lock == LOCK_DIR) {
		getOther(wh) =
			-getHandle(wh) * getOther(wh).length() / getHandle(wh).length();
...
bool Bounds::contains(const Bounds &b) const {
	return b.min.x >= min.x && b.min.y >= min.y && b.max.x <= max.x &&
		   b.max.y <= max.y;
}

Vector2D Bounds::corner(int dx, int dy) const {
	return Vector2D(0.5 * (min.x + max.x + dx * (max.x - min.x)),
...
SymbolPartP read_new<SymbolPart>(Reader &reader) {
	...
	if (type == _("shape") || type.empty())
		return intrusive(new SymbolShape);
	else if (type == _("symmetry"))
		return intrusive(new SymbolSymmetry);
	else if (type == _("group"))
		return intrusive(new SymbolGroup);
	else {
...
void fix(Reader &reader, SymbolShape &shape) {
	...
	if (reader.file_app_version != Version())
		return;
	shape.updateBounds();
	if (shape.bounds.max.x < 100 || shape.bounds.max.y < 100)
		return;
...
String SymbolShape::typeName() const { return _("shape"); }
...
Bounds SymbolShape::calculateBounds(const Vector2D &origin, const Matrix2D &m,
									bool is_identity) {
	Bounds bounds;
	for (int i = 0; i < (int)points.size(); ++i) {
		bounds.update(
			segment_bounds(origin, m, *getPoint(i), *getPoint(i + 1)));
	}
	if (is_identity)
		this->bounds = bounds;
	return bounds;
...
SymbolSymmetry::SymbolSymmetry() : kind(SYMMETRY_ROTATION), copies(2) {}

String SymbolSymmetry::typeName() const { return _("symmetry"); }
...
String SymbolSymmetry::expectedName() const {
	return capitalize(kind == SYMMETRY_ROTATION ? _TYPE_("rotation")
												: _TYPE_("reflection")) +
		   String::Format(_(" (%d)"), copies);
}

Bounds SymbolSymmetry::calculateBounds(const Vector2D &origin,
									   const Matrix2D &m, bool is_identity) {
	Bounds bounds;
	...
	for (auto const &p : parts) {
		for (int i = 0; i < copies; ++i) {
			...
			if (kind == SYMMETRY_ROTATION || i % 2 == 0) {
				...
				bounds.update(
					p->calculateBounds(origin + (center - center * rot) * m,
									   rot * m, is_identity && i == 0));
			} else {
...
SymbolGroup::SymbolGroup() { name = capitalize(_TYPE_("group")); }

String SymbolGroup::typeName() const { return _("group"); }
...
bool SymbolGroup::isAncestor(const SymbolPart &that) const {
	if (this == &that)
		return true;
	for (auto const &p : parts) {
		if (p->isAncestor(that))
			return true;
	}
	return false;
}

Bounds SymbolGroup::calculateBounds(const Vector2D &origin, const Matrix2D &m,
									bool is_identity) {
	Bounds bounds;
	for (auto const &p : parts) {
...
double Symbol::aspectRatio() const {
	...
	// Multiply by 2 (below) to give something in the range [0...1] i.e.
	// [touches the edge...only in the middle]
	...
	// e.g. if the vertical margin is more then the horizontal one, the symbol
	// is 'flat'
...
SymbolView::~SymbolView() {
	if (symbol)
		symbol->actions.removeListener(this);
}
...
void SymbolView::setSymbol(const SymbolP &newSymbol) {
	...
	if (symbol)
		symbol->actions.addListener(this);
	onChangeSymbol();
}
