...
void after_reading(PackType &pt, Version) {
	if (pt.select == SELECT_AUTO) {
		if (pt.filter)
			pt.select = SELECT_NO_REPLACE;
		else if (!pt.items.empty())
			pt.select = SELECT_ALL;
	}
...
PackType::PackType()
	: enabled(true), selectable(indeterminate), summary(indeterminate),
	  select(SELECT_AUTO) {}

PackItem::PackItem() : amount(1), weight(1) {}

PackItem::PackItem(const String &name, int amount)
	: name(name), amount(amount), weight(1) {}
...
bool PackItem::update(Context &ctx) {
	return amount.update(ctx) | weight.update(ctx);
}
...
PackInstance::PackInstance(const PackType &pack_type, PackGenerator &parent)
	: pack_type(pack_type), parent(parent), requested_copies(0), card_copies(0),
	  expected_copies(0) {
	// Filter cards
	if (pack_type.filter) {
	...
	for (auto const &item : pack_type.items) {
	...
	for (auto const &item : pack_type.items) {
		...
		if (pack_type.select == SELECT_ALL) {
			i.expect_copy(copies * item->amount);
				   ...
				   pack_type.select == SELECT_EQUAL_PROPORTIONAL) {
			i.expect_copy(copies * item->amount * item->weight *
...
void PackInstance::request_copy(size_t copies) { requested_copies += copies; }
...
/// Distribute 'total' among the weighted items, higher weight items get chosen
/// more often
void weighted_equal_divide(vector<WeightedItem> &items, int total) {
	...
	if (items.size() == 1) {
		...
		while (total > 0) {
			// repeatedly pick the item that minimizes, after incrementing
			// count:
			//   max_wi  wi->count/wi->weight
			WeightedItem *wi = pq.top();
			pq.pop();
			wi->count++;
			total--;
			pq.push(wi);
...
void PackInstance::generate(vector<CardP> *out) {
	...
	if (pack_type.select == SELECT_ALL) {
		...
		if (!pack_type.items.empty()) {
			throw Error(_("'select:no replace' is not yet supported in "
						  "combination with 'items', only with 'filter'."));
		}
		...
		if (out && !cards.empty()) {
			...
			while (rem > 0) {
				...
				out->insert(out->end(), cards.begin(),
							cards.begin() + min(rem, max_per_batch));
				rem -= max_per_batch;
			}
		}
		...
		if (requested_copies == 1) {
			...
			for (auto const &item : pack_type.items) {
				...
				if (pack_type.select == SELECT_EQUAL_PROPORTIONAL) {
					wi.weight =
						item->weight * parent.get(item->name).total_weight;
				} else if (pack_type.select == SELECT_EQUAL_NONEMPTY) {
					wi.weight = parent.get(item->name).total_weight > 0
									? static_cast<int>(item->weight)
									: 0;
				} else {
			...
			weighted_items.push_back(wi);
			// 2. divide the requested_copies among the cards and the items,
		...
		if (!cards.empty()) {
			...
			card_copies += requested_copies;
			if (out)
				out->insert(out->end(), requested_copies, cards.front());
		} else {
...
void PackGenerator::reset(int seed) { gen.seed((unsigned)seed); }
...
PackInstance &PackGenerator::get(const String &name) {
	...
	if (instance) {
		...
		for (auto const &type : set->pack_types) {
			if (type->name == name) {
		...
		for (auto const &type : set->game->pack_types) {
			if (type->name == name) {
				instance = PackInstanceP(new PackInstance(*type, *this));
				max_depth = max(max_depth, instance->get_depth());
				return *instance;
