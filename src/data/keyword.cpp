...
KeywordParam::KeywordParam() : optional(true), eat_separator(true) {}
...
void read_compat(Reflector &, const Keyword *) {}
void read_compat(Reader &reflector, Keyword *k) {
    if (!k->match.empty())
        return;
    if (reflector.file_app_version >= 301)
        return; // only for versions < 0.3.1
    ...
    if (parameter == (L"no parameter")) {
        parameter
            .clear(); // was used for magic to indicate absence of parameter
    }
    if (!parameter.empty()) {
...
bool Keyword::contains(String const &query) const {
    if (find_i(keyword, query) != String::npos)
        return true;
    if (find_i(rules, query) != String::npos)
        return true;
    if (find_i(match, query) != String::npos)
        return true;
    if (find_i(reminder.get(), query) != String::npos)
        return true;
    return false;
}
    ...
    for (size_t i = 0; i < match.size();) {
        ...
        if (is_substr(match, i, (L"<atom-param"))) {
            ...
            if (!param) {
                ...
                // throw InternalError((L"Unknown keyword parameter type: ") +
                // type);
            ...
            regex += (L"(") + make_non_capturing(param->match) +
                     (param->optional ? (L")?") : (L")"));
...
class KeywordTrie {
  public:
    ...
    map<Char, KeywordTrie *>
        children;             ///< children after a given character (owned)
    ...
    /** return the node where the evaluation will be after matching the
     * character */
...
KeywordTrie::KeywordTrie() : on_any_star(nullptr), finished() {}
...
IMPLEMENT_DYNAMIC_ARG(KeywordUsageStatistics *, keyword_usage_statistics,
                      nullptr);

KeywordDatabase::KeywordDatabase() : root(nullptr) {}

KeywordDatabase::~KeywordDatabase() { clear(); }
...
void KeywordDatabase::add(const Keyword &kw) {
    if (kw.match.empty() || !kw.valid)
        return; // can't handle empty keywords
    // Create root
    if (!root) {
    ...
    for (size_t i = 0; i < kw.match.size();) {
        ...
        if (is_substr(kw.match, i, (L"<atom-param"))) {
            ...
            // enough?
            if (!only_star) {
                // If we have matched anything specific, this is a good time to
                // stop
                // it doesn't really matter how long we go on, since the trie is
                // only used
                // as an optimization to not have to match lots of regexes.
                // As an added bonus, we get a better behaviour of matching
                // earlier keywords first.
                break;
            }
        } else {
            text += c;
...
void KeywordDatabase::prepare_parameters(const vector<KeywordParamP> &ps,
                                         const vector<KeywordP> &kws) {
    for (const auto &kw : kws) {
        kw->prepare(ps);
    }
}
...
void dump(int i, KeywordTrie *t) {
    for (auto &c : t->children) {
        wxLogDebug(String(i, (L' ')) + c.first + (L"     ") +
                   String::Format((L"%p"), c.second));
        dump(i + 2, c.second);
    }
    if (t->on_any_star) {
        wxLogDebug(String(i, (L' ')) + (L".*") + (L"     ") +
                   String::Format((L"%p"), t->on_any_star));
        if (t->on_any_star != t)
            dump(i + 2, t->on_any_star);
    }
}
#endif
...
String KeywordDatabase::expand(const String &text,
    ...
    tagged =
        remove_tag_contents(tagged, (L"<atom-keyword")); // OLD, TODO: REMOVEME
        ...
        for (size_t i = 0; i < tagged.size();) {
            ...
            if (c == (L'<')) {
                if (is_substr(tagged, i, (L"<kw-")) && i + 4 < tagged.size()) {
                    ...
                    tagged = tagged.erase(
                        i, skip_tag(tagged, i) -
                               i); // remove the tag from the string
            ...
            for (auto &kt : current) {
                map<Char, KeywordTrie *>::const_iterator it =
                    kt->children.find(c);
            ...
            swap(current, next);
            // in the MSVC stl clear frees memory, that is a waste, because we
            // need it again in the next iteration
            // next.clear();
            next.resize(0);
            closure(current);
            // are we done?
                ...
                for (auto &n : current) {
                    for (auto &kw : n->finished) {
                        if (kw->fixed != (bool)set_or_game) {
                            continue; // first try set keywords, try game
                                      // keywords in the second round
                        }
                        if (!used.insert(kw).second) {
                            continue; // already seen this keyword
                        }
                        // we have found a possible match, for a keyword which
                        // we have not seen before
                        if (tryExpand(*kw, i, tagged, untagged, result,
                                      expand_type, match_condition,
                                      expand_default, combine_script, ctx, stat,
                                      stat_key)) {
                            // it matches
...
bool KeywordDatabase::tryExpand(
    const Keyword &kw, size_t expand_type_known_upto, String &tagged,
    String &untagged, String &result, char expand_type,
    const ScriptValueP &match_condition, const ScriptValueP &expand_default,
    const ScriptValueP &combine_script, Context &ctx,
    KeywordUsageStatistics *stat, Value *stat_key) const {
    // try to match regex against the *untagged* string
    assert(!kw.match_re.empty());
    Regex::Results match;
    if (!kw.match_re.matches(match, untagged))
        return false;
    ...
    if (start == end)
        return false; // don't match empty keywords
    ...
    // To determine if the case matches exactly we compare plain text parts with
    ...
    // The even captures are parameter values, the odd ones are the plain text
    // in between
    ...
    for (size_t submatch = 1; submatch < match.size(); ++submatch) {
        ...
        size_t part_end = part_len_u > 0
                              ? untagged_to_index(tagged, part_end_u, false)
                              : part_start;
        ...
        if ((submatch % 2) == 0) {
            ...
            String param =
                untagged.substr(part_start_u, part_len_u); // untagged version
            // strip separator_before
            String separator_before, separator_after;
            Regex::Results sep_match;
            if (!kwp.separator_before_re.empty() &&
                kwp.separator_before_re.matches(sep_match, param)) {
                size_t sep_end = sep_match.length();
                assert(sep_match.position() ==
                       0); // should only match at start of param
                ...
                part = get_tags(part, 0, sep_end_t, true, true) +
                       part.substr(sep_end_t);
                // transform?
                if (kwp.separator_script) {
                    ctx.setVariable((L"input"), to_script(separator_before));
                    separator_before =
                        kwp.separator_script.invoke(ctx)->toString();
                }
            }
            // strip separator_after
            if (!kwp.separator_after_re.empty() &&
                kwp.separator_after_re.matches(sep_match, param)) {
                size_t sep_start = sep_match.position();
                assert(sep_match[0].second ==
                       param.end()); // should only match at end of param
                ...
                part = part.substr(0, sep_start_t) +
                       get_tags(part, sep_start_t, part.size(), true, true);
                // transform?
                if (kwp.separator_script) {
            ...
            // to script
            KeywordParamValueP script_param(new KeywordParamValue(
                kwp.name, separator_before, separator_after, param));
            KeywordParamValueP script_part(new KeywordParamValue(
                kwp.name, separator_before, separator_after, part));
            // process param
            if (param.empty()) {
                // placeholder
                ...
                if (kwp.reminder_script) {
                    ...
                    script_param->value =
                        kwp.reminder_script.invoke(ctx)->toString();
                }
            }
            part = separator_before + script_part->toString() + separator_after;
            ctx.setVariable(String((L"param")) << (int)(submatch / 2),
                            script_param);

        } else if (correct_case) {
            // Plain text, check if the case matches
            for (size_t i = part_start_u; i < part_start_u + part_len_u;
                 ++i, ++pos_in_match_string) {
                if (pos_in_match_string > kw.match.size()) {
                    // outside match string, shouldn't happen, strings should be
                    // the same length
                    correct_case = false;
                    break;
                }
            ...
            if (pos_in_match_string < kw.match.size() &&
                is_substr(kw.match, pos_in_match_string, (L"<atom-param"))) {
                pos_in_match_string =
                    match_close_tag_end(kw.match, pos_in_match_string);
            }
        }
...
String KeywordParamValue::toString() const {
    String safe_type = replace_all(
        replace_all(replace_all(type_name, (L"("), (L"-")), (L")"), (L"-")),
        (L" "), (L"-"));
    return (L"<param-") + safe_type + (L">") + value + (L"</param-") +
           safe_type + (L">");
}

int KeywordParamValue::toInt() const {
    return to_script(value)->toInt();
} // a bit of a hack
double KeywordParamValue::toDouble() const {
    return to_script(value)->toDouble();
}
bool KeywordParamValue::toBool() const { return to_script(value)->toBool(); }
AColor KeywordParamValue::toColor() const {
    return to_script(value)->toColor();
}
int KeywordParamValue::itemCount() const {
    return to_script(value)->itemCount();
}

ScriptValueP KeywordParamValue::getMember(const String &name) const {
    if (name == (L"type"))
        return to_script(type_name);
    if (name == (L"separator before"))
        return to_script(separator_before);
    if (name == (L"separator after"))
        return to_script(separator_after);
    if (name == (L"value"))
        return to_script(value);
    if (name == (L"param"))
        return to_script(value);
