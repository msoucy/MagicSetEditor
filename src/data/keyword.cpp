	...
	for (size_t i = 0; i < match.size();) {
		...
		if (is_substr(match, i, _("<atom-param"))) {
			...
			if (!param) {
				...
				// throw InternalError(_("Unknown keyword parameter type: ") +
				// type);
			...
			regex += _("(") + make_non_capturing(param->match) +
					 (param->optional ? _(")?") : _(")"));
...
void dump(int i, KeywordTrie *t) {
	...
	if (t->on_any_star) {
		wxLogDebug(String(i, _(' ')) + _(".*") + _("     ") +
				   String::Format(_("%p"), t->on_any_star));
		if (t->on_any_star != t)
...
String KeywordDatabase::expand(const String &text,
	...
	tagged =
		remove_tag_contents(tagged, _("<atom-keyword")); // OLD, TODO: REMOVEME
...
bool KeywordDatabase::tryExpand(
	...
	for (size_t submatch = 1; submatch < match.size(); ++submatch) {
		...
		if ((submatch % 2) == 0) {
			...
			ctx.setVariable(String(_("param")) << (int)(submatch / 2),
							script_param);
			...
			if (pos_in_match_string < kw.match.size() &&
				is_substr(kw.match, pos_in_match_string, _("<atom-param"))) {
				pos_in_match_string =
...
String KeywordParamValue::toString() const {
	String safe_type = replace_all(
		replace_all(replace_all(type_name, _("("), _("-")), _(")"), _("-")),
		_(" "), _("-"));
	return _("<param-") + safe_type + _(">") + value + _("</param-") +
		   safe_type + _(">");
}
...
ScriptValueP KeywordParamValue::getMember(const String &name) const {
	...
	if (name == _("separator before"))
		return to_script(separator_before);
	if (name == _("separator after"))
