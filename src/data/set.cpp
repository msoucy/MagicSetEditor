...
Set::Set()
	: vcs(intrusive(new VCS())), script_manager(new SetScriptManager(*this)) {}

Set::Set(const GameP &game)
	: game(game), vcs(intrusive(new VCS())),
	  script_manager(new SetScriptManager(*this)) {
	data.init(game->set_fields);
}

Set::Set(const StyleSheetP &stylesheet)
	: game(stylesheet->game), stylesheet(stylesheet), vcs(intrusive(new VCS())),
	  script_manager(new SetScriptManager(*this)) {
	data.init(game->set_fields);
}
...
void Set::updateDelayed() { script_manager->updateDelayed(); }
...
IndexMap<FieldP, ValueP> &Set::stylingDataFor(const CardP &card) {
	if (card && card->has_styling)
		return card->styling_data;
	else
		return stylingDataFor(stylesheetFor(card));
}
...
void Set::validate(Version file_app_version) {
	...
	if (cards.empty())
		cards.push_back(intrusive(new Card(*game)));
	// update scripts
...
IMPLEMENT_REFLECTION(Set) {
	REFLECT(game);
	if (game) {
		REFLECT_IF_READING { data.init(game->set_fields); }
...
void Set::reflect_cards<Writer>(Writer &reflector) {
	...
	if (isZipfile()) {
		...
		for (auto &card : cards) {
			...
			// can't use Package::newFileName, because then we get conflicts
			// with the previous save of the same card
			String filename =
				_("card ") + normalize_internal_filename(
								 clean_filename(card->identification()));
...
void mark_dependency_member(const Set &set, const String &name,
...
template <typename Reflector>
void reflect_set_info_get_member(Reflector &reflector,
								 const IndexMap<FieldP, ValueP> &data) {}
void reflect_set_info_get_member(GetMember &reflector,
								 const IndexMap<FieldP, ValueP> &data) {
...
int Set::positionOfCard(const CardP &card, const ScriptValueP &order_by,
	...
	if (!order) {
		...
		order = intrusive(
			new OrderCache<CardP>(cards, values, filter ? &keep : nullptr));
	}
...
int Set::numberOfCards(const ScriptValueP &filter) {
	...
	if (it != filter_cache.end()) {
		...
		int n = 0;
		for (auto const &c : cards) {
			if (filter->eval(getContext(c))->toBool())
				++n;
		}
