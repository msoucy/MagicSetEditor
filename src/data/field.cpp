...
Field::Field()
    : index(0) // sensible default?
      ,
      editable(true), save_value(true), show_statistics(true), position_hint(0),
      identifying(false), card_list_column(100), card_list_width(100),
      card_list_visible(false), card_list_allow(true),
      card_list_align(ALIGN_LEFT), default_name((L"Default")),
...
void Field::after_reading(Version ver) {
    ...
    if (caption.empty())
        caption = name_to_caption(name);
    if (card_list_name.empty())
        card_list_name = capitalize(caption);
}
...
intrusive_ptr<Field> read_new<Field>(Reader &reader) {
    ...
    if (type == (L"text"))
        return intrusive(new TextField());
    else if (type == (L"choice"))
        return intrusive(new ChoiceField());
    else if (type == (L"multiple choice"))
        return intrusive(new MultipleChoiceField());
    else if (type == (L"boolean"))
        return intrusive(new BooleanField());
    else if (type == (L"image"))
        return intrusive(new ImageField());
    else if (type == (L"symbol"))
        return intrusive(new SymbolField());
    else if (type == (L"color"))
        return intrusive(new ColorField());
    else if (type == (L"info"))
        return intrusive(new InfoField());
    else if (type == (L"package choice"))
        return intrusive(new PackageChoiceField());
    else if (type.empty()) {
...
Style::Style(const FieldP &field)
    : fieldP(field), z_index(0), tab_index(0), left(1000000), top(1000000),
      width(0), height(0), right(1000000), bottom(1000000), angle(0),
      visible(true), automatic_side(AUTO_UNKNOWN), content_dependent(false) {}
...
void init_object(const FieldP &field, StyleP &style) {
    if (!style)
        style = field->newStyle();
}
template <>
StyleP read_new<Style>(Reader &) {
    throw InternalError((L"IndexMap contains nullptr StyleP the application "
                         L"should have crashed already"));
}
...
int Style::update(Context &ctx) {
    int changed = (left.update(ctx) | width.update(ctx) | right.update(ctx) |
                   top.update(ctx) | height.update(ctx) | bottom.update(ctx) |
                   angle.update(ctx)) *
                      CHANGE_SIZE |
    ...
    if (automatic_side == AUTO_UNKNOWN) {
        if (!is_set(right))
            automatic_side = (AutomaticSide)(automatic_side | AUTO_RIGHT);
        else if (!is_setw(width))
            automatic_side = (AutomaticSide)(automatic_side | AUTO_WIDTH);
        else if (!is_set(left))
            automatic_side = (AutomaticSide)(automatic_side | AUTO_LEFT);
        else
            automatic_side = (AutomaticSide)(automatic_side | AUTO_LR);
        if (!is_set(bottom))
            automatic_side = (AutomaticSide)(automatic_side | AUTO_BOTTOM);
        else if (!is_setw(height))
            automatic_side = (AutomaticSide)(automatic_side | AUTO_HEIGHT);
        else if (!is_set(top))
            automatic_side = (AutomaticSide)(automatic_side | AUTO_TOP);
        else
            automatic_side = (AutomaticSide)(automatic_side | AUTO_TB);
        changed |= CHANGE_SIZE;
    }
    if (!changed)
        return CHANGE_NONE;
    // update the automatic_side
    if (automatic_side & AUTO_LEFT)
        left = right - width;
    else if (automatic_side & AUTO_WIDTH)
        width = right - left;
    else if (automatic_side & AUTO_RIGHT)
        right = left + width;
    else {
        int lr = int(left + right);
        left = (lr - width) / 2;
        right = (lr + width) / 2;
    }
    if (automatic_side & AUTO_TOP)
        top = bottom - height;
    else if (automatic_side & AUTO_HEIGHT)
        height = bottom - top;
    else if (automatic_side & AUTO_BOTTOM)
        bottom = top + height;
    else {
        int tb = int(top + bottom);
        top = (tb - height) / 2;
        bottom = (tb + height) / 2;
    }
...
bool Style::isVisible() const {
    return visible && (width()) > 0 && fabs(left()) < 100000 &&
           fabs(right()) < 100000 && (height()) > 0 && fabs(top()) < 100000 &&
           fabs(bottom()) < 100000;
}
...
void Style::checkContentDependencies(Context &ctx,
...
void Style::tellListeners(int changes) {
    for (auto &l : listeners)
        l->onStyleChange(changes);
}

StyleListener::StyleListener(const StyleP &style) : styleP(style) {
    style->addListener(this);
}
StyleListener::~StyleListener() { styleP->removeListener(this); }
...
void parse_errors_to_reader_warnings(Reader &reader,
                                     vector<ScriptParseError> const &errors);
...
void Reader::handle(ScriptValueP &value) {
    ...
    if (formatVersion() < 20001 && field) {
        ...
        if (dynamic_cast<BooleanField const *>(field)) {
            ...
            handle(x);
            value = to_script(x);
        } else if (dynamic_cast<TextField const *>(field) ||
                   dynamic_cast<ChoiceField const *>(field) ||
                   dynamic_cast<PackageChoiceField const *>(field)) {
            ...
            handle(str);
            value =
                script_local_symbol_file(LocalFileName::fromReadString(str));
...
Value::Value(const FieldP &field) : fieldP(field), value(field->initial) {
    assert(value);
}
...
ValueP Value::clone() const { return intrusive(new Value(*this)); }

bool Value::equals(const Value *that) { return this == that; }
...
bool Value::update(Context &ctx, const Action *act) {
    WITH_DYNAMIC_ARG(last_update_age,
                     last_modified.get()); // TODO: this is redundant, since it
                                           // can be got from
                                           // value_being_updated
...
IMPLEMENT_REFLECTION_NAMELESS(Value) {
    if (reflector.isWriting() && !fieldP->save_value)
        return;
    if (reflector.isWriting() && is_default(value))
        return;
...
ValueP read_new<Value>(Reader &) {
    throw InternalError((L"IndexMap contains nullptr ValueP the application "
                         L"should have crashed already"));
}

void mark_dependency_member(const IndexMap<FieldP, ValueP> &value,
                            const String &name, const Dependency &dep) {
