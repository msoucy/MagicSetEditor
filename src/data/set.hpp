...
class Set : public Packaged {
  public:
	...
	/// Get the identification of this set, an identification is something like
	/// a name, title, etc.
	/** May return "" */
	...
	/// Find the position of a card in this set, when the card list is sorted
	/// using the given cirterium
  ...
  protected:
	virtual VCSP getVCS() { return vcs; }
...
inline String type_name(const Set &) { return _TYPE_("set"); }
inline int item_count(const Set &set) { return (int)set.cards.size(); }
