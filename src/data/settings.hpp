...
enum CheckUpdates { CHECK_ALWAYS, CHECK_IF_CONNECTED, CHECK_NEVER };
...
enum FilenameConflicts {
	...
	,
	CONFLICT_NUMBER_OVERWRITE // only add numbers for conflicts inside a set,
							  // overwrite old stuff
};
...
class GameSettings : public IntrusivePtrBase<GameSettings> {
  public:
	...
	/// Where the settings have defaults, initialize with the values from the
	/// game
...
class Settings {
  ...
  public:
	StyleSheetSettings
		default_stylesheet_settings; ///< The default settings for stylesheets
  ...
  public:
	...
	exportOptionsFor(const ExportTemplate &export_template);
...
// --------------------------------------------------- : Update checking
	...
	bool check_updates_all; ///< Check updates of all packages, not just the
							///program
	String website_url;
