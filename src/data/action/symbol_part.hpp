 ...
 *
 *  Actions operating on the insides of SymbolParts/SymbolShapes (ControlPoints
 * and the like).
 */
 ...
 */
Vector2D constrain_vector(const Vector2D &v, bool constrain = true,
                          bool only_diagonal = false);
...
class SegmentModeAction : public Action {
  public:
    SegmentModeAction(const ControlPointP &p1, const ControlPointP &p2,
                      SegmentMode mode);
...
class ControlPointAddAction : public Action {
  public:
    /// Insert a new point in shape, after position insertAfter_, at the time t
    /// on the segment
    ControlPointAddAction(const SymbolShapeP &shape, UInt insert_after,
                          double t);
  ...
  private:
    ...
    ControlPointUpdate point1,
        point2; ///< Update the points around the new point
};
