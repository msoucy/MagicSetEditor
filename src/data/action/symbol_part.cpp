...
Vector2D constrain_vector(const Vector2D &v, bool constrain,
                          bool only_diagonal) {
    if (!constrain)
        return v;
    ...
    if (ax * 2 < ay && !only_diagonal) {
        ...
        return Vector2D( // diagonal
            sgn(v.x) * (ax + ay) / 2, sgn(v.y) * (ax + ay) / 2);
    }
}
...
Vector2D constrain_snap_vector(const Vector2D &v, const Vector2D &d,
                               bool constrain, int steps) {
    if (!constrain)
        return snap_vector(v + d, steps);
...
Vector2D constrain_snap_vector_offset(const Vector2D &off1, const Vector2D &d,
                                      bool constrain, int steps) {
    return constrain_snap_vector(off1, d, constrain, steps) - off1;
}
// calculate constrained delta for the given offset, store in output if it is
// better
void constrain_snap_vector_offset_(const Vector2D &off, const Vector2D &d,
                                   bool constrain, int steps, Vector2D &best,
                                   double &best_length) {
...
Vector2D constrain_snap_vector_offset(const Vector2D &off1,
                                      const Vector2D &off2, const Vector2D &d,
                                      bool constrain, int steps) {
    Vector2D dd;
    ...
    constrain_snap_vector_offset_(Vector2D(off1.x, off2.y), d, constrain, steps,
                                  dd, l);
    constrain_snap_vector_offset_(Vector2D(off2.x, off1.y), d, constrain, steps,
                                  dd, l);
    return dd;
}

String action_name_for(const set<ControlPointP> &points, const String &action) {
    return format_string(action, points.size() == 1 ? _TYPE_("point")
                                                    : _TYPE_("points"));
}
...
ControlPointMoveAction::ControlPointMoveAction(const set<ControlPointP> &points)
    : points(points), constrain(false), snap(0) {
...
HandleMoveAction::HandleMoveAction(const SelectedHandle &handle)
    : handle(handle), old_handle(handle.getHandle()),
      old_other(handle.getOther()), constrain(false), snap(0) {}
...
void HandleMoveAction::move(const Vector2D &deltaDelta) {
    delta += deltaDelta;
    handle.getHandle() = constrain_snap_vector_offset(
        handle.point->pos, old_handle + delta, constrain, snap);
...
void ControlPointUpdate::perform() { swap(other, *point); }

SegmentModeAction::SegmentModeAction(const ControlPointP &p1,
                                     const ControlPointP &p2, SegmentMode mode)
    : point1(p1), point2(p2) {
    if (p1->segment_after == mode)
        return;
...
String SegmentModeAction::getName(bool to_undo) const {
    SegmentMode mode =
        to_undo ? point1.point->segment_after : point1.other.segment_after;
    if (mode == SEGMENT_LINE)
        return _ACTION_("convert to line");
    else
        return _ACTION_("convert to curve");
}
...
void LockModeAction::perform(bool to_undo) { point.perform(); }

// -----------------------------------------------------------------------------
// : Move curve

CurveDragAction::CurveDragAction(const ControlPointP &point1,
                                 const ControlPointP &point2)
...
void CurveDragAction::move(const Vector2D &delta, double t) {
    ...
    //   We now want to change control points b and c, the closer we are to b (t
    //   close to 0)
...
ControlPointAddAction::ControlPointAddAction(const SymbolShapeP &shape,
                                             UInt insert_after, double t)
    : shape(shape), new_point(new ControlPoint()), insert_after(insert_after),
    ...
    if (point1.other.segment_after == SEGMENT_CURVE) {
        ...
        // unlock if needed
        if (point1.other.lock == LOCK_SIZE)
            point1.other.lock = LOCK_DIR;
        if (point2.other.lock == LOCK_SIZE)
            point2.other.lock = LOCK_DIR;
...
void ControlPointAddAction::perform(bool to_undo) {
    if (to_undo) { // remove the point
        ...
        shape->points.insert(shape->points.begin() + insert_after + 1,
                             new_point);
    }
...
class SinglePointRemoveAction
    : public Action,
      public IntrusivePtrBase<SinglePointRemoveAction> {
  public:
...
SinglePointRemoveAction::SinglePointRemoveAction(const SymbolShapeP &shape,
                                                 UInt position)
    : shape(shape), position(position), point(shape->getPoint(position)),
    ...
    if (point1.other.segment_after == SEGMENT_CURVE ||
        ...
        // Also take in acount cases where the point does not correspond to a
        // freshly added point.
        // distance from the point to the curve as it would be in the above case
        // can be used,
        ...
        point1.other.delta_after *=
            ssqrt(dot(distP, point1.other.delta_after) /
                  point1.other.delta_after.lengthSqr()) +
            1;
        point2.other.delta_before *=
            ssqrt(dot(distP, point2.other.delta_before) /
                  point2.other.delta_before.lengthSqr()) +
            1;
...
class ControlPointRemoveAction : public Action {
  public:
    ControlPointRemoveAction(const SymbolShapeP &shape,
                             const set<ControlPointP> &to_delete);
...
ControlPointRemoveAction::ControlPointRemoveAction(
    const SymbolShapeP &shape, const set<ControlPointP> &to_delete) {
    int index = 0;
    ...
    for (auto &point : shape->points) {
        if (to_delete.find(point) != to_delete.end()) {
            // remove this point
            removals.push_back(
                intrusive(new SinglePointRemoveAction(shape, index)));
        }
        ++index;
    }
}
...
String ControlPointRemoveAction::getName(bool to_undo) const {
    return removals.size() == 1 ? _ACTION_("delete point")
                                : _ACTION_("delete points");
}
...
Action *control_point_remove_action(const SymbolShapeP &shape,
                                    const set<ControlPointP> &to_delete) {
...
SymmetryMoveAction::SymmetryMoveAction(SymbolSymmetry &symmetry, bool is_handle)
    : symmetry(symmetry), is_handle(is_handle),
      original(is_handle ? symmetry.handle : symmetry.center), constrain(false),
      snap(0) {}

String SymmetryMoveAction::getName(bool to_undo) const {
    return is_handle ? _ACTION_("move symmetry handle")
                     : _ACTION_("move symmetry center");
}
...
void SymmetryMoveAction::move(const Vector2D &deltaDelta) {
    ...
    if (is_handle) {
        symmetry.handle =
            snap_vector(symmetry.center + original + delta, snap) -
            symmetry.center;
        if (constrain) {
            ...
            symmetry.handle =
                Vector2D(cos(angle), sin(angle)) * symmetry.handle.length();
        }
    } else {
        // Determine actual delta, possibly constrained and snapped
        symmetry.center =
            constrain_snap_vector(original, delta, constrain, snap);
    }
}
...
SymmetryTypeAction::SymmetryTypeAction(SymbolSymmetry &symmetry,
                                       SymbolSymmetryType type)
    : symmetry(symmetry), type(type), old_name(symmetry.name),
...
SymmetryCopiesAction::SymmetryCopiesAction(SymbolSymmetry &symmetry,
                                           int copies_)
    : symmetry(symmetry), copies(copies_), old_name(symmetry.name) {
    if (symmetry.kind == SYMMETRY_REFLECTION && copies % 2 == 1) {
        // make sure it is a multiple of two
        if (copies > symmetry.copies)
            copies++;
        else
            copies--;
    }
