...
String action_name_for(const set<SymbolPartP> &parts, const String &action) {
	return format_string(action, parts.size() == 1 ? (*parts.begin())->name
												   : _TYPE_("shapes"));
}
...
SymbolPartMoveAction::SymbolPartMoveAction(const set<SymbolPartP> &parts,
	...
	: SymbolPartsAction(parts), delta(delta), moved(-delta), constrain(false),
	  snap(0) {
...
void SymbolPartMoveAction::move(const Vector2D &deltaDelta) {
	...
	Vector2D d = constrain_snap_vector_offset(bounds.min, bounds.max, delta,
											  constrain, snap);
...
SymbolPartMatrixAction::SymbolPartMatrixAction(const set<SymbolPartP> &parts,
...
SymbolPartRotateAction::SymbolPartRotateAction(const set<SymbolPartP> &parts,
											   const Vector2D &center)
	: SymbolPartMatrixAction(parts, center), angle(0), constrain(false) {}
...
void SymbolPartRotateAction::rotateTo(Radians newAngle) {
	...
	if (oldAngle != angle)
		rotateBy(angle - oldAngle);
}

void SymbolPartRotateAction::rotateBy(Radians deltaAngle) {
	// Rotation 'matrix'
	transform(Matrix2D(cos(deltaAngle), -sin(deltaAngle), sin(deltaAngle),
					   cos(deltaAngle)));
}
...
SymbolPartShearAction::SymbolPartShearAction(const set<SymbolPartP> &parts,
											 const Vector2D &center)
...
void SymbolPartShearAction::shearBy(const Vector2D &shear) {
	// Shear 'matrix'
	transform(Matrix2D(1, shear.x, shear.y, 1));
}

// -----------------------------------------------------------------------------
// : Scaling symbol parts

SymbolPartScaleAction::SymbolPartScaleAction(const set<SymbolPartP> &parts,
											 int scaleX, int scaleY)
	: SymbolPartsAction(parts), scaleX(scaleX), scaleY(scaleY),
	  constrain(false), snap(0) {
...
void SymbolPartScaleAction::move(const Vector2D &delta_min,
								 const Vector2D &delta_max) {
...
void SymbolPartScaleAction::update() {
	// Move each point so the range [old_min...old_max] maps to
	// [new_min...new_max]
	// we have already moved to the current [new_min...new_max]
	Vector2D tmp_min = old_min,
			 tmp_size = old_size; // the size before any scaling
	old_min = new_min;
	old_size = new_size; // the size before this move
	// the size after the move
	new_min = new_real_min;
	new_size = new_real_size;
	if (constrain && scaleX != 0 && scaleY != 0) {
		...
		new_min += (new_real_size - new_size).mul(
			Vector2D(scaleX == -1 ? 1 : 0, scaleY == -1 ? 1 : 0));
		// TODO : snapping
	} else if (snap >= 0) {
		if (scaleX + scaleY < 0) {
...
void SymbolPartScaleAction::transformPart(SymbolPart &part) {
	...
	if (part.bounds.min.x > part.bounds.max.x)
		swap(part.bounds.min.x, part.bounds.max.x);
	if (part.bounds.min.y > part.bounds.max.y)
		swap(part.bounds.min.y, part.bounds.max.y);
	if (SymbolShape *s = part.isSymbolShape()) {
		// scale all points
...
CombiningModeAction::CombiningModeAction(const set<SymbolPartP> &parts,
										 SymbolShapeCombine mode)
	: SymbolPartsAction(parts) {
	for (auto &p : parts) {
		add(p, mode);
	}
}
void CombiningModeAction::add(const SymbolPartP &part,
							  SymbolShapeCombine mode) {
	if (part->isSymbolShape()) {
		this->parts.push_back(
			make_pair(static_pointer_cast<SymbolShape>(part), mode));
...
SymbolPartNameAction::SymbolPartNameAction(const SymbolPartP &part,
										   const String &name,
										   size_t old_cursor, size_t new_cursor)
	: part(part), part_name(name), old_cursor(new_cursor),
	  new_cursor(old_cursor) // will be swapped
{}
...
AddSymbolPartAction::AddSymbolPartAction(Symbol &symbol,
										 const SymbolPartP &part)
...
RemoveSymbolPartsAction::RemoveSymbolPartsAction(Symbol &symbol,
												 const set<SymbolPartP> &parts)
	: symbol(symbol) {
	check(symbol, parts);
}

void RemoveSymbolPartsAction::check(SymbolGroup &group,
									const set<SymbolPartP> &parts) {
	size_t index = 0;
	size_t removed = 0;
...
String RemoveSymbolPartsAction::getName(bool to_undo) const {
	return _ACTION_1_("remove item", removals.size() == 1 ? _TYPE_("shape")
														  : _TYPE_("shapes"));
}
...
DuplicateSymbolPartsAction::DuplicateSymbolPartsAction(
	Symbol &symbol, const set<SymbolPartP> &parts)
	: symbol(symbol) {
	UInt index = 0;
...
String DuplicateSymbolPartsAction::getName(bool to_undo) const {
	return _ACTION_1_("duplicate", duplications.size() == 1 ? _TYPE_("shape")
															: _TYPE_("shapes"));
}
...
ReorderSymbolPartsAction::ReorderSymbolPartsAction(SymbolGroup &old_parent,
												   size_t old_position,
												   SymbolGroup &new_parent,
												   size_t new_position)
...
UngroupReorderSymbolPartsAction::UngroupReorderSymbolPartsAction(
	SymbolGroup &group_parent, size_t group_pos, SymbolGroup &target_parent,
	size_t target_pos)
...
void UngroupReorderSymbolPartsAction::perform(bool to_undo) {
	if (!to_undo) {
		...
		target_parent.parts.insert(target_parent.parts.begin() + target_pos,
								   group->parts.begin(), group->parts.end());
	} else {
		target_parent.parts.erase(target_parent.parts.begin() + target_pos,
								  target_parent.parts.begin() + target_pos +
									  group->parts.size());
		group_parent.parts.insert(group_parent.parts.begin() + group_pos,
								  group);
	}
}
...
GroupSymbolPartsAction::GroupSymbolPartsAction(SymbolGroup &root,
											   const set<SymbolPartP> &parts,
											   const SymbolGroupP &group)
	: GroupSymbolPartsActionBase(root), group(group) {
...
String GroupSymbolPartsAction::getName(bool to_undo) const {
	return group->isSymbolSymmetry() ? _ACTION_("add symmetry")
									 : _ACTION_("group parts");
}

UngroupSymbolPartsAction::UngroupSymbolPartsAction(
	SymbolGroup &root, const set<SymbolPartP> &parts)
