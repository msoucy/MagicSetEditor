...
void KeywordReminderTextValue::highlight(
    const String &code, const vector<ScriptParseError> &errors) {
    ...
    for (size_t pos = 0; pos < code.size();) {
        ...
        if (c == (L'<')) {
            ...
            new_value += (L"<ref-") + param + (L">") + param + (L"</ref-") +
                         param + (L">");
            pos = end;
        } else {
            new_value += c;
