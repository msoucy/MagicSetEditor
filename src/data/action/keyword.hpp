...
class KeywordListAction : public Action {
  ...
  protected:
	Set &set; // the set owns this action, so the set will not be destroyed
			  // before this
...
class KeywordTextValue : public FakeTextValue {
  public:
	KeywordTextValue(const TextFieldP &field, Keyword *keyword,
					 String *underlying, bool editable, bool untagged = false)
...
class KeywordReminderTextValue : public KeywordTextValue {
  public:
	KeywordReminderTextValue(Set &set, const TextFieldP &field,
							 Keyword *keyword, bool editable);
