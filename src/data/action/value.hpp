...
class ValueAction : public Action {
  public:
	...
	const Card *
		card; ///< The card the value is on, or null if it is not a card value
  private:
...
class SimpleValueAction : public ValueAction {
  public:
	inline SimpleValueAction(const ValueP &value, const ScriptValueP &new_value,
...
class MultipleChoiceValueAction : public SimpleValueAction {
  public:
	inline MultipleChoiceValueAction(const ValueP &value,
									 const ScriptValueP &new_value,
									 const String &changed_choice)
		: SimpleValueAction(value, new_value), changed_choice(changed_choice) {}

	const String
		changed_choice; ///< What choice was toggled by this action (if any)
  private:
	ScriptValueP new_value;
};
...
class TextValueAction : public SimpleValueAction {
  public:
	TextValueAction(const TextValueP &value, size_t start, size_t end,
					size_t new_end, const ScriptValueP &new_value,
					const String &name);
...
class ScriptValueEvent : public Action {
  public:
	inline ScriptValueEvent(const Card *card, const Value *value)
