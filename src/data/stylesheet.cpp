...
StyleSheet::StyleSheet()
	: card_width(100), card_height(100), card_dpi(96),
	  card_background(*wxWHITE), dependencies_initialized(false) {}
...
StyleSheetP StyleSheet::byGameAndName(const Game &game, const String &name) {
	...
	try {
		map<String, String>::const_iterator it =
			stylesheet_alternatives.find(full_name);
		...
		if (stylesheet_for_reading()) {
			// we already have a stylesheet higher up, so just return a null
			// pointer
			return StyleSheetP();
		}
...
StyleP StyleSheet::styleFor(const FieldP &field) {
	if (card_style.containsKey(field)) {
		...
		throw InternalError(_("Can not find styling for field '") +
							field->name + _("'in stylesheet"));
	}
}

void mark_dependency_value(const StyleSheet &stylesheet,
...
void Writer::handle(const StyleSheetP &stylesheet) {
	if (stylesheet)
		handle(stylesheet->stylesheetName());
}
