...
class Field : public IntrusivePtrVirtualBase {
  public:
    ...
    String
        name; ///< Name of the field, for refering to it from scripts and files
    ...
    bool save_value; ///< Should values of this field be written to files? Can
                     ///be false for script generated fields.
    bool show_statistics; ///< Should this field appear as a group by choice in
    ...
    OptionalScript default_script; ///< Script that generates the default value,
                                   ///can be empty if there is no default
    OptionalScript
        sort_script; ///< The script to use when sorting this, if not the value.
    String default_name;  ///< Name of the 'default' choice
    ScriptValueP initial; ///< Initial value of a new value
    Dependencies
        dependent_scripts; ///< Scripts that depend on values of this field
    ...
    /// Add the given dependency to the dependet_scripts list for the variables
...
inline void update_index(FieldP &f, size_t index) { f->index = index; }

inline void after_reading(Field &f, Version ver) { f.after_reading(ver); }

inline String type_name(const Field &) { return _TYPE_("field"); }
...
class Style : public IntrusivePtrVirtualBase {
  public:
    ...
    const FieldP
        fieldP; ///< Field this style is for, should have the right type!
    ...
    enum AutomaticSide {
        ...
        ATTACH_MIDDLE = 0x20,
        ATTACH_BOTTOM = 0x10,
    } automatic_side : 8; ///< Which of (left, width,  right) and (top,  height,
                          ///bottom) is determined automatically?
    ...
    inline RealRect getExternalRect() const {
        return RealRect(left, top, width, height);
    }
    inline RealRect getInternalRect() const {
        return RealRect(0, 0, width, height);
    }
    ...
    /// Update scripted values of this style, return nonzero if anything has
    /// changed.
    /** The caller should tellListeners()
     *  The result is a combination of StyleChange flags
     */
    virtual int update(Context &);
    /// Add the given dependency to the dependent_scripts list for the variables
    ...
    /** In particular, if dep == DEP_DUMMY and name is a content property, set
     * dep.index=true */
    virtual void markDependencyMember(const String &name,
...
enum StyleChange {
    ...
    ,
    CHANGE_ALREADY_PREPARED = 0x80 // hint that the change was the result of a
                                   // content property change, viewers are
                                   // already prepared
};
...
inline String type_name(const Style &) { return _TYPE_("style"); }

inline void after_reading(Style &s, Version ver) { s.after_reading(ver); }
...
class Value : public IntrusivePtrVirtualBase {
  public:
    ...
    const FieldP
        fieldP;        ///< Field this value is for, should have the right type!
    Age last_modified; ///< When was the value last modified? Note: this also
                       ///goes up on undo.
    ///< This variable is used by ScriptManager. It will be incremented by each
     ...
     *  In that case returns true if this and that editing the same undelying
     * value.
     *  If so, this value is updated to reflect the (possibly changed)
     * underlying value.
     */
  ...
  protected:
    ...
    /** Derived classes should put their stuff in between if they need the age
     * in scripts */
...
inline String type_name(const Value &) { return _TYPE_("value"); }
...
#define IMPLEMENT_FIELD_TYPE(Type, NAME)                                       \
    ...
    String Type##Field::typeName() const { return _(NAME); }
