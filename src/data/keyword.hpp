...
class ParamReferenceType : public IntrusivePtrBase<ParamReferenceType> {
  public:
    ...
    StringScript script; ///< Code to insert into the reminder text script,
                         ///input is the actual parameter name
...
class KeywordParam : public IntrusivePtrBase<KeywordParam> {
  public:
    ...
    String placeholder; ///< Placholder for <atom-kwpph>, name is used if this
                        ///is empty
    bool optional; ///< Can this parameter be left out (a placeholder is then
                   ///used)
    String match;  ///< Regular expression to match (including separators)
    String separator_before_is; ///< Regular expression of separator before the
                                ///param
    Regex separator_before_re;  ///< Regular expression of separator before the
                                ///param, compiled
    Regex separator_before_eat; ///< Regular expression of separator before the
                                ///param, if eat_separator
    String
        separator_after_is; ///< Regular expression of separator after the param
    Regex separator_after_re;  ///< Regular expression of separator after the
                               ///param, compiled
    Regex separator_after_eat; ///< Regular expression of separator after the
                               ///param, if eat_separator
    bool eat_separator; ///< Remove the separator from the match string if it
                        ///also appears there (prevent duplicates)
    OptionalScript
        script; ///< Transformation of the value for showing as the parameter
    OptionalScript reminder_script; ///< Transformation of the value for showing
    ...
    vector<ParamReferenceTypeP> refer_scripts; ///< Way to refer to a parameter
                                               ///from the reminder text script
...
class Keyword : public IntrusivePtrVirtualBase {
  public:
    ...
    String
        match; ///< String to match, <atom-param> tags are used for parameters
    ...
    /// Regular expression to match and split parameters, automatically
    /// generated.
    /** The regex has exactly 2 * parameters.size() + 1 captures (excluding the
     * entire match, caputure 0),
     *  captures 1,3,... capture the plain text of the match string
     *  captures 2,4,... capture the separators and parameters
     */
    Regex match_re;
    bool fixed; ///< Is this keyword uneditable? (true for game keywods, false
                ///for set keywords)
    bool valid; ///< Is this keyword okay (reminder text compiles & runs; match
                ///does not match "")
...
inline String type_name(const Keyword &) { return _TYPE_("keyword"); }
...
typedef vector<std::pair<Value *, const Keyword *>> KeywordUsageStatistics;
...
class KeywordDatabase {
  public:
    ...
    static void prepare_parameters(const vector<KeywordParamP> &,
                                   const vector<KeywordP> &);
    ...
    /** @param expand_default script function indicating whether reminder text
     * should be shown by default
     *  @param combine_script script function to combine keyword and reminder
     * text in some way
     *  @param case_sensitive case sensitive matching of keywords?
     *  @param ctx            context for evaluation of scripts
     */
    String expand(const String &text, const ScriptValueP &match_condition,
                  const ScriptValueP &expand_default,
                  const ScriptValueP &combine_script, Context &ctx) const;
  ...
  private:
     ...
     *    - advance the tagged and untagged string by dropping a part from the
     * front
     *    - return true
     */
    bool tryExpand(const Keyword &kw, size_t pos, String &tagged,
                   String &untagged, String &out, char expand_type,
...
class KeywordParamValue : public ScriptValue {
  public:
    KeywordParamValue(const String &type, const String &separator_before,
                      const String &separator_after, const String &value)
        : type_name(type), separator_before(separator_before),
          separator_after(separator_after), value(value) {}
    String type_name;
