...
public:
  ...
  /** Use OnRun instead of OnInit, so we can determine whether or not we need a
   * main loop
...
void nag_about_ascii_version() {
  ...
  if (info.dwMajorVersion >= 5) {
    queue_message(MESSAGE_WARNING, _("This build of Magic Set Editor is "
    ...
    // interpret command line
    {
      ...
      for (int i = 1; i < argc; ++i) {
        ...
        if (args.back() == _("--color"))
          args.pop_back(); // ingnore the --color argument, it is handled by
                           // cli.init()
      }
      if (!args.empty()) {
        // Find the extension
        ...
        if (f.GetExt() == _("mse-symbol")) {
          ...
          wnd->Show();
          return runGUI();
        } else if (f.GetExt() == _("mse-set") || f.GetExt() == _("mse") ||
                   f.GetExt() == _("set")) {
          // Show the set window
          ...
          cli << _("Usage: ") << BRIGHT << argv[0] << NORMAL << _(" [") << PARAM
              << _("OPTIONS") << NORMAL << _("]");
          cli << _("\n\n  no options");
          cli << _("\n         \tStart the MSE user interface showing the "
                   "welcome window.");
          cli << _("\n\n  ") << BRIGHT << _("-?") << NORMAL << _(", ") << BRIGHT
              << _("--help") << NORMAL;
          cli << _("\n         \tShows this help screen.");
          cli << _("\n\n  ") << BRIGHT << _("-v") << NORMAL << _(", ") << BRIGHT
              << _("--version") << NORMAL;
          cli << _("\n         \tShow version information.");
          cli << _("\n\n  ") << PARAM << _("FILE") << FILE_EXT << _(".mse-set")
              << NORMAL << _(", ") << PARAM << _("FILE") << FILE_EXT
              << _(".set") << NORMAL << _(", ") << PARAM << _("FILE")
              << FILE_EXT << _(".mse") << NORMAL;
          ...
          cli << _("\n         \tIf the ") << BRIGHT << _("--local") << NORMAL
              << _(" flag is passed, install packages for this user only.");
          cli << _("\n\n  ") << BRIGHT << _("--symbol-editor") << NORMAL;
          cli << _("\n         \tShow the symbol editor instead of the welcome "
                   "window.");
          cli << _("\n\n  ") << BRIGHT << _("--create-installer") << NORMAL
              << _(" [") << PARAM << _("OUTFILE") << FILE_EXT
              << _(".mse-installer") << NORMAL << _("] [") << PARAM
              << _("PACKAGE") << NORMAL << _(" [") << PARAM << _("PACKAGE")
              << NORMAL << _(" ...]]");
          cli << _("\n         \tCreate an instaler, containing the listed "
                   "packages.");
          cli << _("\n         \tIf no output filename is specified, the name "
                   "of the first package is used.");
          cli << _("\n\n  ") << BRIGHT << _("--export") << NORMAL << PARAM
              << _(" TEMPLATE SETFILE ") << NORMAL << _(" [") << PARAM
              << _("OUTFILE") << NORMAL << _("]");
          cli << _("\n         \tExport a set using an export template.");
          cli << _("\n         \tIf no output filename is specified, the "
                   "result is written to stdout.");
          cli << _("\n\n  ") << BRIGHT << _("--export-images") << NORMAL
              << PARAM << _(" SETFILE") << NORMAL << _(" [") << PARAM
              << _("IMAGE") << NORMAL << _("]");
          cli << _("\n         \tExport the cards in a set to image files,");
          cli << _("\n         \tIMAGE is the same format as for 'export all "
                   "card images'.");
          cli << _("\n\n  ") << BRIGHT << _("--cli") << NORMAL << _(" [")
              << BRIGHT << _("--quiet") << NORMAL << _("] [") << BRIGHT
              << _("--raw") << NORMAL << _("] [") << BRIGHT << _("--script ")
              << NORMAL << PARAM << _("FILE") << NORMAL << _("] [") << PARAM
              << _("SETFILE") << NORMAL << _("]");
          ...
          cli << _("\n         \tUse ") << BRIGHT << _("--raw") << NORMAL
              << _(" for raw output mode.");
          cli << _("\n         \tUse ") << BRIGHT << _("--script") << NORMAL
              << _(" to execute a script file.");
          cli << _(
              "\n\nRaw output mode is intended for use by other programs:");
          cli << _("\n    - The only output is only in response to commands.");
          cli << _("\n    - For each command a single 'record' is written to "
                   "the standard output.");
          cli << _("\n    - The record consists of:");
          cli << _("\n        - A line with an integer status code, 0 for ok, "
                   "1 for warnings, 2 for errors");
          cli << _("\n        - A line containing an integer k, the number of "
                   "lines to follow");
          cli << _("\n        - k lines, each containing UTF-8 encoded string "
                   "data.");
          cli << ENDL;
          cli.flush();
          return EXIT_SUCCESS;
          ...
          // dump version
          cli << _("Magic Set Editor\n");
          cli << _("Version ") << app_version.toString() << version_suffix
              << ENDL;
          cli.flush();
          return EXIT_SUCCESS;
          ...
          for (size_t i = 1; i < args.size(); ++i) {
            ...
            wxFileName f(arg);
            if (f.GetExt() == _("mse-set") || f.GetExt() == _("mse") ||
                f.GetExt() == _("set")) {
              set = import_set(arg);
          ...
          if (scripts.empty()) {
            ...
            FOR_EACH(script, scripts) { cli_interface.run_script_file(script); }
          }
          return EXIT_SUCCESS;
          ...
          // path
          String out =
              args.size() >= 3 ? args[2] : settings.gameSettingsFor(*set->game)
...
#else
  wchar_t file_[1024];
  mbstowcs(file_, file, 1023);
  wchar_t cond_[1024];
  mbstowcs(cond_, cond, 1023);
  wchar_t msg_[1024];
  mbstowcs(msg_, msg, 1023);
