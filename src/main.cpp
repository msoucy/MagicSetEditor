...
class MSE : public wxApp {
  public:
    ...
    /** Use OnRun instead of OnInit, so we can determine whether or not we need
     * a main loop
    ...
    void HandleEvent(wxEvtHandler *handler, wxEventFunction func,
                     wxEvent &event) const;
    /// Hack around some wxWidget idiocies
    int FilterEvent(wxEvent &ev);
/// Fancier assert
#if defined(_MSC_VER) && defined(_DEBUG) && defined(_CRT_WIDE)
    void OnAssert(const wxChar *file, int line, const wxChar *cond,
                  const wxChar *msg);
#endif
};
    ...
    try {
        ...
        // interpret command line
        {
            ...
            for (int i = 1; i < argc; ++i) {
                ...
                if (args.back() == (L"--color"))
                    args.pop_back(); // ingnore the --color argument, it is
                                     // handled by cli.init()
            }
            if (!args.empty()) {
                // Find the extension
                wxFileName f(
                    args[0].Mid(0, args[0].find_last_not_of((L"\\/")) + 1));
                if (f.GetExt() == (L"mse-symbol")) {
                    // Show the symbol editor
                    Window *wnd = new SymbolWindow(nullptr, args[0]);
                    wnd->Show();
                    return runGUI();
                } else if (f.GetExt() == (L"mse-set") ||
                           f.GetExt() == (L"mse") || f.GetExt() == (L"set")) {
                    // Show the set window
                    ...
                    if (inst.prefered_filename.empty()) {
                        throw Error(
                            (L"Specify packages to include in installer"));
                    } else {
                        inst.saveAs(inst.prefered_filename, false);
                    }
                    return EXIT_SUCCESS;
                } else if (args[0] == (L"--help") || args[0] == (L"-?") ||
                           args[0] == (L"/?")) {
                    // command line help
                    cli << (L"Magic Set Editor\n\n");
                    cli << (L"Usage: ") << BRIGHT << argv[0] << NORMAL
                        << (L" [") << PARAM << (L"OPTIONS") << NORMAL << (L"]");
                    cli << (L"\n\n  no options");
                    cli << (L"\n         \tStart the MSE user interface "
                            L"showing the welcome window.");
                    cli << (L"\n\n  ") << BRIGHT << (L"-?") << NORMAL << (L", ")
                    ...
                    cli << (L"\n\n  ") << PARAM << (L"FILE") << FILE_EXT
                        << (L".mse-set") << NORMAL << (L", ") << PARAM
                        << (L"FILE") << FILE_EXT << (L".set") << NORMAL
                        << (L", ") << PARAM << (L"FILE") << FILE_EXT
                        << (L".mse") << NORMAL;
                    ...
                    cli << (L"\n         \tLoad the symbol into the MSE symbol "
                            L"editor.");
                    cli << (L"\n\n  ") << PARAM << (L"FILE") << FILE_EXT
                        << (L".mse-installer") << NORMAL << (L" [") << BRIGHT
                        << (L"--local") << NORMAL << (L"]");
                    cli << (L"\n         \tInstall the packages from the "
                            L"installer.");
                    cli << (L"\n         \tIf the ") << BRIGHT << (L"--local")
                        << NORMAL << (L" flag is passed, install packages for "
                                      L"this user only.");
                    cli << (L"\n\n  ") << BRIGHT << (L"--symbol-editor")
                        << NORMAL;
                    cli << (L"\n         \tShow the symbol editor instead of "
                            L"the welcome window.");
                    cli << (L"\n\n  ") << BRIGHT << (L"--create-installer")
                        << NORMAL << (L" [") << PARAM << (L"OUTFILE")
                        << FILE_EXT << (L".mse-installer") << NORMAL << (L"] [")
                        << PARAM << (L"PACKAGE") << NORMAL << (L" [") << PARAM
                        << (L"PACKAGE") << NORMAL << (L" ...]]");
                    cli << (L"\n         \tCreate an instaler, containing the "
                            L"listed packages.");
                    cli << (L"\n         \tIf no output filename is specified, "
                            L"the name of the first package is used.");
                    cli << (L"\n\n  ") << BRIGHT << (L"--export") << NORMAL
                        << PARAM << (L" TEMPLATE SETFILE ") << NORMAL << (L" [")
                        << PARAM << (L"OUTFILE") << NORMAL << (L"]");
                    cli << (L"\n         \tExport a set using an export "
                            L"template.");
                    cli << (L"\n         \tIf no output filename is specified, "
                            L"the result is written to stdout.");
                    cli << (L"\n\n  ") << BRIGHT << (L"--export-images")
                        << NORMAL << PARAM << (L" SETFILE") << NORMAL << (L" [")
                        << PARAM << (L"IMAGE") << NORMAL << (L"]");
                    cli << (L"\n         \tExport the cards in a set to image "
                            L"files,");
                    cli << (L"\n         \tIMAGE is the same format as for "
                            L"'export all card images'.");
                    cli << (L"\n\n  ") << BRIGHT << (L"--cli") << NORMAL
                        << (L" [") << BRIGHT << (L"--quiet") << NORMAL
                        << (L"] [") << BRIGHT << (L"--raw") << NORMAL
                        << (L"] [") << BRIGHT << (L"--script ") << NORMAL
                        << PARAM << (L"FILE") << NORMAL << (L"] [") << PARAM
                        << (L"SETFILE") << NORMAL << (L"]");
                    cli << (L"\n         \tStart the command line interface "
                            L"for performing commands on the set file.");
                    cli << (L"\n         \tUse ") << BRIGHT << (L"-q") << NORMAL
                        << (L" or ") << BRIGHT << (L"--quiet") << NORMAL
                        << (L" to supress the startup banner and prompts.");
                    cli << (L"\n         \tUse ") << BRIGHT << (L"--raw")
                        << NORMAL << (L" for raw output mode.");
                    cli << (L"\n         \tUse ") << BRIGHT << (L"--script")
                        << NORMAL << (L" to execute a script file.");
                    cli << (L"\n\nRaw output mode is intended for use by other "
                            L"programs:");
                    cli << (L"\n    - The only output is only in response to "
                            L"commands.");
                    cli << (L"\n    - For each command a single 'record' is "
                            L"written to the standard output.");
                    cli << (L"\n    - The record consists of:");
                    cli << (L"\n        - A line with an integer status code, "
                            L"0 for ok, 1 for warnings, 2 for errors");
                    cli << (L"\n        - A line containing an integer k, the "
                            L"number of lines to follow");
                    cli << (L"\n        - k lines, each containing UTF-8 "
                            L"encoded string data.");
                    cli << ENDL;
                    cli.flush();
                    return EXIT_SUCCESS;
                } else if (args[0] == (L"--version") || args[0] == (L"-v") ||
                           args[0] == (L"-V")) {
                    // dump version
                    cli << (L"Magic Set Editor\n");
                    cli << (L"Version ") << app_version.toString()
                        << version_suffix << ENDL;
                    cli.flush();
                    return EXIT_SUCCESS;
                    ...
                    for (size_t i = 1; i < args.size(); ++i) {
                        ...
                        if (f.GetExt() == (L"mse-set") ||
                            ...
                            throw Error((L"Invalid command line argument: ") +
                                        arg);
                        }
                    }
                    ...
                    if (args.size() < 2) {
                        throw Error(
                            (L"No input file specified for --export-images"));
                    }
                    SetP set = import_set(args[1]);
                    // path
                    String out = args.size() >= 3
                                     ? args[2]
                                     : settings.gameSettingsFor(*set->game)
                    ...
                    // export
                    export_images(set, set->cards, path, out,
                                  CONFLICT_NUMBER_OVERWRITE);
                    return EXIT_SUCCESS;
                } else if (args[0] == (L"--export")) {
                    if (args.size() < 2) {
                        throw Error(
                            (L"No export template specified for --export"));
                    } else if (args.size() < 3) {
                        throw Error(
                            (L"No input set file specified for --export"));
                    }
                    String export_template = args[1];
                    ExportTemplateP exp =
                        ExportTemplate::byName(export_template);
...
void MSE::HandleEvent(wxEvtHandler *handler, wxEventFunction func,
                      wxEvent &event) const {
    try {
    ...
    if (wxNotifyEvent *nev = dynamic_cast<wxNotifyEvent *>(&event)) {
        // notifications about for instance list box selection will fail if an
        // exception is thrown
        nev->Veto();
    }
}
...
void MSE::OnAssert(const wxChar *file, int line, const wxChar *cond,
                   const wxChar *msg) {
