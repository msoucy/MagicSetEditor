	...
	try {
		...
		// interpret command line
		{
			...
			if (!args.empty()) {
				// Find the extension
				wxFileName f(
					args[0].Mid(0, args[0].find_last_not_of(_("\\/")) + 1));
				if (f.GetExt() == _("mse-symbol")) {
					// Show the symbol editor
					Window *wnd = new SymbolWindow(nullptr, args[0]);
					wnd->Show();
					return runGUI();
				} else if (f.GetExt() == _("mse-set") ||
						   f.GetExt() == _("mse") || f.GetExt() == _("set")) {
					// Show the set window
					...
					if (inst.prefered_filename.empty()) {
						throw Error(
							_("Specify packages to include in installer"));
					} else {
					...
					// dump version
					cli << _("Magic Set Editor\n");
					cli << _("Version ") << app_version.toString()
						<< version_suffix << ENDL;
					cli.flush();
					return EXIT_SUCCESS;
					...
					for (size_t i = 1; i < args.size(); ++i) {
						...
						if (f.GetExt() == _("mse-set") ||
							...
							throw Error(_("Invalid command line argument: ") +
										arg);
						}
					}
					...
					if (args.size() < 2) {
						throw Error(
							_("No input file specified for --export-images"));
					}
					...
					if (args.size() < 2) {
						throw Error(
							_("No export template specified for --export"));
					} else if (args.size() < 3) {
						throw Error(
							_("No input set file specified for --export"));
					}
