...
class ExportCardSelectionChoice
	: public IntrusivePtrBase<ExportCardSelectionChoice> {
  public:
	...
	vector<CardP> own_cards; ///< Maybe we own the cards, in that case the_cards
							 ///= &own_cards
};
...
class CardSelectWindow : public wxDialog {
  public:
	CardSelectWindow(Window *parent, const SetP &set, const String &label,
					 const String &title, bool sizer = true);
