...
class DropDownList : public wxPopupWindow {
  public:
    ...
    DropDownList(Window *parent, bool is_submenu = false,
                 ValueViewer *viewer = nullptr);
  ...
  protected:
    ...
    // An extra submenu that pops up from an item, or null if there is no popup
    // menu
  ...
  private:
    ...
    DropDownList
        *open_sub_menu;        ///< The sub menu that is currently shown, if any
    DropDownList *parent_menu; ///< The parent menu, only applies to sub menus
    ValueViewer *viewer;       ///< The parent viewer object (optional)
    DropDownHider *hider,
        *hider2;             ///< Class to hide this window when we lose focus
    bool close_on_mouse_out; ///< Was the list kept open after selecting a
                             ///choice, if so, be eager to close it
