...
SymbolBasicShapeEditor::SymbolBasicShapeEditor(SymbolControl *control)
    : SymbolEditorBase(control), mode(ID_SHAPE_CIRCLE), drawing(false) {
...
void SymbolBasicShapeEditor::initUI(wxToolBar *tb, wxMenuBar *mb) {
    sides = new wxSpinCtrl(tb, ID_SIDES, (L"3"), wxDefaultPosition,
                           wxDefaultSize, wxSP_ARROW_KEYS, 3, 50, 3);
    // 	sidesL = new wxStaticText(tb, ID_SIDES, (L" ") + _LABEL_("sides") + (L":
    // "));
    sides->SetHelpText(_HELP_("sides"));
    sides->SetSize(50, -1);
    tb->AddSeparator();
    tb->AddTool(ID_SHAPE_CIRCLE, _TOOL_("ellipse"),
                load_resource_tool_image((L"circle")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("ellipse"), _HELP_("ellipse"));
    tb->AddTool(ID_SHAPE_RECTANGLE, _TOOL_("rectangle"),
                load_resource_tool_image((L"rectangle")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("rectangle"), _HELP_("rectangle"));
    tb->AddTool(ID_SHAPE_POLYGON, _TOOL_("polygon"),
                load_resource_tool_image((L"triangle")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("polygon"), _HELP_("polygon"));
    tb->AddTool(ID_SHAPE_STAR, _TOOL_("star"),
                load_resource_tool_image((L"star")), wxNullBitmap, wxITEM_CHECK,
                _TOOLTIP_("star"), _HELP_("star"));
...
void SymbolBasicShapeEditor::onMouseDrag(const Vector2D &from,
                                         const Vector2D &to, wxMouseEvent &ev) {
    // Resize the object
    if (drawing) {
        end = to;
        makeShape(start, end, ev.ControlDown(), settings.symbol_grid_snap,
                  ev.ShiftDown());
...
inline double sgn(double d) { return d < 0 ? -1 : 1; }

void SymbolBasicShapeEditor::makeShape(Vector2D a, Vector2D b, bool constrained,
                                       bool snap, bool centered) {
    // snap
    if (snap) {
...
void SymbolBasicShapeEditor::makeCenteredShape(const Vector2D &c, Vector2D r,
                                               bool constrained) {
    ...
    case ID_SHAPE_CIRCLE: {
        ...
        // a circle has 4 control points, the first is: (x+r, y) db(0, kr) da(0,
        // -kr)
        // kr is a magic constant
        const double kr = 0.5522847498f; // = 4/3 * (sqrt(2) - 1)
        shape->points.push_back(intrusive(new ControlPoint(
            c.x + r.x, c.y, 0, kr * r.y, 0, -kr * r.y, LOCK_SIZE)));
        shape->points.push_back(intrusive(new ControlPoint(
            c.x, c.y - r.y, kr * r.x, 0, -kr * r.x, 0, LOCK_SIZE)));
        shape->points.push_back(intrusive(new ControlPoint(
            c.x - r.x, c.y, 0, -kr * r.y, 0, kr * r.y, LOCK_SIZE)));
        shape->points.push_back(intrusive(new ControlPoint(
            c.x, c.y + r.y, -kr * r.x, 0, kr * r.x, 0, LOCK_SIZE)));
    ...
    case ID_SHAPE_RECTANGLE: {
        ...
        shape->points.push_back(
            intrusive(new ControlPoint(c.x - r.x, c.y - r.y)));
        shape->points.push_back(
            intrusive(new ControlPoint(c.x + r.x, c.y - r.y)));
        shape->points.push_back(
            intrusive(new ControlPoint(c.x + r.x, c.y + r.y)));
    ...
    default: {
        ...
        if (mode == ID_SHAPE_POLYGON) {
            switch (n) {
            case 3:
                shape->name = capitalize(_TYPE_("triangle"));
                break;
            case 4:
                shape->name = capitalize(_TYPE_("rhombus"));
                break;
            case 5:
                shape->name = capitalize(_TYPE_("pentagon"));
                break;
            case 6:
                shape->name = capitalize(_TYPE_("hexagon"));
                break;
            default:
                shape->name = capitalize(_TYPE_("polygon"));
                break;
            }
        } else { // star
        ...
        //  gamma = (pi - beta) / 2
        // equations:
        //  lb * sin(gamma) == 1               (right angled tri /_\ aXc where X
        //  is halfway fc)
        ...
        if (mode == ID_SHAPE_POLYGON) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                shape->points.push_back(intrusive(new ControlPoint(
                    c.x + ra * r.x * sin(theta), y - ra * r.y * cos(theta))));
            }
        } else {
            ...
            // NOTE: This is only correct for n<=6, but gives acceptable results
            // for higher n
