...
enum SelectMode {
	...
	,
	SELECT_IF_OUTSIDE // define a new selection if the affected part is not
					  // already selected
	,
...
class SymbolPartsSelection {
  public:
	...
	/// Toggle the selection of the parts in a rectangle (a,b) or (a,c) but not
	/// in both
	/** Return true if the selection changed */
	 ...
	 *  Does not select inside groups unless the part in question is already
	 * selected.
	 */
  ...
  private:
	...
	bool selectRect(const SymbolGroup &parent, const Vector2D &a,
