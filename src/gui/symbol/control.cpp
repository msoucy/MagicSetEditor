...
void SymbolControl::switchEditor(const SymbolEditorBaseP &e) {
	if (editor)
		editor->destroyUI(parent->GetToolBar(), parent->GetMenuBar());
	editor = e;
	if (editor)
...
void SymbolControl::onExtraTool(wxCommandEvent &ev) {
	...
	default:
		if (editor)
			editor->onCommand(ev.GetId());
	}
}

void SymbolControl::onAction(const Action &action, bool undone) {
	TYPE_CASE_(action, SymbolPartAction) { Refresh(false); }
}
...
void SymbolControl::signalSelectionChange() { parent->onSelectFromControl(); }
...
void SymbolControl::onMotion(wxMouseEvent &ev) {
	...
	if (ev.LeftIsDown()) {
		if (editor)
			editor->onMouseDrag(last_pos, pos, ev);
	} else {
		if (editor)
			editor->onMouseMove(last_pos, pos, ev);
	}
	last_pos = pos;
}
