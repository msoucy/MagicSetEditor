...
SymbolPointEditor::SymbolPointEditor(SymbolControl *control,
									 const SymbolShapeP &part)
	: SymbolEditorBase(control), part(part), selection(SELECTED_NONE),
...
void SymbolPointEditor::drawHoveredLine(DC &dc) {
	...
	for (int i = 1; i <= 100; ++i) {
		...
		double selectPercent =
			1.0 - 1.2 * sqrt(fabs(hover_line_t - t)); // amount to color
		if (selectPercent > 0) {
			// gradient color
			Color color(col(300 - int(300 * selectPercent)),
						col(300 * int(selectPercent)), col(0));
...
void SymbolPointEditor::drawControlPoint(DC &dc, const ControlPoint &p,
										 bool drawHandleBefore,
										 bool drawHandleAfter) {
	// Position
	...
	if (drawHandleBefore || drawHandleAfter) {
		...
		if (drawHandleBefore && p.segment_before == SEGMENT_CURVE) {
			...
			dc.SetPen(handlePen(handleHovered(p, HANDLE_BEFORE) ? PEN_HOVER
																: PEN_NORMAL,
								p.lock));
		...
		if (drawHandleAfter && p.segment_after == SEGMENT_CURVE) {
			...
			dc.SetPen(handlePen(PEN_LINE, p.lock));
			dc.DrawLine(p0.x, p0.y, p1.x, p1.y);
			dc.SetPen(handlePen(handleHovered(p, HANDLE_AFTER) ? PEN_HOVER
															   : PEN_NORMAL,
								p.lock));
			drawHandleCircle(dc, p1.x, p1.y);
		}
...
wxPen SymbolPointEditor::handlePen(WhichPen p, LockMode lock) {
	...
	case PEN_HOVER:
		return wxPen(col, 2);
	case PEN_LINE:
		return wxPen(col, 1, wxDOT);
	default:
		throw InternalError(_("SymbolPointEditor::handlePen"));
	}
}
...
void SymbolPointEditor::initUI(wxToolBar *tb, wxMenuBar *mb) {
	// Initialize toolbar
	tb->AddSeparator();
	tb->AddTool(ID_SEGMENT_LINE, _TOOL_("line segment"),
				load_resource_tool_image(_("line")), wxNullBitmap, wxITEM_CHECK,
				_TOOLTIP_("line segment"), _HELP_("line segment"));
	tb->AddTool(ID_SEGMENT_CURVE, _TOOL_("curve segment"),
				load_resource_tool_image(_("curve")), wxNullBitmap,
				wxITEM_CHECK, _TOOLTIP_("curve segment"),
				_HELP_("curve segment"));
	tb->AddSeparator();
	tb->AddTool(ID_LOCK_FREE, _TOOL_("free point"),
				load_resource_tool_image(_("lock_free")), wxNullBitmap,
				wxITEM_CHECK, _TOOLTIP_("free point"), _HELP_("free point"));
	tb->AddTool(ID_LOCK_DIR, _TOOL_("smooth point"),
				load_resource_tool_image(_("lock_dir")), wxNullBitmap,
				wxITEM_CHECK, _TOOLTIP_("smooth point"),
				_HELP_("smooth point"));
	tb->AddTool(ID_LOCK_SIZE, _TOOL_("symmetric point"),
				load_resource_tool_image(_("lock_size")), wxNullBitmap,
				wxITEM_CHECK, _TOOLTIP_("symmetric point"),
				_HELP_("symmetric point"));
	tb->Realize();
	// TODO : menu bar
...
void SymbolPointEditor::onUpdateUI(wxUpdateUIEvent &ev) {
	...
	switch (ev.GetId()) {
	case ID_SEGMENT_LINE:
	case ID_SEGMENT_CURVE:
		enabled = selection == SELECTED_LINE;
		break;
	case ID_LOCK_FREE:
	case ID_LOCK_DIR:
	case ID_LOCK_SIZE:
		enabled = selection == SELECTED_POINTS && selected_points.size() == 1 &&
...
void SymbolPointEditor::onLeftDClick(const Vector2D &pos, wxMouseEvent &ev) {
	...
	if (hovering == SELECTED_NEW_POINT) {
		// Add point
		ControlPointAddAction *act =
			new ControlPointAddAction(part, hover_line_1_idx, hover_line_t);
		addAction(act);
		// select the new point
		selectPoint(act->getNewPoint(), false);
		selection = SELECTED_POINTS;
	} else if (hovering == SELECTED_HANDLE &&
			   hover_handle.handle == HANDLE_MAIN) {
		// Delete point
...
void SymbolPointEditor::onMouseMove(const Vector2D &from, const Vector2D &to,
									wxMouseEvent &ev) {
...
void SymbolPointEditor::onMouseDrag(const Vector2D &from, const Vector2D &to,
									wxMouseEvent &ev) {
	Vector2D delta = to - from;
	if (selection == SELECTED_LINE && ev.AltDown()) {
		// Drag the curve
		if (controlPointMoveAction)
			controlPointMoveAction = 0;
		if (!curveDragAction) {
			curveDragAction =
				new CurveDragAction(selected_line1, selected_line2);
		...
		if (!controlPointMoveAction) {
			...
			controlPointMoveAction =
				new ControlPointMoveAction(selected_points);
...
void SymbolPointEditor::onKeyChange(wxKeyEvent &ev) {
	if (ev.GetKeyCode() == WXK_ALT &&
		(hovering == SELECTED_LINE || hovering == SELECTED_NEW_POINT)) {
		if (ev.AltDown()) {
...
void SymbolPointEditor::selectHandle(const SelectedHandle &h,
									 const wxMouseEvent &keystate) {
...
bool SymbolPointEditor::handleHovered(const ControlPointP &pnt,
									  WhichHandle wh) {
	return hovering == SELECTED_HANDLE && hover_handle.point == pnt &&
		   hover_handle.handle == wh;
}
bool SymbolPointEditor::handleHovered(const ControlPoint &pnt, WhichHandle wh) {
	return hovering == SELECTED_HANDLE && hover_handle.point &&
		   hover_handle.point.get() == &pnt && hover_handle.handle == wh;
}
...
void SymbolPointEditor::onChangeSegment(SegmentMode mode) {
	...
	if (selected_line1->segment_after == mode)
		return;
...
bool SymbolPointEditor::checkPosOnCurve(const Vector2D &pos) {
	...
	for (int i = 0; (size_t)i < size; ++i) {
		...
		if (pos_on_segment(pos, range, *hover_line_1, *hover_line_2, new_point,
						   hover_line_t)) {
...
SelectedHandle SymbolPointEditor::findHandle(const Vector2D &pos) {
	...
	for (int i = 0; (size_t)i < part->points.size(); ++i) {
		...
		bool before =
			sel ||
			pointSelected(part->getPoint(i - 1)); // are the handles visible?
...
bool SymbolPointEditor::inRange(const Vector2D &a, const Vector2D &b,
								double range) {
	return fabs(a.x - b.x) <= range && fabs(a.y - b.y) <= range;
}
