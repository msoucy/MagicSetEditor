...
SymbolPartList::SymbolPartList(Window *parent, int id,
							   SymbolPartsSelection &selection, SymbolP symbol)
	: wxScrolledWindow(parent, id, wxDefaultPosition, wxDefaultSize,
					   wxBORDER_THEME_FIX(wxBORDER_THEME) | wxVSCROLL),
	  selection(selection), state_icons(9, 8) {
	SetScrollRate(0, ITEM_HEIGHT + 1);
	// NOTE: this is based on the order of the SymbolShapeCombine and
	// SymbolSymmetryType enums!
...
void SymbolPartList::onAction(const Action &action, bool undone) {
	...
	TYPE_CASE_(action, SymbolPartAction) {
		symbol_preview.up_to_date = false;
		// some part changed, but we don't know which one, assume it is the
		// selection
	...
	TYPE_CASE_(action, SymmetryTypeAction) {
		if (typing_in)
			cursor = typing_in->name.size(); // can change the name
		Refresh(false);
		return;
	}
...
void SymbolPartList::updatePart(const set<SymbolPartP> &parts, int &i,
								bool parent_updated, const SymbolPartP &part) {
...
void SymbolPartList::onLeftDown(wxMouseEvent &ev) {
	...
	if (ev.GetX() < 0 || ev.GetX() >= GetClientSize().x)
		return;
	...
	if (part) {
		...
		selection.select(part,
						 ev.ShiftDown() ? SELECT_TOGGLE : SELECT_OVERRIDE);
...
void SymbolPartList::onLeftUp(wxMouseEvent &ev) {
	...
	if (drag_parent && drop_parent) {
		...
		// move part
		SymbolGroupP par = drag_parent;
		drag_parent = SymbolGroupP();
		if (par != drop_parent || drag_position != drop_position) {
			if (par != drop_parent && par->parts.size() == 1 &&
				!par->isSymbolSymmetry()) {
				// this leaves a group without elements, remove it
				findParent(*par, par, drag_position); // parent of the group
				symbol->actions.addAction(new UngroupReorderSymbolPartsAction(
					*par, drag_position, *drop_parent, drop_position));
			} else {
				symbol->actions.addAction(new ReorderSymbolPartsAction(
					*par, drag_position, *drop_parent, drop_position));
			}
		} else {
			Refresh(false);
...
void SymbolPartList::onMotion(wxMouseEvent &ev) {
	...
	if (drag_parent) {
		...
		// the drop parent must be an ancestor or sibling of ancestor of the
		// drag_parent
		...
		if (drop_parent) {
			...
			while (drop_parent != symbol) {
				...
				findParent(*drop_parent, drop_parent_parent,
		...
		// refresh?
		if (drop_parent != old_drop_parent ||
			drop_position != old_drop_position ||
			drop_inside != old_drop_inside) {
			Refresh(false);
		}
	}
}
...
void SymbolPartList::onChar(wxKeyEvent &ev) {
	if (typing_in) {
		...
		case WXK_HOME:
			if (cursor != 0) {
				cursor = 0;
				Refresh(false); // TODO: without refresh
			}
			break;
		case WXK_END:
		...
		case WXK_BACK:
			if (cursor > 0 && cursor <= typing_in->name.size()) {
				...
				symbol->actions.addAction(new SymbolPartNameAction(
					typing_in, new_name, cursor, cursor - 1));
			}
			break;
		case WXK_DELETE:
			if (cursor < typing_in->name.size()) {
				...
				symbol->actions.addAction(new SymbolPartNameAction(
					typing_in, new_name, cursor, cursor));
			}
			break;
		default:
...
void SymbolPartList::onSize(wxSizeEvent &) { Refresh(false); }
...
void SymbolPartList::findParent(const SymbolPart &of, SymbolGroupP &parent_out,
								size_t &pos_out) {
	if (!findParent(of, symbol, parent_out, pos_out)) {
		throw InternalError(_("Symbol part without a parent"));
	}
}
bool SymbolPartList::findParent(const SymbolPart &of, const SymbolGroupP &g,
								SymbolGroupP &parent_out, size_t &pos_out) {
	if (!g)
		return false;
	for (size_t i = 0; i < g->parts.size(); ++i) {
		...
		if (findParent(of, dynamic_pointer_cast<SymbolGroup>(g->parts[i]),
					   parent_out, pos_out))
			return true;
	}
	return false;
...
bool SymbolPartList::findDropTarget(const SymbolPartP &parent, int &i,
									bool before) {
	if (parent != symbol)
		--i;
...
void SymbolPartList::OnDraw(DC &dc) {
	...
	// clear background
	wxSize size = piecewise_max(GetVirtualSize() + RealSize(0, ITEM_HEIGHT + 1),
								GetClientSize());
...
void SymbolPartList::drawItem(DC &dc, int x, int &i, bool parent_active,
	...
	if (part != symbol) {
		...
		// draw item : border
		dc.SetPen(line_pen);
		dc.DrawLine(x + ITEM_HEIGHT, y, x + ITEM_HEIGHT,
					y + ITEM_HEIGHT + 1); // line after image
		dc.DrawLine(x, y + ITEM_HEIGHT, size.x, y + ITEM_HEIGHT); // line below
		// update caret
		if (selection.size() == 1 && active) {
			updateCaret(dc, x + ITEM_HEIGHT + 3, y + (ITEM_HEIGHT - h) / 2, h,
						part);
		}
		// draw icon
	...
	if (drag_parent && drop_parent) {
		dc.SetPen(
			wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT), 3));
		if (drop_inside) {
			if (part == drop_parent) {
				// drop inside part
...
const Image &SymbolPartList::symbolPreview() {
	if (!symbol_preview.up_to_date) {
		SolidFillSymbolFilter filter(AColor(0, 0, 0, 40),
									 AColor(255, 255, 255, 40));
...
void SymbolPartList::updateCaret(DC &dc, int x, int y, int h,
								 const SymbolPartP &part) {
	int top;
	GetViewStart(0, &top);
	// make caret
