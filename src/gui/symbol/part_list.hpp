...
#define EVT_PART_ACTIVATE(id, handler)                                         \
    EVT_COMMAND(id, EVENT_PART_ACTIVATE, handler)
...
class SymbolPartList : public wxScrolledWindow, public SymbolView {
  public:
    SymbolPartList(Window *parent, int id, SymbolPartsSelection &selection,
  ...
  private:
    ...
    void drawItem(DC &dc, int x, int &i, bool parent_active,
                  const SymbolPartP &part);
    const Image &itemPreview(int i, const SymbolPartP &part);
    const Image &symbolPreview();
    void updatePart(const set<SymbolPartP> &parts, int &i, bool parent_updated,
                    const SymbolPartP &part);
    ...
    void findParent(const SymbolPart &of, SymbolGroupP &parent_out,
                    size_t &pos_out);
    static bool findParent(const SymbolPart &of, const SymbolGroupP &in,
