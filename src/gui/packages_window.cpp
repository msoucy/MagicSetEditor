...
struct HtmlWindowToBrowser : public wxHtmlWindow {
    HtmlWindowToBrowser(Window* parent, int id, const wxPoint& pos, const
wxSize& size, long flags)
...
void PackageInfoPanel::draw(DC &dc) {
    ...
    if (d.icon.Ok()) {
        ...
        dc.DrawBitmap(icon, x + (max_size - icon_w) / 2,
                      y + (max_size - icon_h) / 2);
        x += max_size;
    }
    // package name
    ...
    dc.SetFont(wxFont(16, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL,
                      wxFONTWEIGHT_NORMAL, false, (L"Arial")));
    ...
    // text size?
    int dx = 0, max_dx = 0;
    dc.GetTextExtent(_LABEL_("installed version"), &dx, nullptr);
    max_dx = max(max_dx, dx);
    dc.GetTextExtent(_LABEL_("installable version"), &dx, nullptr);
    max_dx = max(max_dx, dx);
    // dc.GetTextExtent(_LABEL_("installer size"),      &dx, nullptr); max_dx =
    // max(max_dx, dx);
    // dc.GetTextExtent(_LABEL_("installer status"),    &dx, nullptr); max_dx =
    // max(max_dx, dx);
    x += max_dx + 5;
    dc.DrawText(package->installed ? package->installed->version.toString()
                                   : _LABEL_("no version"),
                x, y);
    dc.DrawText(package->installer ? package->description->version.toString()
                                   : _LABEL_("no version"),
                x, y + 1 * dy);
...
wxSize PackageInfoPanel::DoGetBestSize() const { return wxSize(200, 120); }
...
enum Action { KEEP, INSTALL, UPGRADE, REMOVE };
...
void PackagesWindow::init(Window *parent, bool show_only_installable) {
    where = is_install_local(settings.install_type) ? PACKAGE_ACT_LOCAL
                                                    : PACKAGE_ACT_GLOBAL;
    Create(parent, wxID_ANY, _TITLE_("packages window"), wxDefaultPosition,
           wxSize(640, 580),
    ...
    // ui elements
    SetIcon(wxIcon());
    package_list = new PackageUpdateList(
        this, installable_packages, show_only_installable, ID_PACKAGE_LIST);
    package_info = new PackageInfoPanel(this);

    wxToggleButton *keep_button =
        new wxToggleButton(this, ID_KEEP, _BUTTON_("keep package"));
    wxToggleButton *install_button =
        new wxToggleButton(this, ID_INSTALL, _BUTTON_("install package"));
    wxToggleButton *remove_button =
        new wxToggleButton(this, ID_REMOVE, _BUTTON_("remove package"));
    /*
    wxRadioButton* keep_button    = new wxRadioButton(this, ID_KEEP,
    _BUTTON_("keep package"));
    wxRadioButton* install_button = new wxRadioButton(this, ID_INSTALL,
    _BUTTON_("install package"));
    wxRadioButton* upgrade_button = new wxRadioButton(this, ID_UPGRADE,
    _BUTTON_("upgrade package"));
    wxRadioButton* remove_button  = new wxRadioButton(this, ID_REMOVE,
    ...
    */
    ...
    v->Add(CreateButtonSizer(wxOK | wxCANCEL), 0, wxEXPAND | (wxALL & ~wxTOP),
           8);
    SetSizer(v);
...
void PackagesWindow::onActionChange(wxCommandEvent &ev) {
    if (package) {
        PackageAction act = ev.GetId() == ID_INSTALL
                                ? PACKAGE_ACT_INSTALL
                                : ev.GetId() == ID_UPGRADE
                                      ? PACKAGE_ACT_INSTALL
                                      : ev.GetId() == ID_REMOVE
...
void PackagesWindow::onOk(wxCommandEvent &ev) {
    ...
    if (to_remove) {
        int result = wxMessageBox(
            with_modifications == 0
                ? _ERROR_1_("remove packages", String() << to_remove)
                : _ERROR_2_("remove packages modified", String() << to_remove,
                            String() << with_modifications),
    ...
    for (auto &ip : installable_packages) {
        if (ip->has(PACKAGE_ACT_INSTALL) && ip->installer &&
            ...
            if (!progress.Update(step++,
                                 String::Format(_ERROR_("downloading updates"),
                                                ++package_pos, to_download))) {
                return; // aborted
            }
            // download installer
            ...
            if (!is) {
                throw Error(_ERROR_2_("can't download installer",
                                      ip->description->name,
                                      ip->installer->installer_url));
            }
            ip->installer->installer_file =
                wxFileName::CreateTempFileName((L"mse-installer"));
    ...
    for (auto &ip : installable_packages) {
        ...
        if (!progress.Update(step++,
                             String::Format(_ERROR_("installing updates"),
                                            ++package_pos, to_change))) {
            // don't allow abort.
        }
    ...
    wxMessageBox(
        install == success
            ? _ERROR_1_("install packages successful", String() << success)
            : remove == success
                  ? _ERROR_1_("remove packages successful", String() << success)
                  : _ERROR_1_("change packages successful", String()
                                                                << success),
    ...
    //%%       If so, we need to warn with _ERROR_1_("remove packages") and
    //_ERROR_2_("remove packages modified")
    //%%       We probably also refer to the type of package, _TYPE_("package"),
    //for example _TYPE_("locale")
...
void PackagesWindow::onUpdateUI(wxUpdateUIEvent &ev) {
    ...
    case ID_KEEP:
        ...
        w->SetLabel(package && package->installed
                        ? _BUTTON_("keep package")
    ...
    case ID_REMOVE:
        ...
        // w->SetLabel(package && package->... ? _BUTTON_("remove group") :
        // _BUTTON_("remove package"));
        break;
    }
    // TODO: change labels to _BUTTON_("install group"), _BUTTON_("remove
    // group"), _BUTTON_("upgrade group")
}
