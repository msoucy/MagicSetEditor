...
enum PreferedProperty { PREFER_NONE, PREFER_WIDTH, PREFER_HEIGHT };
...
class ImageSlice {
  public:
	...
	// Zoom factor
	inline double zoomX() const {
		return target_size.GetWidth() / (double)selection.width;
	}
	inline double zoomY() const {
		return target_size.GetHeight() / (double)selection.height;
	}
	inline void zoomX(double zoom) {
		selection.width = int(target_size.GetWidth() / zoom);
	}
	inline void zoomY(double zoom) {
		selection.height = int(target_size.GetHeight() / zoom);
	}
};
...
class ImageSliceWindow : public wxDialog {
  public:
	ImageSliceWindow(Window *parent, const Image &source,
					 const wxSize &target_size, const AlphaMask &target_mask);
  ...
  private:
	...
	// Something changed in the selector control, update controls and selection
	// displays
...
class ImageSliceSelector : public wxControl {
  ...
  private:
	...
	wxRect start_selection; ///< selection in slice at start of dragging
	double scaleX,
		scaleY; ///< Amount the source image is scaled to fit in this control
