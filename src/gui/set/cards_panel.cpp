...
CardsPanel::CardsPanel(Window *parent, int id) : SetWindowPanel(parent, id) {
    ...
    collapse_notes = new HoverButton(nodes_panel, ID_COLLAPSE_NOTES,
                                     (L"btn_collapse"), wxNullColour, false);
    ...
    menuCard->Append(ID_CARD_ADD, (L"card_add"), _MENU_("add card"),
                     _HELP_("add card"));
    ...
    menuCard->Append(ID_CARD_REMOVE, (L"card_del"),
                     _MENU_("remove card") + (L" "), _HELP_("remove card"));
    menuCard->AppendSeparator();
    IconMenu *menuRotate = new IconMenu();
    menuRotate->Append(ID_CARD_ROTATE_0, (L"card_rotate_0"), _MENU_("rotate 0"),
                       _HELP_("rotate 0"), wxITEM_CHECK);
    menuRotate->Append(ID_CARD_ROTATE_270, (L"card_rotate_270"),
                       _MENU_("rotate 270"), _HELP_("rotate 270"),
                       wxITEM_CHECK);
    menuRotate->Append(ID_CARD_ROTATE_90, (L"card_rotate_90"),
                       _MENU_("rotate 90"), _HELP_("rotate 90"), wxITEM_CHECK);
    menuRotate->Append(ID_CARD_ROTATE_180, (L"card_rotate_180"),
                       _MENU_("rotate 180"), _HELP_("rotate 180"),
                       wxITEM_CHECK);
    menuCard->Append(wxID_ANY, (L"card_rotate"), _MENU_("orientation"),
...
wxMenu *CardsPanel::makeAddCardsSubmenu(bool add_single_card_option) {
    ...
    if (add_single_card_option) {
        ...
        cards_scripts_menu->Append(ID_CARD_ADD, (L"card_add"),
...
void CardsPanel::initUI(wxToolBar *tb, wxMenuBar *mb) {
    ...
    tb->AddTool(ID_FORMAT_SYMBOL, (L""), load_resource_tool_image((L"symbol")),
                ...
                _HELP_("symbols"));
    tb->AddTool(ID_FORMAT_REMINDER, (L""),
                load_resource_tool_image((L"reminder")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("reminder text"),
                _HELP_("reminder text"));
    tb->AddSeparator();
#if HAVE_TOOLBAR_DROPDOWN_MENU
    toolAddCard =
        tb->AddTool(ID_CARD_ADD, (L""), load_resource_tool_image((L"card_add")),
    ...
    tb->AddTool(ID_CARD_ADD, (L""), load_resource_tool_image((L"card_add")),
                ...
                _HELP_("add card"));
#endif
    tb->AddTool(ID_CARD_REMOVE, (L""), load_resource_tool_image((L"card_del")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("remove card"),
                _HELP_("remove card"));
    tb->AddSeparator();
#if HAVE_TOOLBAR_DROPDOWN_MENU
    wxToolBarToolBase *rot = tb->AddTool(
        ID_CARD_ROTATE, (L""), load_resource_tool_image((L"card_rotate")),
...
#else
    tb->AddTool(ID_CARD_ROTATE, (L""),
                load_resource_tool_image((L"card_rotate")), wxNullBitmap,
...
void CardsPanel::onUpdateUI(wxUpdateUIEvent &ev) {
    ...
    case ID_COLLAPSE_NOTES: {
        ...
        collapse_notes->loadBitmaps(collapse ? (L"btn_collapse")
                                             : (L"btn_expand"));
