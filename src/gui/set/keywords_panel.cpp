...
void KeywordsPanel::initControls() {
	...
	s1->Add(new wxStaticText(panel, wxID_ANY, _LABEL_("mode") + _(":")), 0,
			wxTOP, 2);
	...
	// s->Add(new wxStaticText(this, wxID_ANY, _("Sorry, no keywords for
	...
	menuKeyword->Append(ID_KEYWORD_ADD, _("keyword_add"), _MENU_("add keyword"),
						_HELP_("add keyword"));
	...
	menuKeyword->Append(ID_KEYWORD_REMOVE, _("keyword_del"),
						_MENU_("remove keyword") + _(" "),
...
void KeywordsPanel::initUI(wxToolBar *tb, wxMenuBar *mb) {
	...
	// Toolbar
	tb->AddTool(ID_KEYWORD_ADD, _(""),
				load_resource_tool_image(_("keyword_add")), wxNullBitmap,
				wxITEM_NORMAL, _TOOLTIP_("add keyword"), _HELP_("add keyword"));
	tb->AddTool(ID_KEYWORD_REMOVE, _(""),
				load_resource_tool_image(_("keyword_del")), wxNullBitmap,
...
void KeywordsPanel::onCommand(int id) {
	...
	default:
		if (id >= ID_PARAM_TYPE_MIN && id < ID_PARAM_TYPE_MAX) {
			...
			String to_insert =
				_("<atom-param>") + param->name + _("</atom-param>");
