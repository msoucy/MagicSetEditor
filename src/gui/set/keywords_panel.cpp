...
KeywordsPanel::KeywordsPanel(Window *parent, int id)
    : SetWindowPanel(parent, id), menuKeyword(nullptr) {
...
void KeywordsPanel::initControls() {
    ...
    mode = new wxChoice(panel, ID_KEYWORD_MODE, wxDefaultPosition,
                        wxDefaultSize, 0, nullptr);
    match = new TextCtrl(panel, ID_MATCH, false);
    add_param =
        new wxButton(panel, ID_KEYWORD_ADD_PARAM, _BUTTON_("insert parameter"));
    reminder =
        new TextCtrl(panel, ID_REMINDER, true); // allow multiline for wordwrap
    ref_param =
        new wxButton(panel, ID_KEYWORD_REF_PARAM, _BUTTON_("refer parameter"));
    ...
    wxStaticBitmap *fixedI = new wxStaticBitmap(
        panel, wxID_ANY, wxArtProvider::GetBitmap(wxART_WARNING));
    ...
    sp->Add(fixed, 0, wxEXPAND);
    sp->Show(fixed, false);
    wxSizer *s1 = new wxBoxSizer(wxVERTICAL);
    s1->Add(new wxStaticText(panel, wxID_ANY, _LABEL_("keyword") + (L":")), 0,
            wxTOP, 4);
    s1->Add(keyword, 0, wxEXPAND | wxTOP, 2);
    s1->Add(new wxStaticText(panel, wxID_ANY, _LABEL_("mode") + (L":")), 0,
            wxTOP, 2);
    ...
    // s->Add(new wxStaticText(this, wxID_ANY, (L"Sorry, no keywords for
    // now"),wxDefaultPosition,wxDefaultSize,wxALIGN_CENTER), 1,
    // wxALIGN_CENTER); // TODO: Remove
    /*	wxSizer* s2 = new wxBoxSizer(wxVERTICAL);
    ...
    menuKeyword->Append(ID_KEYWORD_PREV, _MENU_("previous keyword"),
                        _HELP_("previous keyword"));
    menuKeyword->Append(ID_KEYWORD_NEXT, _MENU_("next keyword"),
                        _HELP_("next keyword"));
    menuKeyword->AppendSeparator();
    menuKeyword->Append(ID_KEYWORD_ADD, (L"keyword_add"), _MENU_("add keyword"),
                        _HELP_("add keyword"));
    ...
    menuKeyword->Append(ID_KEYWORD_REMOVE, (L"keyword_del"),
                        _MENU_("remove keyword") + (L" "),
                        _HELP_("remove keyword"));
}

KeywordsPanel::~KeywordsPanel() { delete menuKeyword; }
...
void KeywordsPanel::initUI(wxToolBar *tb, wxMenuBar *mb) {
    ...
    // Toolbar
    tb->AddTool(ID_KEYWORD_ADD, (L""),
                load_resource_tool_image((L"keyword_add")), wxNullBitmap,
                wxITEM_NORMAL, _TOOLTIP_("add keyword"), _HELP_("add keyword"));
    tb->AddTool(ID_KEYWORD_REMOVE, (L""),
                load_resource_tool_image((L"keyword_del")), wxNullBitmap,
                wxITEM_NORMAL, _TOOLTIP_("remove keyword"),
                _HELP_("remove keyword"));
    ...
    if (!filter) {
        filter =
            new FilterCtrl(tb, ID_KEYWORD_FILTER, _LABEL_("search keywords"));
...
void KeywordsPanel::onUpdateUI(wxUpdateUIEvent &ev) {
    ...
    case ID_KEYWORD_PREV:
        ev.Enable(list->canSelectPrevious());
        break;
    case ID_KEYWORD_NEXT:
        ev.Enable(list->canSelectNext());
        break;
    case ID_KEYWORD_REMOVE:
        ev.Enable(list->getKeyword() && !list->getKeyword()->fixed);
        break;
    }
}
...
void KeywordsPanel::onCommand(int id) {
    ...
    case ID_KEYWORD_REF_PARAM: {
        ...
        for (auto &p : list->getKeyword()->parameters) {
            String item = String() << ++param << (L". ") << LEFT_ANGLE_BRACKET
    ...
    default:
        if (id >= ID_PARAM_TYPE_MIN && id < ID_PARAM_TYPE_MAX) {
            // add parameter
            KeywordParamP param =
                set->game->keyword_parameter_types.at(id - ID_PARAM_TYPE_MIN);
            String to_insert =
                (L"<atom-param>") + param->name + (L"</atom-param>");
...
#define CUT_COPY_PASTE(op, return, check)                                      \
    ...
    if (id == ID_KEYWORD_LIST && keyword->IsEnabled()) {                       \
        return list->op();                                                     \
    } else if (check) {                                                        \
        return false;                                                          \
    } else if (id == ID_KEYWORD && keyword->IsEnabled()) {                     \
        return keyword->op();                                                  \
    } else if (id == ID_MATCH && match->IsEnabled()) {                         \
        return match->op();                                                    \
    } else if (id == ID_REMINDER && reminder->IsEnabled()) {                   \
        return reminder->op();                                                 \
    } else if (id == ID_RULES && rules->IsEnabled()) {                         \
        return rules->op();                                                    \
    } else {                                                                   \
        return false;                                                          \
    }
...
bool KeywordsPanel::canCut() const {
    CUT_COPY_PASTE(canCut, return,
                   !list->getKeyword() || list->getKeyword()->fixed)
}
bool KeywordsPanel::canPaste() const {
    CUT_COPY_PASTE(canPaste, return,
                   !list->getKeyword() || list->getKeyword()->fixed)
}
void KeywordsPanel::doCopy() { CUT_COPY_PASTE(doCopy, return (void), false) }
void KeywordsPanel::doCut() {
    CUT_COPY_PASTE(doCut, return (void),
                   !list->getKeyword() || list->getKeyword()->fixed)
}
void KeywordsPanel::doPaste() {
    CUT_COPY_PASTE(doPaste, return (void),
                   !list->getKeyword() || list->getKeyword()->fixed)
}
...
void KeywordsPanel::onAction(const Action &action, bool undone) {
    ...
    TYPE_CASE(action, ValueAction) {
        if (!action.card) {
            {
                KeywordReminderTextValue *value =
                    dynamic_cast<KeywordReminderTextValue *>(
                        action.valueP.get());
            ...
            {
                KeywordTextValue *value =
                    dynamic_cast<KeywordTextValue *>(action.valueP.get());
                if (value && value->underlying == &list->getKeyword()->match) {
                    // match string changes, maybe there are parameters now
                    ref_param->Enable(!value->keyword.fixed &&
                                      !value->keyword.parameters.empty());
                }
            }
        }
    }
    TYPE_CASE(action, ChangeKeywordModeAction) {
        if (&action.keyword == list->getKeyword().get()) {
            ...
            mode->SetSelection(
                (int)list->getKeyword()->findMode(set->game->keyword_modes));
        }
    }
}
...
void KeywordsPanel::onKeywordSelect(KeywordSelectEvent &ev) {
    if (ev.keyword) {
        ...
        keyword->setValue(intrusive(new KeywordTextValue(
            keyword->getFieldP(), &kw, &kw.keyword, !kw.fixed, true)));
        match->setValue(intrusive(new KeywordTextValue(match->getFieldP(), &kw,
                                                       &kw.match, !kw.fixed)));
        rules->setValue(intrusive(new KeywordTextValue(rules->getFieldP(), &kw,
                                                       &kw.rules, !kw.fixed)));
        intrusive_ptr<KeywordReminderTextValue> reminder_value(
            new KeywordReminderTextValue(*set, reminder->getFieldP(), &kw,
                                         !kw.fixed));
        reminder->setValue(reminder_value);
        errors->SetLabel(reminder_value->errors);
        add_param->Enable(!kw.fixed &&
                          !set->game->keyword_parameter_types.empty());
...
void KeywordsPanel::onModeChange(wxCommandEvent &ev) {
    ...
    if (sel >= 0 && (size_t)sel < set->game->keyword_modes.size()) {
        set->actions.addAction(new ChangeKeywordModeAction(
            *list->getKeyword(), set->game->keyword_modes[sel]->name));
    }
}
