...
class ConsoleMessage : public IntrusivePtrBase<ConsoleMessage> {
  public:
    ...
    ConsoleMessage(MessageType type, String const &text = (L""),
...
class MessageCtrl : public wxScrolledWindow {
  public:
    MessageCtrl(wxWindow *parent, int id)
        ...
        icons[MESSAGE_INPUT] =
            wxBitmap(load_resource_image((L"message_input")));
        icons[MESSAGE_OUTPUT] = wxBitmap();
        icons[MESSAGE_INFO] =
            wxBitmap(load_resource_image((L"message_information")));
        icons[MESSAGE_WARNING] =
            wxBitmap(load_resource_image((L"message_warning")));
        icons[MESSAGE_ERROR] =
            wxBitmap(load_resource_image((L"message_error")));
...
class HistoryTextCtrl : public wxTextCtrl {
  public:
    HistoryTextCtrl(wxWindow *parent, wxWindowID id)
        : wxTextCtrl(parent, id, (L""), wxDefaultPosition, wxDefaultSize,
                     wxTE_PROCESS_ENTER),
          history_pos(0) {}
...
void ConsolePanel::get_pending_errors() {
    ...
    // controls might fail, which results in a popup dialog with an event loop
    if (!messages)
        return;
...
void ConsolePanel::update_blinker() {
    ...
    if (blinker_state % 2 == 1 || !new_errors_since_last_view) {
        parent->setPanelIcon(this,
                             load_resource_image((L"tool/window_console")));
    } else if (new_errors_since_last_view == MESSAGE_INFO) {
        parent->setPanelIcon(this,
                             load_resource_image((L"message_information")));
