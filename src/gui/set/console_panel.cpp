...
class MessageCtrl : public wxScrolledWindow {
  public:
	...
	bool canCopy() const { return have_selection(); }
	bool doCopy() {
  ...
  private:
	...
	void onChar(wxKeyEvent &ev) {
		...
		case WXK_UP:
			if (selection < messages.size() && selection > 0)
				select(selection - 1);
			break;
		case WXK_DOWN:
			if (selection < messages.size() && selection + 1 < messages.size())
				select(selection + 1);
			break;
		case WXK_HOME:
			if (!messages.empty())
	...
	size_t find_point(int y) {
		...
		for (size_t i = 0; i < messages.size(); ++i) {
			if (y >= messages[i]->top && y < messages[i]->bottom())
				return i;
		}
		return (size_t)-1;
	}
	...
	static const int LIST_SPACING;
	static const int ICON_PADDING;
	static const int TEXT_PADDING_LEFT;
	static const int TEXT_PADDING_RIGHT;
	static const int TEXT_PADDING_TOP;
	static const int TEXT_PADDING_BOTTOM;
	static const int TEXT_LINE_SPACING;
	...
	void layout_all(size_t start = 0) {
		...
		for (size_t i = start; i < messages.size(); ++i) {
			...
			if (i > 0 && msg.joined_to_previous)
				msg.top -= LIST_SPACING;
			// text size
...
const int MessageCtrl::LIST_SPACING = 1;
const int MessageCtrl::ICON_PADDING = 3;
const int MessageCtrl::TEXT_PADDING_LEFT = ICON_PADDING + 16 + 4;
const int MessageCtrl::TEXT_PADDING_RIGHT = 4;
const int MessageCtrl::TEXT_PADDING_TOP = 4;
const int MessageCtrl::TEXT_PADDING_BOTTOM = 2;
const int MessageCtrl::TEXT_LINE_SPACING = 1;
...
void ConsolePanel::onEnter(wxCommandEvent &ev) { onCommand(ID_EVALUATE); }
