...
class SetWindow : public wxFrame, public SetView {
  ...
  private:
	 ...
	 */
	void addPanel(IconMenu *windowMenu, wxToolBar *tabBar,
				  SetWindowPanel *panel, UInt pos, const String &image_name,
				  const String &name);
	...
	/// Switch this window to the new set, or open another window for it
  ...
  private:
	...
	/** Returns true  if the action can be continued (the set was saved, or need
	 * not be saved)
	 *  Returns false if the action should be canceled (user pressed cancel, or
	 * save failed)
	 */
