...
void RandomCardList::reset() { cards.clear(); }
...
class PackTotalsPanel : public wxPanel {
  public:
    PackTotalsPanel(Window *parent, int id, PackGenerator &generator,
                    bool show_all = false)
...
void PackTotalsPanel::draw(DC &dc) {
    ...
    for (auto &pack : game->pack_types) {
        ...
        if (pack->summary && (show_all || i.has_cards())) {
            drawItem(dc, y, tr(*game, pack->name, capitalize),
                     i.get_card_copies());
...
void PackTotalsPanel::drawItem(DC &dc, int &y, const String &name,
                               double value) {
    wxSize size = dc.GetSize();
    int w, h;
...
void PackTotalsPanel::setGame(const GameP &game) { this->game = game; }
...
class SelectableLabel : public HoverButtonBase {
  public:
    SelectableLabel(wxWindow *parent, int id, const String &label,
                    bool interactive = true)
        : HoverButtonBase(parent, id, false), label(label),
          interactive(interactive), buddy(nullptr) {}
    void draw(DC &dc) {
        Color bg = wxSystemSettings::GetColour(wxSYS_COLOUR_3DFACE);
        Color fg = wxSystemSettings::GetColour(wxSYS_COLOUR_BTNTEXT);
        // clear background
        dc.SetPen(*wxTRANSPARENT_PEN);
        // dc.SetBrush(mouse_down ?
        // wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHT)
        //                       : hover ? lerp(bg,fg,0.1) : bg);
        // dc.SetTextForeground(mouse_down ?
        // wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHTTEXT) : fg);
        ...
        dc.DrawText(interactive && hover ? label + (L"...") : label, 2,
                    (s.y - h) / 2);
    }
    ...
    void setBuddy(wxWindow *buddy) { this->buddy = buddy; }
...
PackAmountPicker::PackAmountPicker(wxWindow *parent, wxFlexGridSizer *sizer,
                                   const PackTypeP &pack, bool active)
    : pack(pack),
      label(new SelectableLabel(parent, ID_PACK_TYPE,
                                capitalize_sentence(pack->name), active)),
      value(new wxSpinCtrl(parent, ID_PACK_AMOUNT, (L"0"), wxDefaultPosition,
                           wxSize(50, -1))) {
...
class CustomPackDialog : public wxDialog {
  public:
    CustomPackDialog(Window *parent, const SetP &set,
                     const PackTypeP &edited_pack, bool can_remove);
...
CustomPackDialog::CustomPackDialog(Window *parent, const SetP &set,
                                   const PackTypeP &edited_pack,
                                   bool can_remove)
    : wxDialog(parent, wxID_ANY, _TITLE_("custom pack"), wxDefaultPosition,
               wxSize(500, 500)),
      set(set), edited_pack(edited_pack) {
    // init ui
    totals = new PackTotalsPanel(this, wxID_ANY, generator, true);
    name = new wxTextCtrl(this, wxID_ANY,
                          edited_pack ? edited_pack->name : (L"custom pack"));
    wxButton *remove =
    ...
    wxSizer *s2 =
        new wxStaticBoxSizer(wxHORIZONTAL, this, _LABEL_("pack name"));
    s2->Add(name, 1, wxEXPAND | wxALL, 4);
    s->Add(s2, 0, wxEXPAND | wxALL, 8);
    wxSizer *s3 = new wxBoxSizer(wxHORIZONTAL);
    wxSizer *s4 =
        new wxStaticBoxSizer(wxHORIZONTAL, this, _LABEL_("pack selection"));
    s4->AddSpacer(2);
    ...
    wxSizer *s5 =
        new wxStaticBoxSizer(wxHORIZONTAL, this, _LABEL_("pack totals"));
    ...
    for (auto &pack : set->game->pack_types) {
        if (pack->selectable)
            continue; // this pack is already selectable from the main UI
...
void CustomPackDialog::storePack() {
    ...
    for (auto &pick : pickers) {
        int copies = pick.value->GetValue();
        if (copies > 0) {
            edited_pack->items.push_back(
                intrusive(new PackItem(pick.pack->name, copies)));
        }
    }
}

bool CustomPackDialog::isDuplicateName(const String &name) {
    for (const auto &pack : set->game->pack_types) {
        if (pack->name == name)
            return true;
    }
    for (const auto &pack : set->pack_types) {
        if (pack->name == name)
            return true;
    }
    return false;
...
void CustomPackDialog::onOk(wxCommandEvent &) {
    ...
    if (isDuplicateName(name->GetValue())) {
        wxMessageBox(_ERROR_1_("pack type duplicate name", name->GetValue()),
                     _TITLE_("custom pack"), wxOK | wxICON_EXCLAMATION);
        return;
    }
    // done
    storePack();
...
void CustomPackDialog::onAmountChange(wxSpinEvent &) { updateTotals(); }
...
void RandomPackPanel::initControls() {
    ...
    generate_button =
        new wxButton(this, ID_GENERATE_PACK, _BUTTON_("generate pack"));
    seed_random =
        new wxRadioButton(this, ID_SEED_RANDOM, _BUTTON_("random seed"));
    ...
    wxSizer *s3 = new wxBoxSizer(wxHORIZONTAL);
    wxSizer *s4 =
        new wxStaticBoxSizer(wxVERTICAL, this, _LABEL_("pack selection"));
    ...
    s4->Add(new wxButton(this, ID_CUSTOM_PACK, _BUTTON_("add custom pack")), 0,
            wxEXPAND | wxALIGN_TOP | (wxALL & ~wxTOP), 4);
    s3->Add(s4, 1, wxEXPAND, 8);
    wxSizer *s5 =
        new wxStaticBoxSizer(wxHORIZONTAL, this, _LABEL_("pack totals"));
    s5->Add(totals, 1, wxEXPAND | wxALL, 4);
...
RandomPackPanel::~RandomPackPanel() { storeSettings(); }
                ...
                if (dlg.get()) {
                    ...
                    for (size_t i = 0; i < set->pack_types.size(); ++i) {
                        if (set->pack_types[i] == pick.pack) {
                            set->actions.addAction(
                                new ChangePackAction(*set, i, dlg.get()));
                        }
                    }
                } else {
                    // delete pack
                    set->actions.addAction(
                        new AddPackAction(REMOVE, *set, pick.pack));
                }
            }
            break;
        }
...
int RandomPackPanel::getSeed() {
    ...
    if (seed_random->GetValue()) {
        ...
        seed = rand() % 1000 * 1000 + clock() % 1000;
    } else {
...
void RandomPackPanel::selectionChoices(ExportCardSelectionChoices &out) {
    ...
    out.push_back(intrusive(new ExportCardSelectionChoice(
        _BUTTON_("export generated packs"), card_list->getCardsPtr())));
}
...
bool RandomPackPanel::canCopy() const {
    return isInitialized() && card_list->canCopy();
}
