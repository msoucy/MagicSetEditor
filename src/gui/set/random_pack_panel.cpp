...
class SelectableLabel : public HoverButtonBase {
  public:
	...
	void draw(DC &dc) {
		...
		dc.DrawText(interactive && hover ? label + _("...") : label, 2,
					(s.y - h) / 2);
	}
...
PackAmountPicker::PackAmountPicker(wxWindow *parent, wxFlexGridSizer *sizer,
	...
	: pack(pack),
	  ...
	  value(new wxSpinCtrl(parent, ID_PACK_AMOUNT, _("0"), wxDefaultPosition,
						   wxSize(50, -1))) {
...
CustomPackDialog::CustomPackDialog(Window *parent, const SetP &set,
	...
	name = new wxTextCtrl(this, wxID_ANY,
						  edited_pack ? edited_pack->name : _("custom pack"));
	wxButton *remove =
...
bool CustomPackDialog::isDuplicateName(const String &name) {
	...
	for (auto const &pack : set->pack_types) {
		if (pack->name == name)
			return true;
	}
	return false;
