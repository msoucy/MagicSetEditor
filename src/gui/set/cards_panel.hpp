...
class CardsPanel : public SetWindowPanel {
  ...
  private:
	...
	FilterCtrl *filter;
	String filter_value; // value of filter, need separate variable because the
	...
	wxMenuItem *insertSymbolMenu; // owned by menuFormat, but submenu owned by
								  // SymbolFont
	wxMenuItem *
		insertManyCardsMenu; // owned my menuCard, but submenu can be changed
