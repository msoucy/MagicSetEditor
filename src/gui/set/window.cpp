...
SetWindow::SetWindow(Window *parent, const SetP &set)
    : wxFrame(parent, wxID_ANY, _TITLE_("magic set editor"), wxDefaultPosition,
              wxDefaultSize,
              wxDEFAULT_FRAME_STYLE | wxNO_FULL_REPAINT_ON_RESIZE),
      current_panel(nullptr), find_dialog(nullptr), find_data(wxFR_DOWN),
    ...
    menuFile->Append(ID_FILE_NEW, (L"new"), _MENU_("new set"),
                     _HELP_("new set"));
    menuFile->Append(ID_FILE_OPEN, (L"open"), _MENU_("open set"),
                     _HELP_("open set"));
    menuFile->Append(ID_FILE_SAVE, (L"save"), _MENU_("save set"),
                     _HELP_("save set"));
    menuFile->Append(ID_FILE_SAVE_AS, _MENU_("save set as"),
                     _HELP_("save set as"));
    menuExport = new IconMenu();
    menuExport->Append(ID_FILE_EXPORT_HTML, (L"export_html"),
                       _MENU_("export html"), _HELP_("export html"));
    menuExport->Append(ID_FILE_EXPORT_IMAGE, (L"export_image"),
                       _MENU_("export image"), _HELP_("export image"));
    menuExport->Append(ID_FILE_EXPORT_IMAGES, (L"export_images"),
                       _MENU_("export images"), _HELP_("export images"));
    menuExport->Append(ID_FILE_EXPORT_APPR, (L"export_apprentice"),
                       _MENU_("export apprentice"),
                       _HELP_("export apprentice"));
    menuExport->Append(ID_FILE_EXPORT_MWS, (L"export_mws"),
                       _MENU_("export mws"), _HELP_("export mws"));
    menuFile->Append(wxID_ANY, (L"export"), _MENU_("export"), _HELP_("export"),
                     wxITEM_NORMAL, menuExport);
    menuFile->AppendSeparator();
    menuFile->Append(ID_FILE_CHECK_UPDATES, _MENU_("check updates"),
                     _HELP_("check updates"));
#if USE_SCRIPT_PROFILING
    menuFile->Append(ID_FILE_PROFILER, _MENU_("show profiler"),
                     _HELP_("show profiler"));
#endif
    //		menuFile->Append(ID_FILE_INSPECT,					(L"Inspect Internal
    //Data..."),	(L"Shows a the data in the set using a tree structure"));
    //		menuFile->AppendSeparator();
    menuFile->Append(ID_FILE_RELOAD, _MENU_("reload data"),
                     _HELP_("reload data"));
    menuFile->AppendSeparator();
    menuFile->Append(ID_FILE_PRINT_PREVIEW, (L"print_preview"),
                     _MENU_("print preview"), _HELP_("print preview"));
    menuFile->Append(ID_FILE_PRINT, (L"print"), _MENU_("print"),
                     _HELP_("print"));
    menuFile->AppendSeparator();
    ...
    menuEdit->Append(ID_EDIT_UNDO, (L"undo"), _MENU_1_("undo", wxEmptyString),
                     _HELP_("undo"));
    menuEdit->Append(ID_EDIT_REDO, (L"redo"), _MENU_1_("redo", wxEmptyString),
                     _HELP_("redo"));
    menuEdit->AppendSeparator();
    menuEdit->Append(ID_EDIT_CUT, (L"cut"), _MENU_("cut"), _HELP_("cut"));
    menuEdit->Append(ID_EDIT_COPY, (L"copy"), _MENU_("copy"), _HELP_("copy"));
    menuEdit->Append(ID_EDIT_PASTE, (L"paste"), _MENU_("paste"),
                     _HELP_("paste"));
    menuEdit->AppendSeparator();
    menuEdit->Append(ID_EDIT_FIND, (L"find"), _MENU_("find"), _HELP_("find"));
    menuEdit->Append(ID_EDIT_FIND_NEXT, _MENU_("find next"),
                     _HELP_("find next"));
    menuEdit->Append(ID_EDIT_REPLACE, _MENU_("replace"), _HELP_("replace"));
    menuEdit->Append(ID_EDIT_AUTO_REPLACE, _MENU_("auto replace"),
                     _HELP_("auto replace"));
    menuEdit->AppendSeparator();
    menuEdit->Append(ID_EDIT_PREFERENCES, _MENU_("preferences"),
                     _HELP_("preferences"));
    menuBar->Append(menuEdit, _MENU_("edit"));

    IconMenu *menuWindow = new IconMenu();
    menuWindow->Append(ID_WINDOW_NEW, _MENU_("new window"),
                       _HELP_("new window"));
    menuWindow->AppendSeparator();
    menuBar->Append(menuWindow, _MENU_("window"));

    IconMenu *menuHelp = new IconMenu();
    menuHelp->Append(ID_HELP_INDEX, (L"help"), _MENU_("index"),
                     _HELP_("index"));
    ...
    tb->AddTool(ID_FILE_NEW, (L""), load_resource_tool_image((L"new")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("new set"),
                _HELP_("new set"));
    tb->AddTool(ID_FILE_OPEN, (L""), load_resource_tool_image((L"open")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("open set"),
                _HELP_("open set"));
    tb->AddTool(ID_FILE_SAVE, (L""), load_resource_tool_image((L"save")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("save set"),
                _HELP_("save set"));
    tb->AddSeparator();
    tb->AddTool(ID_FILE_EXPORT, (L""), load_resource_tool_image((L"export")),
                wxNullBitmap, wxITEM_CHECK, _TOOLTIP_("export"),
                _HELP_("export"));
    tb->AddSeparator();
    tb->AddTool(ID_EDIT_CUT, (L""), load_resource_tool_image((L"cut")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("cut"), _HELP_("cut"));
    tb->AddTool(ID_EDIT_COPY, (L""), load_resource_tool_image((L"copy")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("copy"), _HELP_("copy"));
    tb->AddTool(ID_EDIT_PASTE, (L""), load_resource_tool_image((L"paste")),
                wxNullBitmap, wxITEM_NORMAL, _TOOLTIP_("paste"),
                _HELP_("paste"));
    tb->AddSeparator();
    tb->AddTool(ID_EDIT_UNDO, (L""), load_resource_tool_image((L"undo")),
                wxNullBitmap, wxITEM_NORMAL,
                _TOOLTIP_1_("undo", wxEmptyString));
    tb->AddTool(ID_EDIT_REDO, (L""), load_resource_tool_image((L"redo")),
                wxNullBitmap, wxITEM_NORMAL,
                _TOOLTIP_1_("redo", wxEmptyString));
    tb->AddSeparator();
    tb->Realize();

    // tab bar, sizer
    wxToolBar *tabBar =
        new wxToolBar(this, ID_TAB_BAR, wxDefaultPosition, wxDefaultSize,
                      wxTB_FLAT | wxNO_BORDER | wxTB_HORIZONTAL |
                          wxTB_HORZ_TEXT | wxTB_NOALIGN);
    ...
    // panels
    addPanel(menuWindow, tabBar, new CardsPanel(this, wxID_ANY), 0,
             (L"window_cards"), (L"cards tab"));
    addPanel(menuWindow, tabBar, new StylePanel(this, wxID_ANY), 1,
             (L"window_style"), (L"style tab"));
    addPanel(menuWindow, tabBar, new SetInfoPanel(this, wxID_ANY), 2,
             (L"window_set_info"), (L"set info tab"));
    addPanel(menuWindow, tabBar, new KeywordsPanel(this, wxID_ANY), 3,
             (L"window_keywords"), (L"keywords tab"));
    addPanel(menuWindow, tabBar, new StatsPanel(this, wxID_ANY), 4,
             (L"window_statistics"), (L"stats tab"));
    addPanel(menuWindow, tabBar, new RandomPackPanel(this, wxID_ANY), 5,
             (L"window_random_pack"), (L"random pack tab"));
    addPanel(menuWindow, tabBar, new ConsolePanel(this, wxID_ANY), 6,
             (L"window_console"), (L"console tab"));
    ...
    try {
        ...
        // clean up!
        // if we don't destroy the panel we could crash in ~CardsPanel, since it
        // expected
...
void SetWindow::addPanel(IconMenu *windowMenu, wxToolBar *tabBar,
                         SetWindowPanel *panel, UInt pos,
    ...
    tabBar->AddTool(id, tab_name + (L"   "),
                    load_resource_tool_image(image_name), wxNullBitmap,
                    wxITEM_CHECK, tab_help, description);
    tabBar->AddSeparator();
    // add to menu bar
...
void toolbar_SetToolNormalBitmap(wxToolBar *toolbar, int id,
                                 wxBitmap const &bitmap) {
#if wxVERSION_NUMBER < 2800
    // copied from wx2.8.11, tbar95.cpp
    wxToolBarToolBase *tool =
        wx_static_cast(wxToolBarToolBase *, toolbar->FindById(id));
    if (tool) {
...
#if wxVERSION_NUMBER < 2800
            // This is needed at least in wx2.6, other versions seem not to need
            // it, but maybe they do
            Layout();
#endif
            // TODO: this could be done better, wx requires a full new Realize
            // of the toolbar, but on win32 a single message would do
...
void SetWindow::onChangeSet() {
    ...
    if (set->cards.empty())
        set->cards.push_back(intrusive(new Card(*set->game)));
...
void SetWindow::onAction(const Action &action, bool undone) {
    TYPE_CASE(action, ValueAction) {
        if (!action.card) {
            if (set->data.contains(action.valueP) &&
                action.valueP->fieldP->identifying) {
                updateTitle();
            }
        }
    }
...
void SetWindow::updateTitle() {
    if (!set) {
        ...
        if (identification.empty())
            identification = set->name();
        if (identification.empty())
            identification = _TITLE_("untitled");
...
void SetWindow::onClose(wxCloseEvent &ev) {
    // only ask if we want to save is this is the only window that has the
    // current set opened
...
int ask_save_changes_impl(wxWindow *parent, String const &message,
                          String const &title) {
#if defined(__WXMSW__) && defined(TD_WARNING_ICON) // the last one is a hack to
                                                   // test for precense of
                                                   // TASKDIALOG stuff
    // Do we have the TaskDialogIndirect function?
    HMODULE h = ::LoadLibrary(L"comctl32.dll");
    if (!h)
        return 0;
    typedef HRESULT(WINAPI * type_TaskDialogIndirect)(
        const TASKDIALOGCONFIG *pTaskConfig, int *pnButton, int *pnRadioButton,
        BOOL *pfVerificationFlagChecked);
    type_TaskDialogIndirect func_TaskDialogIndirect =
        !h ? nullptr
           : (type_TaskDialogIndirect)::GetProcAddress(h, "TaskDialogIndirect");
    if (!func_TaskDialogIndirect)
        return 0;

    int nButtonPressed = 0;
    TASKDIALOGCONFIG config = {0};
    const TASKDIALOG_BUTTON buttons[] = {{IDYES, L"&Save"},
                                         {IDNO, L"Do&n't Save"}};
    ...
    config.hwndParent =
        (HWND)(parent->GetHWND()); // without this the dialog is not modal
...
// Fancy save dialog
int ask_save_changes(wxWindow *parent, String const &message,
                     String const &title) {
    int result = ask_save_changes_impl(parent, message, title);
    if (result)
        return result;
    // Use a normal message box
    return wxMessageBox(message, title,
                        wxYES_NO | wxCANCEL | wxICON_EXCLAMATION, parent);
}

bool SetWindow::askSaveAndContinue() {
    if (set->actions.atSavePoint())
        return true;
    int save =
        ask_save_changes(this, _LABEL_1_("save changes", set->short_name),
                         _TITLE_("save changes"));
    if (save == wxYES) {
        // save the set
        try {
            if (set->needSaveAs()) {
                // need save as
                wxFileDialog dlg(
                    this, _TITLE_("save set"), settings.default_set_dir,
                    clean_filename(set->short_name), export_formats(*set->game),
...
void SetWindow::onUpdateUI(wxUpdateUIEvent &ev) {
    ...
    // file menu
    case ID_FILE_SAVE:
        ev.Enable(!set->actions.atSavePoint() || set->needSaveAs());
        break;
    case ID_FILE_EXPORT_IMAGE:
        ev.Enable(!!current_panel->selectedCard());
        break;
    case ID_FILE_EXPORT_APPR:
        ev.Enable(set->game->isMagic());
        break;
    case ID_FILE_EXPORT_MWS:
        ev.Enable(set->game->isMagic());
        break;
    case ID_FILE_EXIT:
        // update for ID_FILE_RECENT done for a different id, because
        // ID_FILE_RECENT may not be in the menu yet
        updateRecentSets();
        break;
    // undo/redo
    case ID_EDIT_UNDO: {
        ...
        GetToolBar()->SetToolShortHelp(ID_EDIT_UNDO,
                                       _TOOLTIP_1_("undo", label));
        break;
    }
    case ID_EDIT_REDO: {
        ...
        GetToolBar()->SetToolShortHelp(ID_EDIT_REDO,
                                       _TOOLTIP_1_("redo", label));
        break;
    }
    // copy & paste & find
    case ID_EDIT_CUT:
        ev.Enable(current_panel->canCut());
        break;
    case ID_EDIT_COPY:
        ev.Enable(current_panel->canCopy());
        break;
    case ID_EDIT_PASTE:
        ev.Enable(current_panel->canPaste());
        break;
    case ID_EDIT_FIND:
        ev.Enable(current_panel->canFind());
        break;
    case ID_EDIT_FIND_NEXT:
        ev.Enable(current_panel->canFind());
        break;
    case ID_EDIT_REPLACE:
        ev.Enable(current_panel->canReplace());
        break;
    // windows
    case ID_WINDOW_KEYWORDS:
        ev.Enable(set->game->has_keywords);
        break;
    case ID_WINDOW_RANDOM_PACK:
        ev.Enable(!set->game->pack_types.empty());
        break;
    // help
    case ID_HELP_INDEX:
        ev.Enable(false);
...
static const int FILE_MENU_SIZE_BEFORE_RECENT_SETS =
    12; // HACK; we should calculate the position to insert!
void SetWindow::updateRecentSets() {
    wxMenuBar *mb = GetMenuBar();
    assert(number_of_recent_sets <=
           (UInt)settings.recent_sets
               .size()); // the number of recent sets should only increase
    UInt i = 0;
    for (auto &file : settings.recent_sets) {
        if (i >= settings.max_recent_sets)
            break;
        if (i < number_of_recent_sets) {
            // menu item already exists, update it
            mb->SetLabel(ID_FILE_RECENT + i, String((L"&")) << (i + 1) << (L" ")
                                                            << file);
        } else {
            // add new item
            int pos = i + FILE_MENU_SIZE_BEFORE_RECENT_SETS; // HUGE HACK, we
                                                             // should calculate
                                                             // the position to
                                                             // insert!
            IconMenu *file_menu = static_cast<IconMenu *>(mb->GetMenu(0));
            file_menu->Insert(pos, ID_FILE_RECENT + i,
                              String((L"&")) << (i + 1) << (L" ") << file,
                              wxEmptyString);
        }
        i++;
...
void SetWindow::onFileNew(wxCommandEvent &) {
    if (!settings.open_sets_in_new_window && isOnlyWithSet() &&
        !askSaveAndContinue())
        return;
    // new set?
...
void SetWindow::onFileOpen(wxCommandEvent &) {
    if (!settings.open_sets_in_new_window && isOnlyWithSet() &&
        !askSaveAndContinue())
        return;
    wxFileDialog dlg(this, _TITLE_("open set"), settings.default_set_dir, (L""),
                     import_formats(), wxFD_OPEN);
    if (dlg.ShowModal() == wxID_OK) {
        settings.default_set_dir = dlg.GetDirectory();
        wxBusyCursor busy;
...
void SetWindow::onFileSaveAs(wxCommandEvent &) {
    wxFileDialog dlg(this, _TITLE_("save set"), settings.default_set_dir,
                     clean_filename(set->short_name),
                     export_formats(*set->game),
                     wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    if (dlg.ShowModal() == wxID_OK) {
        settings.default_set_dir = dlg.GetDirectory();
        export_set(*set, dlg.GetPath(), dlg.GetFilterIndex());
...
void SetWindow::onFileExportImage(wxCommandEvent &) {
    ...
    String name = wxFileSelector(
        ...
        clean_filename(card->identification()), (L""),
        (L"JPEG images (*.jpg)|*.jpg|Windows bitmaps (*.bmp)|*.bmp|PNG images "
         L"(*.png)|*.png|TIFF images (*.tif)|*.tif"),
...
void SetWindow::onFileExportMWS(wxCommandEvent &) { export_mws(this, set); }

void SetWindow::onFileCheckUpdates(wxCommandEvent &) {
    if (!askSaveAndContinue())
        return;
...
void SetWindow::onFileProfiler(wxCommandEvent &) { show_profiler_window(this); }
#endif
...
void SetWindow::onFileReload(wxCommandEvent &) {
    ...
    size_t card_pos = 0;
    {
        vector<CardP>::const_iterator card_it =
            find(set->cards.begin(), set->cards.end(),
                 current_panel->selectedCard());
        if (card_it != set->cards.end())
            card_pos = card_it - set->cards.begin();
    }
...
void SetWindow::onFileExit(wxCommandEvent &) { Close(); }

// -----------------------------------------------------------------------------
// : Window events - menu - edit

void SetWindow::onEditUndo(wxCommandEvent &) { set->actions.undo(); }
void SetWindow::onEditRedo(wxCommandEvent &) { set->actions.redo(); }

void SetWindow::onEditCut(wxCommandEvent &) { current_panel->doCut(); }
void SetWindow::onEditCopy(wxCommandEvent &) { current_panel->doCopy(); }
void SetWindow::onEditPaste(wxCommandEvent &) { current_panel->doPaste(); }
...
void SetWindow::onEditReplace(wxCommandEvent &) {
    delete find_dialog;
    find_dialog = new wxFindReplaceDialog(this, &find_data, (L"Replace"),
                                          wxFR_REPLACEDIALOG);
    find_dialog->Show();
}
...
void SetWindow::onWindowSelect(wxCommandEvent &ev) { selectPanel(ev.GetId()); }
...
EVT_COMMAND_RANGE(ID_CHILD_MIN, ID_CHILD_MAX, wxEVT_COMMAND_BUTTON_CLICKED,
                  SetWindow::onChildMenu)
EVT_COMMAND_RANGE(ID_CHILD_MIN, ID_CHILD_MAX, wxEVT_COMMAND_SPINCTRL_UPDATED,
                  SetWindow::onChildMenu)
EVT_COMMAND_RANGE(ID_CHILD_MIN, ID_CHILD_MAX,
                  wxEVT_COMMAND_RADIOBUTTON_SELECTED, SetWindow::onChildMenu)
EVT_COMMAND_RANGE(ID_CHILD_MIN, ID_CHILD_MAX, wxEVT_COMMAND_TEXT_UPDATED,
                  SetWindow::onChildMenu)
EVT_GALLERY_SELECT(
    ID_FIELD_LIST,
    SetWindow::onChildMenu) // for StatsPanel, because it is not a EVT_TOOL
