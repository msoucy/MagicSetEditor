...
void StatCategoryList::show(const GameP &game) {
    ...
    set<StatsCategoryP, StatComparator<StatsCategoryP>> sorted(
        game->statistics_categories.begin(), game->statistics_categories.end());
...
size_t StatCategoryList::itemCount() const { return categories.size(); }
...
void StatCategoryList::drawItem(DC &dc, int x, int y, size_t item) {
    ...
    // draw name
    RealRect rect(RealPoint(x + 24, y),
                  RealSize(item_size.x - 30, item_size.y));
  ...
  public:
    StatDimensionList(Window *parent, int id, bool show_empty,
        ...
        : GalleryList(parent, id, wxVERTICAL, false),
          ...
          prefered_dimension_count(dimension_count), show_empty(show_empty) {
    ...
    StatsDimensionP getSelection(size_t subcol = (size_t)-1) {
        ...
        if (sel >= itemCount())
            return StatsDimensionP();
        else
            return dimensions.at(sel);
    }
        ...
        for (size_t j = 1; j < subcolumns.size(); ++j) {
            if (j != active_subcolumn && subcolumns[j].selection == item &&
                ...
                subcolumns[j].selection =
                    subcolumns[active_subcolumn].selection;
                changes = true;
                break;
            }
        }
...
void StatDimensionList::show(const GameP &game) {
    ...
    set<StatsDimensionP, StatsComparator<StatsDimensionP>> sorted(
        game->statistics_dimensions.begin(), game->statistics_dimensions.end());
...
StatsPanel::StatsPanel(Window *parent, int id)
    : SetWindowPanel(parent, id), menuGraph(nullptr), up_to_date(true),
      active(false) {
...
void StatsPanel::initControls() {
    ...
    menuGraph->Append(ID_GRAPH_PIE, (L"graph_pie"), _MENU_("pie"),
                      _HELP_("pie"), wxITEM_CHECK);
    menuGraph->Append(ID_GRAPH_BAR, (L"graph_bar"), _MENU_("bar"),
                      _HELP_("bar"), wxITEM_CHECK);
    menuGraph->Append(ID_GRAPH_STACK, (L"graph_stack"), _MENU_("stack"),
                      _HELP_("stack"), wxITEM_CHECK);
    menuGraph->Append(ID_GRAPH_SCATTER, (L"graph_scatter"), _MENU_("scatter"),
                      _HELP_("scatter"), wxITEM_CHECK);
    menuGraph->Append(ID_GRAPH_SCATTER_PIE, (L"graph_scatter_pie"),
                      _MENU_("scatter pie"), _HELP_("scatter pie"),
                      wxITEM_CHECK);
}

StatsPanel::~StatsPanel() { delete menuGraph; }
...
// Toolbar
    ...
    tb->AddTool(ID_GRAPH_PIE, (L""), load_resource_tool_image((L"graph_pie")),
                wxNullBitmap, wxITEM_CHECK, _TOOLTIP_("pie"), _HELP_("pie"));
    tb->AddTool(ID_GRAPH_BAR, (L""), load_resource_tool_image((L"graph_bar")),
                wxNullBitmap, wxITEM_CHECK, _TOOLTIP_("bar"), _HELP_("bar"));
    tb->AddTool(ID_GRAPH_STACK, (L""),
                load_resource_tool_image((L"graph_stack")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("stack"), _HELP_("stack"));
    tb->AddTool(ID_GRAPH_SCATTER, (L""),
                load_resource_tool_image((L"graph_scatter")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("scatter"), _HELP_("scatter"));
    tb->AddTool(ID_GRAPH_SCATTER_PIE, (L""),
                load_resource_tool_image((L"graph_scatter_pie")), wxNullBitmap,
                wxITEM_CHECK, _TOOLTIP_("scatter pie"), _HELP_("scatter pie"));
    tb->Realize();
    // Menu
...
void StatsPanel::onUpdateUI(wxUpdateUIEvent &ev) {
    ...
    case ID_GRAPH_BAR:
    case ID_GRAPH_STACK:
    case ID_GRAPH_SCATTER:
    case ID_GRAPH_SCATTER_PIE: {
...
// find dimensions and layout
    ...
    if (dimensionality(layout) != dims.size()) {
        ...
        layout = dims.size() == 1
                     ? GRAPH_TYPE_BAR
                     : dims.size() == 2 ? (layout == GRAPH_TYPE_SCATTER_PIE ||
                                                   dims[1]->numeric
                                               ? GRAPH_TYPE_SCATTER
                                               : GRAPH_TYPE_STACK)
    ...
    if (!categories->hasSelection())
        return StatsCategory &cat = categories->getSelection();
    // dimensions
    ...
    for (auto &dim : dims) {
        d.axes.push_back(intrusive(new GraphAxis(
            dim->name, dim->colors.empty() ? AUTO_COLOR_EVEN : AUTO_COLOR_NO,
            dim->numeric, dim->bin_size, &dim->colors,
...
void StatsPanel::onGraphSelect(wxCommandEvent &) { filterCards(); }
...
class StatsFilter : public Filter<Card> {
  public:
    ...
    virtual void getItems(const vector<CardP> &cards,
                          vector<VoidP> &out) const {
...
void StatsPanel::filterCards() {
    intrusive_ptr<StatsFilter> filter(
        new StatsFilter(*graph->getData(), graph->getSelectionIndices()));
