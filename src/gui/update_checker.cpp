...
bool update_available() {
	...
	FOR_EACH_CONST(p, update_version_data->packages) {
		if (!settings.check_updates_all && p->package != mse_package)
			continue;
		Version v;
...
class CheckUpdateThread : public wxThread {
  public:
	...
	static void Work() {
		if (checking_updates)
			return; // don't check multiple times simultaniously
		...
		try {
			...
			// ignore all errors, we don't want problems if update checking
			// fails
		}
...
void show_update_dialog(Window *parent) {
	if (!update_available() || shown_dialog)
		return; // we already have the latest version, or this has already been
				// displayed.
	shown_dialog = true;
...
struct HtmlWindowToBrowser : public wxHtmlWindow {
	HtmlWindowToBrowser(Window *parent, int id, const wxPoint &pos,
						const wxSize &size, long style)
		: wxHtmlWindow(parent, id, pos, size,
					   style | wxBORDER_THEME_FIX(wxBORDER_THEME)) {}
...
void show_update_dialog(Window *parent) {
	if (!update_available() || shown_dialog)
		return; // we already have the latest version, or this has already been
				// displayed.
	// Show update dialog
	wxDialog *dlg = new wxDialog(parent, wxID_ANY, _TITLE_("updates available"),
								 wxDefaultPosition);
	// controls
	wxHtmlWindow *html = new HtmlWindowToBrowser(
