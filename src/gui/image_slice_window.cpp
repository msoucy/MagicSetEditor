...
ImageSlice::ImageSlice(const Image &source, const wxSize &target_size)
	: source(source), target_size(target_size),
	  ...
	  allow_outside(false), aspect_fixed(true), sharpen(true),
	  sharpen_amount(25) {}
...
void ImageSlice::constrain(PreferedProperty prefer) {
	...
	// fix aspect ratio
	if (aspect_fixed) {
		int diff = selection.width * target_size.GetHeight() -
				   selection.height * target_size.GetWidth();
...
Image ImageSlice::getSlice() const {
	if (selection.width == target_size.GetWidth() &&
		selection.height == target_size.GetHeight() && selection.x == 0 &&
		selection.y == 0) {
...
#define EVT_SLICE_CHANGED(id, handler)                                         \
	EVT_COMMAND(id, EVENT_SLICE_CHANGED, handler)

// -----------------------------------------------------------------------------
// : ImageSliceWindow

ImageSliceWindow::ImageSliceWindow(Window *parent, const Image &source,
								   const wxSize &target_size,
								   const AlphaMask &mask)
	: wxDialog(parent, wxID_ANY, _TITLE_("slice image"), wxDefaultPosition,
			   wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER |
								  wxFULL_REPAINT_ON_RESIZE),
	  slice(source, target_size), initialized(false) {
	// init controls
	...
	String sizes[] = {_LABEL_("original size"), _LABEL_("size to fit"),
					  _LABEL_("force to fit"), _LABEL_("custom size")};
	size = new wxRadioBox(this, ID_SIZE, _LABEL_("size"), defPos, wxDefaultSize,
						  4, sizes, 1);
	...
	fix_aspect =
		new wxCheckBox(this, ID_FIX_ASPECT, _LABEL_("fix aspect ratio"));
	...
	//	allowOutside= new CheckBox(&this, idSliceAllowOutside, _("Allow
	//selection outside source"))
	...
	s7->Add(new wxStaticText(this, wxID_ANY, _LABEL_("selection left")), 0,
			wxALIGN_CENTER_VERTICAL);
	s7->Add(left, 0, wxEXPAND);
	s7->Add(new wxStaticText(this, wxID_ANY, _LABEL_("selection top")), 0,
			wxALIGN_CENTER_VERTICAL);
	s7->Add(top, 0, wxEXPAND);
	s7->Add(new wxStaticText(this, wxID_ANY, _LABEL_("selection width")), 0,
			wxALIGN_CENTER_VERTICAL);
	s7->Add(width, 0, wxEXPAND);
	s7->Add(new wxStaticText(this, wxID_ANY, _LABEL_("selection height")), 0,
			wxALIGN_CENTER_VERTICAL);
	s7->Add(height, 0, wxEXPAND);
	s6->Add(s7, 1, wxEXPAND | wxALL, 4);
	s5->Add(s6, 0, wxEXPAND | wxALL, 4);
	s5->AddStretchSpacer(1);
	wxSizer *s8 = zoom_sizer =
		new wxStaticBoxSizer(wxVERTICAL, this, _LABEL_("zoom"));
	s8->Add(fix_aspect, 0, wxEXPAND | (wxALL & ~wxBOTTOM), 4);
	wxSizer *s9 = zoom_fixed = new wxFlexGridSizer(0, 3, 4, 5);
	s9->Add(new wxStaticText(this, wxID_ANY, _LABEL_("zoom amount")), 0,
			wxALIGN_CENTER_VERTICAL);
	s9->Add(zoom, 0, wxEXPAND);
	s9->Add(new wxStaticText(this, wxID_ANY, _LABEL_("zoom %")), 0,
			wxALIGN_CENTER_VERTICAL);
	s8->Add(s9, 0, wxEXPAND | wxALL, 4);
	wxSizer *sA = zoom_free = new wxFlexGridSizer(0, 3, 4, 5);
	sA->Add(new wxStaticText(this, wxID_ANY, _LABEL_("zoom amount x")), 0,
			wxALIGN_CENTER_VERTICAL);
	sA->Add(zoom_x, 0, wxEXPAND);
	sA->Add(new wxStaticText(this, wxID_ANY, _LABEL_("zoom %")), 0,
			wxALIGN_CENTER_VERTICAL);
	sA->Add(new wxStaticText(this, wxID_ANY, _LABEL_("zoom amount y")), 0,
			wxALIGN_CENTER_VERTICAL);
...
void ImageSliceWindow::onOk(wxCommandEvent &) { EndModal(wxID_OK); }

Image ImageSliceWindow::getImage() const { return slice.getSlice(); }
...
void ImageSliceWindow::updateControls() {
	if (slice.selection.width == slice.target_size.GetWidth() &&
		slice.selection.height == slice.target_size.GetHeight()) {
		size->SetSelection(0); // original size
	} else if (slice.selection.x == 0 &&
			   slice.selection.width == slice.source.GetWidth() &&
			   slice.selection.y == 0 &&
			   slice.selection.height == slice.source.GetHeight()) {
...
ImageSlicePreview::ImageSlicePreview(Window *parent, int id, ImageSlice &slice,
	...
	: wxControl(parent, id, wxDefaultPosition, wxDefaultSize,
	  ...
	  slice(slice), mask(mask), mouse_down(false) {}
...
void ImageSlicePreview::draw(DC &dc) {
	if (!bitmap.Ok()) {
		...
		assert(image.GetWidth() == slice.target_size.GetWidth() &&
			   image.GetHeight() == slice.target_size.GetHeight());
		mask.setAlpha(image);
		if (image.HasAlpha()) {
			// create bitmap
...
void ImageSlicePreview::onMotion(wxMouseEvent &ev) {
	if (mouse_down) {
		// drag the image
		slice.selection.x =
			int(start_selection.x + (mouseX - ev.GetX()) / slice.zoomX());
		slice.selection.y =
			int(start_selection.y + (mouseY - ev.GetY()) / slice.zoomY());
		// Notify parent
...
ImageSliceSelector::ImageSliceSelector(Window *parent, int id,
									   ImageSlice &slice)
	: wxControl(parent, id, wxDefaultPosition, wxDefaultSize,
				wxBORDER_THEME_FIX(wxBORDER_THEME)),
	  slice(slice), mouse_down(false) {}

void ImageSliceSelector::update() { Refresh(false); }
...
void ImageSliceSelector::createBitmap() {
	...
	// Initialize bitmap_no_sel to be the same bitmap, only with a faded color
	// and blurred
	...
	if (img.HasAlpha()) {
		...
		memcpy(img_no_sel.GetAlpha(), img.GetAlpha(),
	...
	if (mouse_down) {
		...
		if (dragX == 0 && dragY == 0) {
			...
			if (slice.aspect_fixed) {
				...
				double d = (dragX * deltaX * slice.target_size.GetHeight() +
							dragY * deltaY * slice.target_size.GetWidth()) /
						   2;
				...
				/*if (dragX * deltaX * slice.target_size.GetWidth() <
					...
					deltaY = dragX * dragY * deltaX *
				slice.target_size.GetWidth() / slice.target_size.GetHeight();
				} else {
					deltaX = dragX * dragY * deltaY *
				slice.target_size.GetHeight() / slice.target_size.GetWidth();
				}*/
			}
			// move
			if (dragX) {
				slice.selection.x =
					int(start_selection.x + deltaX * (1 - dragX) / 2);
				slice.selection.width =
					int(start_selection.width + deltaX * dragX);
			}
			if (dragY) {
				slice.selection.y =
					int(start_selection.y + deltaY * (1 - dragY) / 2);
				slice.selection.height =
					int(start_selection.height + deltaY * dragY);
			}
		}
...
bool ImageSliceSelector::onHandle(const wxMouseEvent &ev, int dx,
								  int dy) const {
	wxPoint p = handlePos(dx, dy);
	p.x = p.x - 3 + 4 * dx;
	p.y = p.y - 3 + 4 * dy;
	return ev.GetX() >= p.x && ev.GetX() < p.x + 6 && ev.GetY() >= p.y &&
		   ev.GetY() < p.y + 6;
}
bool ImageSliceSelector::onAnyHandle(const wxMouseEvent &ev, int *dxOut,
									 int *dyOut) const {
	for (int dx = -1; dx <= 1; ++dx) {
		for (int dy = -1; dy <= 1; ++dy) {
			if ((dx != 0 || dy != 0) &&
				onHandle(ev, dx, dy)) { // (0,0) == center, not a handle
				*dxOut = dx;
				*dyOut = dy;
				return true;
			}
...
wxPoint ImageSliceSelector::handlePos(int dx, int dy) const {
	return wxPoint(int(scaleX * (slice.selection.x +
								 ((dx + 1) * slice.selection.width) * 0.5) +
					   border),
				   int(scaleY * (slice.selection.y +
								 ((dy + 1) * slice.selection.height) * 0.5) +
					   border));
}
