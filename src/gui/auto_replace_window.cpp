...
AutoReplaceList::AutoReplaceList(Window *parent, int id, const Game &game)
    ...
    // Add columns
    InsertColumn(0, (L""), wxLIST_FORMAT_LEFT,
...
int AutoReplaceList::OnGetItemImage(long pos) const { return -1; }
...
void AutoReplaceWindow::onItemSelect(wxCommandEvent &) { refreshItem(); }
void AutoReplaceWindow::onItemChange(wxCommandEvent &) { updateItem(); }

void AutoReplaceWindow::onEnable(wxCommandEvent &) { enable(); }
...
void AutoReplaceWindow::refreshItem() {
    if (in_event)
        return;
    if (!use_auto_replace->GetValue())
        return;
    in_event = true;
    AutoReplaceP ar = list->getSelected();
    match->Enable(ar && ar->custom);
    replace->Enable(ar != nullptr);
    matchL->Enable(ar != nullptr && ar->custom);
    replaceL->Enable(ar != nullptr);
    enabled->Enable(ar != nullptr);
    whole_word->Enable(ar != nullptr);
    remove->Enable(ar != nullptr && ar->custom);
    if (ar != nullptr) {
