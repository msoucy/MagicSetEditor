...
Image load_resource_image(const String &name) {
	...
	if (hResource == 0)
		throw InternalError(String::Format(_("Resource not found: %s"), name));

	HGLOBAL hData = ::LoadResource(wxGetInstance(), hResource);
	if (hData == 0)
		throw InternalError(
			String::Format(_("Resource not an image: %s"), name));
	...
	else if (wxFileExists(file + _(".bmp")))
		resource.LoadFile(file + _(".bmp"));
	else if (wxFileExists(file + _(".ico")))
		resource.LoadFile(file + _(".ico"));
	else if (wxFileExists(file + _(".cur")))
	...
	if (wxFileExists(file + _(".png")))
		...
		resource.LoadFile(file + _(".cur"));
	if (!resource.Ok())
		handle_error(InternalError(String(_("Cannot find resource file at ")) +
								   path + name + _(" or ") + file));
	return resource;
#else
...
wxIcon load_resource_icon(const String &name) {
	...
	if (wxFileExists(path + name + _(".ico")))
		...
		return wxIcon(local_path + name + _(".ico"), wxBITMAP_TYPE_ICO);
#endif
}
