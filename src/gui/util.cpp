...
int focused_control(const Window *window) {
    ...
    if (focused_window &&
        wxWindow::FindWindowById(focused_window->GetId(), window) ==
            focused_window) {
        return focused_window->GetId();
    } else {
        return -1; // no window has the focus, or it has a different
                   // parent/ancestor
    }
}
...
//  - store the help text of a window in its ClientObject as a
//  StoredStatusString
//  - Connect event handlers that use set_status_text
//  - The event handlers should be members of an EvtHandler somewhere,
//     but it is wasteful to make an object. Instead use nullptr as a 'fake'
//     EvtHandler.
...
struct FakeEvtHandlerClass : public wxEvtHandler {
    void onControlEnter(wxMouseEvent &ev) {
        ...
        if (wnd) {
            StoredStatusString *d =
                static_cast<StoredStatusString *>(wnd->GetClientObject());
...
void set_help_text(Window *wnd, const String &s) {
    ...
    if (d) {
        ...
        wnd->Connect(wxEVT_ENTER_WINDOW,
                     wxMouseEventHandler(FakeEvtHandlerClass::onControlEnter),
                     nullptr, nullptr);
        wnd->Connect(wxEVT_LEAVE_WINDOW,
                     wxMouseEventHandler(FakeEvtHandlerClass::onControlLeave),
                     nullptr, nullptr);
    }
    d->s = s;
}
...
void draw_checker(RotatedDC &dc, const RealRect &rect) {
    ...
    for (double y = 0; y < rect.height; y += checker_size) {
        for (double x = odd * checker_size; x < rect.width;
             x += checker_size * 2) {
            dc.DrawRectangle(RealRect(rect.x + x, rect.y + y,
...
Image load_resource_image(const String &name) {
    ...
    if (hData == 0)
        throw InternalError(
            String::Format((L"Resource not an image: %s"), name));
    ...
    wxImage resource;
    if (wxFileExists(file + (L".png")))
        resource.LoadFile(file + (L".png"));
    else if (wxFileExists(file + (L".bmp")))
        resource.LoadFile(file + (L".bmp"));
    else if (wxFileExists(file + (L".ico")))
        resource.LoadFile(file + (L".ico"));
    else if (wxFileExists(file + (L".cur")))
        resource.LoadFile(file + (L".cur"));
    if (resource.Ok())
        return resource;
    ...
    if (wxFileExists(file + (L".png")))
        ...
        resource.LoadFile(file + (L".cur"));
    if (!resource.Ok())
        handle_error(InternalError(String((L"Cannot find resource file at ")) +
                                   path + name + (L" or ") + file));
    return resource;
#else
...
wxIcon load_resource_icon(const String &name) {
    ...
    if (wxFileExists(path + name + (L".ico")))
        return wxIcon(path + name + (L".ico"), wxBITMAP_TYPE_ICO);
    else
        return wxIcon(local_path + name + (L".ico"), wxBITMAP_TYPE_ICO);
#endif
}
...
void draw_control_box(Window *win, DC &dc, const wxRect &rect, bool focused,
                      bool enabled) {
#if wxUSE_UXTHEME && defined(__WXMSW__)
    RECT r;
    ...
    if (themeEngine && themeEngine->IsAppThemed()) {
        ...
        if (hTheme) {
            ...
            if (themeEngine->IsThemeBackgroundPartiallyTransparent(
                    (HTHEME)hTheme, EP_EDITTEXT, state)) {
                themeEngine->DrawThemeParentBackground((HWND)win->GetHWND(),
                                                       (HDC)dc.GetHDC(), &r);
            }
...
#else
    // draw a 3D border
    draw3DBorder(dc, rect.x - 1, rect.y - 1, rect.x + rect.width,
                 rect.y + rect.height);
#endif
}

void draw_button(Window *win, DC &dc, const wxRect &rect, bool focused,
                 bool down, bool enabled) {
#if wxVERSION >= 2700
    wxRendererNative &rn = wxRendererNative::GetDefault();
    rn.DrawPushButton(win, dc, rect, (focused ? wxCONTROL_FOCUSED : 0) |
                                         (down ? wxCONTROL_PRESSED : 0) |
                                         (enabled ? 0 : wxCONTROL_DISABLED));
#else
    dc.SetPen(*wxTRANSPARENT_PEN);
    dc.SetBrush(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE));
    dc.DrawRectangle(rect.x, rect.y, rect.width, rect.height);
    dc.SetPen(wxSystemSettings::GetColour(down ? wxSYS_COLOUR_BTNSHADOW
                                               : wxSYS_COLOUR_BTNHIGHLIGHT));
    dc.DrawLine(rect.x, rect.y, rect.x + rect.width, rect.y);
    dc.DrawLine(rect.x, rect.y, rect.x, rect.y + rect.height);
    dc.SetPen(wxSystemSettings::GetColour(down ? wxSYS_COLOUR_BTNHIGHLIGHT
...
void draw_menu_arrow(Window *win, DC &dc, const wxRect &rect, bool active) {
    wxPoint pt[] = {wxPoint(0, 0), wxPoint(4, 4), wxPoint(0, 8)};
    dc.SetPen(*wxTRANSPARENT_PEN);
    dc.SetBrush(wxSystemSettings::GetColour(active ? wxSYS_COLOUR_HIGHLIGHTTEXT
                                                   : wxSYS_COLOUR_WINDOWTEXT));
    dc.DrawPolygon(3, pt, rect.x + rect.width - 6,
                   rect.y + (rect.height - 9) / 2);
}

void draw_drop_down_arrow(Window *win, DC &dc, const wxRect &rect,
                          bool active) {
    wxRendererNative &rn = wxRendererNative::GetDefault();
    int w = wxSystemSettings::GetMetric(
        wxSYS_VSCROLL_ARROW_X); // drop down arrow is same size
    if (w == -1) {
        w = wxSystemSettings::GetMetric(
            wxSYS_VSCROLL_X); // Try just the scrollbar, then.
    }
    rn.DrawComboBoxDropButton(
        win, dc, wxRect(rect.x + rect.width - w, rect.y, w, rect.height),
...
// TODO: Windows version?
    ...
    if (checked) {
        dc.DrawCheckMark(
            wxRect(rect.x - 1, rect.y - 1, rect.width + 2, rect.height + 2));
    }
    dc.SetPen(wxSystemSettings::GetColour(enabled ? wxSYS_COLOUR_WINDOWTEXT
                                                  : wxSYS_COLOUR_GRAYTEXT));
    dc.SetBrush(*wxTRANSPARENT_BRUSH);
    dc.DrawRectangle(rect.x, rect.y, rect.width, rect.height);
}

void draw_radiobox(Window *win, DC &dc, const wxRect &rect, bool checked,
                   bool enabled) {
#if wxUSE_UXTHEME && defined(__WXMSW__)
// TODO: Windows version?
#endif
// portable version
    ...
    dc.DrawRoundedRectangle(rect.x, rect.y, rect.width, rect.height,
                            rect.width * 0.5 - 1);
    if (checked) {
        dc.SetBrush(wxSystemSettings::GetColour(
            enabled ? wxSYS_COLOUR_WINDOWTEXT : wxSYS_COLOUR_GRAYTEXT));
        dc.SetPen(*wxTRANSPARENT_PEN);
        // dc.DrawEllipse(rect.x+2,rect.y+2,rect.width-4,rect.height-4);
        dc.DrawRoundedRectangle(rect.x + 3, rect.y + 3, rect.width - 6,
                                rect.height - 6, rect.width * 0.5 - 4);
    }
#endif
}

void draw_selection_rectangle(Window *win, DC &dc, const wxRect &rect,
                              bool selected, bool focused, bool hot) {
#if wxUSE_UXTHEME && defined(__WXMSW__)
...
#define LISS_HOTSELECTED LISS_SELECTED
    ...
    if (themeEngine && themeEngine->IsAppThemed()) {
        ...
        if (hTheme) {
            // themeEngine->SetWindowTheme((HWND)win->GetHWND(), L"Explorer",
            // NULL);
            themeEngine->DrawThemeBackground(
                (HTHEME)hTheme, (HDC)dc.GetHDC(), LVP_LISTITEM,
                hot && selected
                    ? LISS_HOTSELECTED
                    : hot ? LISS_HOT : selected && focused
                                           ? LISS_SELECTED
                                           : selected ? LISS_SELECTEDNOTFOCUS
                                                      : LISS_NORMAL,
                &r, NULL);
            return;
        }
