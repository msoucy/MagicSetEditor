...
AboutWindow::AboutWindow(Window *parent)
    : wxDialog(parent, wxID_ANY, _TITLE_("about"), wxDefaultPosition,
               wxSize(510, 340),
               wxCLIP_CHILDREN | wxDEFAULT_DIALOG_STYLE | wxTAB_TRAVERSAL),
...
#endif
{
    ...
    ok_button->Move(ws.GetWidth() - bs.GetWidth(),
                    ws.GetHeight() - bs.GetHeight()); // align bottom right
}
...
void AboutWindow::draw(DC &dc) {
    ...
    dc.DrawText((L"Version: ") + app_version.toString() + version_suffix, 34,
                110);
...
HoverButtonBase::HoverButtonBase(Window *parent, int id, bool accepts_focus)
    : wxControl(parent, id, wxDefaultPosition, wxDefaultSize, wxNO_BORDER),
      accepts_focus(accepts_focus), hover(false), focus(false),
      mouse_down(false), key_down(false) {}
...
void HoverButtonBase::onMouseLeave(wxMouseEvent &) {
    ...
    refreshIfNeeded();
    if (!help_text.empty())
        set_status_text(this, wxEmptyString);
}
...
void HoverButtonBase::refreshIfNeeded() { Refresh(false); }
...
HoverButton::HoverButton(Window *parent, int id, const String &name,
                         const Color &background, bool accepts_focus)
    : HoverButtonBase(parent, id, accepts_focus), background(background),
      last_drawn(nullptr) {
    loadBitmaps(name);
...
const Bitmap *HoverButton::toDraw() const {
    return (mouse_down && hover) || key_down
               ? &bg_down
               : hover ? &bg_hover : focus ? &bg_focus : &bg_normal;
}
...
void HoverButton::draw(DC &dc) {
    ...
    wxSize ws = GetClientSize();
    dc.SetPen(*wxTRANSPARENT_PEN);
    dc.SetBrush(background != wxNullColour
                    ? background
