...
class PreferencesPage : public wxPanel {
  public:
    PreferencesPage(Window *parent) : wxPanel(parent, wxID_ANY) {}
...
GlobalPreferencesPage::GlobalPreferencesPage(Window *parent)
    ...
    // init controls
    language = new wxComboBox(this, wxID_ANY, (L""), wxDefaultPosition,
                              wxDefaultSize, 0, nullptr, wxCB_READONLY);
    open_sets_in_new_window =
        new wxCheckBox(this, wxID_ANY, _BUTTON_("open sets in new window"));
    // set values
    ...
    for (auto &package : locales) {
        language->Append(package->name() + (L": ") + package->full_name,
                         package.get());
    ...
    s2->Add(new wxStaticText(this, wxID_ANY, _LABEL_("app language")), 0, wxALL,
            4);
    s2->Add(language, 0, wxEXPAND | (wxALL & ~wxTOP), 4);
    s2->Add(new wxStaticText(this, wxID_ANY, _HELP_("app language")), 0, wxALL,
            4);
...
DisplayPreferencesPage::DisplayPreferencesPage(Window *parent)
    ...
    draw_editing =
        new wxCheckBox(this, wxID_ANY, _BUTTON_("show editing hints"));
    spellcheck_enabled =
        new wxCheckBox(this, wxID_ANY, _BUTTON_("spellcheck enabled"));
#if USE_ZOOM_COMBOBOX
    ...
    // wxButton* columns = new wxButton(this, ID_SELECT_COLUMNS,
    // _BUTTON_("select"));
    // set values
    high_quality->SetValue(
        settings.default_stylesheet_settings.card_anti_alias());
    borders->SetValue(settings.default_stylesheet_settings.card_borders());
    draw_editing->SetValue(
        settings.default_stylesheet_settings.card_draw_editing());
    spellcheck_enabled->SetValue(
        settings.default_stylesheet_settings.card_spellcheck_enabled());
    non_normal_export->SetValue(
        !settings.default_stylesheet_settings.card_normal_export());
#if USE_ZOOM_COMBOBOX
    zoom_int = static_cast<int>(
        settings.default_stylesheet_settings.card_zoom() * 100);
    ...
    zoom->SetValue(static_cast<int>(
        settings.default_stylesheet_settings.card_zoom() * 100));
#endif
    // init sizer
    wxSizer *s = new wxBoxSizer(wxVERTICAL);
    wxSizer *s2 =
        new wxStaticBoxSizer(wxVERTICAL, this, _LABEL_("card display"));
    ...
    s3->Add(new wxStaticText(this, wxID_ANY, _LABEL_("zoom")), 0,
            wxALL & ~wxLEFT, 4);
    s3->AddSpacer(2);
    s3->Add(zoom);
    s3->Add(new wxStaticText(this, wxID_ANY, _LABEL_("percent of normal")), 1,
            wxALL & ~wxRIGHT, 4);
    s2->Add(s3, 0, wxEXPAND | wxALL, 4);
    s2->Add(non_normal_export, 0, wxEXPAND | wxALL, 4);
    s2->Add(new wxStaticText(this, wxID_ANY, _HELP_("zoom export")), 0,
            wxALL & ~wxTOP, 4);
    s->Add(s2, 0, wxEXPAND | wxALL, 8);
    /*
    wxSizer* s4 = new wxStaticBoxSizer(wxVERTICAL, this, (L"Card List"));
        wxSizer* s5 = new wxBoxSizer(wxHORIZONTAL);
            s5->Add(new wxStaticText(this, wxID_ANY, (L"Columns: ")),      0,
    wxALL & ~wxLEFT | wxEXPAND, 4);
            s5->Add(columns);
...
void DisplayPreferencesPage::store() {
    settings.default_stylesheet_settings.card_anti_alias =
        high_quality->GetValue();
    settings.default_stylesheet_settings.card_borders = borders->GetValue();
    settings.default_stylesheet_settings.card_draw_editing =
        draw_editing->GetValue();
    settings.default_stylesheet_settings.card_spellcheck_enabled =
        spellcheck_enabled->GetValue();
#if USE_ZOOM_COMBOBOX
    updateZoom();
...
#if USE_ZOOM_COMBOBOX
void DisplayPreferencesPage::onZoomChange(wxCommandEvent &) { updateZoom(); }
...
DirsPreferencesPage::DirsPreferencesPage(Window *parent)
    ...
    wxSizer *s = new wxBoxSizer(wxVERTICAL);
    wxSizer *s2 =
        new wxStaticBoxSizer(wxVERTICAL, this, _LABEL_("external programs"));
    s2->Add(new wxStaticText(this, wxID_ANY, _LABEL_("apprentice")), 0, wxALL,
            4);
    wxSizer *s3 = new wxBoxSizer(wxHORIZONTAL);
...
UpdatePreferencesPage::UpdatePreferencesPage(Window *parent)
    ...
    wxButton *check_now =
        new wxButton(this, ID_CHECK_UPDATES_NOW, _BUTTON_("check now"));
    // set values
    ...
    // init sizer
    wxSizer *s = new wxBoxSizer(wxVERTICAL);
    s->Add(new wxStaticText(this, wxID_ANY, _LABEL_("check at startup")), 0,
           wxALL, 8);
    s->Add(check_at_startup, 0, wxALL & ~wxTOP, 8);
    s->Add(check_now, 0, wxALL & ~wxTOP, 8);
    s->Add(
        new wxStaticText(this, wxID_ANY, _LABEL_("checking requires internet")),
        0, wxALL & ~wxTOP, 8);
    SetSizer(s);
}
...
void UpdatePreferencesPage::store() {
    ...
    if (sel == 0)
        settings.check_updates = CHECK_ALWAYS;
    else if (sel == 1)
        settings.check_updates = CHECK_IF_CONNECTED;
    else
        settings.check_updates = CHECK_NEVER;
}

void UpdatePreferencesPage::onCheckUpdatesNow(wxCommandEvent &) {
    check_updates_now(false);
    if (!update_data_found()) {
        wxMessageBox(_ERROR_("checking updates failed"),
                     _TITLE_("update check"), wxICON_ERROR | wxOK);
    } else if (!update_available()) {
        wxMessageBox(_ERROR_("no updates"), _TITLE_("update check"),
