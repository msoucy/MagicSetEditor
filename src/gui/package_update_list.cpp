...
PackageUpdateList::TreeItem::package_type(const PackageDescription &desc) {
    ...
    if (pos == String::npos)
        return TYPE_OTHER;
    if (is_substr(desc.name, pos, (L".mse-locale")))
        return TYPE_LOCALE;
    if (is_substr(desc.name, pos, (L".mse-game")))
        return TYPE_GAME;
    if (is_substr(desc.name, pos, (L".mse-style")))
        return TYPE_STYLESHEET;
    if (is_substr(desc.name, pos, (L".mse-export-template")))
        return TYPE_EXPORT_TEMPLATE;
    if (is_substr(desc.name, pos, (L".mse-symbol-font")))
        return TYPE_SYMBOL_FONT;
    if (is_substr(desc.name, pos, (L".mse-include")))
        return TYPE_INCLUDE;
    if (is_substr(desc.name, pos, (L".ttf")))
        return TYPE_FONT;
    return TYPE_OTHER;
}
...
class PackageIconRequest : public ThumbnailRequest {
  public:
    PackageIconRequest(PackageUpdateList *list, PackageUpdateList::TreeItem *ti)
        : ThumbnailRequest(
              list, (L"package_") + ti->package->description->icon_url +
                        (L"_") + ti->package->description->version.toString(),
...
String PackageUpdateList::columnText(size_t column) const {
    if (column == 0)
        ...
        throw InternalError((L"Unknown column"));
}
