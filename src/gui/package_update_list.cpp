...
void PackageUpdateList::TreeItem::add(const InstallablePackageP &package,
									  const String &path, int level) {
	// this node
	this->level = level;
	PackageType new_type = package_type(*package->description);
	int new_hint = package->description->position_hint;
	if (new_type < position_type ||
		(new_type == position_type && new_hint < position_hint)) {
...
bool compare_pos_hint(const PackageUpdateList::TreeItemP &a,
					  const PackageUpdateList::TreeItemP &b) {
...
PackageUpdateList::TreeItem::PackageType
PackageUpdateList::TreeItem::package_type(const PackageDescription &desc) {
	if (desc.name == mse_package)
		return TYPE_PROG;
	size_t pos = desc.name.find_last_of(_('.'));
	if (pos == String::npos)
		return TYPE_OTHER;
	if (is_substr(desc.name, pos, _(".mse-locale")))
		return TYPE_LOCALE;
	if (is_substr(desc.name, pos, _(".mse-game")))
		return TYPE_GAME;
	if (is_substr(desc.name, pos, _(".mse-style")))
		return TYPE_STYLESHEET;
	if (is_substr(desc.name, pos, _(".mse-export-template")))
		return TYPE_EXPORT_TEMPLATE;
	if (is_substr(desc.name, pos, _(".mse-symbol-font")))
		return TYPE_SYMBOL_FONT;
	if (is_substr(desc.name, pos, _(".mse-include")))
		return TYPE_INCLUDE;
	if (is_substr(desc.name, pos, _(".ttf")))
		return TYPE_FONT;
	return TYPE_OTHER;
}
...
void PackageUpdateList::TreeItem::setIcon(const Image &img) {
	...
	if (package)
		package->description->icon = image;
...
/// The built in buffer class is too stupid to seek, so we must do it
/// ourselfs
class SeekAtStartInputStream : public wxFilterInputStream {
  public:
	SeekAtStartInputStream(wxInputStream &stream)
		: wxFilterInputStream(stream), buffer_pos(0) {
...
class PackageIconRequest : public ThumbnailRequest {
  public:
	PackageIconRequest(PackageUpdateList *list, PackageUpdateList::TreeItem *ti)
		: ThumbnailRequest(
			  list, _("package_") + ti->package->description->icon_url +
						_("_") + ti->package->description->version.toString(),
			  wxDateTime(1, wxDateTime::Jan, 2000)),
		  list(list), ti(ti) {}

	virtual Image generate() {
		wxURL url(ti->package->description->icon_url);
		const unique_ptr<wxInputStream> isP(url.GetInputStream());
		if (!isP)
			return wxImage();
...
PackageUpdateList::PackageUpdateList(Window *parent,
									 const InstallablePackages &packages,
									 bool show_only_installable, int id)
	: TreeList(parent, id), packages(packages),
	  show_only_installable(show_only_installable) {
	item_height = max(item_height, 19);
	rebuild();
}
PackageUpdateList::~PackageUpdateList() { thumbnail_thread.abort(this); }
	...
	for (auto &i : items) {
		...
		if (p && p->description->icon.Ok()) { // it has an icon
			...
			if (!p->description->icon_url.empty()) {
				// download icon
				thumbnail_thread.request(
					intrusive(new PackageIconRequest(this, &ti)));
			}
...
String PackageUpdateList::columnText(size_t column) const {
	if (column == 0)
		return _LABEL_("package name");
	else if (column == 1)
		return _LABEL_("package status");
	else if (column == 2)
		return _LABEL_("package action");
	else
		throw InternalError(_("Unknown column"));
}
