...
// Class that intercepts all events not directed to a DropDownList, and closes
// the list
class DropDownHider : public wxEvtHandler {
  ...
  private:
	...
	virtual bool ProcessEvent(wxEvent &ev) {
		...
		if (t == wxEVT_LEFT_DOWN || t == wxEVT_RIGHT_DOWN || t == wxEVT_MOVE ||
			t == wxEVT_SIZE || t == wxEVT_MENU_HIGHLIGHT ||
			t == wxEVT_MENU_OPEN || t == wxEVT_MENU_OPEN ||
			t == wxEVT_ACTIVATE || t == wxEVT_CLOSE_WINDOW ||
			t == wxEVT_KILL_FOCUS || t == wxEVT_COMMAND_TOOL_CLICKED) {
			// close the list, and pass on the event
			// don't just use ev.Skip(), because this event handler will be
			// removed by hiding,
			...
			//			if (t !=10093 && t !=10098 && t !=10097 && t !=10099 && t
			//!=10004 && t !=10062
			//			 && t !=10025 && t !=10035 && t !=10034 && t !=10036 && t
			//!=10042 && t !=10119)
			//			{
			//				t=t;//DEBUG
			//			}
...
DropDownList::DropDownList(Window *parent, bool is_submenu, ValueViewer *viewer)
	: wxPopupWindow(parent, wxSIMPLE_BORDER), text_offset(1), item_size(100, 1),
	  icon_size(0, 0), selected_item(NO_SELECTION), mouse_down(false),
	  open_sub_menu(nullptr), parent_menu(nullptr), viewer(viewer),
...
void DropDownList::show(bool in_place, wxPoint pos, RealRect *rect) {
	...
	if (item_size.width == 100) { // not initialized
		...
		for (size_t i = 0; i < count; ++i) {
			...
			item_size.width = max(
				item_size.width, text_width + icon_size.width +
									 14); // 14 = room for popup arrow + padding
		}
	}
	// height
	int line_count = 0;
	for (size_t i = 0; i < count; ++i)
		if (lineBelow(i))
			line_count += 1;
	// size
	RealSize border_size(2,
						 2); // GetClientSize() - GetSize(), assume 1px borders
	RealSize size(item_size.width + marginW * 2,
				  item_size.height * count + marginH * 2 + line_count);
	// placement
	int parent_height = 0;
	if (!in_place && viewer) {
		// Position the drop down list below the editor control (based on the
		// style)
	...
	int room_below =
		wxGetDisplaySize().y - border_size.height - pos.y - parent_height - 50;
	int max_height = max(300, room_below);
	if (size.height > max_height) {
		size.height = max_height;
		size.width += wxSystemSettings::GetMetric(
			wxSYS_VSCROLL_ARROW_X); // width of scrollbar
	...
	// show
	if (selected_item == NO_SELECTION && itemCount() > 0)
		selected_item = 0; // select first item by default
	mouse_down = false;
	close_on_mouse_out = false;
	wxPopupWindow::Show();
...
bool DropDownList::showSubMenu(size_t item, int y) {
	...
	if (sub_menu == open_sub_menu)
		return sub_menu; // no change
	hideSubMenu();
	...
	wxSize size = GetSize();
	sub_menu->show(
		true, sub_menu->GetParent()->ScreenToClient(ClientToScreen(
...
void DropDownList::ensureSelectedItemVisible() {
	...
	if (item_top < marginH) {
		...
		scrollTo(item_top + visible_start + item_size.height - 1 -
				 (cs.y - marginH));
	}
}
...
void DropDownList::drawItem(DC &dc, int y, size_t item) {
	if (y + item_size.height <= 0 || y >= dc.GetSize().y)
		return; // not visible
	// draw background
	dc.SetPen(*wxTRANSPARENT_PEN);
	if (item == selected_item) {
		...
		dc.DrawRectangle(marginW, y, (int)item_size.width,
						 (int)item_size.height);
	...
	dc.DrawText(capitalize(itemText(item)), marginW + (int)icon_size.width + 1,
				y + text_offset);
	// draw popup icon
	if (submenu(item)) {
		draw_menu_arrow(this, dc,
						RealRect(marginW, y, item_size.width, item_size.height),
						item == selected_item);
	}
	// draw line below
	if (lineBelow(item) && item != itemCount()) {
		dc.SetPen(wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT));
		dc.DrawLine(marginW, y + (int)item_size.height,
					marginW + (int)item_size.width, y + (int)item_size.height);
	}
}
...
void DropDownList::onLeftDown(wxMouseEvent &ev) {
	wxSize cs = GetClientSize();
	if (ev.GetX() < 0 || ev.GetX() >= cs.x || ev.GetY() < marginH ||
		ev.GetY() >= cs.y) {
		hide(false);
		ev.Skip();
		return;
	...
	mouse_down =
		true; // prevent closing on mouseup of the click that opened the window
}

void DropDownList::onLeftUp(wxMouseEvent &) {
	if (mouse_down) {
		if (selected_item != NO_SELECTION && !itemEnabled(selected_item))
			return; // disabled item
		// don't hide if there is a child menu
		if (selected_item != NO_SELECTION && submenu(selected_item))
			return;
		hide(true);
	}
}
...
void DropDownList::onMotion(wxMouseEvent &ev) {
	...
	wxSize cs = GetClientSize();
	// inside?
	if (ev.GetX() < marginW || ev.GetX() + marginW >= cs.GetWidth() ||
		ev.GetY() < marginH || ev.GetY() + marginH >= cs.GetHeight()) {
		ev.Skip();
		return;
	}
	// find selected item
...
void DropDownList::onMouseLeave(wxMouseEvent &ev) {
	if (close_on_mouse_out) {
		wxSize cs = GetClientSize();
		if (ev.GetX() < 0 || ev.GetY() < 0 || ev.GetX() >= cs.x ||
			ev.GetY() >= cs.y) {
...
void DropDownList::onMouseWheel(wxMouseEvent &ev) {
	scrollTo(visible_start -
			 item_size.height * ev.GetWheelRotation() / ev.GetWheelDelta());
}
	...
	if (IsShown()) {
		if (open_sub_menu) {
			...
			case WXK_DOWN:
				return selectItem(selected_item + 1);
			case WXK_RETURN:
				if (!showSubMenu() && (selected_item == NO_SELECTION ||
									   itemEnabled(selected_item))) {
			...
			case WXK_RIGHT:
				return showSubMenu();
			default:
				// match first character of an item, start searching just after
				// the current selection
				size_t si =
					selected_item != NO_SELECTION ? selected_item + 1 : 0;
				size_t count = itemCount();
				for (size_t i = 0; i < count; ++i) {
...
#ifdef UNICODE
					if (!c.empty() &&
						toUpper(c.GetChar(0)) == toUpper(ev.GetUnicodeKey())) {
#else
					if (!c.empty() &&
						toUpper(c.GetChar(0)) == toUpper(ev.GetKeyCode())) {
#endif
...
// Note: some DropDownList events get sent to the parent which in turn should
// send them to the DropDownList
