...
PageLayout::PageLayout()
	: margin_left(0), margin_right(0), margin_top(0), margin_bottom(0), rows(0),
	  cols(0), card_landscape(false) {}

void PageLayout::init(const StyleSheet &stylesheet, PageLayoutType type,
					  const RealSize &page_size) {
	...
	if (type == LAYOUT_NO_SPACE) {
		...
		margin_top = vspace * 1. / 3;
		margin_bottom =
			vspace * 2. / 3; // most printers have more margin at the bottom
	} else {
...
class CardsPrintout : public wxPrintout {
  public:
	...
	virtual void GetPageInfo(int *pageMin, int *pageMax, int *pageFrom,
							 int *pageTo);
  ...
  private:
	...
	int pageCount() { return job->num_pages(); }

	/// Draw a card, that is card_nr on this page, find the postion by asking
	/// the layout
	void drawCard(DC &dc, const CardP &card, int card_nr);
};

CardsPrintout::CardsPrintout(PrintJobP const &job) : job(job) {
	viewer.setSet(job->set);
}

void CardsPrintout::GetPageInfo(int *page_min, int *page_max, int *page_from,
								int *page_to) {
...
void CardsPrintout::OnPreparePrinting() {
	if (job->layout.empty()) {
		...
		job->layout.init(*job->set->stylesheet, job->layout_type,
						 RealSize(pw_mm, ph_mm));
	}
}
...
void CardsPrintout::drawCard(DC &dc, const CardP &card, int card_nr) {
	...
	RealPoint pos(
		job->layout.margin_left +
			(job->layout.card_size.width + job->layout.card_spacing.width) *
				col,
		job->layout.margin_top +
			(job->layout.card_size.height + job->layout.card_spacing.height) *
				row);
	// determine rotation
	const StyleSheet &stylesheet = job->set->stylesheetFor(card);
	Radians rotation = 0;
	if ((stylesheet.card_width > stylesheet.card_height) !=
		job->layout.card_landscape) {
		rotation = rad90;
	}
	/*
	...
	// adjust card size, to center card in the available space (from
	job->layout.card_size)?
	// TODO: deal with different sized cards in general
	*/

	// create buffers
	int w = int(stylesheet.card_width),
		h = int(stylesheet.card_height); // in pixels
	if (is_rad90(rotation))
		swap(w, h);
	...
	RotatedDC rdc(bufferDC, rotation, stylesheet.getCardRect(), zoom,
				  QUALITY_AA, ROTATION_ATTACH_TOP_LEFT);
	// render card to dc
	viewer.setCard(card);
...
PrintJobP make_print_job(Window *parent, const SetP &set,
	...
	wxCheckBox *space =
		new wxCheckBox(&wnd, wxID_ANY, L"Put space between cards");
	...
	if (wnd.ShowModal() != wxID_OK) {
		...
		job->layout_type = settings.print_layout =
			space->GetValue() ? LAYOUT_EQUAL_SPACE : LAYOUT_NO_SPACE;
...
void print_preview(Window *parent, const PrintJobP &job) {
	...
	wxPreviewFrame *frame = new wxPreviewFrame(
		new wxPrintPreview(new CardsPrintout(job), new CardsPrintout(job)),
