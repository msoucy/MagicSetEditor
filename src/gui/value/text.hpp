...
class TextValueEditor : public TextValueViewer, public ValueEditor {
  ...
  private:
	size_t selection_start,
		selection_end; ///< Cursor position/selection (if any), cursor positions
	size_t selection_start_i,
		selection_end_i; ///< Cursor position/selection, character indices
	bool selecting;		 ///< Selecting text?
	bool select_words;   ///< Select whole words when dragging the mouse?
	TextValueEditorScrollBar *
		scrollbar;			 ///< Scrollbar for multiline fields in native look
	bool scroll_with_cursor; ///< When the cursor moves, should the
	...
	/// Move the selection to a new location, clears the previously drawn
	/// selection.
	/** t specifies what kind of position new_end is */
	void moveSelection(IndexType t, size_t new_end, bool also_move_start = true,
					   Movement dir = MOVE_MID);
	/// Move the selection to a new location, but does not redraw.
	/** t specifies what kind of position new_end is */
	void moveSelectionNoRedraw(IndexType t, size_t new_end,
							   bool also_move_start = true,
							   Movement dir = MOVE_MID);
	...
	void replaceSelection(const String &replacement, const String &name,
						  bool allow_auto_replace = false,
						  bool select_on_undo = true);
	...
	/// Return a position resulting from moving pos outside the range
	/// [start...end), in the direction dir
	...
	/// Scrolls to ensure the caret stays visible, return true if the control is
	/// scrolled
	...
	/// Find a WordListPos under the mouse cursor (if any), pos is in internal
	/// coordinates
	WordListPosP findWordList(const RealPoint &pos) const;
	/// Find a WordListPos rectangle under the mouse cursor (if any), pos is in
