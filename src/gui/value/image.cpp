...
bool ImageValueEditor::onLeftDClick(const RealPoint &, wxMouseEvent &) {
    String filename = wxFileSelector(
        (L"Open image file"), settings.default_image_dir, (L""), (L""),
        (L"All images|*.bmp;*.jpg;*.png;*.gif|Windows bitmaps "
         L"(*.bmp)|*.bmp|JPEG images (*.jpg;*.jpeg)|*.jpg;*.jpeg|PNG images "
         L"(*.png)|*.png|GIF images (*.gif)|*.gif|TIFF images "
         L"(*.tif;*.tiff)|*.tif;*.tiff"),
...
void ImageValueEditor::sliceImage(const Image &image) {
    if (!image.Ok())
        return;
    // mask
    GeneratedImage::Options options((int)style().width, (int)style().height,
                                    &viewer.getStylePackage(),
                                    &viewer.getLocalPackage());
    AlphaMask mask;
    ...
    if (s.ShowModal() == wxID_OK) {
        ...
        LocalFileName new_image_file = getLocalPackage().newFileName(
            field().name, (L"")); // a new unique name in the package
        Image img = s.getImage();
        img.SaveFile(getLocalPackage().nameOut(new_image_file),
                     wxBITMAP_TYPE_PNG); // always use PNG images, see #69. Disk
                                         // space is cheap anyway.
        addAction(
            value_action(valueP(), script_local_image_file(new_image_file)));
    }
}
...
bool ImageValueEditor::canCopy() const { return !value().value->isNil(); }

bool ImageValueEditor::canPaste() const {
    return wxTheClipboard->IsSupported(wxDF_BITMAP) &&
           !wxTheClipboard->IsSupported(
               CardsDataObject::format); // we don't want to (accidentally)
                                         // paste card images
}
...
bool ImageValueEditor::onChar(wxKeyEvent &ev) {
    if (ev.AltDown() || ev.ShiftDown() || ev.ControlDown())
        return false;
