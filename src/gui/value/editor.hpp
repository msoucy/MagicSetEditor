 ...
 *                  SomeValueEditor
 *
 *  Note that ValueEditor does NOT inherit from ValueViewer, because that leads
...
class ValueEditor {
  public:
	...
	virtual bool onLeftUp(const RealPoint &pos, wxMouseEvent &ev) {
		return false;
	}
	virtual bool onLeftDClick(const RealPoint &pos, wxMouseEvent &ev) {
		return false;
	}
	virtual bool onRightDown(const RealPoint &pos, wxMouseEvent &ev) {
		return false;
	}
	virtual bool onMotion(const RealPoint &pos, wxMouseEvent &ev) {
		return false;
	}
	...
	virtual bool onContextMenu(IconMenu &m, wxContextMenuEvent &ev) {
		return true;
	}
	...
	/** this function should also check the data on the clipboard has the right
	 * format */
	virtual bool canPaste() const { return false; }
	/// Copies from this field editor, returns success
	virtual bool doCopy() { return false; }
	/// Deletes the selection from this field editor, cut = copy + delete,
	/// returns success
	...
	/** should again check if pasting is possible and fail silently if not,
	 * returns success */
	...
	/** If from_start == false: searches only from the current selection onward
	 *(or backward),
...
#define IMPLEMENT_VALUE_EDITOR(Type)                                           \
	ValueViewerP Type##Style::makeEditor(DataEditor &parent) {                 \
		return intrusive(                                                      \
			new Type##ValueEditor(parent, intrusive_from_existing(this)));     \
	}                                                                          \
	Type##ValueEditor::Type##ValueEditor(DataEditor &parent,                   \
										 const Type##StyleP &style)            \
