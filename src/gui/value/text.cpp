...
enum WordListItemFlags { FLAG_ACTIVE = 0x01, FLAG_SUBMENU = 0x02, FLAG_LINE_BELOW = 0x04 };
struct DropDownWordListItem {
	DropDownWordListItem() : flags(0) {}
	DropDownWordListItem(WordListWordP word, int flags = 0)
		: word(word), name(word->name),
		  flags(flags | (word->isGroup() * FLAG_SUBMENU) | (word->line_below * FLAG_LINE_BELOW)) {}
...
class DropDownWordList : public DropDownList {
  public:
	DropDownWordList(Window *parent, bool is_submenu, TextValueEditor &tve, const WordListPosP &pos,
					 const WordListWordP &list);
...
DropDownWordList::DropDownWordList(Window *parent, bool is_submenu, TextValueEditor &tve, const WordListPosP &pos,
								   const WordListWordP &words)
	: DropDownList(parent, is_submenu, is_submenu ? nullptr : &tve), tve(tve), pos(pos), has_checkboxes(false) {
	setWords(words);
}
...
void DropDownWordList::redrawArrowOnParent() { tve.redrawWordListIndicators(true); }

// ----------------------------------------------------------------------------- : TextValueEditor

IMPLEMENT_VALUE_EDITOR(Text)
, selection_start(0), selection_end(0), selection_start_i(0), selection_end_i(0), selecting(false), select_words(false),
...
bool TextValueEditor::onRightDown(const RealPoint &pos, wxMouseEvent &ev) {
	if (dropDownShown())
		return false;
	size_t index = v.indexAt(pos);
	if (index < min(selection_start_i, selection_end_i) || index > max(selection_start_i, selection_end_i)) {
...
wxMenu *TextValueEditor::getMenu(int type) const {
	if (type == ID_INSERT_SYMBOL && (style().always_symbol || style().allow_formating) && style().symbol_font.valid()) {
...
void TextValueEditor::redrawSelection(size_t old_selection_start_i, size_t old_selection_end_i,
...
RealRect TextValueEditor::boundingBox() const {
	...
	FOR_EACH_CONST(wl, word_lists) { r.width = max(r.width, wl->rect.right() + 9); }
	return r;
}
...
bool TextValueEditor::canPaste() const { return wxTheClipboard->IsSupported(wxDF_TEXT); }
...
void TextValueEditor::doFormat(int type) {
	...
	switch (type) {
	case ID_FORMAT_BOLD: {
		addAction(toggle_format_action(valueP(), _("b"), selection_start_i, selection_end_i, selection_start,
									   selection_end, _("Bold")));
		break;
	}
	case ID_FORMAT_ITALIC: {
		addAction(toggle_format_action(valueP(), _("i"), selection_start_i, selection_end_i, selection_start,
									   selection_end, _("Italic")));
		break;
	}
	case ID_FORMAT_SYMBOL: {
		addAction(toggle_format_action(valueP(), _("sym"), selection_start_i, selection_end_i, selection_start,
...
void TextValueEditor::insert(const String &text, const String &action_name) { replaceSelection(text, action_name); }
...
void TextValueEditor::replaceSelection(const String &replacement, const String &name, bool allow_auto_replace,
									   bool select_on_undo) {
	...
	TextValueAction *action =
		typing_action(valueP(), selection_start_i, selection_end_i, select_on_undo ? selection_start : selection_end,
	...
	// move cursor
	{
		...
		if (real_value.size() < expected_value.size() && expected_cursor < expected_value.size() &&
			expected_value.GetChar(expected_cursor) == UNTAG_SEP && real_value.GetChar(start) == UNTAG_SEP &&
			...
			for (size_t i = begin; i < end; ++i) {
				...
				if (match > best_match ||
					(match == best_match &&
					 abs((int)expected_cursor - (int)i) < abs((int)expected_cursor - (int)best_cursor))) {
					best_match = match;
					best_cursor = i;
				}
			}
...
size_t TextValueEditor::prevCharBoundary(size_t pos) const { return max(0, (int)pos - 1); }
...
bool isWordBoundaryChar(Char c) {
	return c == _(' ') || c == _(',') || c == _('.') || c == _(':') || c == _(';') || c == _('(') || c == _(')') ||
		   c == _('\n') || isPunct(c);
}
...
bool TextValueEditor::matchSubstr(const String &s, size_t pos, FindInfo &find) {
	if (pos >= s.size())
		return false;
	if (find.wholeWord()) {
		if (!is_word_end(s, pos - 1) || !is_word_end(s, pos + find.findString().size()))
			return false;
	}
...
void TextValueEditor::determineSize(bool force_fit) {
	...
	if (scrollbar) {
		...
		scrollbar->SetSize((int)(pos.x + rot.trX(style().width) + 1 - sbw), (int)pos.y - 1, (int)sbw,
...
void TextValueEditor::updateScrollbar() {
	...
	scrollbar->SetScrollbar(wxVERTICAL, position, page_size, range, page_size > 1 ? page_size - 1 : 0);
}
...
bool TextValueEditor::dropDownShown() const { return drop_down && drop_down->IsShown(); }
...
WordListPosP TextValueEditor::findWordList(const RealPoint &pos) const {
	FOR_EACH_CONST(wl, word_lists) {
		...
		if (pos.x >= r.right() - 0.5 && pos.x < r.right() + 9 && pos.y >= r.top() && pos.y < r.bottom()) {
			return wl;
		}
	}
	return WordListPosP();
