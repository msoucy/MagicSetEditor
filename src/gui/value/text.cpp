...
TextValueEditorScrollBar::TextValueEditorScrollBar(TextValueEditor &tve)
	: wxWindow(&tve.editor(), wxID_ANY, wxDefaultPosition, wxDefaultSize,
			   wxNO_BORDER | wxVSCROLL | wxALWAYS_SHOW_SB),
	  tve(tve) {}
...
class WordListPos : public IntrusivePtrBase<WordListPos> {
  public:
	WordListPos(size_t start, size_t end, WordListP word_list)
		: start(start), end(end), rect(-1, -1, -1, -1), word_list(word_list) {}
...
struct DropDownWordListItem {
	...
	DropDownWordListItem(WordListWordP word, const String &name, int flags = 0)
		: word(word), name(name), flags(flags) {}
	...
	inline void setActive(bool a) {
		flags = (flags & ~FLAG_ACTIVE) | a * FLAG_ACTIVE;
	}
};

class DropDownWordList : public DropDownList {
  public:
	DropDownWordList(Window *parent, bool is_submenu, TextValueEditor &tve,
					 const WordListPosP &pos, const WordListWordP &list);
  ...
  protected:
	...
	virtual bool lineBelow(size_t item) const {
		return items[item].flags & FLAG_LINE_BELOW;
	}
...
DropDownWordList::DropDownWordList(Window *parent, bool is_submenu,
								   TextValueEditor &tve,
								   const WordListPosP &pos,
								   const WordListWordP &words)
	: DropDownList(parent, is_submenu, is_submenu ? nullptr : &tve), tve(tve),
	  pos(pos), has_checkboxes(false) {
	setWords(words);
}
...
void DropDownWordList::addWordsFromScript(const WordListWordP &w) {
	...
	vector<String> strings;
	{
		...
		while (comma != String::npos) {
			...
			prev = comma + 1;
			if (prev + 1 < str.size() && str.GetChar(prev + 1) == _(' '))
				++prev; // skip space after comma
	...
	for (auto s : strings) {
		if (prev >= items.size() || s != items[prev].name) {
			...
			prev = items.size();
			items.push_back(
				DropDownWordListItem(w, s, w->line_below * FLAG_LINE_BELOW));
		}
	}
}

void DropDownWordList::drawIcon(DC &dc, int x, int y, size_t item,
								bool selected) const {
	if (has_checkboxes) {
		...
		if (radio) {
			draw_radiobox(nullptr, dc, rect, items[item].active(),
						  itemEnabled(item));
		} else {
			draw_checkbox(nullptr, dc, rect, items[item].active(),
						  itemEnabled(item));
		}
	}
}
...
DropDownList *DropDownWordList::submenu(size_t item) const {
	...
	if (i.submenu) {
		// make sure the members of the submenu are still up to date (in
		// particular the pos, see #13)
		i.submenu->pos = pos;
	...
	if (i.flags & FLAG_SUBMENU) {
		// create submenu?
		if (!i.submenu) {
			i.submenu.reset(new DropDownWordList(
				const_cast<DropDownWordList *>(this), true, tve, pos, i.word));
		}
		return i.submenu.get();
	} else {
		return nullptr;
...
size_t DropDownWordList::selection() const {
	// current selection
	String current = untag(tve.value().value->toString().substr(
	...
	for (auto item : items) {
		...
		if (item.active() && (selected == NO_SELECTION ||
...
void DropDownWordList::select(size_t item) {
	...
	tve.replaceSelection(escape(new_value),
						 _ACTION_1_("change", tve.field().name));
	// stay open?
	if (IsShown())
		selection(); // update 'enabled'
}
...
, selection_start(0), selection_end(0), selection_start_i(0),
	selection_end_i(0), selecting(false), select_words(false),
	scrollbar(nullptr), scroll_with_cursor(false), hovered_words(nullptr) {
	if (viewer.nativeLook() && field().multi_line) {
		scrollbar = new TextValueEditorScrollBar(*this);
		style->padding_right =
			max(style->padding_right(), (double)scrollbar->GetSize().x);
	}
}

TextValueEditor::~TextValueEditor() { delete scrollbar; }
...
bool TextValueEditor::onMotion(const RealPoint &pos, wxMouseEvent &ev) {
	...
	if (ev.LeftIsDown() && selecting) {
		...
		if (select_words) {
			...
			// TODO : still not quite right, requires a moveSelection function
			// that moves start & end simultaniously
...
bool TextValueEditor::onChar(wxKeyEvent &ev) {
	...
	case WXK_LEFT:
		// move left (selection?)
		if (ev.ControlDown()) {
			moveSelection(TYPE_INDEX, prevWordBoundary(selection_end_i),
						  !ev.ShiftDown(), MOVE_LEFT);
		} else {
			moveSelection(TYPE_CURSOR, prevCharBoundary(selection_end),
						  !ev.ShiftDown(), MOVE_LEFT);
		}
		break;
	case WXK_RIGHT:
		// move left (selection?)
		if (ev.ControlDown()) {
			moveSelection(TYPE_INDEX, nextWordBoundary(selection_end_i),
						  !ev.ShiftDown(), MOVE_RIGHT);
		} else {
			moveSelection(TYPE_CURSOR, nextCharBoundary(selection_end),
						  !ev.ShiftDown(), MOVE_RIGHT);
		}
		break;
	case WXK_UP:
		if (wordListDropDown(findWordList(selection_end_i)))
			break;
		moveSelection(TYPE_INDEX, v.moveLine(selection_end_i, -1),
					  !ev.ShiftDown(), MOVE_LEFT_OPT);
		break;
	case WXK_DOWN:
		if (wordListDropDown(findWordList(selection_end_i)))
			break;
	...
	case WXK_HOME:
		...
		if (ev.ControlDown()) {
			...
			moveSelection(TYPE_INDEX, v.lineStart(selection_end_i),
						  !ev.ShiftDown(), MOVE_LEFT_OPT);
		}
		break;
	case WXK_END:
		...
		if (ev.ControlDown()) {
			moveSelection(TYPE_INDEX, value().value->toString().size(),
						  !ev.ShiftDown(), MOVE_RIGHT_OPT);
		} else {
			moveSelection(TYPE_INDEX, v.lineEnd(selection_end_i),
						  !ev.ShiftDown(), MOVE_RIGHT_OPT);
		}
		break;
	case WXK_BACK:
		if (selection_start == selection_end) {
			...
			moveSelectionNoRedraw(TYPE_CURSOR, prevCharBoundary(selection_end),
								  false);
			if (selection_start == selection_end) {
	...
	case WXK_DELETE:
		if (selection_start == selection_end) {
			...
			moveSelectionNoRedraw(TYPE_CURSOR, nextCharBoundary(selection_end),
								  false);
			if (selection_start == selection_end) {
...
// This check is need, otherwise pressing a key, say "0" on the numpad produces
// "a0"
// (don't ask me why)
#else
		if (ev.GetKeyCode() >=
			_(' ') /*&& ev.GetKeyCode() == (int)ev.GetRawKeyCode()*/) {
#endif
...
#ifdef UNICODE
			replaceSelection(escape(String(ev.GetUnicodeKey(), 1)),
							 _ACTION_("typing"), true);
#else
			replaceSelection(escape(String((Char)ev.GetKeyCode(), 1)),
							 _ACTION_("typing"), true);
#endif
		} else {
			return false;
...
void spellcheck_language_at(const String &str, size_t error_pos,
							SpellChecker **out) {
...
void get_spelling_suggestions(const String &str, size_t error_pos,
							  vector<String> &suggestions_out) {
...
void TextValueEditor::onFocus() { showCaret(); }
...
bool TextValueEditor::onContextMenu(IconMenu &m, wxContextMenuEvent &ev) {
	...
	if (kwpos != String::npos) {
		m.InsertSeparator(0);
		m.Insert(0, ID_FORMAT_REMINDER, _("reminder"), _MENU_("reminder text"),
				 _HELP_("reminder text"), wxITEM_CHECK);
	}
	// in a spelling error? => show suggestions and "add to dictionary"
	size_t error_pos =
		in_tag(val, _("<error-spelling"), selection_start_i, selection_start_i);
	if (error_pos != String::npos) {
		// TODO: "add to dictionary"
		//%m.InsertSeparator(0);
		//%m.Insert(0,ID_SPELLING_ADD_TO_DICT, _MENU_("add to dictionary"),
		//_HELP_("add to dictionary"));
		// suggestions
		...
		if (suggestions.empty()) {
			m.Insert(0, ID_SPELLING_NO_SUGGEST,
					 _MENU_("no spelling suggestions"),
					 _HELP_("no spelling suggestions"));
		} else {
			int i = 0;
...
bool TextValueEditor::onCommand(int id) {
	if (id >= ID_INSERT_SYMBOL_MENU_MIN && id <= ID_INSERT_SYMBOL_MENU_MAX) {
		// Insert a symbol
		if ((style().always_symbol || style().allow_formating) &&
		...
		String val = value().value->toString();
		size_t error_pos = in_tag(val, _("<error-spelling"), selection_start_i,
								  selection_start_i);
		if (error_pos == String::npos)
			throw InternalError(_("Unexpected spelling suggestion")); // wrong
		...
		// replace it
		replaceSelection(suggestions.at(id - ID_SPELLING_SUGGEST),
						 _ACTION_("correct"));
		return true;
	}
	return false;
...
wxMenu *TextValueEditor::getMenu(int type) const {
	if (type == ID_INSERT_SYMBOL &&
		(style().always_symbol || style().allow_formating) &&
		style().symbol_font.valid()) {
...
void TextValueEditor::redrawSelection(size_t old_selection_start_i,
									  size_t old_selection_end_i,
									  bool old_drop_down_shown) {
	// Hide caret
	if (isCurrent()) {
		wxCaret *caret = editor().GetCaret();
		if (caret->IsVisible())
	...
	// Destroy the clientDC before reshowing the caret, prevent flicker on MSW
	{
		...
		if (!old_drop_down_shown) {
			v.drawSelection(dc, style(), old_selection_start_i,
							old_selection_end_i);
		}
		// scroll?
		...
		if (ensureCaretVisible()) {
			...
			// draw new selection
			if (!dropDownShown()) {
				v.drawSelection(dc, style(), selection_start_i,
								selection_end_i);
			}
...
wxCursor TextValueEditor::cursor(const RealPoint &pos) const {
	...
	if (p) {
		...
		Radians angle =
			viewer.getRotation().getAngle() + deg_to_rad(style().angle);
...
bool TextValueEditor::containsPoint(const RealPoint &pos) const {
	...
	if (TextValueViewer::containsPoint(pos2))
		return true;
	if (word_lists.empty())
		return false;
...
RealRect TextValueEditor::boundingBox() const {
	if (word_lists.empty())
		return ValueViewer::boundingBox();
	RealRect r = style().getInternalRect().grow(1);
	for (auto const wl : word_lists) {
...
bool TextValueEditor::doCopy() {
	...
	String val = value().value->toString();
	if (selection_start_i > val.size())
		selection_start_i = val.size();
	if (selection_end_i > val.size())
		selection_end_i = val.size();
...
bool TextValueEditor::hasFormat(int type) const {
	...
	case ID_FORMAT_REMINDER: {
		size_t tag =
			in_tag(val, _("<kw"), selection_start_i, selection_start_i);
...
void TextValueEditor::doFormat(int type) {
	...
	switch (type) {
	case ID_FORMAT_BOLD: {
		addAction(toggle_format_action(valueP(), _("b"), selection_start_i,
									   selection_end_i, selection_start,
									   selection_end, _("Bold")));
		break;
	}
	case ID_FORMAT_ITALIC: {
		addAction(toggle_format_action(valueP(), _("i"), selection_start_i,
									   selection_end_i, selection_start,
									   selection_end, _("Italic")));
		break;
	}
	case ID_FORMAT_SYMBOL: {
		addAction(toggle_format_action(valueP(), _("sym"), selection_start_i,
									   selection_end_i, selection_start,
									   selection_end, _("Symbols")));
		break;
	}
	case ID_FORMAT_REMINDER: {
...
void TextValueEditor::showCaret() {
	...
	// cursor rectangle
	RealRect cursor =
		v.charRect(selection_end_i, selection_start_i <= selection_end_i);
	cursor.width = 0;
	...
	if (cursor.height == 0) {
		if (style().always_symbol && style().symbol_font.valid()) {
			...
			RealSize s = style().symbol_font.font->defaultSymbolSize(
				rot.trS(style().symbol_font.size));
...
#ifdef __WXGTK__
				// HACK: Some fonts don't get the descender height set
				// correctly.
...
/// compare two cursor positions, determine how much the text matches before and
/// after
size_t match_cursor_position(size_t pos1, const String &text1, size_t pos2,
							 const String &text2) {
	...
	for (before1 = before2 = 0; before1 < pos1 && before2 < pos2;
		 ++before1, ++before2) {
		Char c1 = text1.GetChar(pos1 - before1 - 1),
			 c2 = text2.GetChar(pos2 - before2 - 1);
		if (c1 == c2)
			score += 10000;
	...
	if (pos1 == before1 && pos2 == before2)
		score += 10000;
	// Match part after cursor
	size_t after1, after2;
	for (after1 = after2 = 0;
		 pos1 + after1 < text1.size() && pos2 + after2 < text2.size();
		 ++after1, ++after2) {
		Char c1 = text1.GetChar(pos1 + after1),
			 c2 = text2.GetChar(pos2 + after2);
		if (c1 == c2)
			score += 10;
		else if (toLower(c1) == toLower(c2))
			score += 9;
	...
	if (pos1 + after1 == text1.size() && pos2 + after2 == text2.size())
		score += 2;
	// final score: matching 'before' is more important
	return score;
}

void TextValueEditor::replaceSelection(const String &replacement,
									   const String &name,
									   bool allow_auto_replace,
									   bool select_on_undo) {
	...
	if (selection_end < selection_start)
		swap(selection_end, selection_start);
	fixSelection();
	...
	TextValueAction *action =
		typing_action(valueP(), selection_start_i, selection_end_i,
					  select_on_undo ? selection_start : selection_end,
					  selection_end, replacement, name);
	if (!action) {
	...
	size_t expected_cursor = min(selection_start, selection_end) +
							 untag_for_cursor(replacement).size();
	// perform the action
	// NOTE: this calls our onAction, invalidating the text viewer and moving
	// the selection around the new text
	addAction(action);
	// move cursor
	{
		String real_value = untag_for_cursor(value().value->toString());
		// where real and expected value are the same, nothing has happend, so
		// don't look there
		size_t start, end_min;
		for (start = 0; start < min(real_value.size(), expected_value.size());
			 ++start) {
			if (real_value.GetChar(start) != expected_value.GetChar(start))
				break;
		}
		for (end_min = 0;
			 end_min < min(real_value.size(), expected_value.size());
			 ++end_min) {
			if (real_value.GetChar(real_value.size() - end_min - 1) !=
				expected_value.GetChar(expected_value.size() - end_min - 1))
				break;
		}
		...
		if (real_value.size() < expected_value.size() &&
			...
			// try to find the best match to what text we expected to be around
			// the cursor
			size_t best_match = 0;
			size_t begin = min(start, expected_cursor);
			size_t end =
				min(real_value.size() + 1,
					max(real_value.size() - end_min, expected_cursor) + 1);
			for (size_t i = begin; i < end; ++i) {
				size_t match = match_cursor_position(
					expected_cursor, expected_value, i, real_value);
				if (match > best_match ||
					(match == best_match &&
					 abs((int)expected_cursor - (int)i) <
						 abs((int)expected_cursor - (int)best_cursor))) {
					best_match = match;
					best_cursor = i;
				}
			}
	...
	for (auto ar : gs.auto_replaces) {
		if (ar->enabled && ar->match.size() <= end) {
			...
			if (is_substr(val, start, ar->match) &&
				(!ar->whole_word ||
				 (isWordBoundary(start) && isWordBoundary(end)))) {
				// replace
				selection_start_i = start;
				selection_end_i = end;
				fixSelection(TYPE_INDEX);
				replaceSelection(ar->replace, _ACTION_("auto replace"), false,
								 false);
			}
		}
	}
...
void TextValueEditor::moveSelection(IndexType t, size_t new_end,
									bool also_move_start, Movement dir) {
...
void TextValueEditor::moveSelectionNoRedraw(IndexType t, size_t new_end,
											bool also_move_start,
											Movement dir) {
	if (t == TYPE_INDEX) {
		selection_end_i = new_end;
		if (also_move_start)
			selection_start_i = selection_end_i;
	} else {
...
Movement direction_of(size_t a, size_t b) {
	if (a < b)
		return MOVE_LEFT_OPT;
	if (a > b)
		return MOVE_RIGHT_OPT;
	else
		return MOVE_MID;
}
...
void TextValueEditor::fixSelection(IndexType t, Movement dir) {
	...
	// prepare to move 'inward' (i.e. from start in the direction of end and
	// vice versa)
	selection_start_i = cursor_to_index(
		val, selection_start, direction_of(selection_end, selection_start));
	selection_end_i = cursor_to_index(
		val, selection_end, direction_of(selection_start, selection_end));
	...
	while (seppos != String::npos) {
		...
		if (selection_start_i <= seppos && selection_end_i > seppos) {
			...
			selection_end_i =
				cursor_to_index(val, selection_end,
								direction_of(selection_start, selection_end));
...
size_t TextValueEditor::nextCharBoundary(size_t pos) const {
	return min(index_to_cursor(value().value->toString(), String::npos),
			   pos + 1);
}

static const Char word_bound_chars[] = _(" ,.:;()\n");
bool isWordBoundaryChar(Char c) {
	return c == _(' ') || c == _(',') || c == _('.') || c == _(':') ||
		   c == _(';') || c == _('(') || c == _(')') || c == _('\n') ||
		   isPunct(c);
}
...
bool TextValueEditor::isWordBoundary(size_t pos_i) const {
	...
	while (true) {
		...
		if (c == _('<'))
			pos = skip_tag(val, pos); // skip tags
		else if (isWordBoundaryChar(c))
			return true;
		else
			break;
	}
...
size_t TextValueEditor::move(size_t pos, size_t start, size_t end,
							 Movement dir) {
	if (dir < 0 /*MOVE_LEFT*/)
		return start;
	if (dir > 0 /*MOVE_RIGHT*/)
...
bool TextValueEditor::matchSubstr(const String &s, size_t pos, FindInfo &find) {
	if (pos >= s.size())
		return false;
	if (find.wholeWord()) {
		if (!is_word_end(s, pos - 1) ||
			!is_word_end(s, pos + find.findString().size()))
			return false;
	}
	if (find.caseSensitive()) {
		if (!is_substr(s, pos, find.findString()))
			return false;
	} else {
		if (!is_substr(s, pos, find.findString().Lower()))
			return false;
	}
	// handle
	...
	if (find.select()) {
		...
		selection_end_i =
			untagged_to_index(val, pos + find.findString().size(), true);
		fixSelection(TYPE_INDEX);
		was_selection =
			old_sel_start == selection_start && old_sel_end == selection_end;
	}
...
bool TextValueEditor::search(FindInfo &find, bool from_start) {
	...
	if (!find.caseSensitive())
		v.LowerCase();
	size_t selection_min =
		index_to_untagged(val, min(selection_start_i, selection_end_i));
	size_t selection_max =
		index_to_untagged(val, max(selection_start_i, selection_end_i));
	if (find.forward()) {
		size_t start = min(v.size(), find.searchSelection() ? selection_min
															: selection_max);
		for (size_t i = start; i + find.findString().size() <= v.size(); ++i) {
			if (matchSubstr(v, i, find))
				return true;
		}
	} else {
		size_t start = 0;
		int end =
			(int)(find.searchSelection() ? selection_max : selection_min) -
			(int)find.findString().size();
		if (end < 0)
			return false;
...
void TextValueEditor::determineSize(bool force_fit) {
	...
	if (scrollbar) {
		...
		scrollbar->SetSize((int)(pos.x + rot.trX(style().width) + 1 - sbw),
						   (int)pos.y - 1, (int)sbw,
		...
		dc.SelectObject(bmp);
		dc.SetFont(style().font.toWxFont(1.0));
		style().height = dc.GetCharHeight() + 2 + style().padding_top +
...
bool TextValueEditor::onMouseWheel(const RealPoint &pos, wxMouseEvent &ev) {
	if (scrollbar) {
		int toScroll = ev.GetWheelRotation() * ev.GetLinesPerAction() /
					   ev.GetWheelDelta(); // note: up is positive
		int target = min(max(scrollbar->GetScrollPos(wxVERTICAL) - toScroll, 0),
						 scrollbar->GetScrollRange(wxVERTICAL) -
							 scrollbar->GetScrollThumb(wxVERTICAL));
		scrollTo(target);
		return true;
	}
	return false;
...
bool TextValueEditor::ensureCaretVisible() {
	if (scrollbar && scroll_with_cursor) {
		...
		return v.ensureVisible(style().height - style().padding_top -
								   style().padding_bottom,
							   selection_end_i);
	}
	return false;
...
void TextValueEditor::updateScrollbar() {
	...
	int page_size = (int)v.visibleLineCount(
		style().height - style().padding_top - style().padding_bottom);
	int range = (int)v.lineCount();
	scrollbar->SetScrollbar(wxVERTICAL, position, page_size, range,
...
void TextValueEditor::findWordLists() {
	...
	while (pos != String::npos) {
		...
		if (type_end == String::npos || end == String::npos)
			return;
		...
		// add to word_lists
		if (word_list) {
			word_lists.push_back(
				intrusive(new WordListPos(pos, end, word_list)));
		} else {
			queue_message(MESSAGE_WARNING,
						  _ERROR_1_("word list type not found", name));
		}
		// next
...
void TextValueEditor::clearWordListIndicators(RotatedDC &dc) {
	...
	for (auto wl : word_lists) {
		if (current && drop_down && drop_down->IsShown() &&
			drop_down->getPos() == wl) {
			continue;
		} else if (current && selection_end_i >= wl->start &&
				   selection_end_i <= wl->end && !dropDownShown()) {
			continue;
		...
		// restore background
		if (wl->behind.Ok()) {
			dc.DrawPreRotatedBitmap(wl->behind,
									RealRect(wl->rect.right(), wl->rect.y - 1,
											 10, wl->rect.height + 3));
		}
	}
}
...
void TextValueEditor::redrawWordListIndicators(bool toggling_dropdown) {
	redrawSelection(selection_start_i, selection_end_i,
					dropDownShown() != toggling_dropdown);
}
...
void TextValueEditor::drawWordListIndicators(RotatedDC &dc, bool redrawing) {
	...
	for (auto wl : word_lists) {
		...
		if (r.height < 0) {
			...
			RealRect end = v.charRect(
				wl->end > wl->start ? wl->end - 1 : wl->start, false);
			r.x = start.x;
			r.y = start.y;
		...
		// capture background?
		if (!redrawing) {
			wl->behind = dc.GetBackground(
				RealRect(r.right(), r.top() - 1, 10, r.height + 3));
		}
	...
	for (auto &wl : reverse(word_lists)) {
		...
		if (small) {
			if (what & DRAW_BOXES) {
				dc.DrawRectangle(
					RealRect(r.right(), r.top() - 1, 2, r.height + 2));
			}
		} else {
			...
			wxPoint poly[] = {dc.tr(RealPoint(0,0)), dc.tr(RealPoint(5,0)),
			dc.tr(RealPoint(3,2))};
...
bool TextValueEditor::wordListDropDown(const WordListPosP &wl) {
	...
	if (drop_down) {
		...
		drop_down.reset(
			new DropDownWordList(&editor(), false, *this, wl, wl->word_list));
	}
