...
void DropDownWordList::addWordsFromScript(const WordListWordP &w) {
    ...
    vector<String> strings;
    {
        ...
        while (comma != String::npos) {
            ...
            prev = comma + 1;
            if (prev + 1 < str.size() && str.GetChar(prev + 1) == (L' '))
...
#else
        if (ev.GetKeyCode() >=
            (L' ') /*&& ev.GetKeyCode() == (int)ev.GetRawKeyCode()*/) {
#endif
...
bool TextValueEditor::onContextMenu(IconMenu &m, wxContextMenuEvent &ev) {
    ...
    if (kwpos != String::npos) {
        m.InsertSeparator(0);
        m.Insert(0, ID_FORMAT_REMINDER, (L"reminder"), _MENU_("reminder text"),
                 _HELP_("reminder text"), wxITEM_CHECK);
    }
    // in a spelling error? => show suggestions and "add to dictionary"
    size_t error_pos =
        in_tag(val, (L"<error-spelling"), selection_start_i, selection_start_i);
...
bool TextValueEditor::onCommand(int id) {
    if (id >= ID_INSERT_SYMBOL_MENU_MIN && id <= ID_INSERT_SYMBOL_MENU_MAX) {
        ...
        String val = value().value->toString();
        size_t error_pos = in_tag(val, (L"<error-spelling"), selection_start_i,
                                  selection_start_i);
        if (error_pos == String::npos)
            throw InternalError((L"Unexpected spelling suggestion")); // wrong
...
bool TextValueEditor::hasFormat(int type) const {
    ...
    case ID_FORMAT_REMINDER: {
        size_t tag =
            in_tag(val, (L"<kw"), selection_start_i, selection_start_i);
...
void TextValueEditor::doFormat(int type) {
    ...
    switch (type) {
    case ID_FORMAT_BOLD: {
        addAction(toggle_format_action(valueP(), (L"b"), selection_start_i,
                                       selection_end_i, selection_start,
                                       selection_end, (L"Bold")));
        break;
    }
    case ID_FORMAT_ITALIC: {
        addAction(toggle_format_action(valueP(), (L"i"), selection_start_i,
                                       selection_end_i, selection_start,
                                       selection_end, (L"Italic")));
        break;
    }
    case ID_FORMAT_SYMBOL: {
        addAction(toggle_format_action(valueP(), (L"sym"), selection_start_i,
                                       selection_end_i, selection_start,
                                       selection_end, (L"Symbols")));
        break;
    }
    case ID_FORMAT_REMINDER: {
...
void TextValueEditor::drawWordListIndicators(RotatedDC &dc, bool redrawing) {
    ...
    for (auto wl_it = word_lists.rbegin(); wl_it != word_lists.rend();
         ++wl_it) {
        auto &wl = *wl_it;
        RealRect &r = wl->rect;
        // color
