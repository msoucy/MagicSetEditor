...
void SymbolValueEditor::draw(RotatedDC &dc) {
    ...
    if (symbols.empty()) {
        ...
        dc.DrawText((L"double click to edit symbol"),
                    align_in_rect(ALIGN_MIDDLE_CENTER, text_size,
...
void SymbolValueEditor::drawButton(RotatedDC &dc, int button,
                                   const String &text) {
    ...
    double y = 0;
    // draw button
    draw_button(&editor(), dc.getDC(),
                dc.trRectToBB(RealRect(x, y, width, height)), false, down,
                true);
    // draw text
    RealSize text_size = dc.GetTextExtent(text);
    dc.DrawText(text,
                align_in_rect((Alignment)(ALIGN_BOTTOM | ALIGN_CENTER),
                              text_size, RealRect(x, y, width, height * 0.9)));
    // draw image
    const Bitmap &bmp = button_images[button];
    RealSize image_size(bmp.GetWidth(), bmp.GetHeight());
    dc.DrawBitmap(bmp, align_in_rect(ALIGN_MIDDLE_CENTER, image_size,
                                     RealRect(x, y, width, height * 0.8)));
}

int SymbolValueEditor::findButton(const RealPoint &pos) {
    if (pos.y < 0 || pos.y >= style().height)
        return -1;
...
bool SymbolValueEditor::onLeftDClick(const RealPoint &pos, wxMouseEvent &) {
    // Use SetWindow as parent? Maybe not, the symbol editor will stay open when
    // mainwindow closes
    editSymbol();
    return true;
}

void SymbolValueEditor::determineSize(bool) { style().height = 50; }
...
ValueActionPerformer *SymbolValueEditor::getActionPerformer() {
    return new ValueActionPerformer(valueP(), editor().getCard().get(),
                                    editor().getSetForActions());
}
