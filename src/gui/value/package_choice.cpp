...
DropDownPackageChoiceList::DropDownPackageChoiceList(
    Window *parent, PackageChoiceValueEditor *editor)
    : DropDownList(parent, false, editor), editor(*editor) {
    icon_size.width = 16;
    icon_size.height = 16;
...
String DropDownPackageChoiceList::itemText(size_t item) const {
    if (item == 0 && !editor.field().required)
        return editor.field().empty_name;
    else {
...
void DropDownPackageChoiceList::drawIcon(DC &dc, int x, int y, size_t item,
                                         bool selected) const {
    if (item == 0 && !editor.field().required)
        return;
    size_t i = item - !editor.field().required;
...
bool PackageChoiceValueEditor::onLeftDown(const RealPoint &pos,
                                          wxMouseEvent &ev) {
    if (!drop_down)
        initDropDown();
