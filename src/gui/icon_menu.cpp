...
Image generateDisabledImage(const Image &imgIn) {
	...
	for (int y = 0; y < 16; ++y) {
		for (int x = 0; x < 16; ++x) {
			...
			if (x < 15 && y < 15)
				in1 = Color(in[0], in[1], in[2]);
			// and the pixel to the left+up?
			if (x > 0 && y > 0)
				in2 = Color(in[0 - 3 * 17], in[1 - 3 * 17], in[2 - 3 * 17]);
			// determine output color
			Color col;
			if (in1 != trans && in1 != light)
				col = shadow;
			else if (in2 != trans && in2 != light)
				col = light;
			else
				col = trans;
			out[0] = col.Red();
...
void IconMenu::Append(int id, const String &resource, const String &text,
					  const String &help, wxItemKind kind, wxMenu *submenu) {
...
void IconMenu::Insert(size_t pos, int id, const String &resource,
					  const String &text, const String &help, wxItemKind kind,
					  wxMenu *submenu) {
	// create menu, load icon
	wxMenuItem *item = new wxMenuItem(this, id, text, help, kind, submenu);
	set_menu_item_image(item, resource);
...
void IconMenu::Insert(size_t pos, int id, const String &text,
					  const String &help, wxItemKind kind, wxMenu *submenu) {
	wxMenuItem *item = new wxMenuItem(this, id, text, help, kind, submenu);
	item->SetBitmap(wxNullBitmap);
	wxMenu::Insert(pos, item);
