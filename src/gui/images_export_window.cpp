...
ImagesExportWindow::ImagesExportWindow(
    Window *parent, const SetP &set, const ExportCardSelectionChoices &choices)
    ...
    wxSizer *s2 =
        new wxStaticBoxSizer(wxVERTICAL, this, _LABEL_("export filenames"));
    s2->Add(new wxStaticText(this, -1, _LABEL_("filename format")), 0, wxALL,
            4);
    s2->Add(format, 0, wxEXPAND | (wxALL & ~wxTOP), 4);
    s2->Add(new wxStaticText(this, -1, _HELP_("filename format")), 0,
            wxALL & ~wxTOP, 4);
    s2->Add(new wxStaticText(this, -1, _LABEL_("filename conflicts")), 0, wxALL,
            4);
...
void ImagesExportWindow::onOk(wxCommandEvent &) {
    ...
    if (sel == 0)
        gs.images_export_conflicts = CONFLICT_KEEP_OLD;
    else if (sel == 1)
        gs.images_export_conflicts = CONFLICT_OVERWRITE;
    else if (sel == 2)
        gs.images_export_conflicts = CONFLICT_NUMBER;
    else
        gs.images_export_conflicts = CONFLICT_NUMBER_OVERWRITE;
    // Select filename
    String name = wxFileSelector(
        _TITLE_("export images"), settings.default_export_dir,
        _LABEL_("filename is ignored"), (L""),
        _LABEL_("filename is ignored") + (L"|*"), wxFD_SAVE, this);
    if (name.empty())
        return;
    settings.default_export_dir = wxPathOnly(name);
    // Export
    export_images(set, getSelection(), name, gs.images_export_filename,
                  gs.images_export_conflicts);
    // Done
    EndModal(wxID_OK);
}
