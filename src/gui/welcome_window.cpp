...
WelcomeWindow::WelcomeWindow()
	: wxFrame(nullptr, wxID_ANY, _TITLE_("magic set editor"), wxDefaultPosition,
			  wxSize(520, 380),
			  wxDEFAULT_DIALOG_STYLE | wxTAB_TRAVERSAL | wxCLIP_CHILDREN),
...
// init controls
	...
	wxControl *new_set = new HoverButtonExt(
		this, ID_FILE_NEW, load_resource_image(_("welcome_new")),
		_BUTTON_("new set"), _HELP_("new set"));
	wxControl *open_set = new HoverButtonExt(
		this, ID_FILE_OPEN, load_resource_image(_("welcome_open")),
		_BUTTON_("open set"), _HELP_("open set"));
#if !USE_OLD_STYLE_UPDATE_CHECKER
	wxControl *updates = new HoverButtonExt(
		this, ID_FILE_CHECK_UPDATES, load_resource_image(_("welcome_updates")),
		_BUTTON_("check updates"), _HELP_("check updates"));
#endif
#else
	wxControl *new_set = new wxButton(this, ID_FILE_NEW, _BUTTON_("new set"));
	wxControl *open_set =
		new wxButton(this, ID_FILE_OPEN, _BUTTON_("open set"));
#if !USE_OLD_STYLE_UPDATE_CHECKER
	wxControl *updates =
		new wxButton(this, ID_FILE_CHECK_UPDATES, _BUTTON_("check updates"));
#endif
#endif
	...
	if (!settings.recent_sets.empty()) {
		...
		if (wxFileName::FileExists(filename) ||
			wxFileName::DirExists(filename + _("/"))) {
#ifdef USE_HOVERBUTTON
			wxFileName n(filename);
			open_last = new HoverButtonExt(
				this, ID_FILE_RECENT, load_resource_image(_("welcome_last")),
				_BUTTON_("last opened set"),
				_HELP_1_("last opened set", n.GetName()));
#else
			open_last =
				new wxButton(this, ID_FILE_RECENT, _BUTTON_("last opened set"));
#endif
		}
	}
...
void WelcomeWindow::draw(DC &dc) {
...
#if USE_BETA_LOGO
	dc.DrawBitmap(logo2, ws.GetWidth() - logo2.GetWidth(),
				  ws.GetHeight() - logo2.GetHeight());
#endif
	// draw version number
	...
	String version_string =
		_("version ") + app_version.toString() + version_suffix;
...
void WelcomeWindow::onOpenSet(wxCommandEvent &) {
	wxFileDialog *dlg =
		new wxFileDialog(this, _TITLE_("open set"), settings.default_set_dir,
						 wxEmptyString, import_formats(), wxFD_OPEN);
...
void WelcomeWindow::onNewSet(wxCommandEvent &) { close(new_set_window(this)); }
...
void WelcomeWindow::onOpenLast(wxCommandEvent &) {
	...
	try {
		...
		// remove this package from the recent sets, so we don't get this error
		// again
...
void WelcomeWindow::onCheckUpdates(wxCommandEvent &) {
	Show(false); // hide, so the PackagesWindow will not use this window as its
				 // parent
...
HoverButtonExt::HoverButtonExt(Window *parent, int id, const wxImage &icon,
							   const String &label, const String &sub_label)
	: HoverButton(parent, id, _("btn")), icon(icon), label(label),
	  sub_label(sub_label),
