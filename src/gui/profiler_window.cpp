...
void ProfilerPanel::draw_profiler(wxDC &dc, int x0, int y0) {
    ...
    for (auto &prof : boost::adaptors::reverse(profiles)) {
        ...
        draw_right(dc, wxString::Format((L"%.2f"), prof->avg_time()), pos[2],
                   y);
        draw_right(dc, wxString::Format((L"%.2f"), prof->total_time()), pos[3],
                   y);
        draw_right(dc, wxString::Format((L"%.2f"), prof->max_time()), pos[4],
                   y);
    }
...
void show_profiler_window(wxWindow *parent) {
    wxDialog *dlg = new wxDialog(parent, wxID_ANY, (L"Profiler"),
