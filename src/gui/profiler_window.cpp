...
ProfilerPanel::ProfilerPanel(wxWindow *parent, bool fancy_effects)
	: wxPanel(parent, wxID_ANY), fancy_effects(fancy_effects), timer(this) {
...
void ProfilerPanel::draw_profiler(wxDC &dc, int x0, int y0) {
	...
	int i = 0;
	for (auto &prof : reverse(profiles)) {
		// recently changed?
		if (fancy_effects) {
		...
		draw_right(dc, wxString::Format(_("%.2f"), prof->avg_time()), pos[2],
				   y);
		draw_right(dc, wxString::Format(_("%.2f"), prof->total_time()), pos[3],
				   y);
		draw_right(dc, wxString::Format(_("%.2f"), prof->max_time()), pos[4],
				   y);
	}
...
void ProfilerPanel::onTimer(wxTimerEvent &) { Refresh(false); }

void ProfilerPanel::onSize(wxSizeEvent &) { Refresh(true); }

wxSize ProfilerPanel::DoGetBestSize() const { return wxSize(300, 500); }
...
void show_profiler_window(wxWindow *parent) {
	wxDialog *dlg = new wxDialog(parent, wxID_ANY, _("Profiler"),
