...
class DataEditor : public CardViewer {
  public:
	...
	bool selectLast();
	/// Select the next editable editor, returns false if the current editor is
	/// the last one
	bool selectNext();
	/// Select the previous editable editor, returns false if the current editor
	/// is the first one
	bool selectPrevious();
	...
	/// Insert 'text' into the current editor, using an action with the given
	/// name
  ...
  protected:
	...
	ValueEditor *current_editor; ///< The currently selected editor,
	/// corresponding to the viewer
	ValueViewer *hovered_viewer; ///< The editor under the mouse cursor
	vector<ValueViewer *>
		by_tab_index; ///< The editable viewers, sorted by tab index
  ...
  private:
	...
	void selectField(wxMouseEvent &ev,
