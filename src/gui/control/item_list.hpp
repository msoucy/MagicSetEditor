...
class ItemList : public wxListView {
  ...
  protected:
	...
	/** If focus then the item is also focused and selected in the actual
	 * control.
	 *  This should not be done when the item is selected because it was focused
	 * (leading to a loop).
	 */
	...
	/** if force_focus == true, then the item is highlighted again if it already
	 * is focused. */
	...
	long selected_item_pos; ///< Position of the selected item in the
							///sorted_list, or -1 if no card is selected
	long sort_by_column;	///< Column to use for sorting, or -1 if not sorted
	bool sort_ascending;	///< Sort order
	vector<VoidP> sorted_list; ///< Sorted list of items, can be considered a
							   ///map: pos->item
