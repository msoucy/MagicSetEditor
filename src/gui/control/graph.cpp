...
void GraphDataPre::splitList(size_t axis) {
	...
	for (size_t i = 0; i < count; ++i) {
		...
		while (comma != String::npos) {
			...
			if (is_substr(v, comma, _(", ")))
				++comma; // skip space after it
			v = v.substr(comma + 1);
...
String to_bin(double value, double bin_size) {
	if (bin_size <= 0 || value == 0) {
		...
		return String::Format(_("%.0f%c%.0f"), (bin - 1) * bin_size + 1,
							  EN_DASH, bin * bin_size);
	}
}
...
GraphData::GraphData(const GraphDataPre &d) : axes(d.axes) {
	// find groups on each axis
	size_t i = 0;
	for (auto &a : axes) {
		map<String, UInt, SmartLess>
			counts; // note: default constructor for UInt() does initialize to 0
		...
		if (a->auto_color == AUTO_COLOR_NO && a->colors) {
			...
			for (auto &g : a->groups) {
				double amount =
					a->auto_color == AUTO_COLOR_EVEN
						? 1. / a->groups.size()
						: double(g.size) / a->total; // amount this group takes
				if (!first)
					step += amount / 2;
				if (a->numeric) {
					g.color = hsl2rgb(0.65 - 0.82 * step,
									  0.9 - 0.2 * fabs(step - 0.5),
									  0.3 + 0.35 * step);
				} else {
	...
	for (auto const &e : d.elements) {
		...
		GraphDataElement *de =
			reinterpret_cast<GraphDataElement *>(new char[de_size]);
...
void GraphData::crossAxis(size_t axis1, size_t axis2, size_t axis3,
						  vector<UInt> &out) const {
	size_t a1_size = axes[axis1]->groups.size();
	size_t a2_size = axes[axis2]->groups.size();
...
bool matches(const GraphDataElement *v, const vector<int> &match) {
	for (size_t i = 0; i < match.size(); ++i) {
		if (v->group_nrs[i] == -1 ||
			(match[i] != -1 && v->group_nrs[i] != match[i])) {
			return false;
		}
	}
	return true;
...
void Graph1D::draw(RotatedDC &dc, const vector<int> &current,
				   DrawLayer layer) const {
	draw(dc, axis < current.size() ? current.at(axis) : -1, layer);
}
bool Graph1D::findItem(const RealPoint &pos, const RealRect &screen_rect,
...
RealRect bar_graph_bar(const RealRect &screen_rect, int group, int group_count,
	...
	if (bottom < top)
		swap(top, bottom);
	bottom += 1;
	return RealRect(screen_rect.x + width_space * group + space / 2, top, width,
					bottom - top);
}
...
int find_bar_graph_column(double width, double x, int count) {
	...
	if (col < 0 || col >= count)
		return -1; // not a column
	double in_col = x - col * width_space;
	if (in_col < space / 2)
		return -1; // left
	if (in_col > width_space - space / 2)
		return -1; // right
	return col;
}
...
void BarGraph::draw(RotatedDC &dc, int current, DrawLayer layer) const {
	...
	if (layer == LAYER_SELECTION) {
		...
		if (current >= 0) {
			...
			RealRect bar = bar_graph_bar(screen_rect, current, count, 0,
										 group.size, axis.max);
		...
		int i = 0;
		for (auto const &g : axis.groups) {
			// draw bar
			dc.SetPen(i == current ? fg : lerp(fg, g.color, 0.5));
			dc.SetBrush(g.color);
			RealRect bar =
				bar_graph_bar(screen_rect, i++, count, 0, g.size, axis.max);
			dc.DrawRectangle(bar);
			// redraw axis part
			dc.SetPen(fg);
			dc.DrawLine(bar.bottomLeft() + Vector2D(0, -1),
						bar.bottomRight() + Vector2D(0, -1));
		}
	}
}
int BarGraph::findItem(const RealPoint &pos, const RealRect &screen_rect,
					   bool tight) const {
	if (!data)
		return -1;
	if (pos.y > max(screen_rect.top(), screen_rect.bottom()))
		return -1; // below
	if (pos.y < min(screen_rect.top(), screen_rect.bottom()))
		return -1; // above
	// TODO: tight check
	return find_bar_graph_column(screen_rect.width, pos.x - screen_rect.x,
								 (int)axis_data().groups.size());
}

// -----------------------------------------------------------------------------
// : Bar Graph 2D

void BarGraph2D::draw(RotatedDC &dc, const vector<int> &current,
					  DrawLayer layer) const {
	if (!data || data->axes.size() <= max(axis1, axis2))
		return;
	// Rectangle for bars
	RealRect screen_rect = dc.getInternalRect();
	...
	if (layer == LAYER_SELECTION) {
		...
		if (cur1 >= 0) {
			...
			int start = 0;
			int j = 0;
			for (auto const &g2 : axis2.groups) {
				int end = start + values[j + axis2.groups.size() * cur1];
				if (j == cur2 || cur2 < 0) {
					RealRect bar = bar_graph_bar(screen_rect, cur1, count,
												 start, end, axis1.max);
		...
		for (int i = 0; i < count; ++i) {
			...
			for (auto const &g2 : axis2.groups) {
				...
				if (start != end) {
					...
					RealRect bar = bar_graph_bar(screen_rect, i, count, start,
												 end, axis1.max);
					dc.DrawRectangle(bar);
					// fix up line below
					dc.SetPen(
						active || prevActive
							? fg
							: lerp(fg, lerp(prevColor, g2.color, 0.5), 0.5));
					dc.DrawLine(bar.bottomLeft() + Vector2D(0, -1),
								bar.bottomRight() + Vector2D(0, -1));
					// next
					prevActive = active;
...
bool BarGraph2D::findItem(const RealPoint &pos, const RealRect &screen_rect,
						  bool tight, vector<int> &out) const {
	if (!data || data->axes.size() <= max(axis1, axis2))
		return false;
	if (pos.y > max(screen_rect.top(), screen_rect.bottom()))
	...
	int count = (int)axis1.groups.size();
	int col =
		find_bar_graph_column(screen_rect.width, pos.x - screen_rect.x, count);
	if (col < 0)
		return false;
	// row containing pos...
	int max_value = (int)axis1.max;
	int value =
		(int)((screen_rect.bottom() - pos.y) / screen_rect.height * max_value);
...
void PieGraph::draw(RotatedDC &dc, int current, DrawLayer layer) const {
	...
	// RealPoint pos = align_in_rect(ALIGN_MIDDLE_CENTER, RealSize(size,size),
	// screen_rect);
	// draw items
	if (layer == LAYER_VALUES) {
		...
		for (auto const &g : axis.groups) {
			...
			if (g.size > 0) {
				...
				dc.DrawEllipticArc(pie_pos, active ? pie_size_large : pie_size,
								   end_angle, angle);
				angle = end_angle;
			}
			++i;
		...
		if (axis.groups.size() > 1) {
			...
			for (auto const &g : axis.groups) {
				if (true) {
					int i2 = (i - 1 + (int)axis.groups.size()) %
							 (int)axis.groups.size();
					bool active = i == current || i2 == current;
					dc.SetPen(
						active ? fg : lerp(fg, lerp(g.color,
													axis.groups[i2].color, 0.5),
										   0.5));
...
int PieGraph::findItem(const RealPoint &pos, const RealRect &screen_rect,
					   bool tight) const {
	if (!data)
		return -1;
	// Rectangle for the pie
	...
	double pos_angle =
		atan2(-delta.y, delta.x) - M_PI / 2; // in range [-pi..pi]
	if (pos_angle < 0)
		pos_angle += 2 * M_PI;
	// find angle
...
void ScatterGraph::draw(RotatedDC &dc, const vector<int> &current,
						DrawLayer layer) const {
	if (!data || data->axes.size() <= max(axis1, axis2))
		return;
	...
	int cur1 = this->axis1 < current.size() ? current[this->axis1] : -1;
	int cur2 = this->axis2 < current.size() ? current[this->axis2] : -1;
	RealSize size(screen_rect.width / axis1.groups.size(),
				  screen_rect.height /
					  axis2.groups.size()); // size for a single cell
	// size increments:
	double step =
		min(size.width / max_value_x, size.height / max_value_y) * 0.99;
	// Draw
	if (layer == LAYER_SELECTION) {
		dc.SetPen(*wxTRANSPARENT_PEN);
		Color bg = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW);
		if (cur1 >= 0 && cur2 >= 0) {
			...
			if (value) {
				dc.SetBrush(lerp(bg, lerp(axis1.groups[cur1].color,
										  axis2.groups[cur2].color, 0.5),
								 0.5));
				dc.DrawCircle(
					RealPoint(screen_rect.left() + cur1 * size.width,
							  screen_rect.bottom() - (cur2 + 1) * size.height) +
						size * 0.5,
					scale(value) * step + 5);
			}
		} else if (cur1 >= 0) {
			dc.SetBrush(lerp(bg, axis1.groups[cur1].color, 0.3));
			dc.DrawRectangle(RealRect(screen_rect.x + cur1 * size.width,
									  screen_rect.y, size.width,
									  screen_rect.height));
		} else if (cur2 >= 0) {
			dc.SetBrush(lerp(bg, axis2.groups[cur2].color, 0.3));
			dc.DrawRectangle(RealRect(
				screen_rect.x, screen_rect.bottom() - (cur2 + 1) * size.height,
				screen_rect.width, size.height));
		}
	} else if (layer == LAYER_VALUES) {
		Color fg = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);
		dc.SetPen(fg);
		size_t i = 0;
		int x = 0;
		for (auto const &g1 : axis1.groups) {
			int y = 0;
			for (auto const &g2 : axis2.groups) {
				UInt value = values[i++];
				if (value > 0) {
					Color color = lerp(
						fg,
						lerp(g1.color, g2.color,
							 0.5 - (axis1.auto_color == AUTO_COLOR_NO ? 0.35
																	  : 0.0) +
								 (axis2.auto_color == AUTO_COLOR_NO ? 0.35
																	: 0.0)),
						0.5 + (axis1.auto_color == AUTO_COLOR_NO ||
									   axis2.auto_color == AUTO_COLOR_NO
								   ? 0.5
								   : 0.0));
					bool active = !(cur1 == -1 && cur2 == -1) &&
								  (x == cur1 || cur1 == -1) &&
					...
					dc.SetBrush(color);
					double radius =
						floor(scale(value) * step - 0.5) * 2 + 1; // always odd
					double xx =
						screen_rect.left() + (x + 0.5) * size.width + 0.5;
					double yy =
						screen_rect.bottom() - (y + 0.5) * size.height + 0.5;
...
double ScatterGraph::scale(double x) { return pow(x, 0.75); }

bool ScatterGraph::findItem(const RealPoint &pos, const RealRect &screen_rect,
							bool tight, vector<int> &out) const {
	if (!data || data->axes.size() <= max(axis1, axis2))
		return false;
	// clicked item
	...
	int col = (int)floor((pos.x - screen_rect.x) / screen_rect.width *
						 axis1.groups.size());
	int row = (int)floor((screen_rect.bottom() - pos.y) / screen_rect.height *
						 axis2.groups.size());
	if (col < 0 || col >= (int)axis1.groups.size())
		return false;
	if (row < 0 || row >= (int)axis2.groups.size())
		return false;
	// any values here?
	if (tight && values[row + col * axis2.groups.size()] == 0)
		return false;
	// done
	out.clear();
	out.insert(out.begin(), data->axes.size(), -1);
...
void ScatterGraph::setData(const GraphDataP &d) {
	...
	double allow_overlap_x =
		axis1_data().numeric
			? 0.9
			: 1; // it is okay if we get a bit of a blob on a numeric axis
	max_value_x = 0;
	for (size_t y = 0; y < n2; ++y) {
		max_value_x = max(max_value_x, 2 * scale(values[y])); // left border
		for (size_t x = 0; x + 1 < n1; ++x) {
			double sum_dist =
				scale(values[x * n2 + y]) + scale(values[(x + 1) * n2 + y]);
			max_value_x =
				max(max_value_x, allow_overlap_x * sum_dist); // between two
		}
		max_value_x = max(max_value_x,
						  2 * scale(values[(n1 - 1) * n2 + y])); // right border
	}
	// find maximum (y)
	...
	for (size_t x = 0; x < n1; ++x) {
		...
		for (size_t y = 0; y + 1 < n2; ++y) {
			double sum_dist =
				scale(values[x * n2 + y]) + scale(values[x * n2 + (y + 1)]);
			max_value_y =
				max(max_value_y, allow_overlap_y * sum_dist); // between two
		}
		max_value_y = max(
			max_value_y, 2 * scale(values[x * n2 + (n2 - 1)])); // bottom border
	}
}
...
void ScatterPieGraph::draw(RotatedDC &dc, const vector<int> &current,
						   DrawLayer layer) const {
	if (!data)
		return;
	if (data->axes.size() <= max(max(axis1, axis2), axis3))
		return;
	if (layer == LAYER_SELECTION) {
...
void GraphStats::setData(const GraphDataP &d) {
	...
	if (!axis.numeric)
		return;
	if (axis.groups.empty())
		return;
	values.push_back(
		make_pair(_("max"), String::Format(_("%.2f"), axis.max_value)));
	values.push_back(
		make_pair(_("mean"), String::Format(_("%.2f"), axis.mean_value)));
}
...
RealSize GraphStats::determineSize(RotatedDC &dc) const {
	...
	for (auto const &v : values) {
		...
		this_item_size =
			RealSize(this_item_size.width + 6, this_item_size.height + 5);
		item_size = piecewise_max(item_size, this_item_size);
		label_width = max(label_width, this_label_width);
	}
	item_size.width += label_width;
	size = RealSize(item_size.width + 2,
					values.size() * item_size.height + 3); // margins
	return size;
}
...
RealSize GraphLegend::determineSize(RotatedDC &dc) const {
	...
	for (auto &g : axis.groups) {
		...
		this_item_size =
			RealSize(this_item_size.width + 34, this_item_size.height + 5);
		item_size = piecewise_max(item_size, this_item_size);
	}
	size = RealSize(item_size.width + 2,
					axis.groups.size() * item_size.height + 3); // margins
	return size;
}
	...
	if (layer == LAYER_VALUES) {
		...
		for (int j = 0; j < (int)axis.groups.size(); ++j) {
			...
			if (i == current) {
				...
				dc.SetPen(*wxTRANSPARENT_PEN);
				dc.DrawRectangle(RealRect(pos.x + 1, y, item_size.width,
										  item_size.height + 1));
				dc.SetPen(fg);
			}
			dc.SetBrush(g.color);
			dc.SetPen(i == current ? fg : lerp(fg, g.color, 0.5));
			dc.DrawRectangle(
				RealRect(pos.x + 3, y + 2, 26, item_size.height - 3));
			dc.DrawText(g.name, RealPoint(pos.x + 32, y + 2));
			y += item_size.height;
		}
	}
}
int GraphLegend::findItem(const RealPoint &pos, const RealRect &screen_rect,
						  bool tight) const {
	if (tight)
		return -1;
	RealPoint mypos = align_in_rect(alignment, size, screen_rect);
	RealPoint pos2(pos.x - mypos.x, pos.y - mypos.y);
	if (pos2.x < 0 || pos2.y < 0 || pos2.x >= size.width ||
		pos2.y >= size.height)
		return -1;
	int col = (int)floor((pos2.y - 1) / item_size.height);
	if (col < 0 || col >= (int)axis_data().groups.size())
		return -1;
...
void GraphLabelAxis::draw(RotatedDC &dc, int current, DrawLayer layer) const {
	...
	if (layer == LAYER_SELECTION && current >= 0) {
		...
		if (direction == HORIZONTAL) {
			...
			dc.DrawRectangle(RealRect(screen_rect.x + current * width,
									  screen_rect.bottom(), width,
									  text_size.height + 5));
		} else {
			double height = screen_rect.height / count;
			dc.SetBrush(lerp(bg, group.color, 0.5));
			dc.SetPen(*wxTRANSPARENT_PEN);
			dc.DrawRectangle(RealRect(screen_rect.x, screen_rect.bottom() -
														 (current + 1) * height,
									  -78, height));
		}
		...
		if (direction == HORIZONTAL) {
			...
			for (auto const &g : axis.groups) {
				...
				dc.SetClippingRegion(RealRect(x + 2, screen_rect.bottom() + 3,
											  width - 4, text_size.height));
				dc.DrawText(g.name,
							align_in_rect(ALIGN_TOP_CENTER, text_size,
										  RealRect(x, screen_rect.bottom() + 3,
												   width, 0)));
			...
			if (draw_lines) {
				for (int i = 0; i < count; ++i) {
					...
					if (draw_lines == DRAW_LINES_BETWEEN) {
						dc.DrawLine(RealPoint(screen_rect.x + (i + 1.0) * width,
											  screen_rect.top()),
									RealPoint(screen_rect.x + (i + 1.0) * width,
											  screen_rect.bottom()));
					} else {
						dc.DrawLine(RealPoint(screen_rect.x + (i + 0.5) * width,
											  screen_rect.top()),
									RealPoint(screen_rect.x + (i + 0.5) * width,
			...
			for (auto const &g : axis.groups) {
				...
				RealSize text_size = dc.GetTextExtent(g.name);
				// dc.SetClippingRegion(RealRect(x + 2, screen_rect.bottom() +
				// 3, width - 4, text_size.height));
				dc.DrawText(g.name, align_in_rect(ALIGN_MIDDLE_RIGHT, text_size,
												  RealRect(-4, y, 0, -height)));
				// dc.DestroyClippingRegion();
				y -= height;
			}
			// Draw lines
			if (draw_lines) {
				for (int i = 0; i < count; ++i) {
					dc.SetPen(i == current ? fg : lerp(bg, fg, 0.2));
					if (draw_lines == DRAW_LINES_BETWEEN) {
						dc.DrawLine(RealPoint(screen_rect.left(),
											  screen_rect.bottom() -
												  (i + 1.0) * height),
									RealPoint(screen_rect.right(),
											  screen_rect.bottom() -
												  (i + 1.0) * height));
					} else {
						dc.DrawLine(RealPoint(screen_rect.left() - 2,
											  screen_rect.bottom() -
												  (i + 0.5) * height),
									RealPoint(screen_rect.right(),
											  screen_rect.bottom() -
...
int GraphLabelAxis::findItem(const RealPoint &pos, const RealRect &screen_rect,
							 bool tight) const {
	if (!data)
		return -1;
	GraphAxis &axis = axis_data();
	int col;
	if (direction == HORIZONTAL) {
		col = (int)floor((pos.x - screen_rect.x) / screen_rect.width *
						 axis.groups.size());
		if (pos.y < screen_rect.bottom())
			return -1;
	} else {
		col = (int)floor((screen_rect.bottom() - pos.y) / screen_rect.height *
						 axis.groups.size());
		if (pos.x > screen_rect.left())
			return -1;
	}
	if (col < 0 || col >= (int)axis.groups.size())
		return -1;
	return col;
}
...
void GraphValueAxis::draw(RotatedDC &dc, int current, DrawLayer layer) const {
	...
	GraphAxis &axis = axis_data();
	double step_height =
		screen_rect.height / axis.max; // height of a single value
	dc.SetFont(*wxNORMAL_FONT);
	int label_step = (int)ceil(max(
		1.0, (dc.GetCharHeight()) / step_height)); // values per labeled line
	// Colors
	Color bg = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW);
	Color fg = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);
	...
	int highlight =
		(highlight_value && current >= 0) ? (int)axis.groups[current].size : -1;
	for (int i = 0; i <= (int)axis.max; ++i) {
		if (i % label_step == 0 || i == highlight) {
			...
			dc.DrawLine(RealPoint(screen_rect.left() - 2, y),
						RealPoint(screen_rect.right(), y));
			// draw label, aligned middle right
			if (!((i < highlight && i + label_step > highlight) ||
				  (i > highlight && i - label_step < highlight)) ||
				highlight == -1) {
				...
				dc.DrawText(
					label, align_in_rect(ALIGN_MIDDLE_RIGHT, text_size,
										 RealRect(screen_rect.x - 4, y, 0, 0)));
			}
			// restore font/pen
			if (i == highlight) {
				dc.SetFont(*wxNORMAL_FONT);
	...
	// Draw axis
	dc.SetPen(fg);
	dc.DrawLine(screen_rect.bottomLeft() - RealSize(2, 0),
...
void GraphWithMargins::draw(RotatedDC &dc, const vector<int> &current,
							DrawLayer layer) const {
	RealRect inner = dc.getInternalRect().move(margin_left, margin_top,
...
bool GraphWithMargins::findItem(const RealPoint &pos,
								const RealRect &screen_rect, bool tight,
								vector<int> &out) const {
	RealRect inner =
		screen_rect.move(margin_left, margin_top, -margin_left - margin_right,
						 -margin_top - margin_bottom);
	if (upside_down) {
		inner.y += inner.height;
		inner.height = -inner.height;
...
void GraphContainer::draw(RotatedDC &dc, const vector<int> &current,
						  DrawLayer layer) const {
	for (auto const &g : items) {
...
void GraphContainer::add(const GraphP &graph) { items.push_back(graph); }
...
void GraphControl::setLayout(GraphType type, bool force) {
	...
	case GRAPH_TYPE_PIE: {
		intrusive_ptr<GraphContainer> combined(new GraphContainer());
		combined->add(intrusive(
			new GraphWithMargins(intrusive(new PieGraph(0)), 0, 0, 120, 0)));
	...
	case GRAPH_TYPE_SCATTER: {
		intrusive_ptr<GraphContainer> combined(new GraphContainer());
		combined->add(intrusive(
			new GraphLabelAxis(0, HORIZONTAL, false, DRAW_LINES_MID)));
		combined->add(
			intrusive(new GraphLabelAxis(1, VERTICAL, false, DRAW_LINES_MID)));
...
GraphType GraphControl::getLayout() const { return layout; }
...
size_t GraphControl::getDimensionality() const {
	if (graph)
		return graph->getData()->axes.size();
	else
		return 0;
}
...
void GraphControl::onSize(wxSizeEvent &) { Refresh(false); }

void GraphControl::onMouseDown(wxMouseEvent &ev) {
	if (!graph)
		return;
	wxSize cs = GetClientSize();
	if (graph->findItem(RealPoint(ev.GetX(), ev.GetY()),
						RealRect(RealPoint(0, 0), cs), false, current_item)) {
		onSelectionChange();
	}
	ev.Skip(); // focus
}
void GraphControl::onChar(wxKeyEvent &ev) {
	...
	case WXK_LEFT:
		if (current_item.size() >= 1 && data->axes.size() >= 1 &&
			current_item[0] != -1) {
			current_item[0]--;
			onSelectionChange();
		}
	...
	case WXK_RIGHT:
		if (current_item.size() >= 1 && data->axes.size() >= 1 &&
			current_item[0] + 1 < (int)data->axes[0]->groups.size()) {
			current_item[0]++;
			onSelectionChange();
		}
		break;
	case WXK_UP:
		if (current_item.size() >= 2 && data->axes.size() >= 2 &&
			current_item[1] + 1 < (int)data->axes[1]->groups.size()) {
			current_item[1]++;
			onSelectionChange();
		}
		break;
...
String GraphControl::getSelection(size_t axis) const {
	if (!graph || axis >= current_item.size() ||
		axis >= graph->getData()->axes.size())
		return wxEmptyString;
	GraphAxis &a = *graph->getData()->axes[axis];
	int i = current_item[axis];
	if (i == -1 || (size_t)i >= a.groups.size())
		return wxEmptyString;
	return a.groups[current_item[axis]].name;
}
vector<int> GraphControl::getSelectionIndices() const { return current_item; }
...
void GraphControl::onMotion(wxMouseEvent &ev) {
	...
	if (graph->findItem(RealPoint(ev.GetX(), ev.GetY()),
						RealRect(RealPoint(0, 0), cs), true, hovered_item)) {
		// determine tooltip
		const GraphData &data = *graph->getData();
		String tip;
		for (size_t dim = 0; dim < hovered_item.size(); ++dim) {
			if (hovered_item[dim] != -1 &&
				(size_t)hovered_item[dim] < data.axes[dim]->groups.size()) {
				if (!tip.empty())
					tip += _("\n");
				tip += data.axes[dim]->name + _(": ") +
					   data.axes[dim]->groups[hovered_item[dim]].name;
			}
		}
		UInt count = data.count(hovered_item);
		tip += String::Format(_("\n%d "), count) +
			   (count == 1 ? _TYPE_("card") : _TYPE_("cards"));
		tip.Replace(_(" "), _("\xA0"));
		// set tooltip
		SetToolTip(tip);
	} else {
		// Note: don't use SetToolTip directly, we don't want to create a tip if
		// it doesn't exist
