...
String to_bin(double value, double bin_size) {
    if (bin_size <= 0 || value == 0) {
        ...
        return String::Format((L"%.0f%c%.0f"), (bin - 1) * bin_size + 1,
...
void GraphStats::setData(const GraphDataP &d) {
    ...
    if (axis.groups.empty())
        return;
    values.push_back(
        make_pair((L"max"), String::Format((L"%.2f"), axis.max_value)));
    values.push_back(
        make_pair((L"mean"), String::Format((L"%.2f"), axis.mean_value)));
}
