...
CardP CardSelectEvent::getCard() const { return getTheCardList()->getCard(); }
...
void CardListBase::onBeforeChangeSet() { storeColumns(); }
void CardListBase::onChangeSet() { rebuild(); }
...
void CardListBase::onAction(const Action &action, bool undone) {
    TYPE_CASE(action, AddCardAction) {
        ...
        if (action.action.adding != undone) {
            ...
            refreshList();
            for (const auto &s : action.action.steps)
            ...
            refreshList();
            if (!allowModify()) {
                // Let some other card list do the selecting, otherwise we get
                // conflicting events
                return;
            }
    ...
    TYPE_CASE(action, ReorderCardsAction) {
        if (sort_by_column >= 0)
            return; // nothing changes for us
        if ((long)action.card_id1 < 0 ||
            (long)action.card_id2 >= (long)sorted_list.size())
            return;
        if ((long)action.card_id1 == selected_item_pos ||
            (long)action.card_id2 == selected_item_pos) {
            ...
            selected_item_pos = (long)action.card_id1 == selected_item_pos
                                    ? (long)action.card_id2
                                    : (long)action.card_id1;
    ...
    TYPE_CASE_(action, ScriptValueEvent) {
        // No refresh needed, a ScriptValueEvent is only generated in response
        // to a ValueAction
        return;
    }
...
bool CardListBase::canPaste() const {
    return allowModify() &&
           wxTheClipboard->IsSupported(CardsDataObject::format);
}
...
bool CardListBase::doCopy() {
    ...
    if (cards_to_copy.empty())
        return false;
    // put on clipboard
    if (!wxTheClipboard->Open())
        return false;
    bool ok = wxTheClipboard->SetData(
        new CardsOnClipboard(set, cards_to_copy)); // ignore result
...
void CardListBase::rebuild() {
    ...
    for (auto &f : new_column_fields) {
        ...
        else if (f.second->card_list_align & ALIGN_CENTER)
            align = wxLIST_FORMAT_CENTRE;
        else
            align = wxLIST_FORMAT_LEFT;
        InsertColumn((long)column_fields.size(),
                     tr(*set->game, f.second->card_list_name, identity), align,
                     cs.width);
...
void CardListBase::storeColumns() {
    ...
    GameSettings &gs = settings.gameSettingsFor(*set->game);
    if (sort_by_column >= 0)
        gs.sort_cards_by = column_fields.at(sort_by_column)->name;
    else
        gs.sort_cards_by = wxEmptyString;
...
String CardListBase::OnGetItemText(long pos, long col) const {
    ...
    if (val)
        return val->toFriendlyString();
    else
        return wxEmptyString;
}

int CardListBase::OnGetItemImage(long pos) const { return -1; }

wxListItemAttr *CardListBase::OnGetItemAttr(long pos) const {
    if (!set->game->card_list_color_script)
        return nullptr;
    Context &ctx = set->getContext(getCard(pos));
    item_attr.SetTextColour(
        set->game->card_list_color_script.invoke(ctx)->toColor());
    return &item_attr;
}
...
void CardListBase::onColumnRightClick(wxListEvent &) {
    // show menu
    wxMenu m;
    m.Append(ID_SELECT_COLUMNS, _MENU_("card list columns"),
...
void CardListBase::onSelectColumns(wxCommandEvent &) { selectColumns(); }
        ...
        if (flags & wxLIST_HITTEST_ONITEM) {
            ...
            if (item != selected_item_pos) {
                ...
                set->actions.addAction(
                    new ReorderCardsAction(*set, item, selected_item_pos));
            }
            ev.Skip(false);
        }
    }
...
void CardListBase::onContextMenu(wxContextMenuEvent &) {
    if (allowModify()) {
        ...
        m.Append(wxID_COPY, (L"copy"), _CONTEXT_MENU_("copy"),
                 _HELP_("copy card"));
        m.Append(wxID_PASTE, (L"paste"), _CONTEXT_MENU_("paste"),
                 _HELP_("paste card"));
        m.AppendSeparator();
        m.Append(ID_CARD_ADD, (L"card_add"), _CONTEXT_MENU_("add card"),
                 _HELP_("add card"));
        m.Append(ID_CARD_REMOVE, (L"card_del"), _CONTEXT_MENU_("remove card"),
                 _HELP_("remove card"));
        PopupMenu(&m);
    }
}
