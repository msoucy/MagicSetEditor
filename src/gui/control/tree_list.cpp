...
bool TreeList::hasChildren(size_t item) const {
	return item + 1 < items.size() &&
		   items[item]->level < items[item + 1]->level;
}
...
void TreeList::select(size_t item, bool event) {
	if (item >= items.size() || selection == item)
		return;
	// select
...
void TreeList::calcItemCount() {
	...
	for (auto i : items) {
		if (i->level <= visible_level) {
			...
			++total_lines;
			if (i->expanded)
				visible_level = i->level + 1;
			else
				visible_level = i->level;
		} else {
			i->position = NOTHING;
		}
	}
...
size_t TreeList::findItem(size_t line, size_t start) const {
	for (size_t i = start; i < items.size(); ++i) {
		if (items[i]->visible() && items[i]->position >= line)
			return i;
	}
	return items.size();
}
...
size_t TreeList::findParent(size_t start) const {
	...
	for (size_t i = start - 1; i + 1 > 0; --i) {
		if (items[i]->visible() && items[i]->level < level)
			return i;
	}
	return items.size();
}
...
TreeList::TreeList(Window *parent, int id)
	: wxPanel(parent, id, wxDefaultPosition, wxDefaultSize,
			  wxBORDER_THEME_FIX(wxBORDER_THEME) | wxWANTS_CHARS | wxVSCROLL),
	  selection(NOTHING), total_lines(0), first_line(0) {
	// determine item size
	wxClientDC dc(this);
...
void TreeList::onPaint(wxPaintEvent &ev) {
	...
	for (size_t i = start; i < end; ++i) {
		...
		for (int j = 0; j < 32 && j <= item.level; ++j) {
			if (item.lines & (1 << j)) {
				// draw line
				dc.DrawLine(8 + level_width * j, y, 8 + level_width * j,
							y + item_height);
			}
		}
		dc.DrawLine(x - 1, y + item_height / 2, x - 9, y + item_height / 2);
		if (!(item.lines & (1 << item.level))) {
			dc.DrawLine(8 + level_width * item.level, y,
						8 + level_width * item.level, y + item_height / 2 + 1);
		}
		// draw expand button
		if (hasChildren(i)) {
			wxRect rect(x - 13, y + (item_height - 9) / 2, 9, 9);
			rn.DrawTreeItemButton(this, dc, rect,
								  item.expanded ? wxCONTROL_EXPANDED : 0);
		}
		if (selection == i) {
			dc.SetPen(*wxTRANSPARENT_PEN);
...
void TreeList::onChar(wxKeyEvent &ev) {
	...
	case WXK_PAGEUP: {
		ScrollToLine(first_line > visible_lines_t ? first_line - visible_lines_t
												  : 0);
		break;
	}
	case WXK_PAGEDOWN: {
...
void TreeList::UpdateScrollbar() {
	...
	SetScrollbar(wxVERTICAL, (int)first_line, (int)visible_lines_t,
				 (int)total_lines);
}
...
void TreeList::onMouseWheel(wxMouseEvent &ev) {
	int delta =
		-ev.GetWheelRotation() * ev.GetLinesPerAction() / ev.GetWheelDelta();
