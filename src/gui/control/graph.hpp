...
#define EVT_GRAPH_SELECT(id, handler)                                          \
    EVT_COMMAND(id, EVENT_GRAPH_SELECT, handler)
...
enum AutoColor { AUTO_COLOR_NO, AUTO_COLOR_EVEN, AUTO_COLOR_WEIGHTED };
...
class GraphAxis : public IntrusivePtrBase<GraphAxis> {
  public:
    GraphAxis(const String &name, AutoColor auto_color = AUTO_COLOR_EVEN,
              bool numeric = false, double bin_size = 0,
              const map<String, Color> *colors = nullptr,
              const vector<String> *order = nullptr)
        : name(name), auto_color(auto_color), numeric(numeric),
          bin_size(bin_size), max(0), total(0), mean_value(0),
          max_value(-std::numeric_limits<double>::infinity()), colors(colors),
          order(order) {}

    String name; ///< Name/label of this axis
    AutoColor
        auto_color; ///< Automatically assign colors to the groups on this axis
...
class GraphData : public IntrusivePtrBase<GraphData> {
  public:
    ...
    vector<GraphDataElement *>
        values; ///< All elements, with the group number for each axis, or -1
    ...
    void crossAxis(size_t axis1, size_t axis2, size_t axis3,
...
class Graph : public IntrusivePtrVirtualBase {
  public:
    /// Determine the size of this graph viewer, return -1 if the viewer
    /// stretches
    virtual RealSize determineSize(RotatedDC &dc) const {
        return RealSize(-1, -1);
    }
    /// Draw this graph, filling the internalRect() of the dc.
    virtual void draw(RotatedDC &dc, const vector<int> &current,
                      DrawLayer layer) const = 0;
    /// Find the item at the given position, the rectangle gives the screen size
    virtual bool findItem(const RealPoint &pos, const RealRect &screen_rect,
...
class Graph1D : public Graph {
  ...
  protected:
    ...
    virtual int findItem(const RealPoint &pos, const RealRect &screen_rect,
                         bool tight) const {
        return -1;
    }
...
class ScatterGraph : public Graph2D {
  ...
  protected:
    UInt max_value;
    double max_value_x,
        max_value_y; ///< highest sum of two adjacent scaled values (radii)
...
class ScatterGraphPlus : public ScatterGraph {
  public:
    inline ScatterGraphPlus(size_t axis1, size_t axis2, size_t axis3)
        : ScatterGraph(axis1, axis2), axis3(axis3) {}
    virtual void setData(const GraphDataP &d);
...
class ScatterPieGraph : public ScatterGraphPlus {
  public:
    inline ScatterPieGraph(size_t axis1, size_t axis2, size_t axis3)
        : ScatterGraphPlus(axis1, axis2, axis3) {}
    virtual void draw(RotatedDC &dc, const vector<int> &current,
                      DrawLayer layer) const;
};
...
enum DrawLines { DRAW_LINES_NO, DRAW_LINES_BETWEEN, DRAW_LINES_MID };

/// Draws a horizontal/vertical axis for group labels
class GraphLabelAxis : public Graph1D {
  public:
    inline GraphLabelAxis(size_t axis, Direction direction, bool rotate = false,
                          DrawLines draw_lines = DRAW_LINES_NO,
                          bool label = false)
        : Graph1D(axis), direction(direction), rotate(rotate),
          draw_lines(draw_lines), label(label) {}
    virtual void draw(RotatedDC &dc, int current, DrawLayer layer) const;
    virtual int findItem(const RealPoint &pos, const RealRect &screen_rect,
                         bool tight) const;
...
class GraphValueAxis : public Graph1D {
  public:
    inline GraphValueAxis(size_t axis, bool highlight_value)
        : Graph1D(axis), highlight_value(highlight_value) {}
    virtual void draw(RotatedDC &dc, int current, DrawLayer layer) const;
...
class GraphWithMargins : public Graph {
  public:
    inline GraphWithMargins(const GraphP &graph, double margin_left,
                            double margin_top, double margin_right,
                            double margin_bottom, bool upside_down = false)
        : graph(graph), margin_left(margin_left), margin_top(margin_top),
  ...
  private:
    ...
    bool upside_down; // put the coordinate system upside down, since graphs are
...
class GraphControl : public wxControl {
  ...
  private:
    ...
    /** If the value for an axis is -1, then all groups on that axis are
     * selected */
