...
KeywordList::~KeywordList() { storeColumns(); }
void KeywordList::storeColumns() {
    // TODO
}

void KeywordList::onBeforeChangeSet() { storeColumns(); }
...
void KeywordList::onAction(const Action &action, bool undone) {
    TYPE_CASE(action, AddKeywordAction) {
        if (action.action.adding != undone) {
            ...
            selectItem(action.action.steps[0].item,
                       false /*list will be refreshed anyway*/, true);
            refreshList();
        } else {
    ...
    TYPE_CASE_(action, ChangeKeywordModeAction) { refreshList(); }
}
...
bool KeywordList::doCopy() {
    ...
    bool ok = wxTheClipboard->SetData(
        new KeywordDataObject(set, getKeyword())); // ignore result
...
String match_string(const Keyword &a) {
    return untag(
        replace_all(replace_all(a.match, (L"<atom-param>"), LEFT_ANGLE_BRACKET),
...
bool KeywordList::compareItems(void *a, void *b) const {
    ...
    case 1:
        return ka.match < kb.match;
    case 2:
        return ka.mode < kb.mode;
    case 3:
        return usage(ka) < usage(kb);
    case 4:
        return ka.reminder.getUnparsed() < kb.reminder.getUnparsed();
    default: // TODO: 3
        return ka.keyword < kb.keyword;
    }
}
...
String KeywordList::OnGetItemText(long pos, long col) const {
    ...
    case 0:
        return kw.keyword;
    case 1:
        return match_string(kw);
    case 2:
        return kw.mode;
    case 3:
        return String::Format((L"%d"), usage(kw));
    case 4: {
...
void KeywordList::onContextMenu(wxContextMenuEvent &) {
    IconMenu m;
    m.Append(ID_EDIT_CUT, (L"cut"), _CONTEXT_MENU_("cut"),
             _HELP_("cut keyword"));
    m.Append(ID_EDIT_COPY, (L"copy"), _CONTEXT_MENU_("copy"),
             _HELP_("copy keyword"));
    m.Append(ID_EDIT_PASTE, (L"paste"), _CONTEXT_MENU_("paste"),
             _HELP_("paste keyword"));
    m.AppendSeparator();
    m.Append(ID_KEYWORD_ADD, (L"keyword_add"), _CONTEXT_MENU_("add keyword"),
             _HELP_("add keyword"));
    m.Append(ID_KEYWORD_REMOVE, (L"keyword_del"),
             _CONTEXT_MENU_("remove keyword"), _HELP_("remove keyword"));
    PopupMenu(&m);
}
