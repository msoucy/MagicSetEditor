...
DataEditor::DataEditor(Window *parent, int id, long style)
    : CardViewer(parent, id, style | wxWANTS_CHARS), next_in_tab_order(nullptr),
      current_viewer(nullptr), current_editor(nullptr),
...
void DataEditor::addAction(Action *action) { set->actions.addAction(action); }
...
bool DataEditor::AcceptsFocus() const { return wxWindow::AcceptsFocus(); }
...
struct CompareTabOrder {
    bool operator()(ValueViewer *a, ValueViewer *b) {
        ...
        if (vertical_overlap > 0 && vertical_overlap > horizontal_overlap) {
            ...
            if (as.left < bs.left)
                return true; // horizontal sorting
            if (as.left > bs.left)
                return false;
            if (as.top < bs.top)
                return true; // vertical sorting
        } else {
...
bool DataEditor::selectFirst() {
    // This would be nicer with boost::range, but filtered adaptor was only
    // introduced in boost 1.42(?)
    return selectWithTab(
        std::min_element(boost::make_filter_iterator(
                             is_enabled, viewers.begin(), viewers.end()),
...
bool DataEditor::selectLast() {
    return selectWithTab(
        std::max_element(boost::make_filter_iterator(
                             is_enabled, viewers.begin(), viewers.end()),
                         boost::make_filter_iterator(is_enabled, viewers.end(),
                                                     viewers.end()),
                         CompareTabOrder())
...
bool DataEditor::selectNext() {
    if (!current_viewer)
        return selectFirst();
    return selectWithTab(
        next_element(boost::make_filter_iterator(is_enabled, viewers.begin(),
                                                 viewers.end()),
                     boost::make_filter_iterator(is_enabled, viewers.end(),
                                                 viewers.end()),
                     current_viewer, CompareTabOrder())
            .base());
}
bool DataEditor::selectPrevious() {
    if (!current_viewer)
        return selectLast();
    return selectWithTab(
        prev_element(boost::make_filter_iterator(is_enabled, viewers.begin(),
                                                 viewers.end()),
                     boost::make_filter_iterator(is_enabled, viewers.end(),
                                                 viewers.end()),
                     current_viewer, CompareTabOrder())
...
bool DataEditor::search(FindInfo &find, bool from_start) {
    ...
    for (size_t i = 0; i < by_tab_index.size(); ++i) {
        ValueViewer &viewer =
            *by_tab_index[find.forward() ? i : by_tab_index.size() - i - 1];
        if (&viewer == current_viewer)
            include = true;
        if (include && viewer.getField()->editable &&
            viewer.getStyle()->isVisible()) {
            ValueEditor *editor = viewer.getEditor();
            if (editor &&
                editor->search(find, from_start || &viewer != current_viewer)) {
                return true; // done
            }
        }
    }
...
bool DataEditor::canCut() const {
    return current_editor && current_editor->canCut();
}
bool DataEditor::canCopy() const {
    return current_editor && current_editor->canCopy();
}
bool DataEditor::canPaste() const {
    return current_editor && current_editor->canPaste();
}
bool DataEditor::canFormat(int type) const {
    return current_editor && current_editor->canFormat(type);
}
bool DataEditor::hasFormat(int type) const {
    return current_editor && current_editor->hasFormat(type);
}

void DataEditor::doCut() {
    if (current_editor)
        current_editor->doCut();
}
void DataEditor::doCopy() {
    if (current_editor)
        current_editor->doCopy();
}
void DataEditor::doPaste() {
    if (current_editor)
        current_editor->doPaste();
}
void DataEditor::doFormat(int type) {
    if (current_editor)
        current_editor->doFormat(type);
}
...
void DataEditor::insert(const String &text, const String &action_name) {
    if (current_editor)
        current_editor->insert(text, action_name);
}
...
void DataEditor::onMouseWheel(wxMouseEvent &ev) {
    if (current_editor && current_viewer) {
        RealPoint pos = mousePoint(ev, *current_viewer);
        if (current_viewer->containsPoint(pos)) {
            if (current_editor->onMouseWheel(pos, ev))
                return;
        }
    }
    ev.Skip();
...
void DataEditor::onMotion(wxMouseEvent &ev) {
    ...
    if (!HasCapture()) {
        ...
        if (hovered_viewer && hovered_viewer != new_hovered_viewer) {
            ...
            if (e)
                e->onMouseLeave(pos, ev);
            if (hovered_viewer)
                redraw(*hovered_viewer);
            if (new_hovered_viewer)
                redraw(*new_hovered_viewer);
        }
        ...
        if (hovered_viewer) {
            ...
            if (frame)
                frame->SetStatusText(hovered_viewer->getField()->description);
        } else {
...
void DataEditor::onMouseLeave(wxMouseEvent &ev) {
    ...
    if (hovered_viewer) {
        ValueEditor *e = hovered_viewer->getEditor();
        if (e)
            e->onMouseLeave(mousePoint(ev, *hovered_viewer), ev);
        if (hovered_viewer)
            redraw(*hovered_viewer);
        hovered_viewer = nullptr;
    }
...
void DataEditor::selectField(wxMouseEvent &ev,
                             bool (ValueEditor::*event)(const RealPoint &,
                                                        wxMouseEvent &)) {
    ...
    if (old_editor != current_editor) {
        ...
        // NOTE: after passing mouse down event, otherwise opening combo box
        // produces flicker
        onChange();
    }
}
    ...
    for (auto &v : boost::adaptors::reverse(viewers)) {
        if (ValueEditor *e = v->getEditor()) {
            ...
            if (v->getField()->editable &&
                (v->containsPoint(mousePoint(ev, *v)) ||
                 (nativeLook() &&
                  (y = ev.GetY() + GetScrollPos(wxVERTICAL)) >=
                      v->getStyle()->top &&
                  y < v->getStyle()->bottom))) {
...
void DataEditor::onContextMenu(wxContextMenuEvent &ev) {
    if (current_editor) {
        IconMenu m;
        m.Append(ID_EDIT_CUT, (L"cut"), _MENU_("cut"), _HELP_("cut"));
        m.Append(ID_EDIT_COPY, (L"copy"), _MENU_("copy"), _HELP_("copy"));
        m.Append(ID_EDIT_PASTE, (L"paste"), _MENU_("paste"), _HELP_("paste"));
