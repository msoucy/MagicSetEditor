...
wxSize CardViewer::DoGetBestSize() const {
	...
	if (set) {
		...
		if (is_sideways(deg_to_rad(ss.card_angle())))
			swap(size.x, size.y);
		return size + ws - cs;
	}
	return cs;
...
void CardViewer::redraw(const ValueViewer &v) {
	...
	// drawing another viewer causes styles to be updated for its active card,
...
void CardViewer::onChange() { redraw(); }
...
void CardViewer::onPaint(wxPaintEvent &) {
	...
	if (!buffer.Ok() || buffer.GetWidth() != cs.GetWidth() ||
		buffer.GetHeight() != cs.GetHeight()) {
...
bool CardViewer::shouldDraw(const ValueViewer &v) const {
	...
	//	wxRegion clip = GetUpdateRegion();
	//	clip.Offset(dx, dy);
	return GetUpdateRegion().Contains(
			   getRotation().trRectToBB(v.boundingBox().toRect()).toRect()) !=
		   wxOutRegion;
}
...
class CardViewer::OverdrawDC_aux : private wxClientDC {
  protected:
	...
	OverdrawDC_aux(CardViewer *window) : wxClientDC(window) {
...
Rotation CardViewer::getRotation() const {
	...
	StyleSheetSettings &ss = settings.stylesheetSettingsFor(*stylesheet);
	int dx = GetScrollPos(wxHORIZONTAL), dy = GetScrollPos(wxVERTICAL);
	return Rotation(deg_to_rad(ss.card_angle()),
					stylesheet->getCardRect().move(-dx, -dy, 0, 0),
