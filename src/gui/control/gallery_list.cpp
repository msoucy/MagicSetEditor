...
GalleryList::GalleryList(Window *parent, int id, int direction,
	...
	: wxPanel(parent, id, wxDefaultPosition, wxDefaultSize,
			  wxBORDER_THEME_FIX(wxBORDER_THEME) | wxWANTS_CHARS |
				  (direction == wxHORIZONTAL ? wxHSCROLL : wxVSCROLL)),
	  active_subcolumn(0), direction(direction), column_count(1),
	  always_focused(always_focused), visible_start(0) {
	SubColumn col;
	col.can_select = true;
...
void GalleryList::selectSubColumn(size_t subcol) {
	if (subcol >= subcolumns.size())
		return;
	if (!subcolumns[subcol].can_select)
		return;
	if (active_subcolumn == subcol)
		return;
...
wxPoint GalleryList::itemPos(size_t item) const {
	if (direction == wxHORIZONTAL) {
		...
		return wxPoint(x + MARGIN + BORDER - visible_start,
					   y + MARGIN + BORDER);
	} else {
		int x = (int)(item % column_count) * (item_size.x + SPACING);
		int y = (int)(item / column_count) * (item_size.y + SPACING);
		return wxPoint(x + MARGIN + BORDER,
					   y + MARGIN + BORDER - visible_start);
	}
}
...
void GalleryList::RefreshItem(size_t item) {
	if (item >= itemCount())
		return;
	RefreshRect(wxRect(itemPos(item), item_size).Inflate(BORDER, BORDER),
				false);
}
...
void GalleryList::onMouseWheel(wxMouseEvent &ev) {
	scrollTo(visible_start -
			 (mainSize(item_size) + SPACING) * ev.GetWheelRotation() /
				 ev.GetWheelDelta());
}
...
void GalleryList::onLeftDown(wxMouseEvent &ev) {
	...
	if (item < itemCount()) {
		...
		for (size_t j = 0; j < subcolumns.size(); ++j) {
			...
			if (x >= col.offset.x && y >= col.offset.y &&
				x < col.size.x + col.offset.x &&
				y < col.size.y + col.offset.y) {
...
void GalleryList::onChar(wxKeyEvent &ev) {
	...
	case WXK_RETURN: {
		...
		wxButton *btn =
			tlw ? wxDynamicCast(tlw->GetDefaultItem(), wxButton) : nullptr;
...
void GalleryList::OnDraw(DC &dc) {
	...
	for (size_t i = start; i < end; ++i) {
		...
		for (size_t j = 0; j < subcolumns.size(); ++j) {
			...
			wxRect rect(pos.x + col.offset.x - BORDER,
						pos.y + col.offset.y - BORDER, col.size.x + 2 * BORDER,
						col.size.y + 2 * BORDER);
#if 0
				if (selected) {
...
#else
			Color c = selected ? (focused ? wxSystemSettings::GetColour(
												wxSYS_COLOUR_HIGHLIGHT)
										  : lerp(wxSystemSettings::GetColour(
													 wxSYS_COLOUR_WINDOW),
