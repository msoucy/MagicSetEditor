...
class DropDownMRUList : public DropDownList {
  public:
    DropDownMRUList(Window *parent, vector<String> const &choices)
        : DropDownList(parent), choices(choices) {}
...
FilterCtrl::FilterCtrl(wxWindow *parent, int id, String const &placeholder)
    : wxControl(parent, id, wxDefaultPosition, wxSize(160, 41),
                wxSTATIC_BORDER),
      changing(false), placeholder(placeholder) {
    wxColour bg = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW);
    SetBackgroundColour(bg);
    SetCursor(wxCURSOR_IBEAM);
    filter_ctrl = new TextCtrlWithFocus();
    filter_ctrl->Create(this, wxID_ANY, (L""), wxDefaultPosition,
                        wxSize(130, -1), wxNO_BORDER);
    clear_button =
        new HoverButton(this, wxID_ANY, (L"btn_clear_filter"), bg, false);
...
void FilterCtrl::onClear(wxCommandEvent &) { clearFilter(true); }

void FilterCtrl::onSizeEvent(wxSizeEvent &) { onSize(); }
void FilterCtrl::onSize() {
    wxSize s = GetClientSize();
    wxSize fs = filter_ctrl->GetBestSize();
    wxSize cs = clear_button->GetBestSize();
    int margin = 2;
    filter_ctrl->SetSize(margin, max(margin, (s.y - fs.y) / 2),
                         s.x - cs.x - 3 * margin, fs.y);
...
void FilterCtrl::onKillFocus(wxFocusEvent &) { update(); }
