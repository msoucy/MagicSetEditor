...
#define EVT_GALLERY_SELECT(id, handler)                                        \
	EVT_COMMAND(id, EVENT_GALLERY_SELECT, handler)
/// Handle EVENT_GALLERY_ACTIVATE events
#define EVT_GALLERY_ACTIVATE(id, handler)                                      \
	EVT_COMMAND(id, EVENT_GALLERY_ACTIVATE, handler)

// -----------------------------------------------------------------------------
// : GalleryList

/// A list of items with custom drawing
/** A derived class should implement the abstract members to determine how the
 * items look.
 */
class GalleryList : public wxPanel {
  public:
	GalleryList(Window *parent, int id, int direction = wxHORIZONTAL,
				bool always_focused = true);
  ...
  protected:
	...
	size_t column_count; ///< Number of major level columns (if vertical) or
						 ///rows (if horizontal)
  ...
  private:
	...
	inline int itemEnd(size_t item) const {
		return (int)(item / column_count + 1) *
				   (mainSize(item_size) + SPACING) +
			   MARGIN;
	}
