...
#define EVT_CARD_SELECT(id, handler)                                           \
	DECLARE_EVENT_TABLE_ENTRY(                                                 \
		EVENT_CARD_SELECT, id, -1,                                             \
		(wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction)(      \
			void (wxEvtHandler::*)(CardSelectEvent &))(&handler),              \
		(wxObject *)NULL),

/// Handle EVENT_CARD_ACTIVATE events
#define EVT_CARD_ACTIVATE(id, handler)                                         \
	DECLARE_EVENT_TABLE_ENTRY(                                                 \
		EVENT_CARD_ACTIVATE, id, -1,                                           \
		(wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction)(      \
			void (wxEvtHandler::*)(CardSelectEvent &))(&handler),              \
		(wxObject *)NULL),
...
/*  This class allows the cards to be sorted, and has a _('currentCard'), the
 *selected card
 *  when a card is selected, it raises a CardSelectEvent, that will propage to
 *the parent window.
 *
...
class CardListBase : public ItemList, public SetView {
  public:
	...
	inline CardP getCard() const {
		return static_pointer_cast<Card>(selected_item);
	}
  ...
  public:
	...
	inline CardP getCard(long pos) const {
		return static_pointer_cast<Card>(getItem(pos));
	}
  ...
  private:
	// display stuff
	vector<FieldP>
		column_fields; ///< The field to use for each column (by column index)
	FieldP alternate_sort_field; ///< Second field to sort by, if the column
								 ///doesn't suffice
