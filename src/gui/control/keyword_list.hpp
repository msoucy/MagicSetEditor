...
#define EVT_KEYWORD_SELECT(id, handler)                                        \
	DECLARE_EVENT_TABLE_ENTRY(                                                 \
		EVENT_KEYWORD_SELECT, id, -1,                                          \
		(wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction)(      \
			void (wxEvtHandler::*)(KeywordSelectEvent &))(&handler),           \
		(wxObject *)NULL),
...
class KeywordList : public ItemList, public SetView {
  public:
	...
	inline KeywordP getKeyword() const {
		return static_pointer_cast<Keyword>(selected_item);
	}
  ...
  protected:
	...
	inline KeywordP getKeyword(long pos) const {
		return static_pointer_cast<Keyword>(getItem(pos));
	}
