...
class PackageList : public GalleryList {
  public:
    ...
    /// Shows packages that match a specific patern, and that are of the given
    /// type
    template <typename T>
    ...
    intrusive_ptr<T> getSelection(bool load_fully = true) const {
        intrusive_ptr<T> ret =
            dynamic_pointer_cast<T>(packages.at(getSelectionId()).package);
        if (!ret)
            throw InternalError(
                (L"PackageList: Selected package has the wrong type"));
        if (load_fully)
            ret->loadFully();
        return ret;
    }

    /// Select the package with the given name, if it is not found, selects
    /// nothing
  ...
  private:
    ...
    struct PackageData {
        PackageData() {}
        PackageData(const PackagedP &package, const Bitmap &image)
