...
void NativeLookEditor::drawViewer(RotatedDC &dc, ValueViewer &v) {
	...
	if (!e || e->drawLabel()) {
		...
		Style &s = *v.getStyle();
		draw_control_box(this, dc.getDC(),
						 dc.trRectToBB(s.getInternalRect().grow(1)),
						 current_editor == e, e != nullptr);
		// draw label
...
void NativeLookEditor::resizeViewers() {
	...
	// Determine label width
	{
		...
		for (auto v : viewers) {
			...
			if (!e || e->drawLabel()) {
				...
				Style &s = *v->getStyle();
				String text =
					tr(getStylePackage(), s.fieldP->caption, identity);
	...
	if (y >= h) {
		...
		for(auto v: viewers) {
			...
			ValueEditor* e = v->getEditor();
			s->top  = y;
			s->width = s->width - wxSystemSettings::GetMetric(wxSYS_VSCROLL_X,
		this);
...
void NativeLookEditor::onInit() {
	DataEditor::onInit();
	// Give viewers a chance to show/hide controls (scrollbar) when selecting
	// other editors
	FOR_EACH_EDITOR { e->onShow(true); }
	resizeViewers();
}

wxSize NativeLookEditor::DoGetBestSize() const { return wxSize(200, 200); }
...
void NativeLookEditor::onMouseWheel(wxMouseEvent &ev) {
	...
	int toScroll = 10 * ev.GetWheelRotation() * ev.GetLinesPerAction() /
...
void StylingEditor::onChangeSet() { showStylesheet(set->stylesheet); }
