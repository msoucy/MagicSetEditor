...
CardListColumnSelectDialog::CardListColumnSelectDialog(Window *parent,
													   const GameP &game)
	...
	s->Add(new wxStaticText(this, wxID_ANY, _LABEL_("select columns")), 0,
		   wxALL, 8);
	s->Add(new wxStaticText(this, wxID_ANY, _LABEL_("columns")), 0,
		   wxALL & ~wxBOTTOM, 8);
	wxSizer *s2 = new wxBoxSizer(wxHORIZONTAL);
	s2->Add(list, 1, wxEXPAND | wxLEFT | wxRIGHT, 4);
	wxSizer *s3 = new wxBoxSizer(wxVERTICAL);
	s3->Add(new wxButton(this, ID_MOVE_UP, _BUTTON_("move up")), 0, wxEXPAND,
			2);
	s3->Add(new wxButton(this, ID_MOVE_DOWN, _BUTTON_("move down")), 0,
			wxEXPAND | wxTOP, 2);
	s3->Add(new wxButton(this, ID_SHOW, _BUTTON_("show")), 0, wxEXPAND | wxTOP,
			2);
	s3->Add(new wxButton(this, ID_HIDE, _BUTTON_("hide")), 0, wxEXPAND | wxTOP,
			2);
...
struct SortByPosition {
	...
	const Game &game;
	bool operator()(const CardListColumnSelectDialog::ColumnSettingsF &a,
					const CardListColumnSelectDialog::ColumnSettingsF &b) {
...
void CardListColumnSelectDialog::initColumns() {
	...
	for (auto f : game->card_fields) {
		if (f->card_list_allow) {
			columns.push_back(
				ColumnSettingsF(f, settings.columnSettingsFor(*game, *f)));
		}
	}
	// sorted by position
	...
	int min = 0;
	for (auto c : columns) {
		if (c.settings.position < min)
			c.settings.position = min;
...
void CardListColumnSelectDialog::onMove(wxCommandEvent &ev) {
	...
	if (i == wxNOT_FOUND || i + delta < 0 || i + delta >= (int)columns.size())
		return;
...
void CardListColumnSelectDialog::onUpdateUI(wxUpdateUIEvent &ev) {
	...
	switch (ev.GetId()) {
	case ID_MOVE_UP:
		ev.Enable(i != wxNOT_FOUND && i - 1 >= 0);
		break;
	case ID_MOVE_DOWN:
		ev.Enable(i != wxNOT_FOUND && i + 1 < (int)columns.size());
		break;
	case ID_SHOW:
		ev.Enable(i != wxNOT_FOUND && !columns[i].settings.visible);
		break;
	case ID_HIDE:
		ev.Enable(i != wxNOT_FOUND && columns[i].settings.visible);
		break;
