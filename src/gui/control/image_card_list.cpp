...
ImageCardList::~ImageCardList() { thumbnail_thread.abort(this); }
void ImageCardList::onRebuild() { image_field = findImageField(); }
...
class CardThumbnailRequest : public ThumbnailRequest {
  public:
	CardThumbnailRequest(ImageCardList *parent, const String &key,
						 const GeneratedImageP &imgen)
		: ThumbnailRequest(
			  parent,
...
int ImageCardList::OnGetItemImage(long pos) const {
	if (image_field) {
		...
		const ImageValue &val =
			static_cast<const ImageValue &>(*getCard(pos)->data[image_field]);
		if (val.value->isNil())
			return -1;
		...
		if (it != thumbnails.end()) {
			...
			// request a thumbnail
			thumbnail_thread.request(intrusive(new CardThumbnailRequest(
				const_cast<ImageCardList *>(this), key, image)));
		}
	}
	return -1;
}

void ImageCardList::onIdle(wxIdleEvent &) { thumbnail_thread.done(this); }
...
FilteredImageCardList::FilteredImageCardList(Window *parent, int id,
