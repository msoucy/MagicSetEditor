...
#define DECLARE_DYNAMIC_ARG(Type, name)                                        \
    ...
    class name##_changer {                                                     \
      public:                                                                  \
        ...
        inline ~name##_changer() { name##_private = oldValue; }                \
                                                                               \
      private:                                                                 \
        Type oldValue;                                                         \
    }
...
*  @endcode
*/
#define WITH_DYNAMIC_ARG(name, value) name##_changer name##_dummmy(value)
...
struct ThreadLocalObject {
    ...
    ThreadLocalObject(T def) : def(def) {}

    ~ThreadLocalObject() {
        for (typename map<int, T *>::iterator i = objects.begin();
             i != objects.end(); ++i) {
            delete (*i).second;
        }
    }
...
#define DECLARE_DYNAMIC_ARG(Type, name) extern ThreadLocalObject<Type> name;

#define IMPLEMENT_DYNAMIC_ARG(Type, name, initial)                             \
    ThreadLocalObject<Type> name(initial);

#define WITH_DYNAMIC_ARG(name, value) name.store(value);
