...
/** In addition to what untag() does this function also removes
 * <sep-soft>...</sep-soft>
 */
...
enum Movement {
	...
	,
	MOVE_LEFT_OPT = -1 ///< Move the cursor to the left, but a position inside a
					   ///tag is the same as that before
	,
	MOVE_MID = 0 ///< Move in whichever direction the distance to move is
				 ///shorter (TODO: define shorter)
	,
	MOVE_RIGHT_OPT = 1 ///< Move the cursor to the left, but a position inside a
					   ///tag is the same as that after
	,
 ...
 */
size_t index_to_cursor(const String &str, size_t index,
					   Movement dir = MOVE_MID);

/// Find the range of character indeces corresponding to the given cursor
/// position
/** The output parameters will correspond to the range [start...end) which are
 * all valid character indices.
 */
void cursor_to_index_range(const String &str, size_t cursor, size_t &begin,
						   size_t &end);
...
/// Untag a string for use with cursors, <atom>...</atom> becomes a single
/// character.
 ...
 *  @param inside if inside then it prefers to find positions inside tags (after
 * open tags, before close tags)
 *  @param start_index is the position to start at in the string,
 *                     untagged_to_index(s,_,_,i) ==
 * untagged_to_index(s.substr(i),_,_)+i
 */
...
/** tag doesn't have to be a complete tag, for example remove_tag_contents(str,
 * "<kw-")
 *  removes all tags starting with "<kw-".
 */
 ...
 *  This function makes sure tags still match. It also attempts to cancel out
 * tags.
...
/** e.g. "<tag></tag>" --> ""
 *
 * @param all Merge all tags, if false only merges b,i,sym, and <tag></tag>
 *pairs. But not </tag><tag>.
 */
