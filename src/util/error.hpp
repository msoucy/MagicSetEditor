...
class InternalError : public Error {
  public:
	...
	// not all internal errors are fatal, but we had still better warn the user
	// about them.
...
class ScriptParseError : public ParseError {
  public:
	ScriptParseError(size_t pos, int line, const String &filename,
					 const String &str);
	ScriptParseError(size_t pos, int line, const String &filename,
					 const String &expected, const String &found);
	ScriptParseError(size_t pos1, size_t pos2, int line, const String &filename,
...
class ScriptErrorNoVariable : public ScriptError {
  public:
	inline ScriptErrorNoVariable(const String &var)
		: ScriptError(_("Variable not set: ") + var) {}
};
...
class ScriptErrorConversion : public ScriptError {
  public:
	...
	inline ScriptErrorConversion(const String &value, const String &a,
								 const String &b)
...
/** If the message is a MESSAGE_FATAL_ERROR, and
 * show_message_box_for_fatal_errors==true, then a popup is shown
 */
