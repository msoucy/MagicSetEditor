	  ...
	  protected:
		virtual void OnStackFrame(const wxStackFrame &frame) {
			...
			if (!name.empty()) {
				...
				m_stackTrace
					<< wxString::Format(_("%p"), (void *)frame.GetAddress());
			}

			if (frame.HasSourceLocation()) {
				m_stackTrace << _('\t') << frame.GetFileName() << _(':')
...
ScriptParseError::ScriptParseError(size_t pos1, size_t pos2, int line,
	...
	: ParseError(_("Expected closing '") + close + _("' for this '") + open +
				 _("' instead of '") + found + _("'")),
...
String concat(const vector<ScriptParseError> &errors) {
	String total;
	for (const ScriptParseError &e : errors) {
		if (!total.empty())
			total += _("\n");
		total += e.what();
	}
