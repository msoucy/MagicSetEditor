...
class SpecIterator {
  public:
    SpecIterator(const String &spec, size_t pos = 0) : spec(spec), pos(pos) {}
        ...
        if (pos >= spec.size()) {
            ...
            if (close == 0) {
                ...
                throw ParseError(String::Format(
                    (L"Expected '%c' in sort_rule specification"), close));
            }
        }
            ...
            while (value == (L' ')) {
                if (pos >= spec.size()) {
                    if (close == 0) {
                        ...
                        throw ParseError(String::Format(
                            (L"Expected '%c' in sort_rule specification"),
                            close));
                    }
                }
                value = spec.GetChar(pos++);
    ...
    String readRawParam(Char close1, Char close2 = 0) {
        ...
        while (nextUntil(0, false)) {
            if (escaped) {
                ...
                if (parens == 0 && (value == close1 || value == close2))
                    break;
                if (value == (L'('))
                    parens++;
                else if (value == (L')'))
...
/// Sort a string using a specification using the shortest cycle method, see
/// spec_sort
...
void pattern_sort(const String &pattern, const String &spec, String &input,
                  String &ret) {
            ...
            for (auto &d : input) {
                if (d != REMOVED) {
                    ...
                    d = REMOVED;
                       ...
                       (L"reverse_order("))) { // reverse order of preference
