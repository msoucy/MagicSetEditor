...
class Regex {
  public:
    struct Results : public boost::match_results<String::const_iterator> {
        ...
        inline String format(const String &format) const {
            ...
            boost::match_results<String::const_iterator>::format(
                std::insert_iterator<String>(output, output.end()), fmt,
                boost::format_sed);
            return output;
        }
    };
    ...
    inline bool empty() const { return regex.empty(); }
...
void assign(const String &code);
inline bool matches(const String &str) const { return regex.Matches(str); }
inline bool matches(Results &results, const String &str) const {
    results.regex = &regex;
    results.begin = str.begin();
    return regex.Matches(str);
}
inline bool matches(Results &results, const Char *begin,
...
inline bool empty() const { return !regex.IsValid(); }
