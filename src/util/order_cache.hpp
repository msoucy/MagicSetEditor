...
/// Object that cashes an ordered version of a list of items, for finding the
/// position of objects
/** Can be used as a map "void* -> int" for finding the position of an object */
template <typename T>
class OrderCache : public IntrusivePtrBase<OrderCache<T>> {
  public:
	/// Initialize the order cache, ordering the keys by their string values
	/// from the other vector
	/** Optionally filter the list using a vector of booleans of items to keep
	 * (note: vector<bool> is evil)
	 *  @pre keys.size() == values.size()
	 */
	OrderCache(const vector<T> &keys, const vector<String> &values,
			   vector<int> *keep = nullptr);
...
OrderCache<T>::OrderCache(const vector<T> &keys, const vector<String> &values,
	...
	int i = 0;
	for (typename vector<T>::const_iterator it = keys.begin(); it != keys.end();
		 ++it, ++i) {
		if (!keep || (*keep)[i]) {
	...
	i = 0;
	for (typename vector<KV>::iterator it = positions.begin();
		 it != positions.end(); ++it, ++i) {
		it->second = i;
	}
...
int OrderCache<T>::find(const T &key) const {
	vector<KV>::const_iterator it =
		lower_bound(positions.begin(), positions.end(), &*key, CompareKeys());
	if (it == positions.end() || it->first != &*key)
		return -1;
	return it->second;
}
