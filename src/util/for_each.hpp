...
template <const type_info &ref_type_info>
struct TypeOf {};
...
/// Declare typeof magic for a specific type that doesn't support reverse
/// iterators
#define DECLARE_TYPEOF_NO_REV(T)                                               \
	template <>                                                                \
	struct TypeOf<typeid(T)> {                                                 \
...
#define FOR_EACH_IT_T(Type, Iterator, Collection)                              \
	for (Type Iterator = (Collection).begin(); Iterator != (Collection).end(); \
		 ++Iterator)
...
#define FOR_EACH_REVERSE_IT(Iterator, Collection)                              \
	for (TYPEOF_RIT(Collection) Iterator = (Collection).rbegin();              \
		 Iterator != (Collection).rend(); ++Iterator)

// -----------------------------------------------------------------------------
// : Looping macros

/// Iterate over a collection, with an iterator of type TypeIt, and elements of
/// type TypeElem
/** Usage: FOR_EACH_T(TypeIt,TypeElem,e,collect,begin,end) { body-of-loop }
 *
 *  We need a hack to be able to declare a local variable without needing
 *braces.
 *  To do this we use a nested for loop that is only executed once, and which is
 *optimized away.
 *  To terminate this loop we need an extra bool, which we set to false after
 *the first iteration.
 */
#define FOR_EACH_T(TypeIt, TypeElem, Elem, Collection, begin, end)             \
	for (std::pair<TypeIt, bool> Elem##_IT((Collection).begin(), true);        \
		...
		for (TypeElem Elem = *Elem##_IT.first; Elem##_IT.second;               \
...
#define FOR_EACH_REVERSE(Elem, Collection)                                     \
	FOR_EACH_T(TYPEOF_RIT(Collection), TYPEOF_REF(Collection), Elem,           \
			   Collection, rbegin, rend)
...
#define FOR_EACH_CONST_REVERSE(Elem, Collection)                               \
	FOR_EACH_T(TYPEOF_CRIT(Collection), TYPEOF_CREF(Collection), Elem,         \
			   Collection, rbegin, rend)

/// Iterate over two collection in parallel
/** Usage:
 *FOR_EACH_2_T(TypeIt1,TypeElem1,e1,collect1,TypeIt2,TypeElem2,e2,collect2) {
 *body-of-loop }
 *
 *  Note: This has got to be one of the craziest pieces of code I have ever
 *written :)
 *  It is just an extension of the idea of FOR_EACH_T.
 */
#define FOR_EACH_2_T(TypeIt1, TypeElem1, Elem1, Coll1, TypeIt2, TypeElem2,     \
					 Elem2, Coll2)                                             \
	for (std::pair<std::pair<TypeIt1, TypeIt2>, bool>                          \
		...
		for (TypeElem1 Elem1 = *Elem1##_IT.first.first; Elem1##_IT.second;     \
			 Elem1##_IT.second = false)                                        \
			for (TypeElem2 Elem2 = *Elem1##_IT.first.second;                   \
				 Elem1##_IT.second; Elem1##_IT.second = false)

/// Iterate over two collections in parallel, their type must be declared with
/// DECLARE_TYPEOF.
/** Usage: FOR_EACH_2(e1,collect1, e2,collect2) { body-of-loop }
 */
#define FOR_EACH_2(Elem1, Collection1, Elem2, Collection2)                     \
	FOR_EACH_2_T(TYPEOF_IT(Collection1), TYPEOF_REF(Collection1), Elem1,       \
				 Collection1, TYPEOF_IT(Collection2), TYPEOF_REF(Collection2), \
				 Elem2, Collection2)

/// Iterate over two constants collections in parallel, their type must be
/// declared with DECLARE_TYPEOF.
/** Usage: FOR_EACH_2_CONST(e1,collect1, e2,collect2) { body-of-loop }
 */
#define FOR_EACH_2_CONST(Elem1, Collection1, Elem2, Collection2)               \
	FOR_EACH_2_T(TYPEOF_CIT(Collection1), TYPEOF_CREF(Collection1), Elem1,     \
				 Collection1, TYPEOF_CIT(Collection2),                         \
				 TYPEOF_CREF(Collection2), Elem2, Collection2)
