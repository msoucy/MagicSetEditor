...
enum RotationFlags { ROTATION_NORMAL, ROTATION_ATTACH_TOP_LEFT };
...
class Rotation {
  public:
	...
	/** with the given rectangle of external coordinates and a given rotation
	 * angle and zoom factor.
	 *  if is_internal then the rect gives the internal coordinates, its origin
	 * should be (0,0)
	 */
	Rotation(Radians angle = 0, const RealRect &rect = RealRect(0, 0, 0, 0),
			 double zoom = 1.0, double strectch = 1.0,
			 RotationFlags flags = ROTATION_NORMAL);
	...
	inline RealRect getInternalRect() const {
		return RealRect(RealPoint(0, 0), size);
	}
	/// The size of the external rectangle (as passed to the constructor) ==
	/// trNoNeg(getInternalSize())
	inline RealSize getExternalSize() const { return trSizeToBB(size); }
	/// The external rectangle (as passed to the constructor) ==
	/// trNoNeg(getInternalRect())
	inline RealRect getExternalRect() const {
		return trRectToBB(getInternalRect());
	}
	...
	inline RealSize trS(const RealSize &s) const {
	...
	/// Translate a rectangle, returns the bounding box, the size will be
	/// non-negative
	...
	inline RealSize trInvS(const RealSize &s) const {
		return RealSize(s.width / zoomX, s.height / zoomY);
	}
  ...
  protected:
	...
	double
		zoomX; ///< Zoom factor, zoom = 2.0 means that 1 internal = 2 external
	double zoomY;

	friend class Rotater;

	/// Determine the top-left corner of the bounding box around the rotated box
	/// s (in external coordinates)
...
class RotatedDC : public Rotation {
  public:
	RotatedDC(DC &dc, Radians angle, const RealRect &rect, double zoom,
			  RenderQuality quality, RotationFlags flags = ROTATION_NORMAL);
	RotatedDC(DC &dc, const Rotation &rotation, RenderQuality quality);

	// --------------------------------------------------- : Drawing

	/// Draw text
	void DrawText(const String &text, const RealPoint &pos, int blur_radius = 0,
				  int boldness = 1, double stretch = 1.0);
	void DrawText(const String &text, const RealPoint &pos, AColor color,
				  int blur_radius = 0, int boldness = 1, double stretch = 1.0);
	/// Draw text with the shadow and color settings of the given font
	void DrawTextWithShadow(const String &text, const Font &font,
							const RealPoint &pos, double scale = 1.0,
							double stretch = 1.0);
	/// Draw abitmap, it must already be zoomed!
	void DrawBitmap(const Bitmap &bitmap, const RealPoint &pos);
	/// Draw an image using the given combining mode, the image must already be
	/// zoomed!
	void DrawImage(const Image &image, const RealPoint &pos,
				   ImageCombine combine = COMBINE_DEFAULT);
	/// Draw a bitmap that is already zoomed and rotated.
	/** The rectangle the position in internal coordinates, and the size before
	 * rotating and zooming */
	void DrawPreRotatedBitmap(const Bitmap &bitmap, const RealRect &rect);
	/// Draw an image that is already zoomed and rotated
	void DrawPreRotatedImage(const Image &image, const RealRect &rect,
							 ImageCombine combine = COMBINE_DEFAULT);
	...
	void DrawEllipticArc(const RealPoint &center, const RealSize &size,
						 Radians start, Radians end);
	/// Draw spokes of an ellipse
	void DrawEllipticSpoke(const RealPoint &center, const RealSize &size,
						   Radians start);
