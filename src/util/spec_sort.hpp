 ...
 *   - mixed(abc)   = 'a', 'b' and 'c' go here, in the same order as in the
 *input
 *   - once(abc)    = 'a', 'b' and 'c' go here in that order, and only zero or
 *one time.
 ...
 *                 2. has _('holes') early, a hole means a character that is in
 *the specification
 *                    but not in the input
 *                 3. prefer the one that comes the earliest in the expression
 *(a in this case)
 *   - compound(abc)    = the connect sting "abc" goes gere
 *   - pattern(.. sort) = sort the things matching the pattern using 'sort' and
 *replace them in the pattern
 *
 *  example:
 *    spec_sort("XYZ<0123456789>cycle(WUBRG)",..)  // used by magic
 *     "W1G")      -> "1GW"      // could be "W...G" or "...GW", second is
 *shorter
 *     "GRBUWWUG") -> "WWUUBRGG" // no difference by rule 1,2, could be "WUBRG",
 *"UBRGW", etc.
 *                               // becomes _("WUBRG") by rule 3
 *     "WUR")      -> "RWU"      // by rule 1 could be "R WU" or "WU R", "RWU"
 *has an earlier hole
 */
