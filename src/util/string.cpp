...
const String word_start_chars =
    String((L"[({\"\'")) + LEFT_SINGLE_QUOTE + LEFT_DOUBLE_QUOTE;
const String word_end_chars =
    String((L"])}.,;:?!\"\'")) + RIGHT_SINGLE_QUOTE + RIGHT_DOUBLE_QUOTE;
...
String capitalize(const String &s) {
    ...
    for (String::iterator it = result.begin(); it != result.end(); ++it) {
        if (*it == (L' ') || *it == (L'/')) {
            ...
            // if (it != s.begin() &&
            if (s.begin() != it && (is_substr(result, it, (L"is ")) ||
                                    is_substr(result, it, (L"the ")) ||
                                    is_substr(result, it, (L"in ")) ||
                                    is_substr(result, it, (L"of ")) ||
                                    is_substr(result, it, (L"to ")) ||
                                    is_substr(result, it, (L"at ")) ||
...
Char decompose_char2(Char c) {
    if (c < 0xC6) {
        return 0;
    } else if (c == 0xC6 || c == 0xE6 || c == 0x152 || c == 0x153 ||
               c == 0x1E2 || c == 0x1E3 || c == 0x1FC || c == 0x1FD) {
        return (L'e'); // "ae" or "oe"
...
int smart_compare(const String &sa, const String &sb) {
        ...
        if (da && db) {
            ...
            if (a >= 0x20 && b >= 0x20) {
                ...
                if (la2 || lb2) {
                    ...
                    goto next; // don't move to the next character in both
                               // strings
                }
            } else {
                // control characters
    ...
    if (in_num) {
        ...
        return na - pa == nb - pb
                   ...
                   : na - pa < nb - pb
                         ? -1
                         : 1; // outside number, shorter string comes first
    }
}
...
bool is_substr(const String &str, size_t pos, const Char *cmp) {
    for (String::const_iterator it = str.begin() + pos; *cmp && it < str.end();
         ++cmp, ++it) {
        if (*cmp != *it)
            return false;
    }
...
bool is_substr(const String &str, size_t pos, const String &cmp) {
    return str.size() >= cmp.size() + pos &&
           str.compare(pos, cmp.size(), cmp) == 0;
}

bool is_substr_i(const String &str, size_t pos, const Char *cmp) {
    for (String::const_iterator it = str.begin() + pos; *cmp && it < str.end();
         ++cmp, ++it) {
        if (toLower(*cmp) != toLower(*it))
            return false;
    }
    return *cmp == (L'\0');
}
...
size_t find_i(const String &heystack, const String &needle) {
    ...
    for (size_t i = 0; i + needle.size() <= heystack.size(); ++i) {
        if (is_substr_i(heystack, i, needle))
            return true;
    }
    return String::npos;
}
