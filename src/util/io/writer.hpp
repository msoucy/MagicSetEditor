...
class Writer {
  public:
    ...
    template <typename T>
    void handle(const intrusive_ptr<T> &);
    /// Write a map to the output stream
    template <typename K, typename V>
    void handle(const map<K, V> &);
    /// Write an IndexMap to the output stream
    template <typename K, typename V>
    void handle(const IndexMap<K, V> &);
    template <typename K, typename V>
...
void Writer::handle(const IndexMap<K, V> &m) {
    for (typename IndexMap<K, V>::const_iterator it = m.begin(); it != m.end();
         ++it) {
...
#define REFLECT_OBJECT_WRITER(Cls)                                             \
    template <>                                                                \
    void Writer::handle<Cls>(const Cls &object) {                              \
        const_cast<Cls &>(object).reflect(*this);                              \
    }                                                                          \
    void Cls::reflect(Writer &writer) { reflect_impl(writer); }
...
#define REFLECT_ENUM_WRITER(Enum)                                              \
    template <>                                                                \
    void Writer::handle<Enum>(const Enum &enum_) {                             \
...
class EnumWriter {
  public:
    inline EnumWriter(Writer &writer) : writer(writer) {}
