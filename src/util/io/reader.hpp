...
template <typename T>
inline void after_reading(T &, Version) {}
 ...
 *
 *  The handle functions ensure that afterwards the reader is at the line after
 *the
...
class Reader {
  public:
	 ...
	 */
	Reader(InputStream &input, Packaged *package = nullptr,
		   const String &filename = wxEmptyString, bool ignore_invalid = false);
	...
	inline bool isCompound() const {
		return indent != expected_indent - 1 || value.empty();
	}
	...
	void warning(const String &msg, int line_number_delta = 0,
				 bool warn_on_previous_line = true);
	...
	template <typename T>
	void handle(intrusive_ptr<T> &);
	/// Reads a map from the input stream
	template <typename V>
	void handle(map<String, V> &);
	/// Reads an IndexMap from the input stream, reads only keys that already
	/// exist in the map
	template <typename K, typename V>
	void handle(IndexMap<K, V> &);
	template <typename K, typename V>
	void handle(DelayedIndexMaps<K, V> &);
	template <typename K, typename V>
	void handle(DelayedIndexMapsData<K, V> &);
	template <typename T>
	void handle(Defaultable<T> &);
	template <typename T>
	void handle(Scriptable<T> &);
	void handle(ScriptValueP &);
	// special behaviour: only write the name of the package
	void handle(GameP &);
	void handle(StyleSheetP &);

	/// Indicate that the last value from getValue() was not handled, allowing
  ...
  private:
	...
	enum State {
		...
		HANDLED,   ///< We have handled a value, and moved to the next line,
				   ///previous_value is the value we just handled
	...
	/// Is there a block with the given key under the current cursor? if so,
	/// enter it
	...
	void moveNext();
	/// Reads the next line from the input, and stores it in
	...
	void unknownKey(T &v) {
		if (key == _("include_file")) {
			...
			if (sub_reader.file_app_version == 0) {
				// in an included file, use the app version of the parent if
				// there is none
...
template <typename T>
void update_index(T &, size_t index) {}
...
void Reader::handle(const Char *name, vector<T> &vector) {
	...
	while (enterBlock(vectorKey)) {
		...
		update_index(vector.back(),
					 vector.size() - 1); // update index for IndexMap
		exitBlock();
	}
}
...
void Reader::handle(IndexMap<K, V> &m) {
	for (typename IndexMap<K, V>::iterator it = m.begin(); it != m.end();
		 ++it) {
...
#define REFLECT_OBJECT_READER(Cls)                                             \
	template <>                                                                \
	void Reader::handle<Cls>(Cls & object) {                                   \
		object.reflect(*this);                                                 \
	}                                                                          \
	void Cls::reflect(Reader &reader) { reflect_impl(reader); }
...
#define REFLECT_ENUM_READER(Enum)                                              \
	template <>                                                                \
	void Reader::handle<Enum>(Enum & enum_) {                                  \
...
class EnumReader {
  public:
	...
	/// Handle a possible value for the enum, if the name matches the name in
	/// the input
  ...
  private:
	...
	Char const *first; ///< Has the first (default) value been handled? If so,
					   ///what is its name.
