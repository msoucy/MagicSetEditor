...
Reader::Reader(InputStream &input, Packaged *package, const String &filename,
			   bool ignore_invalid)
	: indent(0), expected_indent(0), state(OUTSIDE),
	  ignore_invalid(ignore_invalid), filename(filename), package(package),
	  line_number(0), previous_line_number(0), input(input) {
	moveNext();
	handleAppVersion();
}
	...
	if (enterBlock(_("mse_version"))) {
		...
		if (app_version < file_app_version) {
			queue_message(MESSAGE_WARNING,
						  _ERROR_2_("newer version", filename,
									file_app_version.toString()));
		}
		exitBlock();
	}
}

void Reader::warning(const String &msg, int line_number_delta,
					 bool warn_on_previous_line) {
	warnings +=
		String(_("\nOn line "))
		<< ((warn_on_previous_line ? previous_line_number : line_number) +
			line_number_delta) << _(": \t") << msg;
}
...
bool Reader::enterAnyBlock() {
	if (state == ENTERED)
		moveNext(); // on the key of the parent block, first move inside it
	if (indent != expected_indent)
		return false; // not enough indentation
	state = ENTERED;
	expected_indent +=
		1; // the indent inside the block must be at least this much
	return true;
}
 ...
 *  2007-09-21: Profiling => Using this class roughly halves the runtime of
 * read_utf8_line,
...
#else
	// NOTE: wx doc is wrong, parameter to GetWritableChar is number of
...
// convert
	...
	size = ::WideCharToMultiByte(CP_ACP, 0, &buf2[0], -1, nullptr, 0, nullptr,
								 nullptr);
	Char *result_buf = result.GetWriteBuf(size);
	::WideCharToMultiByte(CP_ACP, 0, &buf2[0], -1, result_buf, (int)size,
						  nullptr, nullptr);
...
void Reader::readLine(bool in_string) {
	...
	// pragma handler
	if (reader_pragma_handler())
		reader_pragma_handler()(line);
	// read indentation
	indent = 0;
	...
	// read key / value
	if (line.find_first_not_of(_(" \t")) == String::npos ||
		line.GetChar(indent) == _('#')) {
	...
	if (!ignore_invalid && !in_string && starts_with(key, _(" "))) {
		warning(_("key: '") + key +
					_("' starts with a space; only use TABs for indentation!"),
				0, false);
...
void Reader::unknownKey() {
	...
	// else: could be a nameless value, which doesn't call exitBlock to move
	// past its own key
}
...
const String &Reader::getValue() {
	...
	if (state == UNHANDLED) {
		...
		while (indent >= expected_indent && !input.Eof()) {
			previous_value.resize(previous_value.size() + pending_newlines,
								  _('\n'));
			pending_newlines = 0;
			previous_value +=
				line.substr(expected_indent); // strip expected indent
			do {
				readLine(true);
				pending_newlines++;
				// skip empty lines that are not indented enough
			} while (trim(line).empty() && indent < expected_indent &&
					 !input.Eof());
		}
		...
		if (indent >= expected_indent) {
			warning(_("Blank line or comment in text block, that is "
					  "insufficiently indented.\n")
					_("\t\tEither indent the comment/blank line, or add a "
					  "'key:' after it.\n")
...
template <>
void Reader::handle(String &s) {
	s = getValue();
}
template <>
void Reader::handle(int &i) {
	long l = 0;
...
template <>
void Reader::handle(unsigned int &i) {
	long l = 0;
	if (!getValue().ToLong(&l)) {
		warning(_("Expected non-negative integer instead of '") +
				previous_value + _("'"));
	} else if (l < 0) {
		warning(wxString::Format(
			_("Expected non-negative integer instead of %d"), (int)l));
	}
	i = abs(l); // abs, because it will seem strange if -1 comes out as MAX_INT
}
template <>
void Reader::handle(double &d) {
	if (!getValue().ToDouble(&d)) {
		warning(_("Expected floating point number instead of '") +
				previous_value + _("'"));
	}
}
template <>
void Reader::handle(bool &b) {
	...
	if (v == _("true") || v == _("1") || v == _("yes")) {
		...
		warning(_("Expected boolean ('true' or 'false') instead of '") + v +
				_("'"));
	}
}
template <>
void Reader::handle(tribool &tb) {
	bool b;
	handle(b);
	tb = b;
}
...
template <>
void Reader::handle(Vector2D &vec) {
...
template <>
void Reader::handle(LocalFileName &f) {
...
String EnumReader::notDoneErrorMessage() const {
	if (!first)
		throw InternalError(_("No first value in EnumReader"));
