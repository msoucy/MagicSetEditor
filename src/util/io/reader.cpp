...
void Reader::readLine(bool in_string) {
    ...
    if (!ignore_invalid && !in_string && starts_with(key, (L" "))) {
        warning((L"key: '") + key +
                    (L"' starts with a space; only use TABs for indentation!"),
                0, false);
    ...
    if (key.empty() && pos != String::npos)
        key = (L" "); // we don't want an empty key if there was a colon
}
...
const String &Reader::getValue() {
    ...
    if (state == UNHANDLED) {
        ...
        if (indent >= expected_indent) {
            warning(L"Blank line or comment in text block, that is "
                    L"insufficiently indented.\n"
                    L"\t\tEither indent the comment/blank line, or add a "
                    L"'key:' after it.\n"
...
void Reader::handle(unsigned int &i) {
    long l = 0;
    if (!getValue().ToLong(&l)) {
        warning((L"Expected non-negative integer instead of '") +
                previous_value + (L"'"));
    } else if (l < 0) {
        warning(wxString::Format(
            (L"Expected non-negative integer instead of %d"), (int)l));
    }
...
void Reader::handle(double &d) {
    if (!getValue().ToDouble(&d)) {
        warning((L"Expected floating point number instead of '") +
                previous_value + (L"'"));
    }
}
template <>
void Reader::handle(bool &b) {
    ...
    if (v == (L"true") || v == (L"1") || v == (L"yes")) {
        ...
        warning((L"Expected boolean ('true' or 'false') instead of '") + v +
                (L"'"));
    }
}
template <>
...
String EnumReader::notDoneErrorMessage() const {
    if (!first)
        throw InternalError((L"No first value in EnumReader"));
