...
Package::Package() : fileStream(nullptr), zipStream(nullptr) {}
...
bool Package::isOpened() const { return !filename.empty(); }
bool Package::needSaveAs() const { return filename.empty(); }
String Package::name() const {
	...
	if (slash == String::npos && ext == String::npos)
		return filename;
	else if (slash == String::npos)
		return filename.substr(0, ext);
	else if (ext == String::npos)
		return filename.substr(slash + 1);
	else
		return filename.substr(slash + 1, ext - slash - 1);
}
...
const String &Package::absoluteFilename() const { return filename; }
...
void Package::open(const String &n, bool fast) {
	...
	if (wxDirExists(filename)) {
		...
		throw PackageNotFoundError(_("Package not found: '") + filename +
								   _("'"));
	}
}
 ...
 *    Bringing the avarage run time of read_utf8_line from 186k to 54k (in cpu
 * time units)
 */
class BufferedFileInputStream : private BufferedFileInputStream_aux,
								public wxBufferedInputStream {
  public:
...
InputStreamP Package::openIn(const String &file) {
	...
	if (it != files.end() && it->second.wasWritten()) {
	...
	} else if (wxFileExists(filename) && it != files.end() &&
			   it->second.zipEntry) {
		...
		wxFileSystem fs;
		wxFSFile *file = fs.OpenFile(_("file:") + filename + _("#zip:") +
									 it->second.zipEntry->GetInternalName());
		stream = shared_ptr<InputStream>(file->GetStream());
	} else {
...
String Package::nameOut(const String &file) {
	assert(
		wxThread::IsMain()); // Writing should only be done from the main thread
...
void Package::referenceFile(const String &file) {
	...
	if (it == files.end())
		throw InternalError(_("referencing a nonexistant file"));
	it->second.keep = true;
}
...
InputStreamP Package::openAbsoluteFile(const String &name) {
	...
	if (pos == String::npos) {
		...
		if (!f->IsOk())
			throw FileNotFoundError(_("<unknown>"), name);
		return f;
	} else {
...
LocalFileName LocalFileName::fromReadString(String const &fn,
											String const &prefix,
											String const &suffix) {
	if (!fn.empty() && clipboard_package()) {
		// copy file into current package
		try {
			LocalFileName local_name = clipboard_package()->newFileName(
				prefix, _("")); // a new unique name in the package, assume it's
								// an image
...
Package::FileInfo::~FileInfo() { delete zipEntry; }
...
void Package::openSubdir(const String &name) {
	...
	// find files
	String f; // filename
	for (bool ok = d.GetFirst(&f, wxEmptyString, wxDIR_FILES | wxDIR_HIDDEN);
		 ok; ok = d.GetNext(&f)) {
		if (ignore_file(f))
			continue;
	...
	// find subdirs
	for (bool ok = d.GetFirst(&f, wxEmptyString, wxDIR_DIRS | wxDIR_HIDDEN); ok;
		 ok = d.GetNext(&f)) {
...
void Package::openZipfile() {
	...
	if (!fileStream->IsOk())
		throw PackageError(_ERROR_1_("package not found", filename));
...
void Package::saveToDirectory(const String &saveAs, bool remove_unused,
	...
	for (auto &f : files) {
		if (!f.second.keep && remove_unused) {
			...
			if (!(is_copy
					  ? wxCopyFile(f.second.tempName, saveAs + _("/") + f.first)
					  : wxRenameFile(f.second.tempName,
									 saveAs + _("/") + f.first))) {
			...
			if (!wxCopyFile(filename + _("/") + f.first,
							saveAs + _("/") + f.first)) {
				throw PackageError(_ERROR_("unable to store file"));
			}
			vcs->addFile(saveAs + _("/") + f.first);
...
void Package::saveToZipfile(const String &saveAs, bool remove_unused,
	...
	// open zip file
	try {
		const unique_ptr<wxFileOutputStream> newFile(
			new wxFileOutputStream(tempFile));
		if (!newFile->IsOk())
			throw PackageError(_ERROR_("unable to open output file"));
		const unique_ptr<wxZipOutputStream> newZip(
			new wxZipOutputStream(*newFile));
		if (!newZip->IsOk())
			throw PackageError(_ERROR_("unable to open output file"));
		...
		for (auto &f : files) {
			if (!f.second.keep && remove_unused) {
				...
				// can't do this when saving a copy, since it destroys the zip
				// entry
				zipStream->CloseEntry();
...
Package::FileInfos::iterator Package::addFile(const String &name) {
	return files.insert(make_pair(normalize_internal_filename(name),
								  FileInfo())).first;
}
...
template <>
void Reader::handle(PackageDependency &dep) {
	if (!isCompound()) {
		handle(dep.package);
...
template <>
void Writer::handle(const PackageDependency &dep) {
...
Packaged::Packaged() : position_hint(100000), fully_loaded(true) {}
...
template <>
void Reader::handle(JustAsPackageProxy &object) {
...
void Packaged::open(const String &package, bool just_header) {
	...
	if (just_header) {
		...
		try {
			...
			throw FileParseError(err.what(),
								 absoluteFilename() + _("/") +
									 typeName()); // more detailed message
		}
	} else {
		loadFully();
	}
...
void Packaged::loadFully() {
	...
	try {
		...
		fully_loaded = true; // only after loading and validating succeeded, be
...
void Packaged::requireDependency(Packaged *package) {
		...
		if (dep->package == n) {
			if (package->version < dep->version) {
				queue_message(MESSAGE_WARNING,
							  _ERROR_3_("package out of date", n,
										package->version.toString(),
										dep->version.toString()));
			} else if (package->compatible_version > dep->version) {
				queue_message(
					MESSAGE_WARNING,
					_ERROR_4_("package too new", n, package->version.toString(),
							  dep->version.toString(), relativeFilename()));
			} else {
				return; // ok
			}
		}
	...
	queue_message(MESSAGE_WARNING, _ERROR_4_("dependency not given", name(),
...
IMPLEMENT_REFLECTION(IncludePackage) { REFLECT_BASE(Packaged); }
