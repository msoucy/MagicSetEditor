...
void Package::open(const String &n, bool fast) {
    ...
    if (wxDirExists(filename)) {
        ...
        throw PackageNotFoundError((L"Package not found: '") + filename +
                                   (L"'"));
    }
}
...
void Package::referenceFile(const String &file) {
    ...
    if (it == files.end())
        throw InternalError((L"referencing a nonexistant file"));
    it->second.keep = true;
}
...
InputStreamP Package::openAbsoluteFile(const String &name) {
    ...
    if (pos == String::npos) {
        ...
        if (!f->IsOk())
            throw FileNotFoundError((L"<unknown>"), name);
        return f;
    } else {
    ...
    if (!fn.empty() && clipboard_package()) {
        ...
        try {
            LocalFileName local_name = clipboard_package()->newFileName(
                prefix, (L"")); // a new unique name in the package, assume it's
                                // an image
...
void Package::saveToDirectory(const String &saveAs, bool remove_unused,
    ...
    for (auto &f : files) {
        if (!f.second.keep && remove_unused) {
            ...
            if (!(is_copy
                      ? wxCopyFile(f.second.tempName, saveAs + (L"/") + f.first)
            ...
            if (!wxCopyFile(filename + (L"/") + f.first,
                            saveAs + (L"/") + f.first)) {
                throw PackageError(_ERROR_("unable to store file"));
            }
            vcs->addFile(saveAs + (L"/") + f.first);
...
Package::FileInfos::iterator Package::addFile(const String &name) {
    return files.insert(
                    make_pair(normalize_internal_filename(name), FileInfo()))
        .first;
}
