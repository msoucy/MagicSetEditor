...
/// package that will contain the thing being read from the clipboard.
...
class LocalFileName {
  public:
    ...
    static LocalFileName fromReadString(const String &,
                                        const String &prefix = (L"image"),
                                        String const &suffix = (L""));
...
class Packaged : public Package {
  public:
    ...
    inline bool isFullyLoaded() const { return fully_loaded; }
...
inline void Package::readFile(const String &filename, T &obj) {
    ...
    Reader reader(*stream, dynamic_cast<Packaged *>(this),
                  absoluteFilename() + (L"/") + filename);
    try {
        reader.handle_greedy(obj);
    } catch (const ParseError &err) {
        throw FileParseError(err.what(), absoluteFilename() + (L"/") +
