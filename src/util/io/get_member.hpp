...
class GetDefaultMember {
  public:
	...
	template <typename T>
	void handle(const Defaultable<T> &);
	template <typename T>
	void handle(const Scriptable<T> &);
	template <typename T>
	void handle(const vector<T> &c) {
		value = to_script(&c);
	}
	template <typename K, typename V>
	void handle(const map<K, V> &c) {
		value = to_script(&c);
	}
	template <typename K, typename V>
	void handle(const IndexMap<K, V> &c) {
		value = to_script(&c);
	}
	template <typename K, typename V>
	void handle(const DelayedIndexMaps<K, V> &) {}
	template <typename K, typename V>
	void handle(const DelayedIndexMapsData<K, V> &c);
	template <typename T>
	void handle(const intrusive_ptr<T> &p) {
		value = to_script(p);
	}
...
class GetMember {
  public:
	...
	template <typename T>
	void handle(const Char *name, const T &object) {
		if (!gdm.result() && canoncial_name_compare(target_name, name)) {
			gdm.handle(object);
		}
	}
	/// Don't handle a value
	...
	template <typename K, typename V>
	void handle(const IndexMap<K, V> &m) {
		if (gdm.result())
			return;
		for (typename IndexMap<K, V>::const_iterator it = m.begin();
			 it != m.end(); ++it) {
	...
	void handle(const DelayedIndexMaps<K, V> &);
	template <typename K, typename V>
	void handle(const DelayedIndexMapsData<K, V> &);
...
#define REFLECT_OBJECT_GET_DEFAULT_MEMBER(Cls)                                 \
	REFLECT_WRITE_YES(Cls, GetDefaultMember)
#define REFLECT_OBJECT_GET_MEMBER(Cls) REFLECT_WRITE_YES(Cls, GetMember)
#define REFLECT_OBJECT_GET_DEFAULT_MEMBER_NOT(Cls)                             \
	REFLECT_WRITE_NO(Cls, GetDefaultMember)
#define REFLECT_OBJECT_GET_MEMBER_NOT(Cls) REFLECT_WRITE_NO(Cls, GetMember)

#define REFLECT_WRITE_YES(Cls, Reflector)                                      \
	template <>                                                                \
	void Reflector::handle<Cls>(const Cls &object) {                           \
		const_cast<Cls &>(object).reflect(*this);                              \
	}                                                                          \
	void Cls::reflect(Reflector &reflector) { reflect_impl(reflector); }
...
#define REFLECT_ENUM_GET_MEMBER(Enum)                                          \
	template <>                                                                \
	void GetDefaultMember::handle<Enum>(const Enum &enum_) {                   \
...
class EnumGetMember {
  public:
	inline EnumGetMember(GetDefaultMember &gdm) : gdm(gdm) {}
