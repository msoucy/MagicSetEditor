...
class PackageVersionData : public IntrusivePtrVirtualBase {
  public:
	...
	bool    hidden;						///< Not shown in the package list
(installed automatically as a dependency)

	PackageVersionP base_version;		///< The locally installed version (if
installed)
	PackageVersionP local_version;		///< Modifications made to the locally
installed version?
	PackageVersionP remote_version;		///< The version available from the
server (if available)
	bool global;						///< The installed package is in the
global location, not the user-local one
	bool new_global;					///<

	bool source_local;					///< Is the source
	String source;						///< Where can the package be
downloaded/found?
...
class UpdateData {
	vector<PackageDependencyP> packages;         ///<
package/latest-version-number pairs
...
class PackageDirectory {
  ...
  private:
	...
	bool actual_install(const InstallablePackage &package,
  ...
  public:
	...
	intrusive_ptr<T> open(const String &name) {
		...
		if (typedP) {
			...
			throw InternalError(
				format_string(_("Package %s loaded as wrong type"), name));
		}
	}

	/// Open a package with the specified name, the type of package is
	 ...
	 *    - verifies a dependency from that package if an absolute filename is
	 * used
	...
	/** WARNING: this is a bit of a hack, since not all package types support
	 * names in this way.
	 *  It is needed for third party libraries (i.e. hunspell) that load stuff
	 * from files.
	 */
	...
	bool checkDependency(const PackageDependency &dep,
...
class PackageVersion : public IntrusivePtrBase<PackageVersion> {
  public:
	...
	enum Status {
		...
		,
		STATUS_BLESSED = 0x10 ///< Package is an official version, installed
							  ///with the installer
		,
  ...
  public:
	...
	enum FileStatus { FILE_UNCHANGED, FILE_MODIFIED, FILE_ADDED, FILE_DELETED };
	/// Information on files in this package
	struct FileInfo {
		inline FileInfo() {}
		inline FileInfo(const String &file, const DateTime &time,
						FileStatus status)
