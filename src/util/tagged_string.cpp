	...
	for (size_t i = 0; i < str.size(); ++i) {
		...
		if (is_substr(str, i, _("</>"))) {
			...
			if (c == _('<')) {
				...
				intag = false;
				if (!starts_with(tags.top(), _("kw")) &&
					!starts_with(tags.top(), _("atom"))) {
		...
		if (c == _('<')) {
			if (is_substr(str, pos + 1,
				...
				if (pos < start)
					last_start = pos;
				++taglevel;
			} else if (pos + 2 < size && str.GetChar(pos + 1) == _('/') &&
					   is_substr(str, pos + 2,
								 static_cast<const Char *>(tag.c_str()) + 1)) {
				--taglevel; // close tag
			}
...
String close_tag(const String &tag) {
	if (tag.size() < 1)
		...
		return _("</") + tag.substr(1);
}

String anti_tag(const String &tag) {
	if (!tag.empty() && tag.GetChar(0) == _('/'))
		return _("<") + tag.substr(1) + _(">");
	else
		return _("</") + tag + _(">");
}
	...
	for (size_t i = 0; i < str.size();) {
		...
		if (c == _('<')) {
			...
			if (is_substr(str, i, _("<atom")) || is_substr(str, i, _("<sep"))) {
				...
				has_width = false;
			} else if (is_substr(str, i, _("<suffix")) &&
...
String get_tags(const String &str, size_t start, size_t end, bool open_tags,
	...
	for (size_t i = start; i < end; ++i) {
		...
		if (c == _('<') && !intag) {
			...
			if (i + 1 < end &&
				(str.GetChar(i + 1) == _('/') ? close_tags : open_tags)) {
				keeptag = true;
			}
		}
		if (intag && keeptag)
...
bool add_or_cancel_tag(const String &tag, String &stack, bool all = false) {
	if (all || starts_with(tag, _("/")) || starts_with(tag, _("b")) ||
		starts_with(tag, _("i")) || starts_with(tag, _("sym"))) {
...
String simplify_tagged_overlap(const String &str) {
	...
	for (size_t i = 0; i < size; ++i) {
		...
		if (c == _('<')) {
			String tag = tag_at(str, i);
			if (starts_with(tag, _("b")) || starts_with(tag, _("i")) ||
				starts_with(tag, _("sym")) || starts_with(tag, _("/b")) ||
				starts_with(tag, _("/i")) || starts_with(tag, _("/sym"))) {
				// optimize this tag
		...
		if (str.GetChar(i) == _('<')) {
			...
			if (end == String::npos) {
				queue_message(MESSAGE_WARNING,
							  _("Invalid tagged string: missing '>'"));
			}
			for (size_t j = i + 1; j + 1 < end; ++j) {
				Char c = str.GetChar(j);
				if (c == _(' ') || c == _('<')) {
					queue_message(MESSAGE_WARNING,
								  _("Invalid character in tag"));
				}
			}
			if (check_balance) {
	...
	for (Char c : str) {
		if (c == _('\'') || c == LEFT_SINGLE_QUOTE || c == RIGHT_SINGLE_QUOTE) {
			c = curl ? (open ? LEFT_SINGLE_QUOTE : RIGHT_SINGLE_QUOTE)
					 : _('\'');
		} else if (c == _('\"') || c == LEFT_DOUBLE_QUOTE ||
