...
class Action {
  public:
     ...
     *
     *  Perform will only ever be called alternatingly with to_undo =
     * true/false,
 ...
 *
 *  This class also takes on the role of Observable, ActionListeners can
 * register themselfs.
...
class ActionStack {
  public:
    ...
    /** Tells all listeners about the action.
      ...
      * If allow_merge == true then we attempt to merge this action with
     * previous ones
      */
  ...
  private:
    ...
    /// Point at which the file was saved, corresponds to the top of the undo
    /// stack at that point
    Action *save_point;
 ...
 */
#define TYPE_CASE_(variable, Type) if (dynamic_cast<const Type *>(&variable))
...
#define TYPE_CASE(variable, Type)                                              \
    ...
    if (Type##variable.first)                                                  \
        for (const Type &variable = *Type##variable.first;                     \
             Type##variable.second; Type##variable.second = false)
