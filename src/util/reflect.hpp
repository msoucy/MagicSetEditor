 ...
 *
 *  @brief Reflection of classes, currently reflection is used for
 *(de)serialization.
 */
...
/** Reflection allows the member variables of a class to be inspected at
 * runtime.
 */
#define DECLARE_REFLECTION()                                                   \
  protected:                                                                   \
	template <class Reflector>                                                 \
	void reflect_impl(Reflector &reflector);                                   \
	friend class Reader;                                                       \
	friend class Writer;                                                       \
...
/// Declare that a class supports reflection, which can be overridden in derived
/// classes
...
/// Implement the refelection of a class type Cls that only uses
/// REFLECT_NAMELESS
...
/// Implement the refelection of a class type Cls, but only for Reader and
/// Writer,
...
#define REFLECT_IF_READING_COMPOUND_OR(cond)                                   \
	if (reflector.isReading() ? reflector.isCompound() : cond)

/// Add an alias for backwards compatibility
/** If a key 'name' is encountered in the input file, it is interpreted as 'var'
 * for versions < version
 *  Example:
 *  @code
 *   REFLECT_ALIAS(<300, "style", stylesheet) // prior to 0.3.0 style was used
 * instead of stylesheet
 *  @encode
 */
#define REFLECT_COMPAT(cond, name, var)                                        \
	if (reflector.formatVersion() cond)                                        \
	REFLECT_N(name, var)

/// Ignore things for backwards compatibility for versions < 'version'
#define REFLECT_COMPAT_IGNORE(cond, name, Type)                                \
	if (reflector.formatVersion() cond) {                                      \
		Type ignored;                                                          \
		REFLECT_N(name, ignored);                                              \
	}
