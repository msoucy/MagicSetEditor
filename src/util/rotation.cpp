...
Rotation::Rotation(Radians angle, const RealRect &rect, double zoom,
				   double stretch, RotationFlags flags)
	: angle(constrain_radians(angle)), size(rect.size()),
	  origin(rect.position()), zoomX(zoom * stretch), zoomY(zoom) {
	if (stretch != 1.0) {
		size.width /= stretch;
	}
	// set origin
...
RealPoint Rotation::tr(const RealPoint &p) const {
	...
	return RealPoint(c * x + s * y + origin.x, -s * x + c * y + origin.y);
}
...
RealRect Rotation::trRectToBB(const RealRect &r) const {
	...
	if (special_case_optimization && is_rad0(angle)) {
		...
		double s = sin(angle), c = cos(angle);
		RealRect result(c * x + s * y + origin.x, -s * x + c * y + origin.y, 0,
						0);
		if (c > 0) {
			result.width += c * w;
			result.height += c * h;
...
RealPoint Rotation::trInv(const RealPoint &p) const {
	...
	return RealPoint((c * x - s * y) / zoomX, (s * x + c * y) / zoomY);
}
...
Rotater::Rotater(Rotation &rot, const Rotation &by) : old(rot), rot(rot) {
	// apply rotation
...
RotatedDC::RotatedDC(DC &dc, Radians angle, const RealRect &rect, double zoom,
					 RenderQuality quality, RotationFlags flags)
...
void RotatedDC::DrawText(const String &text, const RealPoint &pos,
						 int blur_radius, int boldness, double stretch_) {
	DrawText(text, pos, dc.GetTextForeground(), blur_radius, boldness,
			 stretch_);
}

void RotatedDC::DrawText(const String &text, const RealPoint &pos, AColor color,
						 int blur_radius, int boldness, double stretch_) {
	if (text.empty())
		return;
	if (color.alpha == 0)
	...
	if (quality >= QUALITY_AA) {
		...
		draw_resampled_text(dc, pos2, r_ext, stretch_, angle, color, text,
							blur_radius, boldness);
...
void RotatedDC::DrawTextWithShadow(const String &text, const Font &font,
								   const RealPoint &pos, double scale,
								   double stretch) {
	DrawText(text, pos + font.shadow_displacement * scale, font.shadow_color,
			 font.shadow_blur * scale, 1, stretch);
...
void RotatedDC::DrawImage(const Image &image, const RealPoint &pos,
						  ImageCombine combine) {
	Image rotated = rotate_image(image, angle);
	DrawPreRotatedImage(rotated, RealRect(pos, trInvS(RealSize(image))),
						combine);
}
void RotatedDC::DrawPreRotatedBitmap(const Bitmap &bitmap,
									 const RealRect &rect) {
	RealPoint p_ext = tr(rect.position()) + boundingBoxCorner(rect.size());
	dc.DrawBitmap(bitmap, to_int(p_ext.x), to_int(p_ext.y), true);
}
void RotatedDC::DrawPreRotatedImage(const Image &image, const RealRect &rect,
									ImageCombine combine) {
	RealPoint p_ext = tr(rect.position()) + boundingBoxCorner(rect.size());
...
void RotatedDC::DrawRoundedRectangle(const RealRect &r, double radius) {
	if (is_straight(angle)) {
		wxRect r_ext = trRectToBB(r);
		dc.DrawRoundedRectangle(r_ext.x, r_ext.y, r_ext.width, r_ext.height,
								trS(radius));
	} else {
		// TODO
		DrawRectangle(r);
...
void RotatedDC::DrawEllipticArc(const RealPoint &center, const RealSize &size,
								Radians start, Radians end) {
	wxPoint c_ext = tr(center - size / 2);
	wxSize s_ext = trSizeToBB(size);
	dc.DrawEllipticArc(c_ext.x, c_ext.y, s_ext.x, s_ext.y,
					   rad_to_deg(start + angle), rad_to_deg(end + angle));
}
void RotatedDC::DrawEllipticSpoke(const RealPoint &center, const RealSize &size,
								  Radians angle) {
	wxPoint c_ext = tr(center - size / 2);
	wxSize s_ext = trSizeToBB(size);
	Radians rot_angle = angle + this->angle;
	Radians sin_angle = sin(rot_angle), cos_angle = cos(rot_angle);
	// position of center and of point on the boundary can vary because of
	// rounding errors,
...
void RotatedDC::SetTextForeground(const Color &color) {
	dc.SetTextForeground(color);
}
void RotatedDC::SetLogicalFunction(wxRasterOperationMode function) {
...
void RotatedDC::SetFont(const Font &font, double scale) {
	dc.SetFont(font.toWxFont(trS(scale) *
							 (quality == QUALITY_LOW ? 1 : text_scaling)));
}
...
void RotatedDC::DestroyClippingRegion() { dc.DestroyClippingRegion(); }
