...
class Vector2D {
  public:
    ...
    inline Vector2D(double x, double y) : x(x), y(y) {}
    ...
    inline Vector2D operator*(double r) const { return Vector2D(x * r, y * r); }
    inline Vector2D operator*(int r) const { return Vector2D(x * r, y * r); }
    /// Multiply with a scalar
    ...
    inline Vector2D mul(Vector2D p2) { return Vector2D(x * p2.x, y * p2.y); }
    /// Piecewise division
    inline Vector2D div(Vector2D p2) { return Vector2D(x / p2.x, y / p2.y); }

    /// Returns the square of the length of this vector
    inline double lengthSqr() const { return x * x + y * y; }
    /// Returns the length of this vector
    inline double length() const { return sqrt(lengthSqr()); }
    /// Returns a normalized version of this vector
    /** i.e. length() == 1 */
    inline Vector2D normalized() const { return *this / length(); }
    /// Angle between this vector and the x axis
    inline Radians angle() const { return atan2(y, x); }
...
inline Vector2D piecewise_max(const Vector2D &a, const Vector2D &b) {
    return Vector2D(a.x < b.x ? b.x : a.x, a.y < b.y ? b.y : a.y);
}
...
class Matrix2D {
  public:
    ...
    inline Matrix2D(double a, double b, double c, double d)
        : mx(a, b), my(c, d) {}
};
