...
void filter_symbol(Image &symbol, const SymbolFilter &filter) {
    ...
    // HACK: wxGTK seems to fail sometimes if you ask it to allocate the alpha
    // channel.
    //       This manually allocates the memory and gives it to the image to
    //       handle.
    if (!alpha) {
    ...
    for (UInt y = 0; y < height; ++y) {
        for (UInt x = 0; x < width; ++x) {
            ...
            if (data[0] != data[2]) {
                ...
                SymbolSet point =
                    data[1] ? (data[0] ? SYMBOL_BORDER : SYMBOL_OUTSIDE)
                            : SYMBOL_INSIDE;
                // Call filter
                AColor result =
                    filter.color((double)x / width, (double)y / height, point);
                // Store color
                data[0] = result.Red();
                data[1] = result.Green();
                data[2] = result.Blue();
...
Image render_symbol(const SymbolP &symbol, const SymbolFilter &filter,
                    double border_radius, int width, int height,
                    bool edit_hints, bool allow_smaller) {
...
template <>
void GetMember::handle(const intrusive_ptr<SymbolFilter> &f) {
    handle(*f);
}
...
intrusive_ptr<SymbolFilter> read_new<SymbolFilter>(Reader &reader) {
    ...
    if (fill_type == (L"solid"))
        return intrusive(new SolidFillSymbolFilter);
    else if (fill_type == (L"linear gradient"))
        return intrusive(new LinearGradientSymbolFilter);
    else if (fill_type == (L"radial gradient"))
        return intrusive(new RadialGradientSymbolFilter);
...
AColor SolidFillSymbolFilter::color(double x, double y, SymbolSet point) const {
    if (point == SYMBOL_INSIDE)
        return fill_color;
    else if (point == SYMBOL_BORDER)
        return border_color;
    else
...
bool SolidFillSymbolFilter::operator==(const SymbolFilter &that) const {
    const SolidFillSymbolFilter *that2 =
        dynamic_cast<const SolidFillSymbolFilter *>(&that);
...
template <typename T>
AColor GradientSymbolFilter::color(double x, double y, SymbolSet point,
                                   const T *t) const {
    if (point == SYMBOL_INSIDE)
        return lerp(fill_color_1, fill_color_2, t->t(x, y));
    else if (point == SYMBOL_BORDER)
        return lerp(border_color_1, border_color_2, t->t(x, y));
    else
        return AColor(0, 0, 0, 0);
}
...
String LinearGradientSymbolFilter::fillType() const {
    return (L"linear gradient");
}
...
LinearGradientSymbolFilter::LinearGradientSymbolFilter(
    ...
    const Color &fill_color_2, const Color &border_color_2, double center_x,
    double center_y, double end_x, double end_y)
    : GradientSymbolFilter(fill_color_1, border_color_1, fill_color_2,
                           border_color_2),
      center_x(center_x), center_y(center_y), end_x(end_x), end_y(end_y) {}

AColor LinearGradientSymbolFilter::color(double x, double y,
                                         SymbolSet point) const {
    len = sqr(end_x - center_x) + sqr(end_y - center_y);
    if (len == 0)
        len = 1; // prevent div by 0
    return GradientSymbolFilter::color(x, y, point, this);
}

double LinearGradientSymbolFilter::t(double x, double y) const {
    double t = fabs((x - center_x) * (end_x - center_x) +
                    (y - center_y) * (end_y - center_y)) /
               len;
    return min(1., max(0., t));
}

bool LinearGradientSymbolFilter::operator==(const SymbolFilter &that) const {
    const LinearGradientSymbolFilter *that2 =
        dynamic_cast<const LinearGradientSymbolFilter *>(&that);
    return that2 && equal(*that2) && center_x == that2->center_x &&
           end_x == that2->end_x && center_y == that2->center_y &&
           end_y == that2->end_y;
}
...
String RadialGradientSymbolFilter::fillType() const {
    return (L"radial gradient");
}

AColor RadialGradientSymbolFilter::color(double x, double y,
                                         SymbolSet point) const {
    return GradientSymbolFilter::color(x, y, point, this);
}
...
bool RadialGradientSymbolFilter::operator==(const SymbolFilter &that) const {
    const RadialGradientSymbolFilter *that2 =
        dynamic_cast<const RadialGradientSymbolFilter *>(&that);
