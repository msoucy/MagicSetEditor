...
Image render_symbol(const SymbolP &symbol, double border_radius = 0.05,
                    int width = 100, int height = 100,
                    bool editing_hints = false, bool allow_smaller = false);
...
class SymbolViewer : public SymbolView {
  public:
    // --------------------------------------------------- : Data
    SymbolViewer(const SymbolP &symbol, bool editing_hints, double size = 500,
                 double border_radius = 0.05);
    ...
    Rotation
        rotation; ///< Object that handles rotation, scaling and translation
  ...
  private:
    ...
    void combineSymbolPart(DC &dc, const SymbolPart &part,
                           bool &paintedSomething, bool &buffersFilled,
                           bool allow_overlap, MemoryDCP &borderDC,
                           MemoryDCP &interiorDC);

    /// Combines a symbol part with what is currently drawn, the border and
    /// interior are drawn separatly
    /** directB/directI are true if the border/interior is the screen dc, false
     * if it
     *  is a temporary 1 bit one
     */
    void combineSymbolShape(const SymbolShape &part, DC &border, DC &interior,
                            bool directB, bool directI);

    /// Draw a symbol part, draws the border and the interior to separate DCs
    /** The DCs may be null. directB should be true when drawing the border
     * directly to the screen.
     *  The **Col parameters give the color to use for the (interior of) the
     * border and the interior
     *  default should be white (255) border and black (0) interior.
     */
    void drawSymbolShape(const SymbolShape &shape, DC *border, DC *interior,
                         unsigned char borderCol, unsigned char interiorCol,
                         bool directB, bool oppB);
/*	
