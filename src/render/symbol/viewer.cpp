...
Image render_symbol(const SymbolP &symbol, double border_radius, int width,
					int height, bool editing_hints, bool allow_smaller) {
...
SymbolViewer::SymbolViewer(const SymbolP &symbol, bool editing_hints,
						   double size, double border_radius)
	: border_radius(border_radius), editing_hints(editing_hints),
	  rotation(0, RealRect(0, 0, size, size), size), multiply(size, 0, 0, size),
	  origin(0, 0), in_symmetry(0) {
	setSymbol(symbol);
}
...
void combineBuffers(DC &dc, DC *borders, DC *interior) {
	wxSize s = dc.GetSize();
	if (borders)
		dc.Blit(0, 0, s.GetWidth(), s.GetHeight(), borders, 0, 0, wxOR);
	if (interior)
		dc.Blit(0, 0, s.GetWidth(), s.GetHeight(), interior, 0, 0,
				wxAND_INVERT);
}
...
void SymbolViewer::draw(DC &dc) {
	...
	// Draw all parts
	combineSymbolPart(dc, *symbol, paintedSomething, buffersFilled, true,
					  borderDC, interiorDC);
...
void SymbolViewer::combineSymbolPart(DC &dc, const SymbolPart &part,
									 bool &paintedSomething,
									 bool &buffersFilled, bool allow_overlap,
									 MemoryDCP &borderDC,
									 MemoryDCP &interiorDC) {
	if (const SymbolShape *s = part.isSymbolShape()) {
		if (s->combine == SYMBOL_COMBINE_OVERLAP && buffersFilled &&
			allow_overlap) {
			// We will be overlapping some previous parts, write them to the
			// screen
			combineBuffers(dc, borderDC.get(), interiorDC.get());
			// Clear the buffers
			buffersFilled = false;
		...
		// Draw all parts, in reverse order (bottom to top), also draw rotated
		// copies
		...
		FOR_EACH_CONST_REVERSE(p, s->parts) {
			...
			for (int i = copies - 1; i >= 0; --i) {
				...
				if (s->kind == SYMMETRY_ROTATION || i % 2 == 0) {
					...
					//  = [ cos(a+b)  sin(a+b) !
					//    ! sin(a+b) -cos(a+b) ]
					Matrix2D rot(cos(a + b), sin(a + b), sin(a + b),
								 -cos(a + b));
					multiply = rot * old_m;
					origin = old_o + (s->center - s->center * rot) * old_m;
				}
				...
				combineSymbolPart(dc, *p, paintedSomething, buffersFilled,
								  allow_overlap && i == copies - 1, borderDC,
								  interiorDC);
			}
		}
		multiply = old_m;
...
void SymbolViewer::combineSymbolShape(const SymbolShape &shape, DC &border,
									  DC &interior, bool directB,
									  bool directI) {
	...
	case SYMBOL_COMBINE_MERGE: {
		drawSymbolShape(shape, &border, &interior, 255, interiorCol, directB,
						false);
		break;
	}
	case SYMBOL_COMBINE_SUBTRACT: {
		border.SetLogicalFunction(wxAND);
		drawSymbolShape(shape, &border, &interior, 0, ~interiorCol, directB,
						false);
	...
	case SYMBOL_COMBINE_INTERSECTION: {
		...
		drawSymbolShape(shape, keepBorder.get(), keepInterior.get(), 255, 255,
						false, false);
		// combine the temporary dcs with the result using the AND operator
		wxSize s = border.GetSize();
		border.Blit(0, 0, s.GetWidth(), s.GetHeight(), &*keepBorder, 0, 0,
					wxAND);
		interior.Blit(0, 0, s.GetWidth(), s.GetHeight(), &*keepInterior, 0, 0,
					  wxAND);
		break;
	}
...
void SymbolViewer::drawSymbolShape(const SymbolShape &shape, DC *border,
								   DC *interior, Byte borderCol,
								   Byte interiorCol, bool directB, bool clear) {
	// create point list
	vector<wxPoint> points;
	size_t size = shape.points.size();
	for (size_t i = 0; i < size; ++i) {
		segment_subdivide(*shape.getPoint((int)i), *shape.getPoint((int)i + 1),
						  origin, multiply, points);
	}
	// draw border
	if (border && border_radius > 0) {
		// white/black or, if directB white/green
		border->SetBrush(Color(borderCol,
							   (directB && borderCol == 0 ? 128 : borderCol),
							   borderCol));
...
void SymbolViewer::highlightPart(DC &dc, const SymbolPart &part,
...
void SymbolViewer::highlightPart(DC &dc, const SymbolShape &shape,
								 HighlightStyle style) {
	if (style == HIGHLIGHT_LESS)
		return;
	// create point list
	vector<wxPoint> points;
	size_t size = shape.points.size();
	...
	if (style == HIGHLIGHT_BORDER) {
		...
		dc.DrawPolygon((int)points.size(), &points[0]);
		if (shape.combine == SYMBOL_COMBINE_SUBTRACT ||
			shape.combine == SYMBOL_COMBINE_BORDER) {
...
void SymbolViewer::highlightPart(DC &dc, const SymbolSymmetry &sym,
	...
	// Color?
	Color color = style & HIGHLIGHT_BORDER
					  ? Color(255, 100, 0)
	...
	int copies =
		sym.kind == SYMMETRY_REFLECTION ? sym.copies / 2 * 2 : sym.copies;
	...
	dc.DrawCircle(int(center.x), int(center.y),
				  sym.kind == SYMMETRY_ROTATION ? 7 : 5);
}

void SymbolViewer::highlightPart(DC &dc, const SymbolGroup &group,
								 HighlightStyle style) {
	if (style == HIGHLIGHT_BORDER) {
		dc.SetBrush(*wxTRANSPARENT_BRUSH);
		dc.SetPen(wxPen(Color(255, 0, 0), 2));
