...
class DataViewer : public SetView {
  ...
  protected:
	...
	void setStyles(const StyleSheetP &stylesheet,
				   IndexMap<FieldP, StyleP> &styles,
				   IndexMap<FieldP, StyleP> *extra_styles = nullptr);
	/// Set the data to be shown in the viewers, refresh them
	void setData(IndexMap<FieldP, ValueP> &values,
				 IndexMap<FieldP, ValueP> *extra_values = nullptr);
	...
	vector<ValueViewerP>
		viewers; ///< The viewers for the different values in the data
