...
struct TextViewer::Line {
	...
	vector<double> positions; ///< x position of each character in this line,
	/// gives the number of characters + 1, never empty
	...
	bool justifying;	   ///< Is the text justified? Only true when *really*
	/// justifying.
	...
	double width() const {
		return positions[end_or_soft - start] - positions.front();
	}
	...
	/// Align the contents of this line *horizontally* inside the given
	/// rectangle
	void alignHorizontal(const vector<CharInfo> &chars, const TextStyle &style,
						 const RealRect &s);
};
...
size_t TextViewer::Line::posToIndex(double x) const {
	...
	vector<double>::const_iterator it2 =
		lower_bound(positions.begin(), positions.end(), x);
	if (it2 == positions.begin())
		return start;
	if (it2 == positions.end())
		--it2; // we don't want to find the position beyond the end
	if (it2 == positions.begin())
		return start;
	// first index with pos > x
	vector<double>::const_iterator it1 = it2 - 1;
	if (x - *it1 <= *it2 - x)
		return it1 - positions.begin() + start; // it1 is closer
	else
		return it2 - positions.begin() + start; // it2 is closer
}
		...
		if (l.visible(dc)) {
			if (l.justifying) {
				...
				for (size_t i = 0; i < l.positions.size() - 1; ++i) {
					RealRect rect(l.positions[i], l.top,
								  l.positions[i + 1] - l.positions[i],
								  l.line_height);
					elements.draw(dc, scale, rect, &l.positions[i], what,
								  l.start + i, l.start + i + 1);
				}
			} else {
				RealRect rect(l.positions.front(), l.top, l.width(),
							  l.line_height);
				elements.draw(dc, scale, rect, &*l.positions.begin(), what,
							  l.start, l.end());
			}
		}
	}
...
void TextViewer::drawSelection(RotatedDC &dc, const TextStyle &style,
...
RealRect TextViewer::Line::selectionRectangle(const Rotation &rot,
											  size_t sel_start,
											  size_t sel_end) {
...
void TextViewer::drawSeparators(RotatedDC &dc) {
	...
	for (auto &l : lines) {
		...
		if (separator && l.visible(dc)) {
			...
			y = (y + l.top) / 2;
			dc.DrawLine(RealPoint(0, y),
						RealPoint(dc.getInternalRect().width, y));
		}
...
bool TextViewer::prepare(RotatedDC &dc, const String &text, TextStyle &style,
						 Context &ctx) {
	if (!prepared()) {
		// not prepared yet
...
bool TextViewer::prepared() const { return !lines.empty(); }
...
const TextViewer::Line &TextViewer::findLine(size_t index) const {
	assert(!lines.empty());
	for (const TextViewer::Line &l : lines) {
		if (l.end() >= index)
			return l;
	}
	return lines.front();
...
size_t TextViewer::moveLine(size_t index, int delta) const {
	...
	if (line2 >= &lines.front() && line2 <= &lines.back()) {
		...
		if (idx < 0 || idx >= line1->positions.size())
			return index; // can't move
		return line2->posToIndex(
			line1->positions[idx]); // character at the same position
	} else {
		return index; // can't move
	}
}
...
struct CompareTop {
	...
	inline bool operator()(const TextViewer::Line &a, double b) const {
		return a.top < b;
	}
	inline bool operator()(double a, const TextViewer::Line &b) const {
		return a < b.top;
	}
	inline bool operator()(const TextViewer::Line &a,
						   const TextViewer::Line &b) const {
		return a.top < b.top;
	}
};
size_t TextViewer::indexAt(const RealPoint &pos) const {
	...
	vector<Line>::const_iterator l =
		lower_bound(lines.begin(), lines.end(), pos.y, CompareTop());
	if (l != lines.begin())
		l--;
	assert(l != lines.end());
	// 2. find char on line
...
RealRect TextViewer::charRect(size_t index, bool first) const {
	...
	if (pos + 1 >= l.positions.size()) {
		if (!first && &l < &lines.back()) {
			...
			const Line &l2 = *(&l + 1);
			if (index == l2.start) {
				return RealRect(l2.positions.front(), l2.top, 0,
								l2.line_height);
			}
		}
		return RealRect(l.positions.back(), l.top, 0, l.line_height);
	} else {
		return RealRect(l.positions[pos], l.top,
						l.positions[pos + 1] - l.positions[pos], l.line_height);
	}
}
...
size_t TextViewer::firstVisibleChar(size_t index, int delta) const {
	...
	while (true) {
		...
		while (
			index == l->end() ||
			(pos + delta >= 0 && (size_t)pos + delta < l->positions.size())) {
			if (index == l->end() ||
				l->positions[pos + 1] - l->positions[pos] > 0.0001) {
				return index;
			}
			pos += delta;
			index += delta;
		...
		if (l + delta < &lines.front())
			return 0;
		if (l + delta > &lines.back())
			return l->end();
		index += delta;
		l += delta;
	}
...
size_t TextViewer::lineCount() const { return lines.size(); }
size_t TextViewer::visibleLineCount(double height) const {
	size_t count = 0;
	for (auto const &l : lines) {
		if (l.top + l.line_height > height)
			return count;
		if (l.top >= 0)
			++count;
	}
...
void TextViewer::scrollTo(size_t line_id) { scrollBy(-lines.at(line_id).top); }
...
bool TextViewer::ensureVisible(double height, size_t char_id) {
	...
	if (line.top < 0) {
		...
		for (auto &l : lines) {
			if (l.top > 0)
				scrollBy(-l.line_height); // scroll down a single line ...
			if (line.bottom() <= height)
				break; // ... until we can see the current line
		}
		return true;
	} else {
...
void TextViewer::prepareElements(const String &text, const TextStyle &style,
								 Context &ctx) {
...
void TextViewer::prepareLines(RotatedDC &dc, const String &text,
							  TextStyle &style, Context &ctx) {
	vector<CharInfo> chars;
	prepareLinesTryScales(dc, text, style, chars);
	assert(!lines.empty());

	// store information about the content/layout, allow this to change
	// alignment
	...
	for (auto const &l : reverse(lines)) {
		style.content_height = l.top + l.line_height;
		if (l.line_height)
			break; // not an empty line
	}
	...
	if (lines.size() == 1 && lines[0].width() < 0.0001) {
		if (style.always_symbol && style.symbol_font.valid()) {
			lines[0].line_height =
				style.symbol_font.font->defaultSymbolSize(
											style.symbol_font.size).height;
		} else {
	...
	// HACK : fix empty first line before <line>, do this after align, so layout
	// is not affected
...
inline double bound_on_max_scale(RotatedDC &dc, const TextStyle &style,
								 const vector<TextViewer::Line> &lines,
								 double scale) {
	if (lines.empty())
		return 1.0;
	double tot_height = dc.getInternalSize().height + 1;
	double height =
		min(tot_height, lines.back().bottom() + style.padding_bottom);
	if (height < 1)
		return 1.0;
	return scale * tot_height / height;
}
// bound on min_scale, given that scale doesn't fit and produces the given lines
inline double bound_on_min_scale(RotatedDC &dc, const TextStyle &style,
								 const vector<TextViewer::Line> &lines,
								 double scale) {
	if (lines.empty())
		return 0.0;
...
void TextViewer::prepareLinesTryScales(RotatedDC &dc, const String &text,
	...
	// Assumption:
	//    It is likely that the text should have the same scale as the previous
	//    render attempt
	//    So:
	...
	//               - if it doesn't, we have just (almost) wasted 1 cycle,
	//               start binary search
	//       - if it doesn't
	//           - change max_scale
	...
	if (fits) {
		...
		if (fits) {
			...
			max_scale =
				min(max_scale, bound_on_max_scale(dc, style, lines, scale));
		} else {
			// yay
			scale = min_scale;
			return;
	...
	while (min_scale + scale_step < max_scale) {
		...
		if (fits) {
			min_scale = scale;
			max_scale =
				min(max_scale, bound_on_max_scale(dc, style, lines_try, scale));
			...
			max_scale = scale;
			min_scale =
				max(min_scale, bound_on_min_scale(dc, style, lines_try, scale));
...
bool TextViewer::prepareLinesScale(RotatedDC &dc, const vector<CharInfo> &chars,
								   const TextStyle &style,
								   bool stop_if_too_long,
	...
	while (line.top < style.height &&
		   line_size.width + 1 >= style.width - style.padding_right) {
	...
	for (size_t i = 0; i < chars.size(); ++i) {
		...
		bool accept_word =
			false; // the current word should be added to the line
		bool hide_breaker =
			true; // hide the \n or _(' ') that caused a line break
		if (c.break_after == BREAK_SOFT || c.break_after == BREAK_HARD ||
			c.break_after == BREAK_LINE) {
			break_now = true;
			accept_word = true;
			...
			accept_word = true;
		} else if (c.break_after == BREAK_MAYBE &&
				   style.direction == TOP_TO_BOTTOM) {
			break_now = true;
			accept_word = true;
			hide_breaker = false;
		...
		if (c.break_after != BREAK_LINE) {
			// ^^ HACK: don't count the line height of <line> tags, if they are
		...
		if (accept_word) {
			...
			for (auto const &p : positions_word) {
				line.positions.push_back(line_size.width + p);
			}
			if (word_end_or_soft != 0)
				line.end_or_soft = word_end_or_soft;
		...
		if (break_now) {
			...
			if (line_size.height < 0.01 && !lines.empty()) {
				// if a line has 0 height, use the height of the line above it,
				// but at most once
			} else {
				line.line_height = line_size.height;
			}
			line.end_or_soft =
				max(line.start, min(line.end_or_soft, line.end()));
			// push
			lines.push_back(line);
			// reset line object for next line
			double line_height_multiplier = line.break_after == BREAK_HARD
												? style.line_height_hard
												: line.break_after == BREAK_LINE
													  ? style.line_height_line
													  : style.line_height_soft;
			line.top += line.line_height * line_height_multiplier;
			line.start = word_start;
			line.positions.clear();
			if (line.break_after == BREAK_LINE)
				line.line_height = 0;
	...
	// the last line
	if (line_size.height < 0.01 && !lines.empty()) {
		// if a line has 0 height, use the height of the line above it, but at
		// most once
	} else {
		line.line_height = line_size.height;
	}
	...
	if (style.paragraph_height > 0) {
		...
		for (size_t last = 0; last < lines.size(); ++last) {
			if (lines[last].break_after != BREAK_SOFT || last == lines.size()) {
				max_height =
					max(max_height, lines[last].bottom() - lines[start].top);
				start = last + 1;
			}
		}
		// how many paragraphs would fit?
		int n = int(floor(0.5 +
						  (dc.getInternalSize().height - style.padding_bottom) /
							  style.paragraph_height));
		lines.back().top = max_height * n - lines.back().line_height;
	}
	return lines.back().bottom() <=
		   dc.getInternalSize().height - style.padding_bottom;
}

double TextViewer::lineLeft(RotatedDC &dc, const TextStyle &style,
							double y) const {
	return style.mask.getFromCache().rowLeft(y, dc.getInternalSize()) +
		   style.padding_left;
}
double TextViewer::lineRight(RotatedDC &dc, const TextStyle &style,
							 double y) const {
	return style.mask.getFromCache().rowRight(y, dc.getInternalSize()) -
		   style.padding_right;
}

void TextViewer::alignLines(RotatedDC &dc, const vector<CharInfo> &chars,
							const TextStyle &style) {
	// Size of the box
	RealSize s =
		add_diagonal(dc.getInternalSize(),
					 -RealSize(style.padding_left + style.padding_right,
							   style.padding_top + style.padding_bottom));
	if (style.paragraph_height <= 0) {
		// whole text box alignment
		assert(!lines.empty());
		double top = lines[0].top;
		alignParagraph(0, lines.size(), chars, style,
					   RealRect(RealPoint(0, top), s));
	} else {
		// per paragraph alignment
		size_t start = 0;
		int n = 0;
		for (size_t last = 0; last < lines.size(); ++last) {
			if (lines[last].break_after != BREAK_SOFT || last == lines.size()) {
				alignParagraph(
					start, last + 1, chars, style,
					RealRect(0, style.padding_top + n * style.paragraph_height,
							 s.width, style.paragraph_height));
				start = last + 1;
				++n;
			}
		}
...
void TextViewer::alignParagraph(size_t start_line, size_t end_line,
								const vector<CharInfo> &chars,
								const TextStyle &style, const RealRect &s) {
	...
	if (height < s.height) {
		double d_soft =
			max(0.0, style.line_height_soft_max - style.line_height_soft);
		double d_hard =
			max(0.0, style.line_height_hard_max - style.line_height_hard);
		double d_line =
		...
		for (int i = 1; i < 4 && height < s.height; ++i) {
			...
			for (size_t li = start_line; li < end_line; ++li) {
				...
				if ((soft && l.break_after == BREAK_SOFT) ||
					(hard && l.break_after == BREAK_HARD) ||
					(line && l.break_after == BREAK_LINE))
					add += to_add * l.line_height;
			}
			height += add;
		}
	}
	if (style.alignment == ALIGN_TOP_LEFT)
		return;

	// align
	double vdelta = align_delta_y(style.alignment, s.height, height) + s.y -
...
void TextViewer::Line::alignHorizontal(const vector<CharInfo> &chars,
									   const TextStyle &style,
									   const RealRect &s) {
	double width = this->width();
	bool should_fill =
		(alignment & ALIGN_IF_OVERFLOW ? width > s.width : true) &&
		(alignment & ALIGN_IF_SOFTBREAK
			 ? break_after == BREAK_SOFT || !style.field().multi_line
			 : true);
	if ((alignment & ALIGN_JUSTIFY_ALL) && should_fill) {
		...
		int count = (int)(end_or_soft -
						  start); // distribute it among this many characters
		if (count <= 0)
			count = 1; // prevent div by 0
		int i = 0;
		...
		for (size_t k = start + 1; k < end_or_soft - 1; ++k) {
			if (chars[k].break_after == BREAK_SPACE)
				++count;
		}
		if (count == 0)
			count = 1; // prevent div by 0
		...
		for (auto &c : positions) {
			c += s.x + hdelta * i / count;
			if (j < end_or_soft && chars[j++].break_after == BREAK_SPACE)
				i++;
		}
