...
void FontTextElement::draw(RotatedDC &dc, double scale, const RealRect &rect,
                           const double *xs, DrawWhat what, size_t start,
                           size_t end) const {
    if ((what & draw_as) != draw_as)
        return; // don't draw
    // text
...
void FontTextElement::getCharInfo(RotatedDC &dc, double scale,
    ...
    for (size_t i = start; i < end; ++i) {
        ...
        if (c == (L'\n')) {
            ...
            RealSize s = dc.GetTextExtent(
                content.substr(line_start - this->start, i - line_start + 1));
