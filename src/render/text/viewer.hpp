 ...
 *              For cursor positions char_id = 0 means the cursor is BEFORE
 *character 0
 *  - pos       The position of a character in real world x,y coordinates (in
 *pixels)
 ...
 *  - Line      A line on the screen, this does not neccessarly correspond to
 *explicit linebreaks,
...
class TextViewer {
  public:
	 ...
	 *  before calling draw again with different text/style reset() should be
	 * called
	 */
	void draw(RotatedDC &dc, const TextStyle &style, DrawWhat what);
	/// Draw an indicator for selected text
	void drawSelection(RotatedDC &dc, const TextStyle &style, size_t sel_start,
					   size_t sel_end);
	...
	bool prepare(RotatedDC &dc, const String &text, TextStyle &style,
				 Context &);
	/// Reset the cached data, at a new call to draw it will be recalculated
	/** If related, the new value is related to the old one, and layout
	...
	/// Find the first character index that is at/before/after the given index,
	/// and which has a nonzero width
	/** More precisely: it returns a position so that the character after it in
	 * the direction delta has nonzero width
	 */
  ...
  private:
	...
	void prepareLines(RotatedDC &dc, const String &text, TextStyle &style,
					  Context &ctx);
	/// Find the scale to use for the text
	void prepareLinesTryScales(RotatedDC &dc, const String &text,
							   const TextStyle &style,
							   vector<CharInfo> &chars_out);
	/// Prepare the lines, layout the text; at a specific scale
	/** Stores output in lines_out */
	bool prepareLinesScale(RotatedDC &dc, const vector<CharInfo> &chars,
						   const TextStyle &style, bool stop_if_too_long,
						   vector<Line> &lines_out) const;
	/// Align the lines within the textbox
	void alignLines(RotatedDC &dc, const vector<CharInfo> &chars,
					const TextStyle &style);
	/// Align the lines of a single paragraph (a set of lines)
	void alignParagraph(size_t start_line, size_t end_line,
						const vector<CharInfo> &chars, const TextStyle &style,
						const RealRect &box);

	/// Find the line the given index is on, returns the first line if the index
	/// is not found
	const Line &findLine(size_t index) const;

	// helper : get the start coordinate of a line, this is 0 unless there is a
	// contour mask
	double lineLeft(RotatedDC &dc, const TextStyle &style, double y) const;
	// helper : get the end coordinate of a line, this is width unless there is
	// a contour mask
