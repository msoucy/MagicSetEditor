...
struct CharInfo {
	...
	bool soft : 1; ///< Is this a 'soft' character? soft characters are ignored
				   ///for alignment

	explicit CharInfo() : break_after(BREAK_NO), soft(true) {}
...
class TextElement : public IntrusivePtrBase<TextElement> {
  public:
	...
	virtual void draw(RotatedDC &dc, double scale, const RealRect &rect,
					  const double *xs, DrawWhat what, size_t start,
					  size_t end) const = 0;
	/// Get information on all characters in the range [start...end) and store
	/// them in out
	virtual void getCharInfo(RotatedDC &dc, double scale,
							 vector<CharInfo> &out) const = 0;
...
class TextElements {
  public:
	...
	void draw(RotatedDC &dc, double scale, const RealRect &rect,
			  const double *xs, DrawWhat what, size_t start, size_t end) const;
	// Get information on all characters in the range [start...end) and store
	// them in out
	void getCharInfo(RotatedDC &dc, double scale, size_t start, size_t end,
					 vector<CharInfo> &out) const;
	...
	void fromString(const String &text, size_t start, size_t end,
...
class FontTextElement : public SimpleTextElement {
  public:
	FontTextElement(const String &content, size_t start, size_t end,
					const FontP &font, DrawWhat draw_as, LineBreak break_style)
		: SimpleTextElement(content, start, end), font(font), draw_as(draw_as),
...
class SymbolTextElement : public SimpleTextElement {
  public:
	SymbolTextElement(const String &content, size_t start, size_t end,
					  const SymbolFontRef &font, Context *ctx)
		: SimpleTextElement(content, start, end), font(font), ctx(*ctx) {}
...
class AtomTextElement : public CompoundTextElement {
  public:
	AtomTextElement(size_t start, size_t end)
		: CompoundTextElement(start, end) {}
...
class ErrorTextElement : public CompoundTextElement {
  public:
	ErrorTextElement(size_t start, size_t end)
		: CompoundTextElement(start, end) {}
