...
Package &ValueViewer::getStylePackage() const {
	return viewer.getStylePackage();
}
Package &ValueViewer::getLocalPackage() const {
	return viewer.getLocalPackage();
}
...
Rotation ValueViewer::getRotation() const {
	return Rotation(deg_to_rad(getStyle()->angle),
					getStyle()->getExternalRect(), 1.0, getStretch());
}

#if defined(__WXMSW__)
// on windows, wxDOT is not actually dotted, so use a custom style to achieve
// that
...
#else
wxPen dotted_pen(wxColour const &color) { return wxPen(color, 1, wxDOT); }
#endif

bool ValueViewer::setFieldBorderPen(RotatedDC &dc) {
	if (!getField()->editable)
		return false;
...
void ValueViewer::redraw() { viewer.redraw(*this); }

bool ValueViewer::nativeLook() const { return viewer.nativeLook(); }
bool ValueViewer::isCurrent() const { return viewer.viewerIsCurrent(this); }
