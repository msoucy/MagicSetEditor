...
class ValueViewer : public StyleListener {
  public:
	 ...
	 *  Scripts are re-updated after preparing if they depend on content
	 * properties. */
	...
	/// Get a bounding rectangle for this field (including any border it may
	/// have)
  ...
  protected:
	...
	/// Set the pen for drawing the border, returns true if a border needs to be
	/// drawn
...
#define DECLARE_VALUE_VIEWER(Type)                                             \
  protected:                                                                   \
	inline Type##Style &style() const {                                        \
		return static_cast<Type##Style &>(*ValueViewer::styleP);               \
	}                                                                          \
	inline const Type##Value &value() const {                                  \
		return static_cast<const Type##Value &>(*ValueViewer::valueP);         \
	}                                                                          \
	inline const Type##Field &field() const { return style().field(); }        \
	inline Type##StyleP styleP() const {                                       \
		return static_pointer_cast<Type##Style>(ValueViewer::styleP);          \
	}                                                                          \
	inline Type##ValueP valueP() const {                                       \
		return static_pointer_cast<Type##Value>(ValueViewer::valueP);          \
	}                                                                          \
	inline Type##FieldP fieldP() const {                                       \
		return static_pointer_cast<Type##Field>(style().fieldP);               \
	}                                                                          \
                                                                               \
  public:                                                                      \
...
#define IMPLEMENT_VALUE_VIEWER(Type)                                           \
	ValueViewerP Type##Style::makeViewer(DataViewer &parent) {                 \
		return intrusive(                                                      \
			new Type##ValueViewer(parent, intrusive_from_existing(this)));     \
	}
