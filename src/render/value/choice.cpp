...
void get_options(Rotation &rot, ValueViewer &viewer, const ChoiceStyle &style,
...
bool prepare_choice_viewer(RotatedDC &dc, ValueViewer &viewer,
	...
	if (style.render_style & RENDER_IMAGE) {
		...
		if (img.update(ctx) && img.isReady()) {
			...
			Image image;
			RealSize size;
			img.generateCached(img_options, &style.mask, &combine, &bitmap,
							   &image, &size);
			// store content properties
			if (style.content_width != size.width ||
...
void draw_choice_viewer(RotatedDC &dc, ValueViewer &viewer, ChoiceStyle &style,
	...
	if (style.render_style & RENDER_IMAGE) {
		...
		if (img.isReady()) {
			...
			size = dc.trInvS(size);
			RealRect rect(
				align_in_rect(style.alignment, size, dc.getInternalRect()),
				size);
			if (bitmap.Ok()) {
				// just draw it
	...
	if (style.render_style & RENDER_TEXT) {
		...
		if (style.render_style & RENDER_IMAGE) {
			text_align = ALIGN_MIDDLE_LEFT; // can't align both text and image
											// in the same way
		}
		dc.SetFont(style.font, 1.0);
		RealSize size = dc.GetTextExtent(text);
		RealPoint pos = align_in_rect(text_align, size, dc.getInternalRect()) +
						RealSize(margin, 0);
