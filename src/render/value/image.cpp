...
void ImageValueViewer::draw(RotatedDC &dc) {
    ...
    // reset?
    int w = max(0, (int)dc.trX(style().width)),
        h = max(0, (int)dc.trY(style().height));
    ...
    if (!bitmap.Ok()) {
        ...
        if (!image.Ok() && style().default_image.isReady()) {
            image = style().default_image.generate(GeneratedImage::Options(
                w, h, &getStylePackage(), &getLocalPackage()));
            is_default = true;
            if (what & DRAW_EDITING) {
                bitmap = imagePlaceholder(dc, w, h, image, what & DRAW_EDITING);
                if (alpha_mask.isLoaded() || !is_rad0(a)) {
                    image = bitmap.ConvertToImage(); // we need to convert back
                                                     // to an image
                } else {
                    image = Image();
                }
        ...
        if (!image.Ok() && !bitmap.Ok() && style().width > 40) {
            // placeholder bitmap
            bitmap =
                imagePlaceholder(dc, w, h, wxNullImage, what & DRAW_EDITING);
            if (alpha_mask.isLoaded() || !is_rad0(a)) {
...
void ImageValueViewer::onValueChange() { bitmap = Bitmap(); }

void ImageValueViewer::onStyleChange(int changes) {
    if ((changes & CHANGE_MASK) || ((changes & CHANGE_DEFAULT) && is_default)) {
        bitmap = Bitmap();
    }
...
bool very_light(const Image &image) {
    ...
    int total = (int)data[3 * middle] + (int)data[3 * middle + 1] +
                (int)data[3 * middle + 2];
    return total >= 210 * 3;
}

Bitmap ImageValueViewer::imagePlaceholder(const Rotation &rot, UInt w, UInt h,
