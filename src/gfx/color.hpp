...
class AColor : public Color {
  public:
    ...
    inline AColor(Byte r, Byte g, Byte b, Byte a = 255)
        : Color(r, g, b), alpha(a) {}
...
// it is important to pack this into 3 bytes, so we can directly convert from
// wxImage data
#if defined(_MSC_VER)
#pragma pack(push, 1)
#define MAKE_PACKED
...
struct RGB {
    ...
    inline operator wxColour() const { return wxColour(r, g, b); }
...
inline int bot(int x) {
    return std::max(0, x);
} ///< bottom range check for color values
inline int top(int x) {
    return std::min(255, x);
...
/// Like recolor: map green to similar black/white and blue to complementary
/// white/black
