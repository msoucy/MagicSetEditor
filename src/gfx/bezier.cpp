...
BezierCurve::BezierCurve(const Vector2D &p0, const Vector2D &p1,
...
void deCasteljau(ControlPoint &a, ControlPoint &b, double t,
				 ControlPoint &mid) {
...
void curve_subdivide(const BezierCurve &c, const Vector2D &p0,
					 const Vector2D &p1, double t0, double t1,
					 const Vector2D &origin, const Matrix2D &m,
					 vector<wxPoint> &out, UInt level) {
	if (level <= 0)
		return;
...
Bounds bezier_bounds(const Vector2D &origin, const Matrix2D &m,
	...
	count +=
		solve_quadratic(3 * curve.a.y, 2 * curve.b.y, curve.c.y, roots + count);
...
bool pos_on_bezier(const Vector2D &pos, double range, const ControlPoint &p1,
	...
	// theoretically we would need to check in all directions, but this covers
	// enough
	BezierCurve curve(p1, p2);
	double roots[6];
	UInt count;
	count =
		solve_cubic(curve.a.y, curve.b.y, curve.c.y, curve.d.y - pos.y, roots);
	count += solve_cubic(curve.a.x, curve.b.x, curve.c.x, curve.d.x - pos.x,
						 roots + count); // append intersections
...
bool pos_on_line(const Vector2D &pos, double range, const Vector2D &p1,
				 const Vector2D &p2, Vector2D &pOut, double &t) {
	Vector2D p21 = p2 - p1;
...
UInt intersect_bezier_ray(const ControlPoint &p1, const ControlPoint &p2,
	...
	// we can use the cubic formula to find roots, points where the horizontal
	// line
