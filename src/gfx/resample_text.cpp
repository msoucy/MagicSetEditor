...
// Downsamples the red channel of the input image to the alpha channel of the
// output image
...
void downsample_to_alpha(Bitmap &bmp_in, Image &img_out) {
	...
	if (!img_in.IsOk())
		return;
	// if text_scaling = 4, then the line always is dword aligned, so we need no
	// adjusting
	// we created a bitmap with depth 24, so that is what we should have here
	if (img_in.GetDepth() != 24)
		throw InternalError(_("DIB has wrong bit depth"));
#else
	...
	if (img_in.GetWidth() == img_out.GetWidth() * text_scaling) {
		...
		int w1 = img_in.GetWidth(), w2 = img_out.GetWidth(),
			h = img_in.GetHeight();
		int out_fact =
			(w2 << shift) / w1; // how much to output for 256 input = 1 pixel
	...
	if (img_in.GetHeight() == h * text_scaling) {
		...
		int out_fact =
			(h << shift) / h1; // how much to output for 256 input = 1 pixel
...
void draw_resampled_text(DC &dc, const RealPoint &pos, const RealRect &rect,
	...
	if (color.alpha == 0)
		return;
	// enlarge slightly; some fonts are larger then the GetTextExtent tells us
	// (especially italic fonts)
	int w = static_cast<int>(rect.width) + 3 + 2 * blur_radius,
		h = static_cast<int>(rect.height) + 1 + 2 * blur_radius;
	...
	// draw text
	Bitmap buffer(w * text_scaling, h * text_scaling,
				  24); // should be initialized to black
	wxMemoryDC mdc;
	...
	w += int(w * (stretch - 1) *
			 ca); // GCC makes annoying conversion warnings if *= is used here.
