...
#define COMBINE_FUN(combine, fun)                                              \
    template <>                                                                \
    int Combine<combine>::f(int a, int b) {                                    \
        return fun;                                                            \
    }
...
COMBINE_FUN(COMBINE_OVERLAY,
            a < 128 ? (a * b) >> 7 : 255 - (((255 - a) * (255 - b)) >> 7))
COMBINE_FUN(COMBINE_HARD_LIGHT,
            b < 128 ? (a * b) >> 7 : 255 - (((255 - a) * (255 - b)) >> 7))
...
COMBINE_FUN(COMBINE_SYMMETRIC_OVERLAY, (Combine<COMBINE_OVERLAY>::f(a, b) +
                                        Combine<COMBINE_OVERLAY>::f(b, a)) /
                                           2)
...
void combine_image(Image &a, const Image &b, ImageCombine combine) {
    ...
    switch (combine) {
#define DISPATCH(comb)                                                         \
    case comb:                                                                 \
        combine_image_do<comb>(a, b);                                          \
        return
    case COMBINE_DEFAULT:
    case COMBINE_NORMAL:
        a = b;
        return; // no need to do a per pixel operation
