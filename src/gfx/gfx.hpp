...
/// Resample an image, but preserve the aspect ratio by adding a transparent
/// border around the output if needed.
 ...
 */
void draw_resampled_text(DC &dc, const RealPoint &pos, const RealRect &rect,
						 double stretch, Radians angle, AColor color,
						 const String &text, int blur_radius = 0,
						 int repeat = 1);
 ...
 */
void linear_blend(Image &img1, const Image &img2, double x1, double y1,
				  double x2, double y2);

/// Blends two images together, using a third image as a mask
/** The result is stored in img1
 *  mask is used as a mask, white pixels are taken from img1, black pixels from
 * img2
...
enum ImageCombine {
	COMBINE_DEFAULT // normal combine, but with a low priority, i.e. "apply
					// default instead of add" == "add"
	// it is not representable in scripting/files, so should only be used
	// internally
	,
	COMBINE_NORMAL,
	COMBINE_ADD,
...
void draw_combine_image(DC &dc, UInt x, UInt y, const Image &img,
...
/** It is created by treating black in the source image as transparent and white
 * (red) as opaque
 */
class AlphaMask : public IntrusivePtrBase<AlphaMask> {
  public:
	...
	/// Is the given location opaque (not fully transparent)? when the mask were
	/// stretched to size
	...
	inline bool hasSize(const wxSize &compare_size) const {
		return size == compare_size;
	}
	/// Is the mask loaded?
