...
void linear_blend(Image &img1, const Image &img2, double x1, double y1,
	...
	if (img2.GetWidth() != width || img2.GetHeight() != height) {
		throw Error(
			_ERROR_("images used for blending must have the same size"));
	}
	...
	if (x1 == x2 && y1 == y2)
		throw Error(_ERROR_("coordinates for blending overlap"));
...
void set_alpha(Image &img, const Image &img_alpha) {
	Image img_alpha_resampled =
		resample(img_alpha, img.GetWidth(), img.GetHeight());
	if (!img.HasAlpha())
		img.InitAlpha();
...
void set_alpha(Image &img, Byte *al, const wxSize &alpha_size) {
	if (img.GetWidth() != alpha_size.GetWidth() ||
		img.GetHeight() != alpha_size.GetHeight()) {
