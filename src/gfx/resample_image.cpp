...
// Resample an image only in a single direction, either horizontally or
// vertically
 ...
 */
void resample_pass(const Image &img_in, Image &img_out, int offset_in,
                   int offset_out, int length_in, int delta_in, int length_out,
                   int delta_out, int lines, int line_delta_in,
                   int line_delta_out) {
    bool alpha = img_in.HasAlpha();
    if (alpha && !img_out.HasAlpha())
        img_out.InitAlpha();
    int out_fact = (length_out << shift) /
                   length_in; // how much to output for 256 input = 1 pixel
    ...
    for (int l = 0; l < lines; ++l) {
        ...
        UInt in_rem =
            out_fact +
            out_rest; // remaining to input from the current input pixel

        if (alpha) {
            Byte *in_a = img_in.GetAlpha() + (offset_in + l * line_delta_in);
            Byte *out_a =
                img_out.GetAlpha() + (offset_out + l * line_delta_out);
 ...
 *  - for each row:
 *    - each input pixel becomes a fixed amount of output (in 1<<shift fixed
 * point math)
 *    - for each output pixel:
 *      - (L'eat') input pixels until the total is 1<<shift
 *      - write the total to the output pixel
 *  - to ensure the sum of all the pixel amounts is exacly width<<shift an extra
 * rest amount
...
void resample(const Image &img_in, Image &img_out) {
    resample_and_clip(img_in, img_out,
                      wxRect(0, 0, img_in.GetWidth(), img_in.GetHeight()));
}
...
void resample_and_clip(const Image &img_in, Image &img_out, wxRect rect) {
    ...
    if (img_out.GetHeight() == rect.height) {
        ...
        resample_pass(img_in, img_out, offset_in, 0, rect.width, 1,
                      img_out.GetWidth(), 1, rect.GetHeight(),
                      img_in.GetWidth(), img_out.GetWidth());
    } else {
        Image img_temp(img_out.GetWidth(), rect.height, false);
        resample_pass(img_in, img_temp, offset_in, 0, rect.width, 1,
                      img_temp.GetWidth(), 1, rect.GetHeight(),
                      img_in.GetWidth(), img_temp.GetWidth());
        resample_pass(img_temp, img_out, 0, 0, rect.height, img_temp.GetWidth(),
                      img_out.GetHeight(), img_temp.GetWidth(),
                      img_temp.GetWidth(), 1, 1);
    }
}
...
void resample_preserve_aspect(const Image &img_in, Image &img_out) {
    ...
    if (rheight < img_out.GetHeight())
        rwidth = img_out.GetWidth();
    else if (rwidth < img_out.GetWidth())
        rheight = img_out.GetHeight();
    else {
        rwidth = img_out.GetWidth();
        rheight = img_out.GetHeight();
    }
    ...
    img_temp.InitAlpha();
    resample_pass(img_in, img_temp, 0, 0, img_in.GetWidth(), 1, rwidth, 1,
                  img_in.GetHeight(), img_in.GetWidth(), img_temp.GetWidth());
    resample_pass(img_temp, img_out, 0, offset_out, img_in.GetHeight(),
                  img_temp.GetWidth(), rheight, img_out.GetWidth(), rwidth, 1,
                  1);
}
...
void sharp_resample(const Image &img_in, Image &img_out, int amount) {
    sharp_resample_and_clip(img_in, img_out,
                            wxRect(0, 0, img_in.GetWidth(), img_in.GetHeight()),
                            amount);
}

void sharp_resample_and_clip(const Image &img_in, Image &img_out, wxRect rect,
                             int amount) {
...
void sharp_downsample(const Image &img_in, Image &img_out, int amount) {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            int center_alpha =
                al ? al[0] + al[1] + al[width * 2] + al[width * 2 + 1] : 1;
            int weight = center_weight * center_alpha * 4;
            int sumR = center_weight * center_alpha *
                       (in[0] + in[3] + in[line + 0] + in[line + 3]);
            int sumG = center_weight * center_alpha *
                       (in[1] + in[4] + in[line + 1] + in[line + 4]);
            int sumB = center_weight * center_alpha *
                       (in[2] + in[5] + in[line + 2] + in[line + 5]);
            // edges
            if (x != 0) {
                int a =
                    al
                        ? border_weight * min(2 * (al[-1] + al[width * 2 - 1]),
                                              center_alpha)
                        : border_weight;
            ...
            if (x + 1 != width) {
                int a =
                    al
                        ? border_weight *
                              min(2 * (al[2] + al[width * 2 + 2]), center_alpha)
                        : border_weight;
            ...
            if (y + 1 != height) {
                int a = al
                            ? border_weight * min(2 * (al[width * 2 * 2] +
                                                       al[width * 2 * 2 + 1]),
                                                  center_alpha)
                            : border_weight;
            ...
            // And then avarage the result into a single pixel (downsample by
            // factor 2 in both dimensions)
            if (weight > 0) {
