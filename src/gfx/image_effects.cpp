...
RGB recolor(RGB x, RGB cr, RGB cg, RGB cb, RGB cw) {
	...
	return RGB(static_cast<Byte>(
				   (nr * cr.r + ng * cg.r + nb * cb.r + nw * cw.r) / total),
			   static_cast<Byte>(
				   (nr * cr.g + ng * cg.g + nb * cb.g + nw * cw.g) / total),
			   static_cast<Byte>(
				   (nr * cr.b + ng * cg.b + nb * cb.b + nw * cw.b) / total));
}
...
Byte to_grayscale(RGB x) {
	return (Byte)((6969 * x.r + 23434 * x.g + 2365 * x.b) /
				  32768); // from libpng
}
