...
String GeneratedImage::toFriendlyString() const {
    return (L"<") + _TYPE_("image") + (L">");
}
...
Image conform_image(const Image &img, const GeneratedImage::Options &options) {
    ...
    if ((iw == options.width && ih == options.height) ||
        (options.width == 0 && options.height == 0)) {
        // zoom?
        if (options.zoom != 1.0) {
            image =
                resample(image, int(iw * options.zoom), int(ih * options.zoom));
        } else {
        ...
        int w, h;
        if (iw * options.height >
            ih * options.width) { // too much height requested
            w = options.width;
        ...
        if (options.preserve_aspect == ASPECT_BORDER &&
            ...
            image =
                resample_preserve_aspect(image, options.width, options.height);
        } else {
...
ImageCombine LinearBlendImage::combine() const { return image1->combine(); }
bool LinearBlendImage::operator==(const GeneratedImage &that) const {
    const LinearBlendImage *that2 =
        dynamic_cast<const LinearBlendImage *>(&that);
    return that2 && *image1 == *that2->image1 && *image2 == *that2->image2 &&
           x1 == that2->x1 && y1 == that2->y1 && x2 == that2->x2 &&
           y2 == that2->y2;
}
...
ImageCombine MaskedBlendImage::combine() const { return light->combine(); }
bool MaskedBlendImage::operator==(const GeneratedImage &that) const {
    const MaskedBlendImage *that2 =
        dynamic_cast<const MaskedBlendImage *>(&that);
    return that2 && *light == *that2->light && *dark == *that2->dark &&
           *mask == *that2->mask;
}
...
ImageCombine CombineBlendImage::combine() const { return image1->combine(); }
bool CombineBlendImage::operator==(const GeneratedImage &that) const {
    const CombineBlendImage *that2 =
        dynamic_cast<const CombineBlendImage *>(&that);
    return that2 && *image1 == *that2->image1 && *image2 == *that2->image2 &&
...
bool SetMaskImage::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && *mask == *that2->mask;
}
...
bool SetAlphaImage::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && alpha == that2->alpha;
}
...
ImageCombine SetCombineImage::combine() const { return image_combine; }
...
bool SaturateImage::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && amount == that2->amount;
}
...
bool RecolorImage::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && color == that2->color;
}
...
bool RecolorImage2::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && red == that2->red &&
           green == that2->green && blue == that2->blue &&
           white == that2->white;
}
...
bool FlipImageHorizontal::operator==(const GeneratedImage &that) const {
    const FlipImageHorizontal *that2 =
        dynamic_cast<const FlipImageHorizontal *>(&that);
    return that2 && *image == *that2->image;
}
...
bool FlipImageVertical::operator==(const GeneratedImage &that) const {
    const FlipImageVertical *that2 =
        dynamic_cast<const FlipImageVertical *>(&that);
    return that2 && *image == *that2->image;
}
...
bool RotateImage::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && angle == that2->angle;
}
...
Image EnlargeImage::generate(const Options &opt) const {
    ...
    Options sub_opt(
        ...
        opt.package, opt.local_package, opt.preserve_aspect);
    ...
    for (int y = 0; y < h; ++y) {
        memcpy(data2 + 3 * (dw + (y + dh) * w2), data1 + 3 * y * w,
               3 * w); // copy a line
    }
    if (img.HasAlpha()) {
...
Image CropImage::generate(const Options &opt) const {
    return image->generate(opt).Size(wxSize((int)width, (int)height),
                                     wxPoint(-(int)offset_x, -(int)offset_y));
}
bool CropImage::operator==(const GeneratedImage &that) const {
    const CropImage *that2 = dynamic_cast<const CropImage *>(&that);
    return that2 && *image == *that2->image && width == that2->width &&
           height == that2->height && offset_x == that2->offset_x &&
...
Image DropShadowImage::generate(const Options &opt) const {
    ...
    for (int y = y_start; y < y_end; ++y) {
        for (int x = x_start; x < x_end; ++x) {
            ...
            int a = alpha[p];
            int shad = ((((255 - a) * sa) >> 16) * shadow[p - delta]) /
                       total;              // amount of shadow to add
            int factor = max(1, a + shad); // divide by this
            data[3 * p] =
                (a * data[3 * p] + shad * shadow_color.Red()) / factor;
            data[3 * p + 1] =
                (a * data[3 * p + 1] + shad * shadow_color.Green()) / factor;
            data[3 * p + 2] =
                (a * data[3 * p + 2] + shad * shadow_color.Blue()) / factor;
            alpha[p] = a + shad;
        }
    }
    // memset(data,0,3*w*h);
...
bool DropShadowImage::operator==(const GeneratedImage &that) const {
    ...
    return that2 && *image == *that2->image && offset_x == that2->offset_x &&
           offset_y == that2->offset_y && shadow_alpha == that2->shadow_alpha &&
           shadow_blur_radius == that2->shadow_blur_radius &&
...
Image PackagedImage::generate(const Options &opt) const {
    ...
    if (!opt.package)
        throw ScriptError(
            (L"Can only load images in a context where an image is expected"));
    InputStreamP file = opt.package->openIn(filename);
    Image img;
    if (img.LoadFile(*file)) {
        if (img.HasMask())
            img.InitAlpha(); // we can't handle masks
        return img;
    } else {
        throw ScriptError((L"Unable to load image '") + filename +
                          (L"' from '" + opt.package->name() + (L"'")));
    }
}
...
SymbolToImage::SymbolToImage(bool is_local, const LocalFileName &filename,
                             const SymbolVariationP &variation)
...
Image SymbolToImage::generate(const Options &opt) const {
    ...
    return render_symbol(the_symbol, *variation->filter,
                         variation->border_radius, width, height, false,
                         allow_smaller);
}
...
bool ImageValueToImage::operator==(const GeneratedImage &that) const {
    const ImageValueToImage *that2 =
        dynamic_cast<const ImageValueToImage *>(&that);
    return that2 && filename == that2->filename;
}

String quote_string(String const &str);
String ImageValueToImage::toCode() const {
    return (L"local_image_file(") +
