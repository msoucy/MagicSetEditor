...
Image PackagedImage::generate(const Options &opt) const {
	...
	if (!opt.package)
		throw ScriptError(
			_("Can only load images in a context where an image is expected"));
	...
	if (img.LoadFile(*file)) {
		...
		throw ScriptError(_("Unable to load image '") + filename +
						  _("' from '" + opt.package->name() + _("'")));
	}
}
