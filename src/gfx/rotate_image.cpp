...
struct Rotate90deg {
	...
	inline static int offset(UInt x, UInt y, UInt w, UInt h) {
		...
		int my = w - x - 1;
		return h * my +
			   mx; // note: h, since that is the width of the target image
	}
};
...
Image rotate_image(const Image &image, Radians angle) {
	...
	if (is_rad0(a))
		return image;
	if (is_rad90(a))
		return rotate_image_impl<Rotate90deg>(image);
	if (is_rad180(a))
		return rotate_image_impl<Rotate180deg>(image);
	if (is_rad270(a))
		return rotate_image_impl<Rotate270deg>(image);
	else {
		if (!image.HasAlpha())
			const_cast<Image &>(image).InitAlpha();
