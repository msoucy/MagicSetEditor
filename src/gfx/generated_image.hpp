...
class GeneratedImage : public ScriptValue {
  public:
	...
	struct Options {
		Options(int width = 0, int height = 0, Package *package = nullptr,
				Package *local_package = nullptr,
				PreserveAspect preserve_aspect = ASPECT_STRETCH,
				bool saturate = false)
		...
		mutable int width, height; ///< Width to force the image to, or 0 to
								   ///keep the width of the input
	...
	/// Equality should mean that every pixel in the generated images is the
	/// same if the same options are used
	virtual bool operator==(const GeneratedImage &that) const = 0;
	inline bool operator!=(const GeneratedImage &that) const {
		return !(*this == that);
	}
...
class SimpleFilterImage : public GeneratedImage {
  public:
	inline SimpleFilterImage(const GeneratedImageP &image) : image(image) {}
...
class LinearBlendImage : public GeneratedImage {
  public:
	inline LinearBlendImage(const GeneratedImageP &image1,
							const GeneratedImageP &image2, double x1, double y1,
							double x2, double y2)
...
class MaskedBlendImage : public GeneratedImage {
  public:
	inline MaskedBlendImage(const GeneratedImageP &light,
							const GeneratedImageP &dark,
							const GeneratedImageP &mask)
		: light(light), dark(dark), mask(mask) {}
	virtual Image generate(const Options &opt) const;
	virtual ImageCombine combine() const;
	virtual bool operator==(const GeneratedImage &that) const;
	virtual bool local() const {
		return light->local() && dark->local() && mask->local();
	}
...
/// An image generator that blends two other images using an ImageCombine
/// function
class CombineBlendImage : public GeneratedImage {
  public:
	inline CombineBlendImage(const GeneratedImageP &image1,
							 const GeneratedImageP &image2,
...
class SetMaskImage : public SimpleFilterImage {
  public:
	inline SetMaskImage(const GeneratedImageP &image,
						const GeneratedImageP &mask)
...
class SetCombineImage : public SimpleFilterImage {
  public:
	inline SetCombineImage(const GeneratedImageP &image,
						   ImageCombine image_combine)
...
class RecolorImage2 : public SimpleFilterImage {
  public:
	inline RecolorImage2(const GeneratedImageP &image, Color red, Color green,
						 Color blue, Color white)
		: SimpleFilterImage(image), red(red), green(green), blue(blue),
		  white(white) {}
	virtual Image generate(const Options &opt) const;
	virtual bool operator==(const GeneratedImage &that) const;
...
class CropImage : public SimpleFilterImage {
  public:
	inline CropImage(const GeneratedImageP &image, double width, double height,
...
class DropShadowImage : public SimpleFilterImage {
  public:
	inline DropShadowImage(const GeneratedImageP &image, double offset_x,
						   double offset_y, double shadow_alpha,
						   double shadow_blur_radius, Color shadow_color)
		: SimpleFilterImage(image), offset_x(offset_x), offset_y(offset_y),
		  shadow_alpha(shadow_alpha), shadow_blur_radius(shadow_blur_radius),
...
class PackagedImage : public GeneratedImage {
  public:
	inline PackagedImage(const String &filename) : filename(filename) {}
	virtual Image generate(const Options &opt) const;
	virtual bool operator==(const GeneratedImage &that) const;
...
class BuiltInImage : public GeneratedImage {
  public:
	inline BuiltInImage(const String &name) : name(name) {}
	virtual Image generate(const Options &opt) const;
	virtual bool operator==(const GeneratedImage &that) const;
...
class SymbolToImage : public GeneratedImage {
  public:
	SymbolToImage(bool is_local, const LocalFileName &filename,
